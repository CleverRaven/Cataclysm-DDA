...
{
 #define MISSION(name, goal, diff, urgent, place, start, end) \
 id++; mission_types.push_back( \
		mission_type(id, name, goal, diff, urgent, place, start, end) )
...
// The order of missions should match enum mission_id in mission.h
 ...
 MISSION("Null mission", MGOAL_NULL, 0, false,
         &mission_place::never, &mission_start::standard,
         &mission_end::standard);

 MISSION("Find Antibiotics", MGOAL_FIND_ITEM, 2, true,
