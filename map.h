...
// Items
 ...
 std::vector<item> use_amount(const point origin, const int range, const itype_id type, const int amount,
                              const bool use_container = false);
 std::vector<item> use_charges(const point origin, const int range, const itype_id type, const int amount);
