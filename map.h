  ...
  */
  ...
  void rel2real(int lx, int ly, int px, int py, bool getom=false ) {
    ...
    /* deal with !INBOUND (negative) coordinates.
       ...
       redone @ pos=-24, -48, -96 ....
    */
    while( sub_pos.x < 0 ) { // todo: deprecate for better math used in fromabs
        sub.x--;
        sub_pos.x += 12;
    }
    while( sub_pos.y < 0 ) {
...
// File I/O
 ...
 void clear_spawns();
 void clear_traps();
 std::vector<spawn_point>& getspawns(const int x, const int y);
...
// Items
 ...
 bool add_item(const int x, const int y, item new_item, int maxitems = 64, bool allow_oob = false); // Do we want mapgen and explosions piling up to MAX_ITEM_IN_SQUARE (1024)? NYET!
 bool add_item_anywhere( point worldpos, const int z, item new_item, int maxitems = 64, bool generate_sub = true); // technically this doesn't need to be part of map, but only map uses it so far.
...
protected:
 ...
 bool offloadn(game *g, const int x, const int y, const int z, const int gridx, const int gridy,
 ...
 std::vector<spawn_point> nulspawns;
