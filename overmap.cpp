...
point overmap::display_notes(int const z) const
{
 ...
 do{
  ...
  for (cur_it = start; cur_it < start + maxitems && cur_line < 23; cur_it++) {
   ...
   mvwprintz(w_notes, cur_line, 2, c_ltgray, "- %s", it->second.notes[cur_it].text.c_str());
   } else{
...
bool overmap::generate_sub(int const z)
{
 ...
 for (int i = 0; i < OMAPX; i++) {
  for (int j = 0; j < OMAPY; j++) {
   if (ter(i, j, z + 1) >= ot_sub_station_north &&
    ...
    zg.push_back(mongroup(mcat_ant, i * 2, j * 2, z, size * 1.5, rng(6000, 8000)));
    ...
    zg.push_back(mongroup(mcat_spiral, i * 2, j * 2, z, 2, 200));
 ...
 for (int i = 0; i < cities.size(); i++) {
  if (one_in(3))
   zg.push_back(
    mongroup(mcat_chud, cities[i].x * 2, cities[i].y * 2, z,
             cities[i].s, cities[i].s * 20));
  if (!one_in(8))
   zg.push_back(
    mongroup(mcat_sewer, cities[i].x * 2, cities[i].y * 2, z,
...
bool overmap::build_lab(int x, int y, int z, int s)
{
 ...
 zg.push_back(mongroup(mcat_lab, (x * 2), (y * 2), z, s, 400));
 ...
 if (!OPTIONS[OPT_STATIC_SPAWN]) {
  ...
  for (unsigned int i = 0; i < cities.size(); i++) {
   if (!one_in(16) || cities[i].s > 5)
    zg.push_back (mongroup(mcat_zombie, (cities[i].x * 2), (cities[i].y * 2), 0,
 ...
 for (int x = 3; x < OMAPX - 3; x += 7) {
  for (int y = 3; y < OMAPY - 3; y += 7) {
   ...
   if (swamp_count >= 25) // ~25% swamp or ~50% river
    zg.push_back(mongroup(mcat_swamp, x * 2, y * 2, 0, 3,
...
// Place the "put me anywhere" groups
 ...
 for (int i = 0; i < numgroups; i++) {
  zg.push_back(
	mongroup(mcat_worm, rng(0, OMAPX * 2 - 1), rng(0, OMAPY * 2 - 1), 0,
	         rng(20, 40), rng(500, 1000)));
 }

// Forest groups cover the entire map
 zg.push_back(
	mongroup(mcat_forest, 0, OMAPY, OMAPY, 0,
                 rng(2000, 12000)));
 zg.back().diffuse = true;
 zg.push_back(
	mongroup(mcat_forest, 0, OMAPY * 2 - 1, OMAPY, 0,
                 rng(2000, 12000)));
 zg.back().diffuse = true;
 zg.push_back(
	mongroup(mcat_forest, OMAPX, 0, OMAPX, 0,
                 rng(2000, 12000)));
 zg.back().diffuse = true;
 zg.push_back(
	mongroup(mcat_forest, OMAPX * 2 - 1, 0, OMAPX, 0,
                 rng(2000, 12000)));
 zg.back().diffuse = true;
}
...
// DEBUG VARS
 ...
 if (fin.is_open()) {
  ...
  while (fin >> datatype) {
    ...
    fin >> ct >> cx >> cy >> cz >> cs >> cp >> cd;
    zg.push_back(mongroup(moncat_id(ct), cx, cy, cz, cs, cp));
    zg.back().diffuse = cd;
    nummg++;
