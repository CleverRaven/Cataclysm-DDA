...
calendar::calendar(int turn)
{
 ...
 int season_param = day_param / (awo_populated?ACTIVE_WORLD_OPTIONS:OPTIONS)["SEASON_LENGTH"]; //OPTIONS["SEASON_LENGTH"];
 second = 6 * (turn % 10);
 minute = minute_param % 60;
 hour = hour_param % 24;
 day = 1 + day_param % (int)(awo_populated?ACTIVE_WORLD_OPTIONS:OPTIONS)["SEASON_LENGTH"];
...
int calendar::get_turn() const
{
 ...
 ret += hour * 600;
 ret += day * 14400;
 ret += int(season) * 14400 * (int)(awo_populated?ACTIVE_WORLD_OPTIONS:OPTIONS)["SEASON_LENGTH"];
 ret += year * 14400 * 4 * (int)(awo_populated?ACTIVE_WORLD_OPTIONS:OPTIONS)["SEASON_LENGTH"];
 return ret;
}
...
calendar& calendar::operator =(int rhs)
{
 ...
 int hour_param = minute_param / 60;
 int day_param = hour_param / 24;
 int season_param = day_param / (awo_populated?(awo_populated?ACTIVE_WORLD_OPTIONS:OPTIONS):OPTIONS)["SEASON_LENGTH"];
 second = 6 * (rhs % 10);
 minute = minute_param % 60;
 hour = hour_param % 24;
 day = day_param % (int)(awo_populated?(awo_populated?ACTIVE_WORLD_OPTIONS:OPTIONS):OPTIONS)["SEASON_LENGTH"];
 season = season_type(season_param % 4);
 year = season_param / 4;
 return *this;
}
...
void calendar::standardize()
{
 ...
 if (day >= (awo_populated?ACTIVE_WORLD_OPTIONS:OPTIONS)["SEASON_LENGTH"]) {
  tmpseason += day / (awo_populated?ACTIVE_WORLD_OPTIONS:OPTIONS)["SEASON_LENGTH"];
  day %= (int)(awo_populated?ACTIVE_WORLD_OPTIONS:OPTIONS)["SEASON_LENGTH"];
 }
 if (tmpseason >= 4) {
  year += tmpseason / 4;
  tmpseason %= 4;
...
{
 int phase = day / ((awo_populated?ACTIVE_WORLD_OPTIONS:OPTIONS)["SEASON_LENGTH"] / 4);
 //phase %= 4;   Redundant?
 if (phase == 3)
  return MOON_HALF;
 else
...
calendar calendar::sunrise() const
{
 ...
 double percent = double(double(day) / (awo_populated?ACTIVE_WORLD_OPTIONS:OPTIONS)["SEASON_LENGTH"]);
