...
args.add_argument("type", action="store", help="provide item type")
...
def object_key_check(jo, object_key, old_field, new_field):
    if jo["type"] != object_key:
        return None
    if old_field in jo:
        jo[new_field] = jo[old_field]
        del jo[old_field]
    return jo
...
if __name__ == "__main__":
    for root, directories, filenames in os.walk(args_dict["dir"]):
        for filename in filenames:
            ...
            new = gen_new(path, args_dict["type"], args_dict["old_object_field"], args_dict["new_object_field"])
            if new is not None:
