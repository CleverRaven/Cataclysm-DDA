        ...
        if len(thing) == 1:
            ...
            if (
                ...
                if (
                    ...
                    # Because we changed the reference (read: type) of `thing`
                    # (from `int` to `list`), the changes won't be reflected in
                    # the outer scope, even if list.append() would normally
                    # work. Therefore, we must return the new reference.
                    return thing # return the modified thing
            else:
                offset(thing[0])
        else:
...
def tile_convert(otile, main_id, new_tile_number):
    ...
    for g in ('fg', 'bg'):
        ...
        ntile[g] = otile[g]
        new_thing = thing_convert(
            ntile[g], # thing
            g, # g
            ntile, # ntile
            ntile['id'], # thing_id
            ('rotates' in otile and otile['rotates'] == True), # rotates
            height, # height
        )
    ...
    if 'additional_tiles' in otile:
        ...
        for otilea in otile['additional_tiles']:
            print ' handling additional_tile ' + otile['id']
            new_tile = tile_convert(otilea, main_id, ntile['ntn'])
...
if __name__ == "__main__":
    ...
    if args.dryrun>1:
        ...
        with open(new_tileset_name + '/tile_config.json', 'w') as new_tile_config_json_file:
            json.dump(ntc, new_tile_config_json_file, sort_keys=False, indent=2, separators=(',', ': '))
