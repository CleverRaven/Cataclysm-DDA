...
struct uimenu_entry {
    ...
    uimenu_entry(std::string T) : retval(-1), enabled(false), hotkey(-1), txt(T) {text_color=C_UNSET_MASK;};
    uimenu_entry(std::string T, int K) : retval(-1), enabled(false), hotkey(K), txt(T) {text_color=C_UNSET_MASK;};
