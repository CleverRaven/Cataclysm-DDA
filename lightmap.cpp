 ...
 */
 ...
 for (int i = 0; i < g->num_zombies(); ++i) {
  ...
  if (INBOUNDS(mx, my)) {
   ...
   if (monid == "mon_zombie_electric") { luminance = 1;}
   else if (monid == "mon_turret") { luminance = 2;}
   else if (monid == "mon_flaming_eye") { luminance = LIGHT_SOURCE_BRIGHT;}
   else if (monid == "mon_manhack") { luminance = LIGHT_SOURCE_LOCAL;}

   if (luminance)
   {
       apply_light_source(mx, my, luminance, trigdist);
   }
  }
 }
