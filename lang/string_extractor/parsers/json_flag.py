...
def parse_json_flag(json, origin):
    ...
    if "item_prefix" in json:
        write_text(json["item_prefix"], origin, c_format=False, comment="This is custom prefix added to item name. \"{}\"".format(json["id"]))
    if "item_suffix" in json:
        write_text(json["item_suffix"], origin, c_format=False, comment="This is custom suffix added to item name. \"{}\"".format(json["id"]))
