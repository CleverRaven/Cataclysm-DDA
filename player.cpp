...
void player::update_bodytemp(game *g)
{
    ...
    for (int i = 0 ; i < num_bp ; i++)
    {
        ...
        case bp_mouth :
            ...
            break;
        case bp_hands :
            temp_equalizer(bp_hands, bp_arms);
            break;
        case bp_feet  :
            temp_equalizer(bp_feet, bp_legs);
            break;
        }
        ...
        // Disintergration
        if (has_trait(PF_ROT1))
        {
            temp_conv[i] -= 250;
        }
        else if (has_trait(PF_ROT2))
        {
            temp_conv[i] -= 750;
        }
        else if (has_trait(PF_ROT3))
        {
            temp_conv[i] -= 1500;
        }
        // Radioactive
        ...
        else if (has_trait(PF_RADIOACTIVE2))
        {
            temp_conv[i] += 750;
        }
        else if (has_trait(PF_RADIOACTIVE3))
        {
            temp_conv[i] += 1500;
        }
        // Chemical Imbalance
...
// Next, draw encumberment.
    ...
    for (int i=0; i < 8; i++)
    {
        ...
        {
            color = c_green;    // More than cold is comfortable
        }
        else if (temp_conv[i] >  BODYTEMP_VERY_COLD)
        {
            color = c_ltblue;
...
// Find hottest/coldest bodypart
    ...
    for (int i = 0; i < num_bp ; i++ )
    {
        if      (temp_cur[i] > BODYTEMP_HOT  && temp_cur[i] > temp_cur[max])
        {
            max = i;
        }
        else if (temp_cur[i] < BODYTEMP_COLD && temp_cur[i] < temp_cur[min])
        {
            min = i;
        }
    }
// Compare which is most extreme
    int print;
    if (temp_cur[max] - BODYTEMP_NORM > BODYTEMP_NORM + temp_cur[min])
    {
        print = max;
    }
    else
...
// Assign zones to temp_cur and temp_conv for comparison
    ...
    else if (temp_cur[print] >  BODYTEMP_COLD)
    {
        cur_zone = 4;
    }
    ...
    int conv_zone = 0;
    if      (temp_conv[print] >  BODYTEMP_SCORCHING)
    {
        conv_zone = 7;
    }
    else if (temp_conv[print] >  BODYTEMP_VERY_HOT)
    {
        conv_zone = 6;
    }
    else if (temp_conv[print] >  BODYTEMP_HOT)
    {
        conv_zone = 5;
    }
    else if (temp_conv[print] >  BODYTEMP_COLD)
    {
        conv_zone = 4;
    }
    else if (temp_conv[print] >  BODYTEMP_VERY_COLD)
    {
        conv_zone = 3;
    }
    else if (temp_conv[print] >  BODYTEMP_FREEZING)
    {
        conv_zone = 2;
    }
    else if (temp_conv[print] <= BODYTEMP_FREEZING)
...
bool player::in_climate_control(game *g)
{
    ...
    if(int(g->turn) >= next_climate_control_check)
    {
        ...
        if(!regulated_area)
        {
            next_climate_control_check+=40;    // Takes longer to cool down / warm up with AC, than it does to step outside and feel cruddy.
        }
    }
    else
    {
...
int player::encumb(body_part bp, int &layers, int &armorenc)
{
    ...
    {
    case bp_feet  :
        if (is_wearing("socks") || is_wearing("socks_wool"))
        {
            layers--;
        }
        break;
    case bp_legs  :
        if (is_wearing("long_underpants"))
        {
            layers--;
        }
        break;
    case bp_hands :
        if (is_wearing("gloves_liner"))
        {
            layers--;
        }
        break;
    case bp_torso :
        if (is_wearing("under_armor"))
        {
            layers--;
        }
        break;
...
{
    if( this->power_level < 3 || !this->has_active_bionic("bio_uncanny_dodge") )
    {
        return false;
    }
...
point player::adjacent_tile()
{
    ...
    for (int i=posx-1; i <= posx+1; i++)
    {
        for (int j=posy-1; j <= posy+1; j++)
        {
            if (i == posx && j == posy)
            {
                continue;    // don't consider player position
            }
