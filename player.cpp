...
void player::process_active_items(game *g)
{
 ...
 if (weapon.is_artifact() && weapon.is_tool())
  ...
  if (weapon.has_flag(IF_CHARGE)) { // We're chargin it up!
   if (weapon.charges == 8) {
    bool maintain = false;
    if (use_charges_if_available("UPS_on", 4) ) {
     maintain = true;
    } else if (use_charges_if_available("UPS_off", 4)) {
     maintain = true;
    }
    if (maintain) {
     if (one_in(20)) {
    ...
    if (use_charges_if_available("UPS_on", 1 + weapon.charges)) {
     weapon.poison++;
    } else if (use_charges_if_available("UPS_off", 1 + weapon.charges)) {
     weapon.poison++;
    } else {
...
bool player::use_charges_if_available(itype_id it, int quantity)
{
