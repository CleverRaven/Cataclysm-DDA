...
void player::update_bodytemp(game *g) // TODO bionics, diseases and humidity (not in yet) can affect body temp.
{
    ...
    for (int i = 0 ; i < num_bp ; i++)
    {
        ...
        {
            temp_equalizer(bp_head, bp_eyes);
        ...
        {
            add_disease(dis_type(cold_pen), 1, g, 3, 3);
            frostbite_timer[i] += 3;
        }
        else if (temp_cur[i] < BODYTEMP_VERY_COLD)
        {
            add_disease(dis_type(cold_pen), 1, g, 2, 3);
            frostbite_timer[i] += 2;
        }
        else if (temp_cur[i] < BODYTEMP_COLD)
        {
            add_disease(dis_type(cold_pen), 1, g, 1, 3);    // Frostbite timer does not go down if you are still cold.
            frostbite_timer[i] += 1;
        }
        else if (temp_cur[i] > BODYTEMP_SCORCHING)
        {
            add_disease(dis_type(hot_pen),  1, g, 3, 3);    // If body temp rises over 15000, disease.cpp (DI_HOT_HEAD) acts weird and the player will die
        }
...
int player::encumb(body_part bp, int &layers, int &armorenc, int &warmth)
{
    ...
    {
    case bp_feet  :
        if (!(is_wearing("socks") || is_wearing("socks_wool"))) break;
        else layers--;
    case bp_legs  :
        if (!is_wearing("long_underpants")) break;
        else layers--;
    case bp_hands :
        if (!is_wearing("gloves_liner")) break;
        else layers--;
    case bp_torso :
        if (!is_wearing("under_armor")) break;
        else layers--;
    }
    if (layers > 1)
