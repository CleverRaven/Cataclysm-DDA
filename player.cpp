...
{
 int ret = light_level;
 if (has_active_bionic(bio_night_vision) && ret < 12)
  ret = 12;
...
void player::suffer(game *g)
{
 ...
 if (!has_disease(DI_SLEEP)) {
  ...
  if (has_trait(PF_MOODSWINGS) && one_in(3600)) {
   int shift = rng(-30, 25);
   if (shift < 0)
    g->add_msg("You suddenly feel depressed.");
   else if (shift > 0)
    g->add_msg("You suddenly feel ecstatic!");
   morale += shift;;
  }
