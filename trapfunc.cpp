...
void trapfunc::sinkhole(game *g, int x, int y)
{
 ...
 if (g->u.has_amount(itm_rope_30, 1) &&
  ...
  int throwroll = rng(g->u.skillLevel(Skill::skill("throw")).level(),
                      g->u.skillLevel(Skill::skill("throw")).level() + g->u.str_cur + g->u.dex_cur);
  if (throwroll >= 12) {
   g->add_msg("The rope catches something!");
   if (rng(g->u.skillLevel(Skill::skill("unarmed")).level(),
           g->u.skillLevel(Skill::skill("unarmed")).level() + g->u.str_cur) > 6) {
