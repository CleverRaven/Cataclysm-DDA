...
// Creature::add_effect
// Character::add_effect: (wraps Creature::add_effect)
// monster::add_effect: (wraps Creature::add_effect without specific body part)
//
// TODO:
// Parameters:
// - Required: id, duration
// - Optional: bodypart, permanent, intensity
...
// - Mutate to a main body_part if necessary(??)
...
TEST_CASE( "monster add_effect", "[creature][effect][add]" )
{
    ...
    GIVEN( "monster is susceptible to effect" ) {
        ...
        WHEN( "monster add_effect is called" ) {
            mummy.add_effect( effect_grabbed, 1_minutes );
            THEN( "monster has_effect is true" ) {
                CHECK( mummy.has_effect( effect_grabbed ) );
            }
        }
    }
    ...
    GIVEN( "monster is immune to effect" ) {
        ...
        THEN( "monster add_effect is called with force = false" ) {
            ...
            THEN( "monster has_effect is false" ) {
