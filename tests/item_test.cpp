...
TEST_CASE( "uncraft_sanity_check", "[item]" )
{
    ...
    for( const itype *type : all_items ) {
        ...
        if( !is_within_tolerance ) {
            INFO( string_format( "Item %s weight %s gram, but it's uncrafting recipe (including crafting with 'reversible: false') has components with total weight of %s gram.  It should be within %.0f%%.",
