...
{
    item &gun = *shooter.get_wielded_weapon();
...
{
    {
        ...
        INFO( "Max aim speed: " << shooter.aim_per_move( *shooter.get_wielded_weapon(), MAX_RECOIL ) );
        INFO( "Min aim speed: " << shooter.aim_per_move( *shooter.get_wielded_weapon(), shooter.recoil ) );
...
static void test_fast_shooting( npc &shooter, const int moves, float hit_rate )
{
    ...
    INFO( "Min aim speed: " << shooter.aim_per_move( *shooter.get_wielded_weapon(), shooter.recoil ) );
    CAPTURE( shooter.ranged_dispersion_modifier() );
    CAPTURE( shooter.get_wielded_weapon()->gun_skill().str() );
    CAPTURE( shooter.get_skill_level( shooter.get_wielded_weapon()->gun_skill() ) );
    CAPTURE( shooter.get_dex() );
    CAPTURE( to_milliliter( shooter.get_wielded_weapon()->volume() ) );
