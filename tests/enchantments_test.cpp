...
static void test_generic_ench( avatar &p, enchant_test enc_test )
{
    ...
    CHECK( get_talker_for( p )->trial_chance_mod( "lie" ) == static_cast<int>( ( enc_test.lie_before + 15 ) * 1.5 ) );
    CHECK( get_talker_for( p )->trial_chance_mod( "persuade" ) == static_cast<int>( ( enc_test.persuade_before + 15 ) *
           1.5 ) );
    CHECK( get_talker_for( p )->trial_chance_mod( "intimidate" ) == static_cast<int>( ( enc_test.intimidate_before + 1 )
           * 1.5 ) );
