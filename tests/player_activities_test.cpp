...
TEST_CASE( "boltcut", "[activity][boltcut]" )
{
    ...
    auto setup_dummy = [&dummy]() -> item_location {
        ...
        REQUIRE( dummy.get_wielded_weapon()->typeId() == itype_test_boltcutter );

        return item_location{dummy, dummy.get_wielded_weapon()};
    };
    ...
    SECTION( "boltcut turn checks" ) {
        GIVEN( "player is in mid activity" ) {
            ...
            REQUIRE( dummy.get_wielded_weapon()->typeId() == itype_test_boltcutter_elec );

            item_location boltcutter_elec{dummy, dummy.get_wielded_weapon()};
