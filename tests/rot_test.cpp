...
TEST_CASE( "Rate_of_rotting", "[rot]" )
{
    SECTION( "Passage of time" ) {
        ...
        normal_item.process( get_map(), nullptr, tripoint::zero, 1, temperature_flag::NORMAL );
        sealed_item.process( get_map(), nullptr, tripoint::zero, 1, temperature_flag::NORMAL );
        freeze_item.process( get_map(), nullptr, tripoint::zero, 1, temperature_flag::NORMAL );
        ...
        sealed_item.process( get_map(), nullptr, tripoint::zero, 1, temperature_flag::NORMAL );
        normal_item.process( get_map(), nullptr, tripoint::zero, 1, temperature_flag::NORMAL );
        freeze_item.process( get_map(), nullptr, tripoint::zero, 1, temperature_flag::FREEZER );
...
TEST_CASE( "Items_rot_away", "[rot]" )
{
    SECTION( "Item in reality bubble rots away" ) {
        ...
        test_item.process( get_map(), nullptr, tripoint::zero, 1, temperature_flag::HEATER );

        // Set rot to >2 days and process again. process_temperature_rot should return true.
        calendar::turn += 20_minutes;
        test_item.mod_rot( 2_days );

        CHECK( test_item.process_temperature_rot( 1, tripoint::zero, get_map(), nullptr,
