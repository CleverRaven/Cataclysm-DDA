...
static void set_player_bmi( Character &dummy, float bmi )
{
    ...
    REQUIRE( dummy.get_bmi_fat() == Approx( bmi ).margin( 0.001f ) );
}
...
TEST_CASE( "stored kcal ratio influences body mass index", "[biometrics][kcal][bmi]" )
{
    ...
    CHECK( true_bmi_at_kcal_ratio( dummy, 0.6f ) == Approx( 21.0f ).margin( 0.01f ) );
    // Normal (fat bmi 3.6-5)
    CHECK( true_bmi_at_kcal_ratio( dummy, 0.7f ) == Approx( 21.5f ).margin( 0.01f ) );
