...
Item_spawn_data_dissect_cattle_sample_single( "dissect_cattle_sample_single" );
                         ...
                         int *sample_count, int *other_count )
{
    ...
    for( int i = 0; i < max_iters; i++ ) {
        ...
        for( const item &it : here.i_at( cow_loc ) ) {
            if( it.is_bionic() ) {
                ( *cbm_count )++;
            } else if( item_group::group_contains_item( Item_spawn_data_dissect_cattle_sample_single,
                       it.typeId() ) ) {
                ( *sample_count )++;
            } else {
                ( *other_count )++;
