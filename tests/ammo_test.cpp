...
TEST_CASE( "battery_energy_test", "[ammo][energy][item]" )
{
    ...
    SECTION( "Integer drain from battery" ) {
        ...
        units::energy consumed = test_battery.energy_consume( 40_kJ, tripoint_bub_ms_zero, nullptr );
    ...
    SECTION( "Integer over-drain from battery" ) {
        REQUIRE( test_battery.energy_remaining( nullptr ) == 56_kJ );
        units::energy consumed = test_battery.energy_consume( 400_kJ, tripoint_bub_ms_zero, nullptr );
    ...
    SECTION( "Non-integer drain from battery" ) {
        ...
        REQUIRE( test_battery.energy_remaining( nullptr ) == 56_kJ );
        units::energy consumed = test_battery.energy_consume( 4500_J, tripoint_bub_ms_zero, nullptr );
    ...
    SECTION( "Tiny Non-integer drain from battery" ) {
        ...
        REQUIRE( test_battery.energy_remaining( nullptr ) == 56_kJ );
        for( int i = 0; i < 133; ++i ) {
            units::energy consumed = test_battery.energy_consume( 2_J, tripoint_bub_ms_zero, nullptr );
            CHECK( consumed == 2_J );
        }
    ...
    SECTION( "Non-integer over-drain from battery" ) {
        REQUIRE( test_battery.energy_remaining( nullptr ) == 56_kJ );
        units::energy consumed = test_battery.energy_consume( 500500_J, tripoint_bub_ms_zero, nullptr );
        CHECK( test_battery.energy_remaining( nullptr ) == 0_kJ );
        CHECK( consumed == 56_kJ );
    }
    ...
    SECTION( "zero drain from battery" ) {
        REQUIRE( test_battery.energy_remaining( nullptr ) == 56_kJ );
        units::energy consumed = test_battery.energy_consume( 0_J, tripoint_bub_ms_zero, nullptr );
