...
TEST_CASE( "item_contents" )
{
    ...
    CHECK( !tool_belt.can_contain( hammer ).success() );

    tool_belt.force_insert_item( hammer, item_pocket::pocket_type::CONTAINER );
    ...
    tool_belt.remove_items_if( []( item & it ) {
    ...
    CHECK( tool_belt.contents_empty() );
}
...
TEST_CASE( "overflow on combine", "[item]" )
{
    ...
    capture_debugmsg_during( [&purse, &overfull_contents]() {
        purse.combine_contents( overfull_contents );
    } );
    map &here = get_map();
    here.i_clear( origin );
