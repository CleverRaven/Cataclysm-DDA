           ...
           "[craft][attention][reservation][enforcement]" )
{
    ...
    GIVEN( "ground pot bound by a live step" ) {
        here.add_item( pot_pos, item( itype_pot ) );
    ...
    GIVEN( "a carried pot bound by a live step" ) {
        ...
        THEN( "crafter's own carried provider is hidden too" ) {
            u.invalidate_crafting_inventory();
            CHECK_FALSE( u.crafting_inventory( origin,
                                               PICKUP_RANGE ).has_quality( qual_BOIL, 1, 1 ) );
        }
    ...
    GIVEN( "a bound pot inside a worn container" ) {
        ...
        THEN( "whole container is pruned, since the guard cuts at the root" ) {
            u.invalidate_crafting_inventory();
            CHECK_FALSE( u.crafting_inventory( origin,
                                               PICKUP_RANGE ).has_quality( qual_BOIL, 1, 1 ) );
        }
           ...
           "[craft][attention][reservation][enforcement]" )
{
    ...
    GIVEN( "a live site-only craft inside a container" ) {
        ...
        // site-only recipe binds nothing, so contains_reserved can't stand in for
        // craft check here.
        REQUIRE( nested->get_reservations().empty() );
        REQUIRE_FALSE( craft_reservation::contains_reserved( bag_loc ) );

        THEN( "the container reads as holding a live craft" ) {
            CHECK( craft_reservation::contains_live_craft( bag_loc ) );
        }
    ...
    GIVEN( "same container when craft is not running" ) {
        REQUIRE( nested->get_passive_started_at() == calendar::before_time_starts );

        THEN( "nothing about it is off limits" ) {
            CHECK_FALSE( craft_reservation::contains_live_craft( bag_loc ) );
            std::vector<item_location> no_activity_items;
           ...
           "[craft][attention][reservation][npc]" )
{
    ...
    GIVEN( "claimed food item and a free one in the same pack" ) {
        ...
        THEN( "free meal is still offered, since eating is node-local" ) {
           ...
           "[craft][attention][reservation][npc][pickup]" )
{
    ...
    // Binding picks one, the other is the free instance for the whole case
    ...
    GIVEN( "a stack holding one bound provider and one free item" ) {
        WHEN( "the executor sweeps the tile" ) {
            ...
            THEN( "bound provider is left where it lies" ) {
    ...
    GIVEN( "same stack in vehicle cargo" ) {
        ...
        WHEN( "the executor sweeps the cargo" ) {
            ...
            THEN( "claimed item stays in cargo space" ) {
           ...
           "[craft][attention][reservation][npc]" )
{
    ...
    GIVEN( "reserved furniture in NPC's way" ) {
    ...
    GIVEN( "craft site under a bashable tile" ) {
    ...
    GIVEN( "unreserved bashable tile" ) {
        const tripoint_bub_ms free_pos( 63, 60, 0 );
        here.furn_set( free_pos, furn_test_f_reserve_qual );

        THEN( "bashing is still allowed, so the guard didn't disable it wholesale" ) {
    ...
    GIVEN( "a passable tile holding a reserved item" ) {
        THEN( "it's fine to walk over it, since only bashing destroys" ) {
           ...
           "[craft][attention][reservation][farm]" )
{
    ...
    GIVEN( "unclaimed fertilizer" ) {
        ...
        WHEN( "the fertilize actor finishes" ) {
            ...
            THEN( "it's consumed, so filter didn't over-block" ) {
           ...
           "[craft][attention][reservation][quality]" )
{
    ...
    GIVEN( "a bound high-level provider beside a free lower-level one" ) {
        ...
        THEN( "selection returns the free one, not the bound better one" ) {
        ...
        THEN( "unfiltered pair still offers the bound one, so its owner can use it" ) {
           ...
           "[craft][attention][reservation][npc]" )
{
    ...
    // weapon selectors only fire when NPC thinks it's in danger
    arm_shooter( player_character, itype_M24 );

    GIVEN( "NPC carrying a reserved good weapon and a free worse one" ) {
    ...
    GIVEN( "a free container in the NPC's pack whose contents are reserved" ) {
        ...
        THEN( "it passes over the whole subtree, not just the reserved item" ) {
            const item *best = hostile.evaluate_best_weapon();
            CHECK( ( best == nullptr || best->typeId() != itype_bat ) );
        }
    ...
    GIVEN( "same container with nothing reserved in it" ) {
        item backpack( itype_backpack );
        backpack.force_insert_item( item( itype_bat ), pocket_type::CONTAINER );
        hostile.i_add( backpack );
        hostile.regen_ai_cache();
        ...
        THEN( "the nested weapon is still reachable, so the guard hid it" ) {
            const item *best = hostile.evaluate_best_weapon();
           ...
           "[craft][attention][reservation][enforcement][vehicle]" )
{
    ...
    GIVEN( "welding rig beside the work spot, no welder carried" ) {
        vehicle *veh = here.add_vehicle( vehicle_prototype_none, beside, 0_degrees, 0,
                                         veh_spawn_status::UNDAMAGED );
        REQUIRE( veh != nullptr );
        REQUIRE( veh->install_part( here, point_rel_ms::zero, vpart_frame ) != -1 );
        ...
        WHEN( "supplying part is claimed by a craft" ) {
            claim_part( rig_uid );
    ...
    GIVEN( "faucet + full tank on one vehicle" ) {
        vehicle *veh = here.add_vehicle( vehicle_prototype_none, beside, 0_degrees, 0,
                                         veh_spawn_status::UNDAMAGED );
        REQUIRE( veh != nullptr );
        REQUIRE( veh->install_part( here, point_rel_ms::zero, vpart_frame ) != -1 );
        ...
        WHEN( "nothing is reserved" ) {
            ...
            ovp->form_inventory( here, inv, vehicles_found );

            THEN( "you can reach the water in the tank via the faucet" ) {
                CHECK( inv.charges_of( itype_water_clean ) == 100 );
            }
        }

        WHEN( "faucet part is claimed by craft" ) {
            claim_part( faucet_uid );
            temp_crafting_inventory inv;
            std::set<vehicle *> vehicles_found;
            ovp->form_inventory( here, inv, vehicles_found );

            THEN( "tank's water goes with it" ) {
           ...
           "[craft][attention][reservation][enforcement][automation]" )
{
    ...
    GIVEN( "a spill to mop and one mop in the pack" ) {
        // New water takes the tile's temperature at once. turn_zero's weather at
        // wherever an earlier test left the map can be below freezing.
        restore_on_out_of_scope restore_temp( get_weather().forced_temperature );
           ...
           "[craft][attention][reservation][charges]" )
{
    ...
    GIVEN( "a step whose quality binding and charge draw share a tool type" ) {
        ...
        WHEN( "free tool is empty, lease lapsed" ) {
            ...
            THEN( "the step doesn't drain the provider its own binding names" ) {
                CHECK( bound.ammo_remaining() == bound_charges );
            }
        }

        WHEN( "draw is sourced from both pools" ) {
            ...
            THEN( "shortfall is seen, not the bucket recorded unpaid" ) {
                CHECK( bound.ammo_remaining() == bound_charges );
           ...
           "[craft][attention][reservation][enforcement][vehicle]" )
{
    ...
    GIVEN( "two welding rigs on one mount" ) {
        vehicle *veh = here.add_vehicle( vehicle_prototype_none, beside, 0_degrees, 0,
                                         veh_spawn_status::UNDAMAGED );
        REQUIRE( veh != nullptr );
        REQUIRE( veh->install_part( here, point_rel_ms::zero, vpart_frame ) != -1 );
        ...
        WHEN( "first one is claimed by a craft" ) {
            craft_reservation_index::record rec;
            rec.craft_uid = 424242;
            rec.expires_at = calendar::turn + 1_hours;
