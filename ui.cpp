...
void uimenu::show() {
   if (!started) {
     ...
     if ( w_auto ) w_width = text.size()+4;
     bool h_auto = (w_height == -1);
     if ( h_auto ) w_height = 4;
     ...
     for ( int i = 0; i < entries.size(); i++ ) {
       if(entries[ i ].enabled) {
         ...
         if ( w_auto && w_width < entries[ i ].txt.size() + pad + 4 ) w_width = entries[ i ].txt.size() + pad + 4;
       } else {
         if ( w_auto && w_width < entries[ i ].txt.size() + pad + 4 ) w_width = entries[ i ].txt.size() + pad + 4; // todo: or +5 if header
       }
     }
...
void uimenu::query(bool loop) {
  ...
  do {
    ...
    if ( keypress == KEY_UP ) {
      ...
      if ( hilight_disabled==true ) iter=1;
      while ( iter > 0 ) {
        iter--;
