...
`"price_rules"`       | array, allows defining `premium`, `markup`, `price` and/or `fixed_adj` for items selected by `item`/`category`/`group`/`item_condition`.<br/><br/>`premium` is a price multiplier that applies to both sides.<br/> `markup` is only used when an NPC is selling to the avatar and defaults to `1`.<br/>`price` replaces the item's `price_postapoc`.<br/>`fixed_adj` is used instead of adjustment based on social skill and intelligence stat and can be used to define secondary currencies.<br/><br/>Lower entries override higher ones.<br/><br/>Entries can be enabled by a [`condition`](EFFECT_ON_CONDITION.md#condition). Entry condition uses the avatar as alpha and the evaluating npc as beta.<br/><br/>`item_condition` uses the avatar as alpha and the item as beta.
