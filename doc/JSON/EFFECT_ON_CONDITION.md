...
| character_takeoff_item | triggers when character removes a worn item. If using `run_inv_eocs`, remember that the event fires before the items are actually removed. Deleting the item to takeoff immediately here will cause a crash (using time_in_future to delay deletion is safe). | { "character", `character_id` },<br/> { "itype", `itype_id` } |character / item to takeoff |
