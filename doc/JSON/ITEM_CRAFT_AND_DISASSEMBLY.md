...
"name" : "with quern", // optional string to further describe recipe that will append to the result name. Example if the result name is "flour", the final name will be "flour with quern". Especially useful for recipes with "id_suffix" that produce the same result as other recipes.
