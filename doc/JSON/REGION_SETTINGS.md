...
A region often spans multiple overmaps (see doc/JSON/OVERMAP.md) or even the entire game world.
...
but if more than one mod is loaded, an extended `region_settings` object must be overwritten.
...
- `region_settings_highways`
- `region_settings_forest_trails`
...
By adding an "overlay_id" to the `map_extra_collection`, the above example would
