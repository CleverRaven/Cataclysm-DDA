...
- ```SNOWPLOW``` Bodies struck by this part are shoved off to the side instead of being thrown forwards, and the collision costs the vehicle far less of its speed, but only where the vehicle outweighs the body several times over; against anything in its own weight class the part rams as usual.  How far to the side, and how cheap the collision is, is set by `collision_deflection`.
