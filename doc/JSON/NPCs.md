...
| coord_x( `v` )<br/>coord_y( `v` )<br/>coord_z( `v` )  |  ✅  |  ✅  | g | Pass a variable with coordinates, and return or modify it's x, y or z coordinate. Example:<br/>`"condition: { "math": [ "coord_z(_pos) > 0 " ] }`<br/>`"math": [ "coord_z(_loc_to_teleport) =+ 2" ]`<br/>`"math": [ " coord_z(_loc_to_teleport) = 5" ]`
