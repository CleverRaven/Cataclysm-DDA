...
Effect | Required | Optional | Description 
---|---|---|---
`u_add_effect`<br />`npc_add_effect`  | `effect_id`[^string]<br />`duration`[^time][^var] | `target_part`[^string]<br />`intensity`[^int][^var]<br />`force`[^bool] | Target will gain the [effect](./EFFECTS_JSON.md) (not script effect) for `duration`, at `intensity` 0 if not specified. If `force_bool` is `true` immunity will be ignored. If `target_part` is supplied that body part will get the effect, rather than the whole body.<br />
Special arguments: `target_part` accepts `RANDOM`; `duration` accepts `"PERMANENT"`.
`u_lose_effect`<br/>`npc_lose_effect` | `effect_id`[^string] | None | Target will lose the effect if they have it.
`u_add_wet`<br/>`npc_add_wet` | `wetness`[^int][^var] | Target will be wettened by specified amount as if they were in the rain.
