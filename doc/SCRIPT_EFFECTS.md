...
Effect | Required | Optional | Description 
---|---|---|---
`custom_light_level` | `custom_light_level`[^int][^var] `duration`[^time][^var] | None | Sets the ambient light from the sun/moon to be `custom_light_level`.  This can vary naturally between 0 and 125.  This lasts `duration`.
...
`revert_location` | `location`[^var]<br />`time_in_future`[^time][^var] | None | `revert_location` stores a location variable.  The terrain, furniture, and traps at that location will be saved and restored at `time_in_future`.
`u_set_field`<br />`npc_set_field` | `field_id`[^string] | `intensity`[^int][^var]<br />`age`[^int][^var]<br />`radius`[^int][^var]<br />`outdoor_only`[^bool]<br />`hit_player`[^bool]<br />`target`[^var] | Add a field centered on you or the npc, of type `field_type_id`, of intensity `intensity` ( default 1), of radius `radius` (defaults to 10000000), and age `age` (defaults 1).  If `outdoor_only` is true (default false) it will only spawn outdoors.  If `hits_player` is false (default true) the field then if the newly spawned field includes the player's location the player will not be treated as having just entered the field. If `target` is a location variable, the effect will be centered on it.
`u_spawn_monster`<br />`npc_spawn_monster`| `mtype_id`[^string] | `group`[^bool]<br />`hallucination_count`[^int][^var]<br />`real_count`[^int][^var]<br />`min_radius`[^int][^var]<br />`max_radius`[^int][^var]<br />`outdoor_only`[^bool]<br />`target_range`[^int][^var]<br />`lifespan`[^string][^var]<br />`target`[^var]<br />`spawn_message`[^string]<br />`spawn_message_plural`[^string]<br />`true_eocs`[^string][^array]<br />`false_eocs`[^string][^array] | Spawns monsters of type `mtype_id` near you or npc.  The number is specified in `real_count` and `hallucination_count` (the latter adds hallucinatory monsters).<br /><br />If `group` is true (default false) a random monster from monster_group `mtype_id` will be used.<br />If `mtype_id` is empty it will spawn a random monster within `target_range` spaces of you.<br />The spawns will happen between `min_radius` (default 1) and `max_radius` (default 10) spaces of the target.<br />If `outdoor_only` is true (default false) the spawner will only choose outdoor spaces.<br />If `lifespan` is provided the monster will disappear after that time.<br />If `target` is a location variable, this effect will be centered on it.<br />If at least one spawned creature is visible `spawn_message` will be displayed; if `spawn_message_plural` is defined and more than one spawned creature is visible, it will be used instead.<br />If any monster is spawned, then all of the effect_on_conditions in `true_eocs` are run, otherwise those in `false_eocs` are run.
`u_transform_radius`<br />`npc_transform_radius` | `transform_radius`[^int][^var]<br />`ter_furn_transform`[^string] | `target`[^var]<br />`time_in_future`[^time][^var] | Applies the `ter_furn_transform` of specified id in radius `translate_radius`. If `target` is a location variable, the effect centers on it.  If `time_in_future` is set the transform will that far in the future, in this case however the target location and radius will be determined now and not changed even if their variables update.
