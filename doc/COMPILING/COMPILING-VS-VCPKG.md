...
4. (Optional) Install (or upgrade) neccessary packages with following command line:
...
2. Open the provided solution (`msvc-full-features\Cataclysm-vcpkg-static.sln`) in `Visual Studio`, select configuration (`Release` or `Debug`) and platform (`x64` or `x86`) and build it. All dependencies should be built by vcpkg automatically.
