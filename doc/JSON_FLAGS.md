...
- ```GRAB``` This effect is a grab, creatures will attempt to break it as such
- ````GRAB_FILTER``` This effect is a grab filter effect, assigning grabs to their grabbing monster.  No mechanical effect on its own, but bodypart `grabbing_effects` should have it defined.
