...
`u_translate_radius, npc_translate_radius: translate_radius_int or translate_radius_variable_object, to: to_string, from: from_string, `(*optional* `toggle: toggle_bool`), (*optional*` same_submap:  same_submap_bool`) | Changes all terrain of type `from` in radius `translate_radius` into type `to`. If `toggle`(default false) is true the change will go both ways, if `same_submap`(default false) is true the change will be limited to the current submap
