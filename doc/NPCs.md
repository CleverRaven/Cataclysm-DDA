...
`u_transform_radius, npc_transform_radius: transform_radius_int or transform_radius_variable_object, ter_furn_transform: ter_furn_transform_string`, (*optional*` same_submap:  same_submap_bool`) | Applies the ter_furn_transform of id `ter_furn_transform`(See [the transform docs](TER_FURN_TRANSFORM.md))  in radius `translate_radius`. If `same_submap`(default false) is true the change will be limited to the current submap.
