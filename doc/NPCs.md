...
| spell_count(`s`/`v`)    |  ✅   |   ✅  | u, n  | Return number of spells the character knows.<br/> Variable is school ID.(Optional) If given, return number of spells known of that school.<br/> Example:<br/>`"condition": { "math": [ "u_spell_count('MAGUS')", ">=", "10"] }`|
...
| spell_level(`s`/`v`)    |  ✅   |   ✅  | u, n  | Return or set level of a given spell. -1 means the spell is not known when read and that the spell should be forgotten if written.<br/>Variable is spell ID.(Optional) If omitted, return the highest level of spells the character knows.<br/> Example:<br/>`"condition": { "math": [ "u_spell_level('SPELL_ID')", "==", "-1"] }`|
