...
| distance(`s`/`v`,`s`/`v`)    |  ✅   |   ❌  | g  | Return distance between two targets.<br/> Variables are location variables or special strings (`u`, `npc`). `u` means your location. `npc` means NPC's location.<br/><br/> Example:<br/>`"condition": { "math": [ "distance('u', loc)", "<=", "50"] }`|
