...
`u_transform_radius, npc_transform_radius: transform_radius_int or transform_radius_variable_object, ter_furn_transform: ter_furn_transform_string`, (*optional* `set_var: set_var_object`), (*optional* `target_var: target_var_object`) | Applies the ter_furn_transform of id `ter_furn_transform` (See [the transform docs](TER_FURN_TRANSFORM.md)) in radius `translate_radius`. If `set_var` is set the location this is centered on will be saved to a variable with its name.  If `target_var` is set this effect will be centered on a location saved to a variable with its name.  Both `set_var` and `target_var` are objects with `value`,`type` and `context` as string values.
