...
`run_eoc_with :` effect_on_condition_array or single effect_condition_object, `variables :` Object with variable names and values as pairs, `names: ` string or variables, `keys: ` single character strings, `title: ` string | Opens a menu with title `title` that lets you select one of the EOCs, they each get a matched `names` and `keys` value if provided, selected EOC runs with the provided variables as context variables.  EOC should either be the id of an effect_on_condition or an inline effect_on_condition.
