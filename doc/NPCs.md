...
| skill_exp(`s`/`v`, `s`/`v`)    |  ✅  |   ✅   | u, n  | Return or set skill experience<br/> Variables are skill ID, format (optional). Format must be `true` (as raw value) or `false` (as percentage, default)<br/> Example:<br/>`{ "math": [ "u_skill_exp('driving')", "+=", "10"] }`|
