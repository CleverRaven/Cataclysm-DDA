...
void complete_vehicle (game *g)
{
    ...
    case 'i':
    {
        bool tire_change = type == SEL_JACK && veh->part_flag(part, vpf_wheel);
        // Handle "tire change" first so mount point will be free for new tire.
        if( tire_change ) {
            ...
            if( parts.size() ) {
                ...
                if (!broken) {
                    used_item = veh->item_from_part( replaced_wheel );
