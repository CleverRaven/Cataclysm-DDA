...
function handle_previous_version()
    ...
    local character_eligible_to_migration = false
    local calendar = game.get_calendar_turn()
    if( function_exists( "calendar.days" ) or calendar:years() >= 1 ) then
        character_eligible_to_migration =  = game.get_calendar_turn():days() >= 1
    else
        character_eligible_to_migration = calendar:years() >= 1
    end
    if( (not loaded_version or loaded_version < 2) and character_eligible_to_migration ) then --handle upgrading from original StatsThroughSkills to version 2
