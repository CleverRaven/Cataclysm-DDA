...
void addict_effect(game *g, addiction &add)
{
 ...
 case ADD_CIG:
  if (in > 20 || one_in((500 - 20 * in))) {
   ...
   g->u.add_morale("Nicotine Craving", -15, -50);
   if (one_in(800 - 50 * in))
    g->u.fatigue++;
   if (one_in(400 - 20 * in))
    g->u.stim--;
 ...
 case ADD_ALCOHOL:
  ...
  if (one_in(20) && rng(0, 20) < in) {
   ...
   g->u.add_morale("Alcohol Craving", -35, -120);
...
// EXCEPT!  Prolong this addiction longer than usual.
 ...
 case ADD_SPEED:
  ...
  if (dice(2, 100) < in) {
   ...
   g->u.add_morale("Speed Craving", -25, -200);
 ...
 case ADD_COKE:
  ...
  if (in >= 30 || one_in((900 - 30 * in))) {
   ...
   g->u.add_morale("Cocaine Craving", -20, -250);
  }
  if (dice(2, 80) <= in) {
   g->add_msg("You feel like you need a bump.");
   g->cancel_activity_query("You have a craving for cocaine.");
