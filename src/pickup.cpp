...
void Pickup::pick_one_up( const point &pickup_target, std::vector<item> &here, vehicle *veh,
                          ...
                          std::map<std::string, item> &item_invlet, bool autopickup )
{
    int moves_taken = 100;
    ...
    if( newit.made_of(LIQUID) ) {
        ...
        item_invlet[newit.tname()] = g->u.i_add(newit);
        picked_up = true;
    }
                        ...
                        std::list<int> &indices, std::list<int> &quantities, bool autopickup )
{
    ...
    std::map<std::string, item> item_invlet;
                                    ...
                                    bool &picked_up)
{
    ...
    if(quivered > 0) {
        ...
        item_invlet.insert(std::pair<std::string, item>(here.tname(), it));
                                 ...
                                 std::map<std::string, item> &item_invlet)
{
    ...
    std::map<std::string, item>::iterator ii_iter = item_invlet.begin();
