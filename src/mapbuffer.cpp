        ...
        while( !jsin.end_object() ) {
            ...
            if( submap_member_name == "version" ) {
                    ...
                    for( int j = 0; j < SEEY; j++ ) {
                        for( int i = 0; i < SEEX; i++ ) {
                            ...
                            if ( tid == "t_rubble" ) {
                                ...
                                sm->frn[i][j] = furnmap[ furn_str_id( "f_rubble" ) ].loadid;
                                sm->itm[i][j].push_back( rock );
                                sm->itm[i][j].push_back( rock );
                            } else if ( tid == "t_wreckage" ){
                                sm->ter[i][j] = ter_id( "t_dirt" );
                                sm->frn[i][j] = furnmap[ furn_str_id( "f_wreckage" ) ].loadid;
                                sm->itm[i][j].push_back( chunk );
                                sm->itm[i][j].push_back( chunk );
                            } else if ( tid == "t_ash" ){
                                sm->ter[i][j] = ter_id(  "t_dirt" );
                                sm->frn[i][j] = furnmap[ furn_str_id( "f_ash" ) ].loadid;
                ...
                while( !jsin.end_array() ) {
                    ...
                    int i = jsin.get_int();
                    int j = jsin.get_int();
                    sm->frn[i][j] = furnmap[ furn_str_id( jsin.get_string() ) ].loadid;
                    jsin.end_array();
                }
