                                  ...
                                  bool switch_ammo, bool tiny )
{
    ...
    if( mode == TARGET_MODE_FIRE ) {
        mvwprintz( w_target, text_y++, 1, c_white, _( "[%c] to steady your aim. (10 moves)" ), front_or( "AIM", ' ' ) );
    ...
    for( const aim_type &type : aim_types ) {
        ...
        if( display_type == "numbers" ) {
            ...
            [&]( const confidence_rating & config ) {
                ...
                last_chance += chance;
                return string_format( "%s: <color_%s>%3d%%</color>", _( config.label/*.c_str()*/ ), config.color, chance );
...
static int print_aim( const player &p, const catacurses::window &w, int line_number, input_context &ctxt, item *weapon,
...
static int draw_throw_aim( const player &p, const catacurses::window &w, int line_number, input_context &ctxt,
