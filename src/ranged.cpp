...
static const ammo_effect_str_id ammo_effect_id_CUSTOM_EXPLOSION( "CUSTOM_EXPLOSION" );
static const ammo_effect_str_id ammo_effect_id_EMP( "EMP" );
static const ammo_effect_str_id ammo_effect_id_EXPLOSIVE( "EXPLOSIVE" );
...
static const ammo_effect_str_id ammo_effect_id_LASER( "LASER" );
static const ammo_effect_str_id ammo_effect_id_LIGHTNING( "LIGHTNING" );
static const ammo_effect_str_id ammo_effect_id_MATCHHEAD( "MATCHHEAD" );
static const ammo_effect_str_id ammo_effect_id_MULTI_EFFECTS( "MULTI_EFFECTS" );
static const ammo_effect_str_id ammo_effect_id_NON_FOULING( "NON_FOULING" );
...
static const ammo_effect_str_id ammo_effect_id_PLASMA( "PLASMA" );
...
static const ammo_effect_str_id ammo_effect_id_SHOT( "SHOT" );
static const ammo_effect_str_id ammo_effect_id_TANGLE( "TANGLE" );
static const ammo_effect_str_id ammo_effect_id_WHIP( "WHIP" );
static const ammo_effect_str_id ammo_effect_id_WIDE( "WIDE" );
...
bool Character::handle_gun_damage( item &it )
{
    ...
    } else if( curammo_effects.count( ammo_effect_id_RECYCLED ) && one_in( 256 ) ) {
    ...
    } else if( it.has_flag( flag_CONSUMABLE ) && !curammo_effects.count( ammo_effect_id_LASER ) &&
               !curammo_effects.count( ammo_effect_id_PLASMA ) && !curammo_effects.count( ammo_effect_id_EMP ) ) {
    ...
    if( !it.has_flag( flag_PRIMITIVE_RANGED_WEAPON ) ) {
        ...
        if( !curammo_effects.count( ammo_effect_id_NON_FOULING ) && !it.has_flag( flag_NON_FOULING ) ) {
            if( dirt < static_cast<int>( dirt_max_dbl ) ) {
                dirtadder = curammo_effects.count( ammo_effect_id_BLACKPOWDER ) * ( 200 -
                ...
                if( curammo_effects.count( ammo_effect_id_BLACKPOWDER ) ||
                    dirt < 7150 ) {
            ...
            if( dirt > 0 && curammo_effects.count( ammo_effect_id_BLACKPOWDER ) ) {
    ...
    if( ( dirt_dbl > 5000 && x_in_y( dirt_dbl * dirt_dbl * dirt_dbl, 5555555555555 ) ) ||
        ( curammo_effects.count( ammo_effect_id_MATCHHEAD ) && one_in( 256 ) ) ) {
...
int Character::fire_gun( const tripoint &target, int shots, item &gun, item_location ammo )
{
    ...
    if( gun.has_flag( flag_CHOKE ) && !gun.ammo_effects().count( ammo_effect_id_SHOT ) ) {
        ...
        const std::optional<tripoint> &blind_throw_from_pos )
{
    ...
    if( thrown.has_flag( flag_ACT_ON_RANGED_HIT ) ) {
        proj_effects.insert( ammo_effect_id_ACT_ON_RANGED_HIT );
        thrown.active = true;
    }
    ...
    if( weight > 500_gram ) {
        proj_effects.insert( ammo_effect_id_HEAVY_HIT );
    }

    proj_effects.insert( ammo_effect_id_NO_ITEM_DAMAGE );

    if( thrown.active ) {
        // Can't have Molotovs embed into monsters
        // Monsters don't have inventory processing
        proj_effects.insert( ammo_effect_id_NO_EMBED );
    }

    if( do_railgun ) {
        proj_effects.insert( ammo_effect_id_LIGHTNING );
    ...
    if( thrown.has_flag( flag_TANGLE ) ) {
        proj_effects.insert( ammo_effect_id_TANGLE );
    }
...
static projectile make_gun_projectile( const item &gun )
{
    ...
    if( ( gun.ammo_data() && gun.ammo_data()->phase == phase_id::LIQUID ) ||
        fx.count( ammo_effect_id_SHOT ) || fx.count( ammo_effect_id_BOUNCE ) ) {
        fx.insert( ammo_effect_id_WIDE );
    }
    ...
    if( gun.ammo_data() ) {
        ...
        if( recover && !fx.count( ammo_effect_id_IGNITE ) && !fx.count( ammo_effect_id_EXPLOSIVE ) ) {
            ...
            drop.active = fx.count( ammo_effect_id_ACT_ON_RANGED_HIT );
        ...
        if( fx.count( ammo_effect_id_MULTI_EFFECTS ) ) {
        ...
        if( fx.count( ammo_effect_id_CUSTOM_EXPLOSION ) > 0 ) {
...
static void cycle_action( item &weap, const itype_id &ammo, const tripoint &pos )
{
    ...
    if( !!ammo->ammo->casing ) {
        ...
        if( ( *ammo->ammo ).ammo_effects.count( ammo_effect_id_BLACKPOWDER ) ) {
...
item::sound_data item::gun_noise( const bool burst ) const
{
    ...
    if( fx.count( ammo_effect_id_LASER ) || fx.count( ammo_effect_id_PLASMA ) ) {
        if( noise < 20 ) {
            return { noise, _( "Fzzt!" ) };
        } else if( noise < 40 ) {
            return { noise, _( "Pew!" ) };
    ...
    } else if( fx.count( ammo_effect_id_LIGHTNING ) ) {
        if( noise < 20 ) {
            return { noise, _( "Bzzt!" ) };
        } else if( noise < 40 ) {
            return { noise, _( "Bzap!" ) };
    ...
    } else if( fx.count( ammo_effect_id_WHIP ) ) {
        return { noise, _( "Crack!" ) };
