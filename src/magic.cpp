        ...
        case item_energy: {
            ...
            if ( !a_filter.empty() ) {
                if ( a_filter[ 0 ]->count_by_charges() ) {
                    return player_inv.has_charges(id, energy_cost ( p ) );
                }
                else {
                    return static_cast<int>( a_filter.size() ) > energy_cost( p );
                }
            }
            else {
                return false;
...
std::string spell::energy_cur_string( const player &p ) const
{
    ...
    if ( energy_source() == item_energy ) {
        if ( energy_cost( p ) == 0 ) {
            return colorize( "Infinite", c_white );
        }
        const itype_id id = energy_item();
        const inventory& player_inv = p.inv;
        std::vector<const item*> a_filter = player_inv.items_with( [id] (const item& it ) {
...
bool known_magic::has_enough_energy( const player &p, spell &sp ) const
{
    ...
    switch( sp.energy_source() ) {
        ...
        case item_energy: {
            ...
            std::vector<const item*> a_filter = p.inv.items_with( [id](const item& it ) {
                return it.typeId() == id;
                } );

            int count = 0;
            for ( const item * it : a_filter ) {
                if ( it->count_by_charges( id ) ) {
                    count += it->charges;
                }
                else {
                    count++;
