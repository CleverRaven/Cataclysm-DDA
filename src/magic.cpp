...
class spellcasting_callback : public uilist_callback
{
    ...
    public:
        ...
        void refresh( __attribute__((unused)) uilist *menu ) override {
            ...
            ImGui::TextColored( cataimgui::imvec4_from_color( casting_ignore ? red : green ),
                                "%s %s", "[I]", ignore_string.c_str() );
...
void spellcasting_callback::display_spell_info( size_t index )
{
    ...
    if( ImGui::BeginTable( "data", 2 ) ) {
        ...
        ImGui::SameLine( 0, 0 );
        ImGui::TextColored( cataimgui::imvec4_from_color( green ), "%d", sp.xp() );
        ImGui::TableNextColumn();
        ImGui::Text( "%s: ", _( "to Next Level" ) );
        ImGui::SameLine( 0, 0 );
        ImGui::TextColored( cataimgui::imvec4_from_color( green ), "%d", sp.exp_to_next_level() );
    ...
    if( pc.magic->has_enough_energy( pc, sp ) ) {
        ...
        auto red = c_red;
        ImGui::TextColored( cataimgui::imvec4_from_color( red ), "%s", _( "Not Enough Stamina" ) );
        ImGui::SameLine( 0, 0 );
    ...
    if( sp.effect() == "attack" ) {
        if( damage > 0 ) {
            ...
            auto color = sp.damage_type_color();
            ImGui::TextColored( cataimgui::imvec4_from_color( color ),
                                "%s: %s %s%s", _( "Damage" ),
            ...
            ImGui::SameLine( 0, 0 );
            auto green = c_light_green;
            ImGui::TextColored( cataimgui::imvec4_from_color( green ),
...
static void draw_spellbook_info( const spell_type &sp )
{
    ...
    cataimgui::draw_colored_text( spell_name, gray );
    ImGui::SameLine();
    ...
    ImGui::TextColored( cataimgui::imvec4_from_color( yellow ), "%s", spell_class.c_str() );
    ...
    if( ImGui::BeginTable( "stats", 4,
        ...
        const dbl_or_var & inc_d, const dbl_or_var & max_d, bool check_minmax = false ) {
            ...
            ImGui::TableNextRow();
            ImGui::TableNextColumn();
            ImGui::TextColored( cataimgui::imvec4_from_color( gray ), "%s", label.c_str() );
            ImGui::TableNextColumn();
            cataimgui::draw_colored_text( color_number( min ), gray );
            ImGui::TableNextColumn();
            cataimgui::draw_colored_text( color_number( inc ), gray );
            ImGui::TableNextColumn();
            cataimgui::draw_colored_text( color_number( max ), gray );
        };
