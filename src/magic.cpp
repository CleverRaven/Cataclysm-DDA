...
void spell::gain_exp( const Character &guy, int nxp )
{
    ...
    if( guy.is_avatar() && oldLevel != get_level() ) {
        get_event_bus().send<event_type::player_levels_spell>( guy.getID(), id(), get_level(), spell_class() );
    }
}
    ...
    if( !knows_spell( sp ) ) {
        ...
        if( new_exp >= 0 ) {
            ...
            if( guy->is_avatar() && old_level != temp_sp.get_level() ) {
                get_event_bus().send<event_type::player_levels_spell>( guy->getID(), sp->id, temp_sp.get_level(), sp->spell_class);
            }
        } else {
            get_event_bus().send<event_type::character_forgets_spell>( guy->getID(), sp->id );
            spellbook.erase( sp );
