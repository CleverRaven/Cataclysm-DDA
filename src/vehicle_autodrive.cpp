...
{
    data.view_bounds = { {0, 0}, {NAV_VIEW_SIZE_X, NAV_VIEW_SIZE_Y} };
    data.nav_bounds = { {0, 0}, {NAV_MAP_SIZE_X, NAV_MAP_SIZE_Y} };
...
void vehicle::autodrive_controller::precompute_data()
{
    ...
    const tripoint_abs_omt next_omt = driven_veh.omt_path.back();
    const tripoint_abs_omt next_next_omt = driven_veh.omt_path.size() >= 2 ?
                                           driven_veh.omt_path[driven_veh.omt_path.size() - 2] : next_omt;
...
std::vector<std::tuple<point, int, std::string>> vehicle::get_debug_overlay_data() const
{
    ...
    for( const std::string &debug_str : debug_what ) {
        if( debug_str == "profiles" ) {
            ...
            static const std::vector<point> corners = {{0, 0}, {0, OMT_SIZE - 1}, {OMT_SIZE - 1, 0}, {OMT_SIZE - 1, OMT_SIZE - 1}};
