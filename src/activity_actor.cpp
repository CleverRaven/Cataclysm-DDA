...
bool tool_activity_actor::start_finish( map &here, player_activity &act, Character &who,
                                        ...
                                        const cata::value_ptr<activity_data_common> map_data_common_t::* const tool_member, bool finish,
                                        activity_data_common *&data )
{
    if( here.has_furn( target ) ) {
        const furn_id &furn_type = here.furn( target );
        const auto furn_tool_ad = ( *furn_type.*tool_member );
        if( !furn_tool_ad->valid() ) {
            if( !testing ) {
        ...
        if( finish ) {
            const furn_str_id new_furn = std::get<furn_str_id>( furn_tool_ad->result() );
            ...
            moves = to_moves<int>( ( *furn_type.*tool_member )->duration() );
        }
    } else if( const ter_id &ter_type = here.ter( target ); !ter_type->is_null() ) {
        const auto ter_tool_ad = ( *ter_type.*tool_member );
        if( !ter_tool_ad->valid() ) {
            if( !testing ) {
        ...
        if( finish ) {
            const ter_str_id new_ter = std::get<ter_str_id>( ter_tool_ad->result() );
            if( !new_ter.is_valid() ) {
                if( !testing ) {
            ...
            moves = to_moves<int>( ter_tool_ad->duration() );
        }
    } else {
        if( !testing ) {
...
void hacksaw_activity_actor::start( player_activity &act, Character &who )
{
    ...
    if( !start_finish( here, act, who, moves_before_quality, _( "hacksaw" ),
...
void hacksaw_activity_actor::finish( player_activity &act, Character &who )
{
    ...
    start_finish( here, act, who, moves_discard, _( "hacksaw" ), &map_data_common_t::hacksaw,
                  true, data );
}
...
void boltcutting_activity_actor::start( player_activity &act, Character &who )
{
    ...
    if( !start_finish( here, act, who, act.moves_total, _( "boltcut" ), &map_data_common_t::boltcut,
                       false, ad_discard ) ) {
        return;
    }
...
void boltcutting_activity_actor::finish( player_activity &act, Character &who )
{
    ...
    if( !start_finish( here, act, who, moves_discard, _( "boltcut" ), &map_data_common_t::boltcut,
                       true, data ) ) {
        return;
    }
...
void oxytorch_activity_actor::start( player_activity &act, Character &who )
{
    ...
    if( !start_finish( here, act, who, act.moves_total, _( "oxytorch" ), &map_data_common_t::oxytorch,
                       false, ad_discard ) ) {
        return;
    }
...
void oxytorch_activity_actor::finish( player_activity &act, Character &who )
{
    ...
    if( !start_finish( here, act, who, moves_discard, _( "oxytorch" ), &map_data_common_t::oxytorch,
                       true, data ) ) {
        return;
    }
...
void prying_activity_actor::start( player_activity &act, Character &who )
{
    ...
    if( !start_finish( here, act, who, moves_discard, _( "prying" ), &map_data_common_t::prying,
                       false, ad_discard ) ) {
        return;
    }
