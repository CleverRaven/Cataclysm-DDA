...
static const oter_str_id afs_ruins_dynamic("afs_ruins_dynamic");
...
static const update_mapgen_id update_mapgen_faction_afs_ruin( "pp_afs_ruin_json" );
...
static void GENERATOR_aftershock_ruin( map &md, const tripoint_abs_omt &p )
{
    ...
    bool above_open_air = overmap_buffer.ter(tripoint_abs_omt(p.x(), p.y(), p.z() + 1)) == open_air;
    bool above_ruined = overmap_buffer.ter(tripoint_abs_omt(p.x(), p.y(), p.z() + 1)) == afs_ruins_dynamic;


    //fully ruined
    if ((x_in_y(1, 2) && above_open_air) || above_ruined ) {
        overmap_buffer.ter_set(p, afs_ruins_dynamic);
        tripoint_bub_ms explosion_loc = random_entry(all_points_in_map);
        if (p.z() > 1) {
            explosion_handler::explosion(nullptr, explosion_loc, 15000, .8, false);
            explosion_handler::explosion(nullptr, explosion_loc, 15000, .8, false);
        }
        ...
        for (const tripoint_bub_ms& current_tile : all_points_in_map) {
            ...
            if (md.has_flag_ter(ter_furn_flag::TFLAG_WALL, current_tile)) {
                !one_in(5) ? md.ter_set(current_tile.xy(), ter_t_wall_prefab_metal) : md.ter_set(current_tile.xy(), ter_t_metal_floor);
            }
            if (md.has_flag_ter(ter_furn_flag::TFLAG_INDOORS, current_tile)) {
                if (!one_in(4)) {
                    p.z() == 0 ? md.ter_set(current_tile.xy(), ter_t_snow_metal_floor) : md.ter_set(current_tile.xy(), ter_t_snow);
    ...
    for (size_t i = 0; i < all_points_in_map.size(); i++) {
        ...
        if (md.has_flag_ter(ter_furn_flag::TFLAG_NATURAL_UNDERGROUND, current_tile) || md.is_open_air(current_tile)) {
            continue;
        }
