    ...
    switch(op) {
        case JMAPGEN_PLACEGROUP_MONSTER: {
            for (int i = 0; i < trepeat; i++) {
                // Messages::add_msg("%s %d %d,%d %d,%d %.4f", gid.c_str(), chance, x.val, y.val, x.valmax, y.valmax, ( density == -1.0f ? mdensity : density ) );
                m->place_spawns(gid, chance, x.val, y.val, x.valmax, y.valmax, ( density == -1.0f ? mdensity : density ) );
            }
        } break;
        case JMAPGEN_PLACEGROUP_ITEM: {
            for (int i = 0; i < trepeat; i++) {
                // Messages::add_msg("%s %d %d,%d %d,%d", gid.c_str(), chance, x.val, y.val, x.valmax, y.valmax);
                   ...
                   const int zlevel, const regional_settings * rsettings)
{
    ...
    if ( fmapit != oter_mapgen.end() && !fmapit->second.empty() ) {
        ...
        //Messages::add_msg("draw_map: %s (%s): %d/%d roll %d/%d den %.4f", terrain_type.c_str(), function_key.c_str(), fidx+1, fmapit->second.size(), roll, rlast, density );
