...
static void GENERATOR_aftershock_ruin( map &md, const tripoint_abs_omt &p )
{
    ...
    bool above_open_air = overmap_buffer.ter(tripoint_abs_omt(p.x(), p.y(), p.z() + 1)) == oter_open_air;
    bool above_ruined = overmap_buffer.ter(tripoint_abs_omt(p.x(), p.y(), p.z() + 1)) == oter_afs_ruins_dynamic;
