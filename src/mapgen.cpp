                    ...
                    bool run_post_process )
{
    ...
    for( int gridz = OVERMAP_HEIGHT; gridz >= -OVERMAP_DEPTH; gridz-- ) {
            ...
            if( mx_iter != settings_mx.extras.end() ) {
                ...
                if( this_ex.chance > 0 && ex.values.empty() && !this_ex.values.empty() ) {
                    ...
                    if( extra == nullptr ) {
                        ...
                        MapExtras::apply_function( *extra, *this, tripoint_abs_sm( abs_sub ) );
                    }
                }
            }
        ...
        // check args, if they are true, apply corresponding post-process
        overmap_buffer.mapgen_args( p );

        // dat.get_arg()
        // Apply post-process generators
        if( run_post_process ) {
            for( const auto &x : dat.get_applied_post_generators() ) {
                const tripoint_abs_omt omt_point = { p.x(), p.y(), gridz };
                pp_generator::apply_pp_generators( *this, dat.get_applied_post_generators(), omt_point );
            }
        }
    }
