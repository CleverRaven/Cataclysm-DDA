...
bool Character::handle_melee_wear( item_location shield, float wear_multiplier )
{
    ...
    double enchant_multiplier = calculate_by_enchantment( 1, enchant_vals::mod::EQUIPMENT_DAMAGE_CHANCE );
    ...
    int damage_chance = static_cast<int>( ( stat_factor * material_factor / (wear_multiplier * enchant_multiplier) ) );
        ...
        if( worn_shield && shield->covers( bp_hit ) ) {
            ...
            if( source != nullptr && !source->is_hallucination() ) {
                for( damage_unit &du : dam.damage_units ) {
                    shield->damage_armor_durability( du, bp_hit, calculate_by_enchantment( 1, enchant_vals::mod::EQUIPMENT_DAMAGE_CHANCE ) );
                }
            }
