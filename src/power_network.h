...
class power_network_manager
{
    ...
    private:
        ...
        std::unordered_map<tripoint_abs_ms, int> old_pos_to_id_;
        std::unordered_set<int> claimed_ids_;
        std::map<int, power_network> pending_networks_;
        int rebuild_next_id_ = 1;
};
