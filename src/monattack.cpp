...
void mattack::boomer(monster *z, int index)
{
    ...
    Creature *target = z->attack_target();
    if( target == nullptr || 
        rl_dist( z->posx(), z->posy(), target->xpos(), target->ypos() ) > 3 || 
        !z->sees( target, t ) ) {
        return;
    }

    player *foe = dynamic_cast< player* >( target );
    std::vector<point> line = line_to( z->posx(), z->posy(), target->xpos(), target->ypos(), t );
    z->reset_special(index); // Reset timer
...
void mattack::spit_sap(monster *z, int index)
{
    ...
    Creature *target = z->attack_target();
    if( target == nullptr || 
        ( dist = rl_dist( z->posx(), z->posy(), target->xpos(), target->ypos() ) ) > 12 || 
        !z->sees( target, t ) ) {
        return;
    }
...
void mattack::upgrade(monster *z, int index)
{
    ...
    for (size_t i = 0; i < g->num_zombies(); i++) {
        ...
        if( zed.type->id == "mon_zombie" &&
            rl_dist(z->posx(), z->posy(), zed.posx(), zed.posy()) <= 5 &&
...
void mattack::flesh_golem(monster *z, int index)
{
    ...
    Creature *target = z->attack_target();
    if( target == nullptr || 
        ( dist = rl_dist( z->posx(), z->posy(), target->xpos(), target->ypos() ) ) > 20 || 
        !z->sees( target, t ) ) {
        return;
    }
...
void mattack::suicide(monster *z, int index)
{
    ...
    Creature *target = z->attack_target();
    if( target == nullptr || 
        rl_dist( z->posx(), z->posy(), target->xpos(), target->ypos() ) > 2 || 
        !z->sees( target, t ) ) {
        return;
    }
    z->die(z);
