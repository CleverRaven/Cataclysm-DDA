...
bool mattack::shriek_stun( monster *z )
{
    ...
    for( const tripoint &cone : g->m.points_in_radius( z->pos(), 4 ) ) {
        ...
        Creature *nTarget = g->critter_at( cone ); //If a monster is there, chance for stun
        if( nTarget == nullptr ) {
            continue;
        }
        if( one_in( dist / 2 ) && !( nTarget->is_immune_effect( effect_deaf ) ) ) {
            nTarget->add_effect( effect_dazed, rng( 1_minutes, 2_minutes ), num_bp, false, rng( 1,
                                ( 15 - dist ) / 3 ) );
        }
