...
bool mattack::eat_crop( monster *z )
{
    ...
    for( const auto &p : here.points_in_radius( z->pos(), 1 ) ) {
        if( here.has_flag( flag_PLANT,  p ) && one_in( num_targets ) ) {
            num_targets++;
            target = p;
        }
    }
...
bool mattack::plant( monster *z )
{
    ...
    const bool is_fungi = here.has_flag_ter( flag_FUNGUS, monster_position );
