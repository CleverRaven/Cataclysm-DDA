...
// is_empty() because we can't hatch an ant under the player, a monster, etc.
 ...
 if (!ants.empty()) {
  ...
  if (g->u_see(z->posx(), z->posy()) && g->u_see(ant->posx(), ant->posy()))
   Messages::add_msg(_("The %s feeds an %s and it grows!"), z->name().c_str(),
              ant->name().c_str());
  ...
  if (g->u_see(z->posx(), z->posy()))
   Messages::add_msg(_("The %s lays an egg!"), z->name().c_str());
  ...
  if (g->u_see(z->posx(), z->posy()))
   Messages::add_msg(_("The %s tends nearby eggs, and they hatch!"), z->name().c_str());
    ...
    for (int i = 0; i < line.size(); i++) {
        if (g->m.hit_with_acid(line[i].x, line[i].y)) {
            if (g->u_see(line[i].x, line[i].y)) {
                Messages::add_msg(_("A glob of acid hits the %s!"),
...
void mattack::shockstorm(monster *z)
{
    ...
    z->sp_timeout = z->type->sp_freq; // Reset timer
    Messages::add_msg(_("A bolt of electricity arcs towards you!"));
...
void mattack::boomer(monster *z)
{
 ...
 if (u_see)
  Messages::add_msg(_("The %s spews bile!"), z->name().c_str());
 for (int i = 0; i < line.size(); i++) {
...
// If bile hit a solid tile, return.
  if (g->m.move_cost(line[i].x, line[i].y) == 0) {
   ...
   if (g->u_see(line[i].x, line[i].y))
    Messages::add_msg(_("Bile splatters on the %s!"),
               g->m.tername(line[i].x, line[i].y).c_str());
   return;
  }
 }
 if (!g->u.uncanny_dodge()) {
  if (rng(0, 10) > g->u.get_dodge() || one_in(g->u.get_dodge()))
   g->u.infect("boomered", bp_eyes, 3, 12, false, 1, 1);
    ...
    Messages::add_msg(_("You dodge it!"));
...
// Find all corpses that we can see within 4 tiles.
 ...
 if (sees_necromancer)
  Messages::add_msg(_("The %s throws its arms wide..."), z->name().c_str());
 z->sp_timeout = z->type->sp_freq; // Reset timer
 ...
 if (raised > 0) {
  if (raised == 1)
   Messages::add_msg(_("A nearby corpse rises from the dead!"));
  else if (raised < 4)
   Messages::add_msg(_("A few corpses rise from the dead!"));
  else
   Messages::add_msg(_("Several corpses rise from the dead!"));
 } else if (sees_necromancer)
  Messages::add_msg(_("...but nothing seems to happen."));
}
...
void mattack::science(monster *z) // I said SCIENCE again!
{
 ...
 case 1: // Shock the player
  if (!g->u.uncanny_dodge()) {
   Messages::add_msg(_("The %s shocks you!"), z->name().c_str());
   z->moves -= 150;
   g->u.hurtall(rng(1, 2));
 ...
 case 2: // Radioactive beam
  Messages::add_msg(_("The %s opens it's mouth and a beam shoots towards you!"),
             z->name().c_str());
  z->moves -= 400;
  if (!g->u.uncanny_dodge()) {
   if (g->u.get_dodge() > rng(0, 16) && !one_in(g->u.get_dodge()))
    Messages::add_msg(_("You dodge the beam!"));
   else if (one_in(6))
    g->u.mutate();
   else {
    Messages::add_msg(_("You get pins and needles all over."));
    g->u.radiation += rng(20, 50);
   }
  }
  break;
 case 3: // Spawn a manhack
  Messages::add_msg(_("The %s opens its coat, and a manhack flies out!"),
             z->name().c_str());
  z->moves -= 200;
  ...
  break;
 case 4: // Acid pool
  Messages::add_msg(_("The %s drops a flask of acid!"), z->name().c_str());
  z->moves -= 100;
 ...
 case 5: // Flavor text
  switch (rng(1, 4)) {
  case 1:
   Messages::add_msg(_("The %s gesticulates wildly!"), z->name().c_str());
   break;
  case 2:
   Messages::add_msg(_("The %s coughs up a strange dust."), z->name().c_str());
   break;
  case 3:
   Messages::add_msg(_("The %s moans softly."), z->name().c_str());
   break;
  case 4:
   Messages::add_msg(_("The %s's skin crackles with electricity."), z->name().c_str());
   z->moves -= 80;
   break;
  }
  break;
    ...
    if (one_in(4)) { // 1 in 4 chance to grow a tree
     ...
     if (mondex != -1) {
      if (g->u_see(z->posx() + i, z->posy() + j))
       Messages::add_msg(_("A tree bursts forth from the earth and pierces the %s!"),
...
// Player is hit by a growing tree
  if (!g->u.uncanny_dodge()) {
       ...
       if (one_in(4))
        ...
        hit = bp_feet;
       Messages::add_msg(_("A tree bursts forth from the earth and pierces your %s!"),
      ...
      if (npcdex != -1) { // An NPC got hit
       ...
       if (one_in(4))
        ...
        hit = bp_feet;
       if (g->u_see(z->posx() + i, z->posy() + j))
        Messages::add_msg(_("A tree bursts forth from the earth and pierces %s's %s!"),
...
// Underbrush => young tree
      ...
      if (mondex != -1) {
       if (g->u_see(z->posx() + i, z->posy() + j))
        Messages::add_msg(_("Underbrush forms into a tree, and it pierces the %s!"),
                   g->zombie(mondex).name().c_str());
       int rn = rng(10, 30);
       rn -= g->zombie(mondex).get_armor_cut(bp_torso);
       if (rn < 0)
  ...
  if (!g->u.uncanny_dodge()) {
        ...
        if (one_in(4))
         ...
         hit = bp_feet;
        Messages::add_msg(_("The underbrush beneath your feet grows and pierces your %s!"),
                   body_part_name(hit, side).c_str());
        g->u.hit(z, hit, side, 0, rng(10, 30));
       }
      } else {
       ...
       if (npcdex != -1) {
        ...
        if (one_in(4))
         ...
         hit = bp_feet;
        if (g->u_see(z->posx() + i, z->posy() + j))
         Messages::add_msg(_("Underbrush grows into a tree, and it pierces %s's %s!"),
                    g->active_npc[npcdex]->name.c_str(),
                    body_part_name(hit, side).c_str());
        g->active_npc[npcdex]->hit(z, hit, side, 0, rng(10, 30));
       }
  ...
  for (int y = z->posy() - 1; y <= z->posy() + 1; y++) {
   if (g->u.posx == x && g->u.posy == y) {
    if ( g->u.uncanny_dodge() ) {
     ...
     Messages::add_msg(_("The %s lashes your %s!"), z->name().c_str(),
...
// TODO: Friendly biollantes?
 ...
 if (missed_by > 1.) {
  if (g->u_see(z->posx(), z->posy()))
   Messages::add_msg(_("The %s spits sap, but misses you."), z->name().c_str());
  ...
  for (int i = 0; i < line.size() && dam > 0; i++) {
   ...
   if (dam == 0 && g->u_see(line[i].x, line[i].y)) {
    Messages::add_msg(_("A glob of sap hits the %s!"),
               g->m.tername(line[i].x, line[i].y).c_str());
    return;
   }
  }
 ...
 if (g->u_see(z->posx(), z->posy()))
  Messages::add_msg(_("The %s spits sap!"), z->name().c_str());
 ...
 Messages::add_msg(_("A glob of sap hits you!"));
...
void mattack::triffid_heartbeat(monster *z)
{
 ...
 if (rl_dist(z->posx(), z->posy(), g->u.posx, g->u.posy) > 5 &&
  ...
  Messages::add_msg(_("The root walls creak around you."));
...
void mattack::fungus(monster *z)
{
    ...
    if (g->u_see(z->posx(), z->posy())) {
        Messages::add_msg(_("Spores are released from the %s!"), z->name().c_str());
    }
    for (int i = -1; i <= 1; i++) {
        for (int j = -1; j <= 1; j++) {
            if (i == 0 && j == 0) {
...
void mattack::fungus_growth(monster *z)
{
    ...
    if (g->u_see(z->posx(), z->posy())) {
        Messages::add_msg(_("The %s young fungaloid grows into an adult!"),
                      z->name().c_str());
    }
...
void mattack::fungus_sprout(monster *z)
{
    ...
    for (int x = z->posx() - 1; x <= z->posx() + 1; x++) {
        for (int y = z->posy() - 1; y <= z->posy() + 1; y++) {
            if (g->u.posx == x && g->u.posy == y) {
                Messages::add_msg(_("You're shoved away as a fungal wall grows!"));
...
void mattack::leap(monster *z)
{
    ...
    if (seen) {
        Messages::add_msg(_("The %s leaps!"), z->name().c_str());
    }
}
...
void mattack::dermatik(monster *z)
{
    ...
    {
        Messages::add_msg(_("The %s tries to land on you, but you dodge."), z->name().c_str());
        z->stumble(false);
    ...
    if (g->u.has_trait("TAIL_CATTLE")) {
        Messages::add_msg(_("You swat at the %s with your tail!"), z->name().c_str());
        player_swat += ((g->u.dex_cur + g->u.skillLevel("unarmed")) / 2);}
    if (player_swat > dodge_roll) {
        Messages::add_msg(_("The %s lands on you, but you swat it off."), z->name().c_str());
        if (z->hp >= z->type->hp / 2) {
            z->hurt(1);
        }
    ...
    if (4 < g->u.get_armor_cut(targeted) / 3) {
        Messages::add_msg(_("The %s lands on your %s, but can't penetrate your armor."),
    ...
    Messages::add_msg(_("The %s sinks its ovipositor into your %s!"), z->name().c_str(),
...
void mattack::dermatik_growth(monster *z)
{
    ...
    if (g->u_see(z->posx(), z->posy())) {
        Messages::add_msg(_("The %s dermatik larva grows into an adult!"),
                      z->name().c_str());
    }
...
void mattack::plant(monster *z)
{
    ...
    if (!g->spread_fungus(z->posx(), z->posy()) && one_in(20)) {
        if (g->u_see(z->posx(), z->posy())) {
            Messages::add_msg(_("The %s takes seed and becomes a young fungaloid!"),
                          z->name().c_str());
        }
        ...
        if (g->u_see(z->posx(), z->posy())) {
        Messages::add_msg(_("The %s falls to the ground and bursts!"),
                       z->name().c_str());
        }
        z->hp = 0;
    }
...
void mattack::dogthing(monster *z)
{
 ...
 Messages::add_msg(_("The %s's head explodes in a mass of roiling tentacles!"),
            z->name().c_str());
...
void mattack::tentacle(monster *z)
{
    ...
    Messages::add_msg(_("The %s lashes its tentacle at you!"), z->name().c_str());
    z->moves -= 100;
    z->sp_timeout = z->type->sp_freq; // Reset timer
    ...
    Messages::add_msg(_("Your %s is hit for %d damage!"), body_part_name(hit, side).c_str(), dam);
...
// Make sure that the player's butchering is interrupted!
 if (g->u.activity.type == ACT_BUTCHER &&
  ...
  Messages::add_msg(_("The buffeting winds interrupt your butchering!"));
    ...
    if (distance > 0) {
     ...
     for (int i = 0; i < traj.size() && dam > 0; i++) {
      ...
      if (! g->u.uncanny_dodge()) {
        body_part hit = random_body_part();
        int side = random_side(hit);
        Messages::add_msg(_("A %s hits your %s for %d damage!"), thrown.tname().c_str(),
...
// Throw monsters
   ...
   if (mondex != -1) {
    ...
    if (distance > 0) {
     if (g->u_see(thrown))
      Messages::add_msg(_("The %s is thrown by winds!"), thrown->name().c_str());
     std::vector<point> traj = continue_line(from_monster, distance);
     bool hit_wall = false;
     for (int i = 0; i < traj.size() && !hit_wall; i++) {
      ...
      if (i > 0 && monhit != -1 && !g->zombie(monhit).digging()) {
       if (g->u_see(traj[i].x, traj[i].y))
        Messages::add_msg(_("The %s hits a %s!"), thrown->name().c_str(),
   ...
   if (g->u.posx == x && g->u.posy == y) { // Throw... the player?! D:
      if ((g->u.has_trait("LEG_TENT_BRACE")) && (!(g->u.wearing_something_on(bp_feet))) ) {
          Messages::add_msg(_("You secure yourself using your tentacles!"));
      }
      if (g->u.is_throw_immune()) {
          Messages::add_msg(_("You deftly maintain your footing!"));
      }
    ...
    (!(g->u.wearing_something_on(bp_feet))) ) && (!(g->u.is_throw_immune())) )) {
     ...
     Messages::add_msg(_("You're thrown by winds!"));
     bool hit_wall = false;
     int damage = rng(5, 10);
     for (int i = 0; i < traj.size() && !hit_wall; i++) {
      int monhit = g->mon_at(traj[i].x, traj[i].y);
      if (i > 0 && monhit != -1 && !g->zombie(monhit).digging()) {
       if (g->u_see(traj[i].x, traj[i].y))
        Messages::add_msg(_("You hit a %s!"), g->zombie(monhit).name().c_str());
       if (g->zombie(monhit).hurt(damage))
       ...
       Messages::add_msg(_("You slam into a %s"),
...
void mattack::gene_sting(monster *z)
{
 ...
 z->moves -= 150;
 z->sp_timeout = z->type->sp_freq;
 Messages::add_msg(_("The %s shoots a dart into you!"), z->name().c_str());
 g->u.mutate();
}
...
void mattack::para_sting(monster *z)
{
    ...
    z->sp_timeout = z->type->sp_freq;
    Messages::add_msg(_("The %s shoots a dart into you!"), z->name().c_str());
    Messages::add_msg(_("You feel poison enter your body!"));
...
void mattack::triffid_growth(monster *z)
{
    ...
    if (g->u_see(z->posx(), z->posy())) {
        Messages::add_msg(_("The %s young triffid grows into an adult!"),
                      z->name().c_str());
    }
...
void mattack::stare(monster *z)
{
 ...
 if (g->sees_u(z->posx(), z->posy(), j)) {
  Messages::add_msg(_("The %s stares at you, and you shudder."), z->name().c_str());
  g->u.add_disease("teleglow", 800);
 } else {
  Messages::add_msg(_("A piercing beam of light bursts forth!"));
...
void mattack::fear_paralyze(monster *z)
{
 if (g->u_see(z->posx(), z->posy())) {
  ...
  if (g->u.has_artifact_with(AEP_PSYSHIELD)) {
   Messages::add_msg(_("The %s probes your mind, but is rebuffed!"), z->name().c_str());
  } else if (rng(1, 20) > g->u.int_cur) {
   Messages::add_msg(_("The terrifying visage of the %s paralyzes you."),
              z->name().c_str());
   g->u.moves -= 100;
  } else
   Messages::add_msg(_("You manage to avoid staring at the horrendous %s."),
              z->name().c_str());
 }
}
...
void mattack::photograph(monster *z)
{
 ...
 z->sp_timeout = z->type->sp_freq;
 z->moves -= 150;
 Messages::add_msg(_("The %s takes your picture!"), z->name().c_str());
...
void mattack::smg(monster *z)
{
    ...
    if (z->friendly != 0) {
        ...
        if (target == NULL) {// Couldn't find any targets!
            if(boo_hoo > 0 && g->u_see(z->posx(), z->posy()) ) { // because that stupid oaf was in the way!
                Messages::add_msg(ngettext("Pointed in your direction, the %s emits an IFF warning beep.",
    ...
    if (g->u_see(z->posx(), z->posy())) {
        Messages::add_msg(_("The %s fires its smg!"), z->name().c_str());
    }
...
void mattack::laser(monster *z)
{
    ...
    if (!sunlight) {
        if (one_in(3)) {
            if (g->u_see(z->posx(), z->posy())) {
                Messages::add_msg(_("The %s's barrel spins but nothing happens!"), z->name().c_str());
            }
        } else if (one_in(4)) {
            g->sound(z->posx(), z->posy(), 6, _("boop-boop!"));
        }
    ...
    if (g->u_see(z->posx(), z->posy())) {
        Messages::add_msg(_("The %s's barrel spins and fires!"), z->name().c_str());
    }
...
void mattack::flamethrower(monster *z)
{
    ...
    for (int i = 0; i < traj.size(); i++)
    {
        ...
        {
            if (g->u_see(traj[i].x, traj[i].y))
                Messages::add_msg(_("The tongue of flame hits the %s!"),
            g->m.tername(traj[i].x, traj[i].y).c_str());
            return;
        }
...
void mattack::ratking(monster *z)
{
    ...
    switch (rng(1, 5)) { // What do we say?
        case 1: Messages::add_msg(_("\"YOU... ARE FILTH...\"")); break;
        case 2: Messages::add_msg(_("\"VERMIN... YOU ARE VERMIN...\"")); break;
        case 3: Messages::add_msg(_("\"LEAVE NOW...\"")); break;
        case 4: Messages::add_msg(_("\"WE... WILL FEAST... UPON YOU...\"")); break;
        case 5: Messages::add_msg(_("\"FOUL INTERLOPER...\"")); break;
    }
...
void mattack::upgrade(monster *z)
{
 ...
 if (g->u_see(z->posx(), z->posy()))
  Messages::add_msg(_("The black mist around the %s grows..."), z->name().c_str());
 if (g->u_see(target->posx(), target->posy()))
  Messages::add_msg(_("...a zombie becomes a %s!"), target->name().c_str());
}
...
void mattack::bite(monster *z) {
    ...
    z->sp_timeout = z->type->sp_freq; // Reset timer
    Messages::add_msg(_("The %s lunges forward!"), z->name().c_str());
    z->moves -= 100;
    ...
    if (dam > 0) {
        Messages::add_msg(_("The %s bites your %s!"), z->name().c_str(), body_part_name(hit, side).c_str());
        ...
        Messages::add_msg(_("The %s bites your %s, but your armor protects you."), z->name().c_str(), body_part_name(hit, side).c_str());
    }
...
void mattack::brandish(monster *z){
    ...
    Messages::add_msg("He's brandishing a knife!");
    Messages::add_msg("Quiet, quiet");
}
...
void mattack::flesh_golem(monster *z)
{
    ...
    z->sp_timeout = z->type->sp_freq; // Reset timer
    Messages::add_msg(_("The %s swings a massive claw at you!"), z->name().c_str());
    z->moves -= 100;
    ...
    body_part hit = random_body_part();
    int dam = rng(5, 10), side = random_side(hit);
    Messages::add_msg(_("Your %s is battered for %d damage!"), body_part_name(hit, side).c_str(), dam);
    g->u.hit(z, hit, side, dam, 0);
...
void mattack::darkman(monster *z)
{
    ...
    g->add_zombie( tmp );
    if( g->u_see(z->posx(), z->posy()) ) {
        Messages::add_msg( _("A shadow splits from the %s!"),
                    z->name().c_str() );
    }
    int linet;
    if( !g->sees_u(z->posx(), z->posy(), linet) ){
    ...
    case 1: Messages::add_msg(_("\"Stop it please\"")); break;
    case 2: Messages::add_msg(_("\"Let us help you\"")); break;
    case 3: Messages::add_msg(_("\" We wish you no harm \"")); break;
    case 4: Messages::add_msg(_("\"Do not fear\"")); break;
    case 5: Messages::add_msg(_("\"We can help you\"")); break;
    case 6: Messages::add_msg(_("\"We are friendly\"")); break;
    case 7: Messages::add_msg(_("\"Please dont\"")); break;
    }
...
void mattack::slimespring(monster *z)
{
    ...
    if (g->u.morale_level() <= 1) {
        ...
        //~ Lowercase is intended: they're small voices.
            case 1: Messages::add_msg(_("\"hey, it's gonna be all right!\""));
            g->u.add_morale(MORALE_SUPPORT, 10, 50);
            break;
            case 2: Messages::add_msg(_("\"we'll get through this!\""));
            g->u.add_morale(MORALE_SUPPORT, 10, 50);
            break;
            case 3: Messages::add_msg(_("\"i'm here for you!\""));
            g->u.add_morale(MORALE_SUPPORT, 10, 50);
            break;
        }
    }
    if (rl_dist(z->posx(), z->posy(), g->u.posx, g->u.posy) <= 3) {
        if ( (g->u.has_disease("bleed")) || (g->u.has_disease("bite")) ) {
            Messages::add_msg(_("\"let me help!\""));
            // Yes, your slimespring(s) handle/don't all Bad Damage at the same time.
            if (g->u.has_disease("bite")) {
                if (one_in(3)) {
                    g->u.rem_disease("bite");
                    Messages::add_msg(_("The slime cleans you out!"));
                }
                else {
                    Messages::add_msg(_("The slime flows over you, but your gouges still ache."));
                }
            }
            if (g->u.has_disease("bleed")) {
                if (one_in(2)) {
                    g->u.rem_disease("bleed");
                    Messages::add_msg(_("The slime seals up your leaks!"));
                }
                else {
                    Messages::add_msg(_("The slime flows over you, but your fluids are still leaking."));
                }
            }
        }
    }
...
bool mattack::thrown_by_judo(monster *z)
{
    ...
    if (g->u.is_throw_immune()) {
        ...
        if ( ((g->u.dex_cur + g->u.skillLevel("unarmed")) > (z->type->melee_skill + rng(0, 3))) ) {
            Messages::add_msg(_("but you grab its arm and flip it to the ground!"));
            ...
            if ( !one_in(4) && !g->u.has_active_bionic("bio_faraday") &&
                ...
                Messages::add_msg(_("The flip does shock you..."));
            ...
            Messages::add_msg(_("but you deftly spin out of its grasp!"));
        }
        return true;
    } else {
        return false;
...
void mattack::bio_op_takedown(monster *z)
{
    ...
    z->sp_timeout = z->type->sp_freq; // Reset timer
    Messages::add_msg(_("The %s mechanically grabs at you!"), z->name().c_str());
    z->moves -= 100;
    ...
    Messages::add_msg(_("The zombie kicks your %s for %d damage..."), body_part_name(hit, side).c_str(), dam);
    g->u.hit(z, hit, side, dam, 0);
    ...
    (g->u.wearing_something_on(bp_feet))) ) {
        if (one_in(4)) {
            ...
            Messages::add_msg(_("and slams you, face first, to the ground for %d damage!"), dam);
            g->u.hit(z, hit, side, dam, 0);
        }
        else {
            hit = bp_torso;
            dam = rng(6, 18);
            Messages::add_msg(_("and slams you to the ground for %d damage!"), dam);
            g->u.hit(z, hit, side, dam, 0);
        }
    ...
    if (!thrown_by_judo(z)) {
        ...
        hit = bp_torso;
            dam = rng(3, 9);
            Messages::add_msg(_("and slams you for %d damage!"), dam);
            g->u.hit(z, hit, side, dam, 0);
    }
    g->u.practice(calendar::turn, "dodge", z->type->melee_skill);
}
