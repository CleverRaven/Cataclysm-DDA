...
std::function<int( const T & )> conditional_t<T>::get_get_int( const JsonObject &jo )
{
    if( jo.has_member( "const" ) ) {
        ...
        if( weather_aspect == "temperature" ) {
            return []( const T & ) {
                return int(get_weather().weather_precise->temperature);
            };
        } else if( weather_aspect == "windpower" ) {
            return []( const T & ) {
                return int(get_weather().weather_precise->windpower);
            };
        } else if( weather_aspect == "humidity" ) {
            return []( const T & ) {
                return int(get_weather().weather_precise->humidity);
            };
        } else if( weather_aspect == "pressure" ) {
            return []( const T & ) {
                return int(get_weather().weather_precise->pressure);
            };
        }
        ...
        if( checked_value == "strength" ) {
            ...
            return [is_npc]( const T & d ) {
                // Energy in milijoule
                return int(d.actor( is_npc )->power_cur().value());
            };
        } else if( checked_value == "power_max" ) {
            return [is_npc]( const T & d ) {
                // Energy in milijoule
                return int(d.actor( is_npc )->power_max().value());
            };
