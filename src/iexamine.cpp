    ...
    for( auto item_it = items.begin(); item_it != items.end(); ++item_it ) {
        if( item_it->made_of( LIQUID ) ) {
            ...
            if( one_in( 10 + p.get_dex() ) ) {
                ...
                g->liquid_handling.handle_liquid_from_ground( item_it, examp, 1 );
            }
            return;
        }
    }
...
void iexamine::toilet( player &p, const tripoint &examp )
{
    ...
    if( water == items.end() ) {
        ...
        ( void ) p; // TODO: use me
        g->liquid_handling.handle_liquid_from_ground( water, examp );
    }
}
...
void iexamine::fvat_empty( player &p, const tripoint &examp )
{
    ...
    if( !brew_present ) {
        ...
        switch( selectmenu.ret ) {
            ...
            case REMOVE_BREW: {
                g->liquid_handling.handle_liquid_from_ground( g->m.i_at( examp ).begin(), examp );
                return;
            }
            case START_FERMENT: {
                ferment = true;
...
void iexamine::fvat_full( player &p, const tripoint &examp )
{
    ...
    if( g->liquid_handling.handle_liquid_from_ground( items_here.begin(), examp ) ) {
        g->m.furn_set( examp, f_fvat_empty );
...
void iexamine::keg( player &p, const tripoint &examp )
{
    ...
    if( !liquid_present ) {
        ...
        switch( selectmenu.ret ) {
            case DISPENSE:
                if( g->liquid_handling.handle_liquid_from_ground( drink, examp ) ) {
            ...
            for( auto &it : items ) {
                if( ( it.is_bucket() || it.is_watertight_container() ) && !it.is_container_empty() ) {
                    ...
                    if( liquid.typeId() == "maple_sap" ) {
                        g->liquid_handling.handle_liquid_from_container( it, PICKUP_RANGE );
                    }
                }
            }
...
void iexamine::water_source( player &p, const tripoint &examp )
{
    ...
    ( void ) p; // TODO: use me
    g->liquid_handling.handle_liquid( water, nullptr, 0, &examp );
}
