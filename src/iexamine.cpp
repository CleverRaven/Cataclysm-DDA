...
void iexamine::tree_maple_tapped( Character &you, const tripoint_bub_ms &examp )
{
    ...
    map &here = get_map();
    map_cursor cur( examp );
    cur.visit_items(
    [&]( item_location node ) {
        if( node->will_spill() || node->is_watertight_container() ) {
            container = node.get_item();
            if( node->typeId() == itype_maple_sap ) {
                has_sap = true;
        ...
        if( node->get_quality_nonrecursive( qual_TREE_TAP ) > 0 ) {
            spile = node.get_item();
        }
        return VisitResponse::NEXT;
    }
