...
void smoker_activate( player &p, const tripoint &examp )
{
    ...
    if( food_volume > units::from_liter( 20 ) ) {
        ...
        add_msg( string_format( _( "You think that you can load about %s %s in it." ),
                                format_volume( units::from_liter( 20 ) ), volume_units_long() ) );
        return;
    }
...
void smoker_finalize( player &, const tripoint &examp )
{
    ...
    for( size_t i = 0; i < items.size(); i++ ) {
        ...
        if( item_it.typeId() == "meat_smoked" ) {
            ...
            product = "dry_beans";
        } else if( item_it.typeId() == "sweet_fruit" || item_it.typeId() == "coconut" ||
...
void smoker_load_food( player &p, const tripoint &examp )
{
    ...
    for( item m : moved ) {
        ...
        p.mod_moves( -p.item_handling_cost( m ) );
        add_msg( m_info, string_format( _( "You carefully place %s %s in the rack." ), amount,
                                        m.nname( m.typeId(), amount ) ) );
    }
    p.invalidate_crafting_inventory();
}
...
void iexamine::smoker_options( player &p, const tripoint &examp )
{
    ...
    if( c_check ) {
        smenu.addentry( 5, true, 'c', "%s",
                        _( "Remove charcoal from rack; has: " ) + std::to_string( char_quantity ) );
    } else {
