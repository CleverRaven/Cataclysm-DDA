...
struct sid_or_sid {
    ...
    int chance;                // 68
    sid_or_sid( const std::string &s1, const int i, const::std::string s2 ) : primary_str( s1 ),
        secondary_str( s2 ), chance( i ) { }
};
...
void apply_region_overlay( JsonObject &jo, regional_settings &region )
{
    ...
    if( region.field_coverage.boost_chance > 0.0f &&
        region.field_coverage.boosted_percent_str.size() == 0 ) {
...
void overmap::init_layers()
{
    for( int z = 0; z < OVERMAP_LAYERS; ++z ) {
        oter_id default_type = ( z < OVERMAP_DEPTH ) ? "empty_rock" : ( z == OVERMAP_DEPTH ) ?
                               settings.default_oter :
                               "open_air";
...
bool overmap::generate_sub( int const z )
{
    ...
    for( int i = 0; i < OMAPX; i++ ) {
        for( int j = 0; j < OMAPY; j++ ) {
            ...
            if( is_ot_type( "house_base", oter_above ) ) {
                ...
                add_mon_group( mongroup( mongroup_id( "GROUP_ANT" ), i * 2, j * 2, z, ( size * 3 ) / 2, rng( 6000,
                                         8000 ) ) );
    ...
    for( auto &i : cities ) {
        ...
        if( !one_in( 8 ) ) {
            add_mon_group( mongroup( mongroup_id( "GROUP_SEWER" ), i.x * 2, i.y * 2, z, ( i.s * 7 ) / 2,
                                     i.s * 70 ) );
        }
    }
...
bool overmap::allowed_terrain( const tripoint &p, int width, int height,
...
void overmap::open()
{
    ...
    const auto serialise_reader = ( const std::function<void( std::istream & )> & ) std::bind(
                                      &overmap::unserialize, this, _1 );
    const auto unserialise_view_reader = ( const std::function<void( std::istream & )> & ) std::bind(
            &overmap::unserialize_view, this, _1 );
    if( read_from_file_optional( terfilename, serialise_reader ) ) {
        read_from_file_optional( plrfilename, unserialise_view_reader );
...
void regional_settings::setup_oter( oter_weight &oter )
{
    if( oter.ot_iid == -1 ) {
        ...
        if( it == obasetermap.end() ) {
            debugmsg( "Bad oter_weight_list entry in region settings: overmap_terrain '%s' not found.",
                      oter.ot_sid.c_str() );
            oter.ot_iid = 0;
        } else {
