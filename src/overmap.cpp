...
// Like map::points_in_rectangle, but for overmaps
static tripoint_range points_in_rectangle( const tripoint &from, const tripoint &to )
{
    ...
    const int maxx = std::min( OMAPX - 1, std::max( from.x, to.x ) );
    const int maxy = std::min( OMAPY - 1, std::max( from.y, to.y ) );
 ...
 */
void overmap::build_city( const tripoint &city_origin, int size )
{
    ...
    ter( city_origin ) = oter_id( "road_nesw" );
    city new_city;
    ...
    while( !to_build.empty() ) {
        ...
        */
        ...
        int max_length = free_or_road_len( *this, origin, cur_dir, static_cast<int>( std::min( size / 2.0f, leftover_area / 5.0f ) ) ) - 1; 
        int max_width = free_or_road_len( *this, origin, width_dir, 7 ) - 1;
        ...
        */
        ...
        block_blueprint max_block_blueprint = max_block_area( *this, inner,
        ...
        */
        ...
        const auto path_1 = lay_out_street( local_road, origin_pt, cur_dir, length + 2, true );
        build_connection( local_road, path_1, 0 );
        const auto path_2 = lay_out_street( local_road, origin_pt + om_direction::displace( width_dir, width + 1 ), cur_dir, length + 2, true );
        build_connection( local_road, path_2, 0 );
        const auto path_3 = lay_out_street( local_road, origin_pt, width_dir, width + 2, true );
        build_connection( local_road, path_3, 0 );
        const auto path_4 = lay_out_street( local_road, origin_pt + om_direction::displace( cur_dir, length + 1 ), width_dir, width + 2, true );
        build_connection( local_road, path_4, 0 );

        // Furthest point (from inner) that is still in the road-bound rectangle
        const tripoint last = origin + om_direction::displace( cur_dir, length ) + om_direction::displace( width_dir, width );
        for( const tripoint &p : points_in_rectangle( inner, last ) ) {
            oter_id &cur_id = ter( p );
            if( cur_id == get_settings().default_oter ) {
                place_special( settings.city_spec.pick_house().obj(), p, om_direction::type::north, new_city );
            } else {
                place_special( settings.city_spec.pick_house().obj(), p, om_direction::type::south, new_city );
            }
        }
...
pf::path overmap::lay_out_street( const overmap_connection &connection, const point &source, om_direction::type dir,
                                  size_t len, bool force ) const
{
    ...
    while( actual_len < len ) {
        ...
        if( !force && actual_len > 1 && connection.has( ter_id ) ) {
            break;  // Stop here.
        }
    }
