...
void debug_console::execute()
{
    ...
    on_out_of_scope flush_guard{ [this]()
    {
        if( debug_capture::is_initialized() ) {
            debug_capture::instance().flush_jsonl();
        }
                            ...
                            std::string &edit_buf )
{
    ...
    if( can_edit && edit_table == table_id ) {
        ...
        if( ImGui::BeginPopupModal( "var_edit_popup", nullptr,
            ...
            ImGui::InputText( "##var_edit_value", &edit_buf );
            ImGui::TextDisabled( "%s",
                                 _( "Numeric input stored as double, otherwise as string" ) );
            if( ImGui::Button( _( "Save##var_edit" ) ) ) {
                writer.write( edit_key, edit_buf );
                edit_table.clear();
                edit_key.clear();
                edit_buf.clear();
                ImGui::CloseCurrentPopup();
...
void tab_data_view::draw_var_browser( debug_console &/*host*/ )
{
    ...
    auto parse_value = []( const std::string & s ) -> diag_value {
        size_t pos = 0;
        try
        {
    ...
    var_table_writer global_writer{
        [parse_value]( const std::string & k, const std::string & v )
        {
            get_globals().set_global_value( k, parse_value( v ) );
        },
        []( const std::string & k )
        {
    ...
    var_table_writer avatar_writer{
        [parse_value]( const std::string & k, const std::string & v )
        {
            get_avatar().set_value( k, parse_value( v ) );
        },
        []( const std::string & k )
        {
    ...
    if( ImGui::TreeNodeEx( _( "Global" ), ImGuiTreeNodeFlags_DefaultOpen ) ) {
        ...
        ImGui::InputTextWithHint( "##new_global_key", _( "new key" ),
                                  &var_browser.new_key );
        ImGui::SameLine();
        ImGui::SetNextItemWidth( 160.0f );
        ImGui::InputTextWithHint( "##new_global_val", _( "value (number or text)" ),
                                  &var_browser.new_val );
        ImGui::SameLine();
        ImGui::BeginDisabled( var_browser.new_key.empty() );
        const bool add_clicked = ImGui::SmallButton( _( "Add##global_var" ) );
        if( ImGui::IsItemHovered() ) {
            ImGui::SetTooltip( "%s",
                               _( "Create global variable from key + value" ) );
        }
        if( add_clicked ) {
            global_writer.write( var_browser.new_key, var_browser.new_val );
            var_browser.new_key.clear();
            var_browser.new_val.clear();
        }
        ImGui::EndDisabled();

        draw_var_table( "global_vars", get_globals().get_global_values(),
                        var_browser.filter, 150.0f, "global", global_provider, global_writer,
                        var_browser.edit_table, var_browser.edit_key, var_browser.edit_buf );
        ImGui::TreePop();
    }
    ...
    avatar &player_character = get_avatar();
    {
        ...
        if( ImGui::TreeNode( label.c_str() ) ) {
            draw_var_table( "avatar_vars", player_character.get_values(),
                            var_browser.filter, 120.0f, "avatar", avatar_provider,
                            avatar_writer,
                            var_browser.edit_table, var_browser.edit_key, var_browser.edit_buf );
            ImGui::TreePop();
        }
        ...
        for( npc &guy : g->all_npcs() ) {
            ...
            if( ImGui::TreeNode( label.c_str() ) ) {
                ...
                var_table_writer npc_writer{
                    [parse_value, npc_cid]( const std::string & k, const std::string & v )
                    {
                        if( npc *p = g->find_npc( npc_cid ) ) {
                            p->set_value( k, parse_value( v ) );
                        }
                    },
                ...
                draw_var_table( string_format( "npc_vars_%d", npc_idx ).c_str(),
                                guy.get_values(), var_browser.filter, 120.0f,
                                string_format( "npc#%d", npc_cid.get_value() ),
                                npc_provider, npc_writer,
                                var_browser.edit_table, var_browser.edit_key, var_browser.edit_buf );
                ImGui::TreePop();
            }
            npc_idx++;
        ...
        for( monster &mon : g->all_monsters() ) {
            ...
            if( ImGui::TreeNode( label.c_str() ) ) {
                draw_var_table( string_format( "mon_vars_%d", mon_idx ).c_str(),
                                mon.get_values(), var_browser.filter, 120.0f,
                                std::string(), {}, {},
                                var_browser.edit_table, var_browser.edit_key, var_browser.edit_buf );
                ImGui::TreePop();
            }
...
void tab_trace_view::draw_body( debug_console &host )
{
    ...
    framed_section( "trace_capture", _( "Capture" ), [&]() {
        ...
        ImGui::Spacing();
        ImGui::Checkbox( _( "Store on disk" ), &s.jsonl.enabled );
        if( ImGui::IsItemHovered() ) {
            ImGui::SetTooltip( "%s",
        ...
        int fmt_idx = static_cast<int>( s.jsonl.format );
        ...
        if( ImGui::Combo( "##disk_fmt", &fmt_idx, fmt_items, IM_ARRAYSIZE( fmt_items ) ) ) {
            ...
            if( new_fmt != s.jsonl.format ) {
                s.jsonl.format = new_fmt;
                const std::string desired = new_fmt == capture_format::csv
                                            ? ".csv"
                                            : ".jsonl";
                const std::string &p = s.jsonl.path;
                const size_t dot = p.find_last_of( '.' );
                const size_t slash = p.find_last_of( '/' );
                if( dot != std::string::npos
                    && ( slash == std::string::npos || dot > slash ) ) {
                    s.jsonl.path = p.substr( 0, dot ) + desired;
                } else if( !p.empty() ) {
                    s.jsonl.path = p + desired;
                }
            }
        }
        if( ImGui::IsItemHovered() ) {
        ...
        ImGui::InputTextWithHint( "##jsonl_path", _( "path" ), &s.jsonl.path );
        if( ImGui::IsItemHovered() ) {
            ImGui::SetTooltip( "%s",
                               _( "Output path (relative to game working directory)" ) );
        }
        ImGui::SameLine();
        ImGui::Checkbox( _( "auto-flush" ), &s.jsonl.auto_flush );
        if( ImGui::IsItemHovered() ) {
            ImGui::SetTooltip( "%s",
