...
bool mtype::made_of( const std::string &material ) const
{
...
field_id mtype::bloodType() const
{
    ...
    if( has_flag( MF_WARM ) && made_of( "flesh" ) ) {
        return fd_blood;
    }
    return fd_null;
}
...
itype_id mtype::get_meat_itype() const
{
    if( has_flag( MF_POISON ) ) {
        if( made_of( "flesh" ) || made_of( "hflesh" ) ) {
            return "meat_tainted";
        } else if( made_of( "iflesh" ) ) {
            //In the future, insects could drop insect flesh rather than plain ol' meat.
            return "meat_tainted";
        } else if( made_of( "veggy" ) ) {
