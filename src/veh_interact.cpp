    ...
    switch( static_cast<char>( p.activity.index ) ) {
        case 'i': {
            ...
            if( !reqs.can_make_with_inventory( inv, is_crafting_component ) ) {
                p.add_msg_if_player( m_info, _( "You don't meet the requirements to install the %s." ), vpinfo.name() );
                break;
            }
            ...
            if( base.is_null() ) {
                if( !p.has_trait( trait_DEBUG_HS ) ) {
                    p.add_msg_if_player( m_info, _( "Could not find base part in requirements for %s." ), vpinfo.name() );
                    break;
                } else {
        ...
        case 'c':
            ...
            if( !parts.empty() ) {
                ...
                if( !broken ) {
                    g->m.add_item_or_charges( point( p.posx(), p.posy() ), removed_wheel );
                }
                p.add_msg_if_player( _( "You replace one of the %1$s's tires with a %2$s." ),
