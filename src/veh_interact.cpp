...
void veh_interact::do_siphon()
{
    ...
    auto sel = [&]( const vehicle_part & pt ) {
        return( pt.is_tank() && !pt.base.contents_empty() &&
...
void act_vehicle_siphon( vehicle *veh )
{
    ...
    for( const vpart_reference &vp : veh->get_any_parts( VPFLAG_FLUIDTANK ) ) {
        if( vp.part().get_base().legacy_front().made_of( phase_id::LIQUID ) ) {
            has_liquid = true;
            break;
        }
    }
    ...
    auto sel = []( const vehicle_part & pt ) {
        return pt.is_tank() && pt.get_base().legacy_front().made_of( phase_id::LIQUID );
    };
...
void veh_interact::complete_vehicle( player &p )
{
    ...
    switch( static_cast<char>( p.activity.index ) ) {
        ...
        case 'f': {
            ...
            if( pt.is_tank() && src->is_container() && !src->contents_empty() ) {
