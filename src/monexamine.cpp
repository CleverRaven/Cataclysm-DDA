...
void monexamine::shear_animal( monster &z )
{
    ...
    if( !z.has_effect( effect_tied ) ) {
        z.add_effect( effect_tied, 1_turns, bodypart_id( "bp_null" ), true );
...
bool monexamine::mech_hack( monster &z )
{
    ...
    if( player_character.has_amount( card_type, 1 ) ) {
        if( query_yn( _( "Swipe your ID card into the mech's security port?" ) ) ) {
            ...
            z.add_effect( effect_pet, 1_turns, bodypart_id( "bp_null" ), true );
            z.friendly = -1;
    ...
    switch( bot_menu.ret ) {
        case 1:
            ...
            if( amount > 0 ) {
                ...
                z.add_effect( effect_paid, time_bought, bodypart_id( "bp_null" ), true );
                z.friendly = -1;
...
void monexamine::attach_or_remove_saddle( monster &z )
{
    if( z.has_effect( effect_monster_saddled ) ) {
        ...
        z.add_effect( effect_monster_saddled, 1_turns, bodypart_id( "bp_null" ), true );
...
void monexamine::attach_bag_to( monster &z )
{
    ...
    z.add_effect( effect_has_bag, 1_turns, bodypart_id( "bp_null" ), true );
...
bool monexamine::add_armor( monster &z )
{
    ...
    add_msg( pgettext( "pet armor", "You put the %1$s on your %2$s." ), armor.display_name(),
             pet_name );
    loc.remove_item();
    z.add_effect( effect_monster_armor, 1_turns, bodypart_id( "bp_null" ), true );
...
void monexamine::milk_source( monster &source_mon )
{
    ...
    if( milkable_ammo->second > 0 ) {
        ...
        if( temp_tie ) {
            source_mon.add_effect( effect_tied, 1_turns, bodypart_id( "bp_null" ), true );
            player_character.activity.str_values.push_back( "temp_tie" );
        }
