...
std::string empty_line( size_t len )
{
...
void draw_points( WINDOW *w, points_left &points, int netPointCost )
{
    ...
    mvwprintz( w, 3, 2, c_black, empty_line( getmaxx( w ) - 3 ).c_str() );
...
tab_direction set_stats(WINDOW *w, player *u, points_left &points)
{
    ...
    <color_light_green>%s</color> to decrease the statistic."),
        ...
        const std::string clear_line = empty_line( getmaxx( w ) - iSecondColumn - 1 );
        ...
        for (int iCurrentPage = 0; iCurrentPage < 2; iCurrentPage++) { //Good/Bad
            ...
            for( int i = start_y; i < (int)traits_size[iCurrentPage]; i++ ) {
                ...
                if (cur_line_y == i && iCurrentPage == iCurWorkingPage) {
                    ...
                    mvwprintz( w, 3, 41, c_ltgray, empty_line( getmaxx( w ) - 41 - 1 ).c_str() );
                ...
                mvwprintz( w, cur_line_y, cur_line_x, c_ltgray, empty_line( page_width ).c_str() ); 
...
tab_direction set_profession(WINDOW *w, player *u, points_left &points)
{
    ...
    do {
        ...
        const std::string clear_line = empty_line( getmaxx( w ) - 2 );
...
tab_direction set_skills(WINDOW *w, player *u, points_left &points)
{
    ...
    do {
        ...
        // Clear the bottom of the screen.
        werase(w_description);
        mvwprintz(w, 3, 31, c_ltgray, empty_line( getmaxx( w ) - 32 ).c_str() );
        ...
        for (int i = first_i; i < end_i; ++i) {
            ...
            // Clear the line
            mvwprintz(w, base_y + i, 2, c_ltgray, empty_line( getmaxx( w ) - 3 ).c_str() );
...
tab_direction set_scenario(WINDOW *w, player *u, points_left &points)
{
    ...
    do {
        ...
        const std::string clear_line = empty_line( getmaxx( w_description ) );
...
tab_direction set_description(WINDOW *w, player *u, const bool allow_reroll, points_left &points)
{
    ...
    WINDOW *w_guide = newwin(4, TERMX - 3, getbegy(w) + 19, getbegx(w) + 2);
...
void save_template( player *u )
{
    ...
    std::string name = string_input_popup()
                       .title( title )
                       .width( TERMX - utf8_width( title ) - 8 )
                       .query_string();
    if( name.length() == 0 ) {
        return;
    }
