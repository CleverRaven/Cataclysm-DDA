...
void map::step_in_field(int x, int y)
{
    ...
    for(std::map<field_id, field_entry*>::iterator field_list_it = curfield.getFieldStart();
        ...
        case fd_electricity:
            if (g->u.has_artifact_with(AEP_RESIST_ELECTRICITY) || g->u.has_active_bionic("bio_faraday")|| g->u.worn_with_flag("ELECTRIC_IMMUNE")) //Artifact or bionic stops electricity.
