    ...
    */
    const auto create_hot_air = [this] ( const tripoint &p, int density )
    {
        ...
        for( int counter = 0; counter < 5; counter++ ) {
            tripoint dst( p.x + rng_int( -1, 1 ), p.y + rng_int( -1, 1 ), p.z );
        ...
        for( locy = 0; locy < SEEY; locy++ ) {
            ...
            for( auto it = curfield.begin(); it != curfield.end();) {
                ...
                switch (curtype) {
                        ...
                        if( !ter_furn_has_flag( ter, frn, TFLAG_SUPPRESS_SMOKE ) &&
                            ...
                            rng(3, 35) < cur.getFieldDensity() * 10 ) {
                                ...
                                if( smoke_up ) {
                                    ...
                                    const auto &dst_ter = dst.get_ter_t();
                                    if( dst_ter.has_flag( TFLAG_NO_FLOOR ) ) {
                                        dst.add_field( fd_smoke, rng_int( 1, cur.getFieldDensity() ), 0 );
                                    } else {
                    ...
                    {
                        dirty_transparency_cache = true;
                        int extra_radiation = rng_int(0, cur.getFieldDensity());
                        ...
                        if (!one_in(5)) {   // 4 in 5 chance to spread
                            ...
                            if (impassable( p ) && cur.getFieldDensity() > 1) { // We're grounded
                                ...
                                while (tries < 10 && cur.getFieldAge() < 50 && cur.getFieldDensity() > 1) {
                                    pnt.x = p.x + rng_int(-1, 1);
                                    pnt.y = p.y + rng_int(-1, 1);
                                    if( passable( pnt ) ) {
                    ...
                    case fd_shock_vent:
                        if (cur.getFieldDensity() > 1) {
                            ...
                            for (int i = 0; i < num_bolts; i++) {
                                ...
                                int dist = rng_int(4, 12);
                                int boltx = p.x, bolty = p.y;
                                for (int n = 0; n < dist; n++) {
                                    boltx += xdir;
                                    bolty += ydir;
                                    add_field( tripoint( boltx, bolty, p.z ), fd_electricity, rng_int(2, 3), 0 );
                                    if (one_in(4)) {
                                        if (xdir == 0) {
                                            xdir = rng_int(0, 1) * 2 - 1;
                                        } else {
                                            xdir = 0;
                                        }
                                    }
    ...
    for( auto field_list_it = curfield.begin(); field_list_it != curfield.end(); ++field_list_it){
        ...
        case fd_electricity:
        {
            ...
            for( size_t i = 0; i < num_hp_parts; i++ ) {
                ...
                const int dmg = rng_int( 1, cur.getFieldDensity() );
    ...
    for( auto field_list_it = curfield.begin(); field_list_it != curfield.end(); ++field_list_it ) {
        ...
        case fd_acid:
            if( !z.has_flag( MF_FLIES ) ) {
                const int d = rng_int( cur.getFieldDensity(), cur.getFieldDensity() * 3 );
        ...
        case fd_fire:
            ...
            if (cur.getFieldDensity() == 1) {
                ...
                if (!z.has_flag(MF_FLIES)) {
                    z.moves -= 20;
                    if( dam > 0 ) {
                        z.add_effect( effect_onfire, rng_int(dam / 2, dam * 2));
                    }
                }
        ...
        case fd_tear_gas:
            if ((z.made_of( material_id( "flesh" ) ) || z.made_of( material_id( "hflesh" ) ) || z.made_of( material_id( "veggy" ) ) || z.made_of( material_id( "iflesh" ) )) &&
                ...
                if (cur.getFieldDensity() == 3) {
                    z.add_effect( effect_stunned, rng_int(10, 20));
                    dam += rng(4, 10);
                } else if (cur.getFieldDensity() == 2) {
                    z.add_effect( effect_stunned, rng_int(5, 10));
                    dam += rng(2, 5);
                } else {
                    z.add_effect( effect_stunned, rng_int(1, 5));
                }
                if (z.made_of( material_id( "veggy" ) )) {
        ...
        case fd_relax_gas:
            if ((z.made_of( material_id( "flesh" ) ) || z.made_of( material_id( "hflesh" ) ) || z.made_of( material_id( "veggy" ) ) || z.made_of( material_id( "iflesh" ) )) &&
                !z.has_flag(MF_NO_BREATHE)) {
                z.add_effect( effect_stunned, rng_int(cur.getFieldDensity() * 4, cur.getFieldDensity() * 8));
            }
            break;
        ...
        case fd_dazzling:
            if (z.has_flag(MF_SEES)) {
                ...
                z.add_effect( effect_stunned, cur.getFieldDensity() * rng_int(5, 12));
            }
            break;
        ...
        case fd_fatigue:
            if (rng(0, 2) < cur.getFieldDensity()) {
                ...
                do {
                    newpos.x = rng_int(z.posx() - SEEX, z.posx() + SEEX);
                    newpos.y = rng_int(z.posy() - SEEY, z.posy() + SEEY);
                    tries++;
