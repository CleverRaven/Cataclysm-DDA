...
bool player::eat(item &food, bool force)
{
    ...
    if (!will_vomit && !(has_bionic(bio_digestion) || has_trait(trait_id("PARAIMMUNE")))) {
        if (food.get_comestible()->parasites > 0 && !food.has_flag("NO_PARASITES") &&
            ...
            if (!has_active_bionic(bionic_id("afs_bio_nodisease"))) { // Aftershock-specific bionic; clears parasites and causes side effects
                switch (rng(0, 3)) {
                case 0:
                ...
                add_msg_if_player(m_mixed,
                    _("Your augmented immune system flares up.  It must be cleansing something in what you just ate."));
