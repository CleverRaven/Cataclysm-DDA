...
std::string format_pad(const std::string &str, unsigned int length, bool right_align)
{
    return format_pad(str.c_str(), length, right_align);
}

std::string format_pad(const char *str, unsigned int length, bool right_align)
{
    std::string ret = str;
    if(ret.size() > length) {
        ret.clear();
        ...
        if(right_align == true) {
            ret = std::string(length - ret.size(), ' ') + ret;
        } else {
            ret = ret + std::string(length - ret.size(), ' ');
        }
    }
    return ret;
}

std::string format_pad(int number, unsigned int length, bool right_align)
{
    std::string ret;
    ...
    // format the number by dividing 10 repeatedly to extract number of zeroes
        ...
        // take into account the first digit and possible sign
        int i = length - ((number < 0) ? 2 : 1);
//        int i = length - 1;
        for(int j = num; j >= 10; j = j / 10) {
            --i;
        }
        if(i >= 0) {
            // display '-' sign?
            bool sign = (number / ((num == 0) ? 1 : num)) == -1;
            const char *fmt = (sign == true) ? "-%d" : "%d";
            if(right_align == true) {
                ret = std::string(i, ' ') + string_format(fmt, num);
            } else {
                ret = string_format(fmt, num) + std::string(i, ' ');
            }
        }
    }
    return ret;
