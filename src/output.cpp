...
bool query_yn(const char *mes, ...)
{
    ...
    // make the localized warning string show up right
    std::string ucwarning = _(" (Case Sensitive)");
    ...
    int win_width = utf8_width(message.c_str()) + utf8_width(selectors.c_str()) + 2 + 4;
    if (force_uc) {
        win_width += utf8_width(ucwarning.c_str());
    }
