...
void event::actualize()
{
 switch (type) {
  ...
  case EVENT_TEMPLE_OPEN: {
   ...
   if (saw_grate)
    Messages::add_msg(_("The nearby grates open to reveal a staircase!"));
  } break;
...
// Check if we should print a message
   if (flood_buf[g->u.posx][g->u.posy] != g->m.ter(g->u.posx, g->u.posy)) {
    if (flood_buf[g->u.posx][g->u.posy] == t_water_sh) {
     Messages::add_msg(_("Water quickly floods up to your knees."));
     ...
     Messages::add_msg(_("Water fills nearly to the ceiling!"));
 ...
 switch (type) {
  case EVENT_WANTED: {
   ...
   if (g->levz >= 0 && one_in(100) && !ACTIVE_WORLD_OPTIONS["CLASSIC_ZOMBIES"]) {
    ...
    if (g->u_see(place.x, place.y))
     Messages::add_msg(_("An eyebot swoops down nearby!"));
   }
  } break;
  ...
  case EVENT_SPAWN_WYRMS:
   ...
   if (int(calendar::turn) % 3 == 0)
    Messages::add_msg(_("You hear screeches from the rock above and around you!"));
   break;

  case EVENT_AMIGARA:
   Messages::add_msg(_("The entire cavern shakes!"));
   break;

  case EVENT_TEMPLE_OPEN:
   Messages::add_msg(_("The earth rumbles."));
   break;
