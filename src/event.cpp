...
void event::actualize()
{
    ...
    case EVENT_SPAWN_WYRMS: {
        ...
        int num_wyrms = rng_int(1, 4);
        for (int i = 0; i < num_wyrms; i++) {
            int tries = 0;
            tripoint monp = g->u.pos();
            do {
                monp.x = rng_int(0, SEEX * MAPSIZE);
                monp.y = rng_int(0, SEEY * MAPSIZE);
                tries++;
    ...
    case EVENT_AMIGARA: {
        ...
        for (int i = 0; i < num_horrors; i++) {
            ...
            do {
                if (horizontal) {
                    monx = rng_int(faultx, faultx + 2 * SEEX - 8);
                    for (int n = -1; n <= 1; n++) {
                    ...
                    mony = rng_int(faulty, faulty + 2 * SEEY - 8);
                    for (int n = -1; n <= 1; n++) {
...
// flood_buf is filled with correct tiles; now copy them back to g->m
   ...
   g->events.add( EVENT_TEMPLE_FLOOD, calendar::turn + rng( 2_turns, 3_turns ) );
    ...
    case EVENT_TEMPLE_SPAWN: {
        ...
        int tries = 0, x, y;
        do {
            x = rng_int(g->u.posx() - 5, g->u.posx() + 5);
            y = rng_int(g->u.posy() - 5, g->u.posy() + 5);
            tries++;
