...
{
    std::vector<std::string> all_basements = region.city_spec.basements.all;
    for( auto b : all_basements ) {
        if( t_below == oter_id( b ) ) {
            return true;
        }
    }
    return false;
...
void mapgen_generic_house(map *m, oter_id terrain_type, mapgendata dat, const time_point &turn, float density, int variant)
{
    ...
    if( dat.has_basement() ) {
        place_basement_stairs( m, terrain_type, dat, actual_house_height, lw, rw );
    }
...
void place_basement_stairs( map *m, oter_id terrain_type, mapgendata dat, const int actual_house_height, const int lw, const int rw )
{
    // For rotation
