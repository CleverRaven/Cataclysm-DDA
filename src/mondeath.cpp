...
void mdeath::normal(monster *z) {
    if (g->u_see(z)) {
        Messages::player_messages.add_msg(_("The %s dies!"), z->name().c_str()); //Currently it is possible to get multiple messages that a monster died.
    }
...
void mdeath::acid(monster *z) {
    if (g->u_see(z)) {
        if(z->type->dies.size() == 1) //If this death function is the only function. The corpse gets dissolved.
            ...
            Messages::player_messages.add_msg(_("The %s's body leaks acid."), z->name().c_str());
        }
    }
            ...
            if (g->m.move_cost(sporex, sporey) > 0) {
                if (mondex != -1) {
                    ...
                    // Spores hit the player
                        ...
                        Messages::player_messages.add_msg(_("The spores land on you, but you quickly swat them off with your tail!"));
                        return;
                        }
                    bool hit = false;
...
void mdeath::worm(monster *z) {
    if (g->u_see(z)) {
        if(z->type->dies.size() == 1)
            ...
            Messages::player_messages.add_msg(_("Two worms crawl out of the %s's corpse."), z->name().c_str());
        }
    }
...
void mdeath::blobsplit(monster *z) {
    ...
    if (g->u_see(z)) {
        if(z->type->dies.size() == 1)
            ...
            Messages::player_messages.add_msg(_("Two small blobs slither out of the corpse."), z->name().c_str());
        }
    }
    blob.hp = blob.speed;
...
void mdeath::darkman(monster *z) {
     ...
     if (g->u_see(z))
        Messages::player_messages.add_msg(_("The %s melts away. And the world returns to normaliity"), z->name().c_str());
}
