...
// temporary until "turn" is factored out of game 
//#include "game.h"
...
void Messages::add_msg_string(const std::string &s)
{
	...
	messages.push_back(game_message());
	messages.back().count=1;
	messages.back().turn = calendar::turn;
	messages.back().message = s;
}
...
void Messages::display_messages(WINDOW* ipk_target, int left, int top, int right, int bottom, int offset, bool display_turns)
{
	...
	for (int i = messages.size()-offset-1; i >= 0 && line >= top; i--) {
		...
		for (int j = folded.size() - 1; j >= 0 && line >= top; j--, line--) {
			mvwprintz(ipk_target, line, left, col, folded[j].c_str());
		}
		if (display_turns)
		{
