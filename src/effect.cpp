...
std::string effect::disp_desc( bool reduced ) const
{
    ...
    if( !permanent || !( eff_type->max_duration <= time_duration::from_turns( 2 ) &&
                         eff_type->max_duration == time_duration::from_turns( 0 ) ) ) {
        if( get_player_character().has_trait( trait_SELFAWARE ) ) {
            ret += string_format( _( "Effect ends in: <color_white>%s</color>" ),
                                  to_string_clipped( duration ) );
        } else {
            time_duration remaining_est = get_player_character().estimate_effect_dur( skill_id( "firstaid" ),
                                          eff_type->id, ( effect_dur_elapsed + duration ) / 2, 6, get_player_character() );
            if( to_turns<int>( remaining_est ) == 0 ) {
                ret += _( "Effect ends <color_white>any second now...</color>" );
            } else {
                ret += string_format( _( "Effect ends in <color_white>%s</color>" ),
                                      to_string_approx( remaining_est ) );
            }
        }
    }
