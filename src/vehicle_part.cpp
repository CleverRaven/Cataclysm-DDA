...
int vehicle_part::ammo_capacity( const ammotype &ammo ) const
{
    if( is_tank() ) {
        ...
        const int max_charges_volume = ammo_type->charges_per_volume( base.get_volume_capacity(item_pocket::ok_like_old_default_behavior) );
...
int vehicle_part::item_capacity( const itype_id &stuffing_id ) const
{
    ...
    if( stuffing->count_by_charges() ) {
        max_amount_volume = stuffing->charges_per_volume( base.get_volume_capacity(item_pocket::ok_like_old_default_behavior) );
    } else {
        max_amount_volume = base.get_volume_capacity(item_pocket::ok_like_old_default_behavior) / stuffing->volume;
    }
