...
/** Calculate the cost to travel between OMT centers
  ...
  * @param is_cardinal is this N/E/S/W travel, not NE/SE/SW/NW travel
  */
int omt_travel_cost( int cost1, int cost2, bool is_cardinal )
{
    if( is_cardinal ) {
    ...
    while( !open_set.empty() ) {
        ...
        if( cur_point == dest ) {
            ...
            do {
                ...
                if( next_node != nullptr && !( addr == start ) ) {
                    ret.dist += is_cardinal( next_node->get_prev_dir() ) ? 24 : 34;
                }
                next_node = &node;
        ...
        for( direction dir : enumerate_directions( cur_node.allow_z_change, allow_diagonal ) ) {
            ...
            const int cumulative_cost = cur_node.cumulative_cost + omt_travel_cost( cur_node.node_cost,
                                        next_score.node_cost, is_cardinal( dir ) );
