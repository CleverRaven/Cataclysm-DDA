        ...
        const std::function<void( const std::string &msg )> &report ) const
{
    ...
    auto should_avoid_functor = [this]( const tripoint & p ) {
        return is_dangerous_tile( p );
    };
    for( const tripoint_bub_ms &p : here.points_in_radius( target, threshold, 0 ) ) {
        if( should_avoid_functor( p.raw() ) ) {
            continue; // dont route to dangerous tiles
        }
        const route_t route = here.route( who.pos_bub(), p, who.get_pathfinding_settings(),
                                          should_avoid_functor );
        if( route.empty() ) {
            continue; // no route
        }
