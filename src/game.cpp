...
void game::init_ui()
{
    ...
    if( use_narrow_sidebar() ) {
        ...
        messHshort = TERRAIN_WINDOW_TERM_HEIGHT - ( locH + statH +
                     pixelminimapH ); // 1 for w_location + 4 for w_stat, w_messages starts at 0
        if( messHshort < 9 ) {
            pixelminimapH -= 9 - messHshort;
            messHshort = 9;
        }
...
bool game::cleanup_at_end()
{
    ...
    if( uquit == QUIT_DIED || uquit == QUIT_SUICIDE ) {
        ...
        sTemp = _( "Kills:" );
        mvwprintz( w_rip, 1 + iInfoLine++, ( FULL_SCREEN_WIDTH / 2 ) - 5, c_ltgray,
                   ( sTemp + " " ).c_str() );
        wprintz( w_rip, c_magenta, ssTemp.str().c_str() );
                            ...
                            int catch_duration ) // catching function
{
    ...
    m.add_item_or_charges( pos, item::make_corpse( catchables[index]->type->id,
                           calendar::turn + int( rng( 0, catch_duration ) ) ) );
    ...
    if( !u.is_dead_state() ) {
        switch( act ) {
            ...
            case ACTION_MOVE_N:
                ...
                if( ( u.get_value( "remote_controlling" ) != "" ) && ( ( u.has_active_item( "radiocontrol" ) ) ||
...
bool game::load_master( std::string worldname )
{
    ...
    const auto reader = ( const std::function<void( std::istream & )> & ) std::bind(
                            &game::unserialize_master, this, _1 );
...
void game::load( std::string worldname, std::string name )
{
    ...
    const auto unserialise_reader = ( const std::function<void( std::istream & )> & ) std::bind(
                                        &game::unserialize, this, _1 );
    if( !read_from_file( playerfile, unserialise_reader ) ) {
        return;
    }
    const auto load_weather_reader = ( const std::function<void( std::istream & )> & ) std::bind(
                                         &game::load_weather, this, _1 );
    read_from_file_optional( worldpath + name + ".weather", load_weather_reader );
    nextweather = int( calendar::turn );
    const auto load_memorial_file_reader = ( const std::function<void( std::istream & )> & ) std::bind(
            &player::load_memorial_file, &u, _1 );
...
{
    const std::string playerfile = world_generator->active_world->world_path + "/" + base64_encode(
                                       u.name );
    ...
    while( true ) {
        ...
        if( redraw ) {
            ...
            mvwprintz( w_info, 3, 0, c_white,
                       _( "Food Supply:       %s" ), fac_food_supply_text( cur_frac->food_supply,
                               cur_frac->size ).c_str() );
...
void game::draw_sidebar()
{
    ...
    if( get_levz() < 0 ) {
        ...
        mvwprintz( w_location, 0, 18, weather_data( weather ).color, "%s",
...
{
    int distance = ( get_option<int>( "SAFEMODEPROXIMITY" ) <= 0 ) ? 60 :
...
int game::mon_info( WINDOW *w )
{
    ...
    int newseen = 0;
    const int iProxyDist = ( get_option<int>( "SAFEMODEPROXIMITY" ) <= 0 ) ? 60 :
                           get_option<int>( "SAFEMODEPROXIMITY" );
    ...
    do {
        ...
        if( zone_num == 0 ) {
            ...
            for( auto &i : zones.zones ) {
                if( iNum >= start_index &&
                    iNum < start_index + ( ( max_rows > zone_num ) ? zone_num : max_rows ) ) {
                    ...
                    if( iNum >= iStartPos && iNum < iStartPos + ( ( iMaxRows > iItemNum ) ? iItemNum : iMaxRows ) ) {
                        ...
                        if( mSortCategory[iNum] != "" ) {
                            ...
                            trim_and_print( w_items, iNum - iStartPos, 1, width - 9,
                                            ( ( iNum == iActive ) ? c_ltgreen : ( high ? c_yellow : ( low ? c_red :
                                                    iter->example->color_in_inventory() ) ) ),
                                            "%s", ( sText.str() ).c_str() );
                            ...
                            mvwprintz( w_items, iNum - iStartPos, width - ( 6 + numw ),
                                       ...
                                       direction_name_short( direction_from( 0, 0, iter->vIG[iThisPage].pos.x,
                                                             iter->vIG[iThisPage].pos.y ) ).c_str()
                                     );
                        }
                    }
...
int game::list_monsters( const int iLastState )
{
    ...
    do {
        ...
        if( vMonsters.empty() ) {
            ...
            if( bVMonsterLookFire ) {
                mvwprintz( w_monsters, getmaxy( w_monsters ) - 1, 1, c_ltgreen, "%s",
...
extern void serialize_liquid_source( player_activity &act, const vehicle &veh,
                                     const itype_id &ftype );
                          ...
                          const vehicle *const source_veh )
{
    ...
    const std::string liquid_name = liquid.has_infinite_charges() ? liquid.tname() :
...
void game::read()
{
    ...
    auto loc = inv_map_splice( []( const item & it ) {
        return it.is_book();
    }, _( "Read:" ), 1,
...
void game::plswim( const tripoint &p )
{
    ...
    if( u.is_underwater() ) {
        drenchFlags |= mfb( bp_head ) | mfb( bp_eyes ) | mfb( bp_mouth ) | mfb( bp_hand_l ) | mfb(
                           bp_hand_r );
    }
