...
void game::create_factions()
{
    ...
    for(std::vector<std::string>::reverse_iterator it = faction_vector.rbegin();
        it != faction_vector.rend(); ++it) {
            ...
            for (int gate_x = -1; gate_x <= 1; gate_x++) {
                for (int gate_y = -1; gate_y <= 1; gate_y++) {
                    if ((wall_x + wall_y == 1 || wall_x + wall_y == -1) &&
                        ...
                        ((wall_y != 0 && (g->m.ter(examx + wall_x, examy + wall_y) == h_wall_type)) ||
                         //horizontal orientation of the gate
                         (wall_x != 0 &&
                          (g->m.ter(examx + wall_x, examy + wall_y) == v_wall_type)))) { //vertical orientation of the gate
...
bool zlave_menu(monster *z)
{
    ...
    if (attach_bag == choice) {
        ...
        add_msg(_("You mount the %s on your zombie slave, ready to store gear."),
    ...
    do {
        if (!vMonsters.empty() || iLastState == 1) {
            if (action == "UP") {
                ...
                if (iMonDex >= 0 && iMonDex < num_zombies() &&
...
bool game::plmove(int dx, int dy)
{
    if (run_mode == 2) {
        ...
        add_msg(m_warning,
                _("Monster spotted--safe mode is on! (%s to turn it off or %s to ignore monster.)"),
                press_x(ACTION_TOGGLE_SAFEMODE).c_str(),
                from_sentence_case(press_x(ACTION_IGNORE_ENEMY)).c_str());
        return false;
    }
