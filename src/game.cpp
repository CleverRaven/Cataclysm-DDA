...
bool game::plmove(int dx, int dy, int dz)
{
    ...
    if( waste_moves || dest_loc.z != u.posz() ) {
        ...
        if( veh != nullptr ) {
            // redefine variable as id of obtacle part
            part = veh->obstacle_at_part( part );
            if( part > 0 ) {
                obtacle_name = veh->parts[part].info().name;
            }
        } else {
            obtacle_name = m.name( dest_loc );
        }
        add_msg( _( "You bump into a %s!" ), obtacle_name.c_str() );
...
void game::place_player( const tripoint &dest_loc )
{
    ...
    if( !m.has_flag( "SEALED", u.pos() ) ) {
        if ((u.has_effect( effect_blind) || u.worn_with_flag("BLIND") || u.has_active_bionic("bio_blindfold")) && !m.i_at(u.pos()).empty()) {
