...
void game::start_game(std::string worldname)
{
    ...
    if (scen->has_flag("FIRE_START")){
            m.add_field(u.pos().x + 5, u.pos().y + 3, u.pos().z, field_from_ident("fd_fire"), 3 );
            m.add_field(u.pos().x + 7, u.pos().y + 6, u.pos().z, field_from_ident("fd_fire"), 3 );
            m.add_field(u.pos().x + 3, u.pos().y + 4, u.pos().z, field_from_ident("fd_fire"), 3 );
    }
...
bool game::do_turn()
{
    ...
    if (calendar::turn % 50 == 0) { // Hunger, thirst, & fatigue up every 5 minutes
        ...
        if (u.has_bionic("bio_solar") && is_in_sunlight(u.posx, u.posy, u.posz)) {
            u.charge_power(25);
        }
    ...
    if ((driving_view_offset.x != 0 || driving_view_offset.y != 0)) {
        ...
        vehicle *veh = m.veh_at(u.posx, u.posy, u.posz);
        if (veh == 0) {
    ...
    for(auto it = MAPBUFFER.begin(); it != MAPBUFFER.end(); ++it) {
        ...
        tripoint sm_topleft = overmapbuffer::sm_to_ms_copy(tripoint(sm_loc.x, sm_loc.y, sm_loc.z));
        tripoint in_reality = m.getlocal(sm_topleft);
        ...
        for(size_t i = 0; i < sm->vehicles.size(); i++) {
            ...
            veh->power_parts();
            if (sm_loc.z == levz) {
                veh->idle(m.inbounds(in_reality.x, in_reality.y, in_reality.z));
            }
        }
    }
    m.process_fields();
    m.process_active_items();
    m.step_in_field(u.posx, u.posy, u.posz);
...
{
    vehicle *veh = NULL;
    veh = m.veh_at(u.activity.placement.x, u.activity.placement.y, u.activity.placement.z);
    ...
    for(int i = -1; i <= 1; i++) {
        for(int j = -1; j <= 1; j++) {
            if( m.ter(u.posx + i, u.posy + j, u.posz) == t_gas_pump ||
                m.ter_at(u.posx + i, u.posy + j, u.posz).id == "t_gas_pump_a" ||
                m.ter(u.posx +i, u.posy + j, u.posz) == t_diesel_pump ) {
                for( auto it = m.i_at(u.posx + i, u.posy + j, u.posz).begin();
                     it != m.i_at(u.posx + i, u.posy + j, u.posz).end(); ) {
                    if( it->type->id == "gasoline" || it->type->id == "diesel" ) {
                        ...
                        if (gas->charges > lack) {
                            ...
                            it = m.i_at(u.posx + i, u.posy + j, u.posz).erase(it);
                            u.activity.moves_left = 0;
                        }
                        i = 2;
                        j = 2;
...
void game::activity_on_finish_make_zlave()
{
    ...
    std::vector<item> &items = m.i_at(u.posx, u.posy, u.posz);
...
void game::rod_fish(int sSkillLevel, int fishChance) // fish-with-rod fish catching function
{
    if (sSkillLevel > fishChance) {
        ...
        if (fishables.size() < 1){
            ...
            m.add_item_or_charges(u.posx, u.posy, u.posz, fish);
...
void game::catch_a_monster(std::vector<monster*> &catchables, int posx, int posy, player *p, int catch_duration) // catching function
{
    ...
    m.add_item_or_charges(posx, posy, p->posz, fish);
...
void game::activity_on_finish_vehicle()
{
    ...
    vehicle *veh = m.veh_at(u.activity.values[0], u.activity.values[1], u.activity.values[2]);
    complete_vehicle();
...
//        debugmsg("Generating weather for turn %d", int(calendar::turn));
        ...
        if (weather != old_weather && weather_data[weather].dangerous &&
            levz >= 0 && m.is_outside(u.posx, u.posy, u.posz)
...
{
    return temperature + m.temperature(u.posx, u.posy, u.posz);
}
...
int game::reserve_random_mission(mission_origin origin, point p, int npc_id)
{
    ...
    mission_place place;
    for (std::vector<mission_type>::iterator mission = mission_types.begin();
         mission != mission_types.end(); ++mission) {
        for (std::vector<mission_origin>::iterator orig =
                 mission->origins.begin();
             orig != mission->origins.end(); ++orig) {
            if (*orig == origin &&
                (place.*mission->place)(p.x, p.y)) {
                valid.push_back(mission->id);
                break;
            }
        }
    }
...
bool game::mission_complete(int id, int npc_id)
{
    ...
    case MGOAL_RECRUIT_NPC_CLASS: {
        ...
        for (std::vector<npc *>::iterator it = npcs.begin();
             it != npcs.end(); ++it) {
            if ((*it)->myclass == miss->recruit_class) {
                if ((*it)->attitude == NPCATT_FOLLOW) {
                    return true;
                }
            }
        }
...
void game::process_missions()
{
    for (std::vector<mission>::iterator it = active_missions.begin();
         it != active_missions.end(); ++it) {
        if (it->deadline > 0 && !it->failed &&
            int(calendar::turn) > it->deadline) {
            fail_mission(it->uid);
        }
    }
}
...
void game::rcdrive(int dx, int dy)
{
    ...
    if( m.move_cost(cx + dx, cy + dy, cz) == 0 || !m.can_put_items(cx + dx, cy + dy, cz) ||
        m.has_furn(cx + dx, cy + dy, cy) ) {
        sound(cx + dx, cy + dy, cz, 7, _("sound of a collision with an obstacle."));
        return;
    } else if( m.add_item_or_charges(cx + dx, cy + dy, cz, *rc_car ) ) {
        //~ Sound of moving a remote controlled car
        sound(cx, cy, cz, 6, _("zzz..."));
        u.moves -= 50;
        m.i_rem( cx, cy, cz, rc_car );
...
bool game::handle_action()
{
        ...
        if (action == "SELECT" || action == "SEC_SELECT") {
            ...
            if (action == "SELECT") {
                ...
                if (!destination_preview.empty()) {
                    tripoint final_destination = destination_preview.back();
                ...
                if (mouse_selected_mondex != -1) {
                    ...
                    act = ACTION_FIRE;
                           ...
                           m.close_door(mx, my, u.posz, !m.is_outside(u.posx, u.posy, u.posz), true)) {
    ...
    case ACTION_MOVE_N:
        ...
        if( u.get_value( "remote_controlling" ) != "" ) {
            ...
            continue_auto_move = plmove(0, -1, 0);
        }
        break;
    ...
    case ACTION_MOVE_NE:
        ...
        if( u.get_value( "remote_controlling" ) != "" ) {
            ...
            continue_auto_move = plmove(1, -1, 0);
        }
        break;
    ...
    case ACTION_MOVE_E:
        ...
        if( u.get_value( "remote_controlling" ) != "" ) {
            ...
            continue_auto_move = plmove(1, 0, 0);
        }
        break;
    ...
    case ACTION_IGNORE_ENEMY:
        if (safe_mode == SAFE_MODE_STOP) {
            ...
            for(std::vector<int>::iterator it = new_seen_mon.begin();
                ...
                critter.ignoring = rl_dist(tripoint(u.posx, u.posy, u.posz), critter.pos());
            }
            safe_mode = SAFE_MODE_ON;
        }
        break;
    ...
    for (int x = scentmap_minx - 1; x <= scentmap_maxx + 1; ++x) {
        for (int y = scentmap_miny; y <= scentmap_maxy; ++y) {
            ...
            if (y == scentmap_miny) {  // Setting y-1 y-0, when we are at the top row...
                for (int i = y - 1; i <= y; ++i) {
                    blocks_scent[x][i] = m.has_flag(TFLAG_WALL, x, i, u.posz);
                    reduces_scent[x][i] = m.has_flag(TFLAG_REDUCE_SCENT, x, i, u.posz);
                }
            }
            blocks_scent[x][y + 1] = m.has_flag(TFLAG_WALL, x, y + 1, u.posz); // ...so only y+1 here.
            reduces_scent[x][y + 1] = m.has_flag(TFLAG_REDUCE_SCENT, x, y + 1, u.posz);
    ...
    for (int x = scentmap_minx; x <= scentmap_maxx; ++x) {
        for (int y = scentmap_miny; y <= scentmap_maxy; ++y) {
            if (not blocks_scent[x][y]) {
                ...
                const int fslime = m.get_field_strength(tripoint(x, y, u.posz), fd_slime) * 10;
...
bool game::is_game_over()
{
    if (uquit == QUIT_SUICIDE) {
        if (u.in_vehicle) {
            m.unboard_vehicle(u.posx, u.posy, u.posz);
        }
        return true;
    }
    if (uquit != QUIT_NO) {
...
void game::load(std::string worldname, std::string name)
{
    ...
    update_map(u.posx, u.posy, u.posz);
...
std::vector<std::string> game::list_active_characters()
{
    ...
    for( auto it = worldsaves.begin(); it != worldsaves.end(); ++it ) {
        saves.push_back(base64_decode(*it));
    }
    return saves;
}
...
void game::debug()
{
    ...
    case 7: {
        ...
        if (!active_npc.empty()) {
            for( auto it = active_npc.begin(); it != active_npc.end(); ++it ) {
                tripoint t = (*it)->global_sm_location();
                add_msg(m_info, _("%s: map (%d:%d) pos (%d:%d)"),
                        (*it)->name.c_str(), t.x, t.y,
                        (*it)->posx, (*it)->posy);
            }
    ...
    case 8:
        for( auto it = active_npc.begin(); it != active_npc.end(); ++it ) {
            add_msg(_("%s's head implodes!"), (*it)->name.c_str());
            (*it)->hp_cur[bp_head] = 0;
        }
        break;
    ...
    case 10:
        if (m.veh_at(u.posx, u.posy, u.posz)) {
            ...
            if (veh_num < (int)opts.size() - 1) {
                ...
                if (veh != NULL) {
                    m.board_vehicle(u.posx, u.posy, u.posz, &u);
                }
            }
        }
        break;
    ...
    case 14: {
        ...
        int npcdex = npc_at(pos.x, pos.y, pos.z);
        if (npcdex == -1) {
    ...
    case 15: {
        ...
        m.create_anomaly(center.x, center.y, u.posz, prop);
        m.spawn_natural_artifact(center.x, center.y, u.posz, prop);
    }
    break;
...
void game::mondebug()
{
    ...
    for (size_t i = 0; i < num_zombies(); i++) {
        ...
        if (critter.has_flag(MF_SEES) &&
            m.sees(critter.posx(), critter.posy(), critter.posz(), u.posx, u.posy, u.posz, -1, tc)) {
...
void game::calculate_footstep_markers(std::vector<point> &result)
{
    ...
    for (size_t i = 0; i < footsteps.size(); i++) {
        if (!u_see(footsteps_source[i]->posx(), footsteps_source[i]->posy())) {
            ...
            for (std::vector<point>::iterator it = footsteps[i].begin();
                 it != footsteps[i].end(); ++it) {
                if (!u_see(it->x, it->y)) {
                    unseen_points.push_back(*it);
                }
            }
...
void game::draw_ter(int posx, int posy)
{
    ...
    if (!destination_preview.empty()) {
        ...
        tripoint final_destination = destination_preview.back();
        tripoint center = tripoint(u.posx + u.view_offset_x, u.posy + u.view_offset_y, u.posz);
        draw_line(final_destination.x, final_destination.y, final_destination.z, center, destination_preview);
...
bool game::sees_u(int x, int y, int z, int &t)
{
    const int mondex = mon_at(x, y, z);
    if (mondex != -1) {
    ...
    return (
               m.sees(x, y, z, u.posx, u.posy, u.posz, -1, t) &&
               !u.is_invisible()
           );
}
...
void game::monmove()
{
    ...
    for (size_t i = 0; i < num_zombies(); i++) {
        monster *critter = &critter_tracker.find(i);
        while (!critter->is_dead() && !critter->can_move_to(critter->posx(), critter->posy(), critter->posz())) {
            // If we can't move to our current position, assign us to a new one
                dbg(D_ERROR) << "game:monmove: " << critter->name().c_str()
                             << " can't move to its location! (" << critter->posx()
                             << ":" << critter->posy() << "), "
                             << m.tername(critter->posx(), critter->posy(), critter->posz()).c_str();
                add_msg( m_debug, "%s can't move to its location! (%d:%d), %s", critter->name().c_str(),
                         critter->posx(), critter->posy(), m.tername(critter->posx(), critter->posy(), critter->posz()).c_str());
            bool okay = false;
            ...
            for (int x = startx; x != endx && !okay; x += xdir) {
                for (int y = starty; y != endy && !okay; y += ydir) {
                    if (critter->can_move_to(x, y, critter->posz()) && is_empty(x, y, critter->posz())) {
                        critter->setpos(x, y);
                        okay = true;
                    }
                }
        ...
        m.mon_in_field(critter->posx(), critter->posy(), critter->posz(), critter);
...
bool game::ambient_sound(int x, int y, int z, int vol, std::string description)
{
    return sound( x, y, z, vol, description, true );
}

bool game::sound(int x, int y, int z, int vol, std::string description, bool ambient)
{
    ...
    for (int i = 0, numz = num_zombies(); i < numz; i++) {
        ...
        if (vol_goodhearing > 0 && critter.can_hear()) {
            ...
            if (volume > 0) {
                ...
                critter.wander_to(target_x, target_y, critter.posz(), wander_turns);
...
void game::do_blast(const int x, const int y, const int z, const int power, const int radius, const bool fire)
{
    int dam;
    for (int i = x - radius; i <= x + radius; i++) {
        for (int j = y - radius; j <= y + radius; j++) {
            for (int h = z - radius; h <= z + radius; h++){
                ...
                m.bash(i, j, h, dam);
                m.bash(i, j, h, dam); // Double up for tough doors, etc.

                int mon_hit = mon_at(i, j, h), npc_hit = npc_at(i, j, h);
                if (mon_hit != -1) {
                ...
                if (fire) {
                    m.add_field(i, j, h, fd_fire, dam / 10);
                }
            }
        }
    }
...
void game::explosion(int x, int y, int z, int power, int shrapnel, bool fire, bool blast)
{
    ...
    if (power >= 30) {
        sound(x, y, z, noise, _("a huge explosion!"));
    } else if (power >= 4) {
        sound(x, y, z, noise, _("an explosion!"));
    } else {
        sound(x, y, z, 3, _("a loud pop!"));
    }
    if (blast) {
        do_blast(x, y, z, power, radius, fire);
        // Draw the explosion
        draw_explosion(x, y, z, radius, c_red);
    }
    ...
    for (int i = 0; i < shrapnel; i++) {
        ...
        for (size_t j = 0; j < traj.size(); j++) {
            ...
            const int npcdex = npc_at(tx, ty, z);
            if (zid != -1) {
                ...
                m.shoot(tx, ty, z, dam, j == traj.size() - 1, shrapnel_effects);
            }
        }
    }
}

void game::flashbang(int x, int y, int z, bool player_immune)
{
    draw_explosion(x, y, z, 8, c_white);
    int dist = rl_dist(u.posx, u.posy, x, y), t;
    if (dist <= 8 && !player_immune) {
        if (!u.has_bionic("bio_ears") && !u.is_wearing("rm13_armor_on")) {
            u.add_effect("deaf", 40 - dist * 4);
        }
        if (m.sees(u.posx, u.posy, u.posz, x, y, z, 8, t)) {
            int flash_mod = 0;
            if (u.has_trait("PER_SLIME")) {
                if (one_in(2)) {
    ...
    for (size_t i = 0; i < num_zombies(); i++) {
        ...
        if (dist <= 8) {
            if (critter.has_flag(MF_SEES) && m.sees(critter.posx(), critter.posy(), critter.posz(), x, y, z, 8, t)) {
    ...
    sound(x, y, z, 12, _("a huge boom!"));
    // TODO: Blind/deafen NPC
}

void game::shockwave(int x, int y, int z, int radius, int force, int stun, int dam_mult,
                     bool ignore_player)
{
    draw_explosion(x, y, z, radius, c_blue);

    sound(x, y, z, force * force * dam_mult / 2, _("Crack!"));
    for (size_t i = 0; i < num_zombies(); i++) {
        monster &critter = critter_tracker.find(i);
    ...
    for (std::vector<npc *>::iterator it = active_npc.begin(); it != active_npc.end(); ++it) {
        if (rl_dist((*it)->posx, (*it)->posy, x, y) <= radius) {
            add_msg(_("%s is caught in the shockwave!"), (*it)->name.c_str());
            knockback(x, y, (*it)->posx, (*it)->posy, force, stun, dam_mult);
        }
    }
...
void game::knockback(std::vector<point> &traj, int force, int stun, int dam_mult)
{
    ...
    int ty = traj.front().y;
    const int zid = mon_at(tx, ty);
    if (zid == -1 && npc_at(tx, ty,u.posz) == -1 && (u.posx != tx && u.posy != ty)) {
    ...
    if (zid != -1) {
        ...
        for (size_t i = 1; i < traj.size(); i++) {
            if (m.move_cost(traj[i].x, traj[i].y, targ->posz()) == 0) {
                targ->setpos(tripoint(traj[i - 1].x,traj[i-1].y,targ->posz()));
                ...
                m.bash(traj[i].x, traj[i].y, targ->posz(), 2 * dam_mult * force_remaining);
                break;
            } else if (mon_at(traj[i].x, traj[i].y) != -1 || npc_at(traj[i].x, traj[i].y, u.posz) != -1 ||
                ...
                if (mon_at(traj.front().x, traj.front().y) != -1) {
                    add_msg(_("%s collided with something else and sent it flying!"),
                            targ->name().c_str());
                } else if (npc_at(traj.front().x, traj.front().y, u.posz) != -1) {
                    if (active_npc[npc_at(traj.front().x, traj.front().y, u.posz)]->male) {
            ...
            targ->setpos(tripoint(traj[i].x,traj[i].y,targ->posz()));
            if (m.has_flag("LIQUID", targ->posx(), targ->posy(), targ->posz()) && !targ->can_drown() && !targ->is_dead()) {
                targ->die( nullptr );
                if (u_see(targ)) {
                    add_msg(_("The %s drowns!"), targ->name().c_str());
                }
            }
            if (!m.has_flag("LIQUID", targ->posx(), targ->posy(), targ->posz()) && targ->has_flag(MF_AQUATIC) &&
                !targ->is_dead()) {
                targ->die( nullptr );
                if (u_see(targ)) {
    ...
    } else if (npc_at(tx, ty, u.posz) != -1) {
        npc *targ = active_npc[npc_at(tx, ty, u.posz)];
        if (stun > 0) {
            targ->add_effect("stunned", stun);
            add_msg(ngettext("%s was stunned for %d turn!",
                             "%s was stunned for %d turns!", stun),
        ...
        for (size_t i = 1; i < traj.size(); i++) {
            if (m.move_cost(traj[i].x, traj[i].y, targ->posz) == 0) { // oops, we hit a wall!
                ...
                m.bash(traj[i].x, traj[i].y, targ->posz, 2 * dam_mult * force_remaining);
                break;
            } else if (mon_at(traj[i].x, traj[i].y) != -1 || npc_at(traj[i].x, traj[i].y, targ->zpos()) != -1 ||
                       (u.posx == traj[i].x && u.posy == traj[i].y)) {
                targ->posx = traj[i - 1].x;
                targ->posy = traj[i - 1].y;
                force_remaining = traj.size() - i;
                ...
                if (mon_at(traj.front().x, traj.front().y) != -1) {
                    add_msg(_("%s collided with something else and sent it flying!"),
                            targ->name.c_str());
                } else if (npc_at(traj.front().x, traj.front().y, targ->zpos()) != -1) {
                    if (active_npc[npc_at(traj.front().x, traj.front().y, targ->zpos())]->male) {
                        add_msg(_("%s collided with someone else and sent him flying!"),
                                targ->name.c_str());
                    } else {
                        add_msg(_("%s collided with someone else and sent her flying!"),
        ...
        for (size_t i = 1; i < traj.size(); i++) {
            if (m.move_cost(traj[i].x, traj[i].y,u.zpos()) == 0) { // oops, we hit a wall!
                u.posx = traj[i - 1].x;
                u.posy = traj[i - 1].y;
                force_remaining = traj.size() - i;
                if (stun != 0) {
                ...
                m.bash(traj[i].x, traj[i].y, u.zpos(), 2 * dam_mult * force_remaining);
                break;
            } else if (mon_at(traj[i].x, traj[i].y) != -1 || npc_at(traj[i].x, traj[i].y, u.zpos()) != -1) {
                u.posx = traj[i - 1].x;
                u.posy = traj[i - 1].y;
                force_remaining = traj.size() - i;
                if (stun != 0) {
                ...
                } else if (npc_at(traj.front().x, traj.front().y, u.zpos()) != -1) {
                    if (active_npc[npc_at(traj.front().x, traj.front().y, u.zpos())]->male) {
            ...
            if (m.has_flag("LIQUID", u.posx, u.posy, u.zpos()) && force_remaining < 1) {
...
void game::use_computer(int x, int y, int z)
{
    ...
    computer *used = m.computer_at(x, y, z);
...
void game::resonance_cascade(int x, int y, int z)
{
    ...
    int startz = (z < 8 ? 0 : z - 8), endz = (z + 8 >= SEEZ * 3 ? SEEZ * 3 - 1 : z + 8);
    for (int i = startx; i <= endx; i++) {
        for (int j = starty; j <= endy; j++) {
            for (int h = startz; h <= endz; h++){
                ...
                case  9:
                case 10:
                    m.add_trap(i, j, h, tr_portal);
                    break;
                case 11:
                case 12:
                    m.add_trap(i, j, h, tr_goo);
                    break;
                case 13:
                case 14:
                case 15:
                    spawn_details = MonsterGroupManager::GetResultFromGroup("GROUP_NETHER");
                    invader = monster(GetMType(spawn_details.name), i, j, h);
                    add_zombie(invader);
                    break;
                case 16:
                case 17:
                case 18:
                    ...
                    break;
                case 19:
                    explosion(i, j, h, rng(1, 10), rng(0, 1) * rng(0, 6), one_in(4));
                    break;
                }
            }
        }
...
void game::scrambler_blast(int x, int y, int z)
{
...
void game::emp_blast(int x, int y, int z)
{
    int rn;
    if (m.has_flag("CONSOLE", x, y, z)) {
        add_msg(_("The %s is rendered non-functional!"), m.tername(x, y, z).c_str());
        m.ter_set(x, y, z, t_console_broken);
        return;
    }
    // TODO: More terrain effects.
    if (m.ter(x, y, z) == t_card_science || m.ter(x, y, z) == t_card_military) {
        rn = rng(1, 100);
        if (rn > 92 || rn < 40) {
            add_msg(_("The card reader is rendered non-functional."));
            m.ter_set(x, y, z, t_card_reader_broken);
        }
        if (rn > 80) {
            add_msg(_("The nearby doors slide open!"));
            for (int i = -3; i <= 3; i++) {
                for (int j = -3; j <= 3; j++) {
                    if (m.ter(x + i, y + j, z) == t_door_metal_locked) {
                        m.ter_set(x + i, y + j, z, t_floor);
                    }
                }
            }
        }
    ...
    if (mondex != -1) {
        ...
        if (critter.has_flag(MF_ELECTRONIC)) {
            ...
            if( !mon_item_id.empty() && deact_chance != 0 && one_in( deact_chance ) ) {
                ...
                m.add_item_or_charges( x, y, z, critter.to_item() );
                for( auto & ammodef : critter.ammo ) {
                    if( ammodef.second > 0 ) {
                        m.spawn_item( x, y, z, ammodef.first, 1, ammodef.second, calendar::turn );
                    }
                }
    ...
    for (std::vector<item>::iterator it = m.i_at(x, y, z).begin();
         it != m.i_at(x, y, z).end(); ++it) {
        if (it->is_tool() &&
...
int game::npc_at(const int x, const int y, const int z) const
{
    for (size_t i = 0; i < active_npc.size(); i++) {
        if (active_npc[i]->posx == x && active_npc[i]->posy == y && active_npc[i]->posz == z && !active_npc[i]->is_dead()) {
            return (int)i;
        }
    }
    return -1;
...
Creature *game::critter_at(int x, int y, int z)
{
    const int mindex = mon_at(x, y, z);
    if (mindex != -1) {
        return &zombie(mindex);
    }
    if (x == u.posx && y == u.posy && z == u.posz) {
        return &u;
    }
    const int nindex = npc_at(x, y, z);
    if (nindex != -1) {
...
bool game::update_zombie_pos(const monster &critter, const int newx, const int newy, const int newz)
{
    return critter_tracker.update_pos(critter, newx, newy, newz);
}
...
int game::mon_at(const int x, const int y, const int z) const
{
    return critter_tracker.mon_at(x, y, z);
}

int game::mon_at(tripoint p) const
{
...
bool game::is_empty(const int x, const int y, const int z)
{
    return ((m.move_cost(x, y, z) > 0 || m.has_flag("LIQUID", x, y, z)) &&
            npc_at(x, y, z) == -1 && mon_at(x, y, z) == -1 &&
            (u.posx != x || u.posy != y || u.posz != z));
}

bool game::is_in_sunlight(int x, int y, int z)
{
    return (m.is_outside(x, y, z) && light_level() >= 40 &&
            (weather == WEATHER_CLEAR || weather == WEATHER_SUNNY));
}

bool game::is_sheltered(int x, int y, int z)
{
...
bool game::revive_corpse(int x, int y, int z, int n)
{
    if ((int)m.i_at(x, y, z).size() <= n) {
        debugmsg("Tried to revive a non-existent corpse! (%d, %d), #%d of %d", x, y, n, m.i_at(x,
                 y, z).size());
        return false;
    }
    if (!revive_corpse(x, y, z, &m.i_at(x, y, z)[n])) {
        return false;
    }
    m.i_rem(x, y, z, n);
    return true;
...
void game::open()
{
    ...
    if (veh) {
        ...
        if (openable >= 0) {
            const vehicle *player_veh = m.veh_at(u.posx, u.posy, u.posz);
    ...
    bool didit = m.open_door(openx, openy, u.posz, !m.is_outside(u.posx, u.posy, u.posz));

    if (!didit) {
        const std::string terid = m.get_ter(openx, openy, u.posz);
...
void game::close(int closex, int closey)
{
    ...
    bool didit = false;
    const bool inside = !m.is_outside(u.posx, u.posy, u.posz);

    std::vector<item> &items_in_way = m.i_at(closex, closey, u.posz);
    int vpart;
    ...
    if (zid != -1) {
        ...
        if (openable >= 0) {
            ...
            if (veh->part_info(openable).has_flag("OPENCLOSE_INSIDE")) {
                const vehicle *in_veh = m.veh_at(u.posx, u.posy, u.posz);
    ...
    } else if (m.has_furn(closex, closey, u.posz) && m.furn_at(closex, closey, u.posz).close.empty()) {
        add_msg(m_info, _("There's a %s in the way!"), m.furnname(closex, closey, u.posz).c_str());
    } else if (!m.close_door(closex, closey, u.posz, inside, true)) {
        // ^^ That checks if the PC could close something there, it
        // does not actually do anything.
        std::string door_name;
        if (m.has_furn(closex, closey, u.posz)) {
            door_name = furnlist[m.furn(closex, closey, u.posz)].name;
        } else {
            door_name = terlist[m.ter(closex, closey, u.posz)].name;
        }
        // Print a message that we either can not close whatever is there
        // or (if we're outside) that we can only close it from the
        // inside.
        if (!inside && m.close_door(closex, closey, u.posz, true, true)) {
        ...
        if (m.furn(closex, closey, u.posz) != f_safe_o && !items_in_way.empty()) {
        ...
        didit = m.close_door(closex, closey, u.posz, inside, false);
        if (didit && m.has_flag_ter_or_furn("NOITEM", closex, closey, u.posz)) {
            // Just plopping items back on their origin square will displace them to adjacent squares
            // since the door is closed now.
            for (std::vector<item>::iterator cur_item = items_in_way.begin();
                 cur_item != items_in_way.end(); ++cur_item) {
                m.add_item_or_charges(closex, closey, u.posz, *cur_item);
            }
...
void game::smash()
{
    ...
    if( m.get_field( tripoint( smashx, smashy, u.posz ), fd_web ) != nullptr ) {
        m.remove_field( smashx, smashy, u.posz, fd_web );
        sound( smashx, smashy, u.posz, 2, "" );
        add_msg( m_info, _( "You brush aside some webs." ) );
        u.moves -= 100;
        return;
    }
    static const int full_pulp_threshold = 4;
    for (auto it = m.i_at(smashx, smashy, u.posz).begin(); it != m.i_at(smashx, smashy, u.posz).end(); ++it) {
        if (it->type->id == "corpse" && it->damage < full_pulp_threshold) {
            ...
            u.activity.placement = tripoint(smashx, smashy, u.posz);
            return; // don't smash terrain if we've smashed a corpse
        }
    }
    didit = m.bash(smashx, smashy, u.posz, smashskill).first;
    if (didit) {
        u.handle_melee_wear();
        u.moves -= move_cost;
        ...
        if (u.weapon.made_of("glass") &&
            ...
            for (std::vector<item>::iterator it = u.weapon.contents.begin();
                ...
                m.add_item_or_charges(u.posx, u.posy, u.posz, *it);
            }
            sound(u.posx, u.posy, u.posz, 24, "");
        ...
        if (smashskill < m.bash_resistance(smashx, smashy, u.posz) && one_in(10)) {
            if (m.has_furn(smashx, smashy, u.posz) && m.furn_at(smashx, smashy, u.posz).bash.str_min != -1) {
                // %s is the smashed furniture
                add_msg(m_neutral, _("You don't seem to be damaging the %s."), m.furnname(smashx, smashy, u.posz).c_str());
            } else {
                // %s is the smashed terrain
                add_msg(m_neutral, _("You don't seem to be damaging the %s."), m.tername(smashx, smashy, u.posz).c_str());
            }
        }
    } else {
...
void game::activity_on_turn_pulp()
{
    ...
    for( std::vector<item>::iterator it = m.i_at(smashx, smashy, u.posz).begin();
         it != m.i_at(smashx, smashy, u.posz).end(); ++it ) {
        if (!(it->type->id == "corpse" && it->damage < full_pulp_threshold)) {
                ...
                for (int x = smashx - 1; x <= smashx + 1; x++) {
                    for (int y = smashy - 1; y <= smashy + 1; y++) {
                        if (!one_in(damage + 1) && type_blood != fd_null) {
                            m.add_field(x, y, u.posz, type_blood, 1);
                        }
                    }
                }
            }
...
bool game::vehicle_near()
{
    for (int dx = -1; dx <= 1; dx++) {
        for (int dy = -1; dy <= 1; dy++) {
            if (m.veh_at(u.posx + dx, u.posy + dy, u.posz)) {
                return true;
            }
        }
    }
...
bool game::forced_gate_closing(int x, int y, int z, ter_id door_type, int bash_dmg)
{
    ...
    if (bash_dmg < 0 && !m.i_at(x, y, z).empty()) {
        return false;
    }
    if(bash_dmg == 0) {
        for(std::vector<item>::iterator it = m.i_at(x, y, z).begin();
    ...
    m.ter_set(x, y, z, door_type);
    if (m.has_flag("NOITEM", x, y, z)) {
        std::vector<item> &items = m.i_at(x, y, z);
        while (!items.empty()) {
            ...
            m.add_item_or_charges(kbx, kby, u.posz, items[0]);
            items.erase(items.begin());
        }
    }
    return true;
            ...
            for (int gate_x = -1; gate_x <= 1; gate_x++) {
                for (int gate_y = -1; gate_y <= 1; gate_y++) {
                    if ((wall_x + wall_y == 1 || wall_x + wall_y == -1) &&
                        ...
                        ((wall_y != 0 && (m.ter(examx + wall_x, examy + wall_y, u.posz) == h_wall_type)) ||
                         //horizontal orientation of the gate
                         (wall_x != 0 &&
                          (m.ter(examx + wall_x, examy + wall_y, u.posz) == v_wall_type)))) { //vertical orientation of the gate

                        int cur_x = examx + wall_x + gate_x;
                        int cur_y = examy + wall_y + gate_y;

                        if (!close &&
                            (m.ter(examx + wall_x + gate_x, examy + wall_y + gate_y, u.posz) == door_type)) {  //opening the gate...
                            open = true;
                            while (m.ter(cur_x, cur_y, u.posz) == door_type) {
                                m.ter_set(cur_x, cur_y, u.posz, floor_type);
                                cur_x = cur_x + gate_x;
                                cur_y = cur_y + gate_y;
                            }
                        }
                        ...
                        if (!open &&
                            (m.ter(examx + wall_x + gate_x, examy + wall_y + gate_y, u.posz) == floor_type)) {  //closing the gate...
                            close = true;
                            while (m.ter(cur_x, cur_y, u.posz) == floor_type) {
                                forced_gate_closing(cur_x, cur_y, u.posz, door_type, bash_dmg);
                                cur_x = cur_x + gate_x;
                                cur_y = cur_y + gate_y;
                            }
                        }
...
void game::moving_vehicle_dismount(int tox, int toy)
{
    ...
    if (!m.has_flag("SWIMMABLE", u.posx, u.posy, u.posz)) {
        if (veh->velocity > 0) {
...
{
    int veh_part;
    vehicle *veh = m.veh_at(u.posx, u.posy, u.posz, veh_part);
    ...
    if (veh && veh->player_in_control(&u)) {
        ...
        veh = m.veh_at(examx, examy, u.posz, veh_part);
        if (!veh) {
...
bool pet_menu(monster *z)
{
    ...
    if (push_zlave == choice) {
        ...
        z->move_to(z->posx() + deltax, z->posy() + deltay, z->posz());
    ...
    if (drop_all == choice) {
        for (std::vector<item>::iterator it = z->inv.begin();
             it != z->inv.end(); ++it) {
            g->m.add_item_or_charges(z->posx(), z->posy(), z->posz(), *it);
        }
...
void game::examine(int examx, int examy)
{
    ...
    if (veh) {
        ...
        std::vector<item> here_ground = m.i_at(examx, examy, u.posz);
    ...
    if (m.has_flag("CONSOLE", examx, examy, u.posz)) {
        use_computer(examx, examy, u.posz);
        return;
    }
    const furn_t *xfurn_t = &furnlist[m.furn(examx, examy, u.posz)];
    const ter_t *xter_t = &terlist[m.ter(examx, examy, u.posz)];
    iexamine xmine;
    const int player_x = u.posx;
    const int player_y = u.posy;
    const int player_z = u.posz;

    if (m.has_furn(examx, examy, u.posz)) {
    ...
    if (critter_at(examx, examy, u.posz) != NULL) {
        Creature *c = critter_at(examx, examy, u.posz);
    ...
    if (m.has_flag("SEALED", examx, examy, u.posz)) {
        if (none) {
            if (m.has_flag("UNSTABLE", examx, examy, u.posz)) {
                add_msg(_("The %s is too unstable to remove anything."), m.name(examx, examy, u.posz).c_str());
            } else {
                add_msg(_("The %s is firmly sealed."), m.name(examx, examy, u.posz).c_str());
            }
        }
    } else {
        //examx,examy has no traps, is a container and doesn't have a special examination function
        if (m.tr_at(examx, examy, u.posz) == tr_null && m.i_at(examx, examy, u.posz).empty() &&
            m.has_flag("CONTAINER", examx, examy, u.posz) && none) {
    ...
    if(m.tr_at(examx, examy, u.posz) != tr_null) {
...
void game::peek(int peekx, int peeky)
{
    ...
    //TODO: Make peeking towards open air look at the level below?
    if (peekx == 0 && peeky == 0) {
        if (!choose_adjacent(_("Peek where?"), peekx, peeky)) {
            return;
        }

        if (m.move_cost(peekx, peeky, u.posz) == 0) {
            return;
        }
    }
...
void game::print_all_tile_info(int lx, int ly, int lz, WINDOW *w_look, int column, int &line,
                               bool mouse_hover)
{
    print_terrain_info(lx, ly, lz, w_look, column, line);
    print_fields_info(lx, ly, lz, w_look, column, line);
    print_trap_info(lx, ly, lz, w_look, column, line);
    print_object_info(lx, ly, lz, w_look, column, line, mouse_hover);
}

void game::print_terrain_info(int lx, int ly, int lz, WINDOW *w_look, int column, int &line)
{
    int ending_line = line + 3;
    std::string tile = m.tername(lx, ly, lz);
    if (m.has_furn(lx, ly, lz)) {
        furn_t furn = m.furn_at(lx, ly, lz);
        tile += "; " + furn.name;
        if (furn.has_flag("PLANT")) {
            // Plant types are defined by seeds.
            item plantType = m.i_at(lx, ly, lz)[0];
    ...
    mvwprintw(w_look, ++line, column, "%s", m.features(lx, ly, lz).c_str());
    if (line < ending_line) {
        line = ending_line;
    }
}

void game::print_fields_info(int lx, int ly, int lz, WINDOW *w_look, int column, int &line)
{
    const field &tmpfield = m.field_at(lx, ly, lz);
...
void game::print_trap_info(int lx, int ly, int lz, WINDOW *w_look, const int column, int &line)
{
    trap_id trapid = m.tr_at(lx, ly, lz);
    if (trapid == tr_null) {
        return;
    }
    if (traplist[trapid]->can_see(u, lx, ly, lz)) {
        mvwprintz(w_look, line++, column, traplist[trapid]->color, "%s", traplist[trapid]->name.c_str());
    }
}

void game::print_object_info(int lx, int ly, int lz, WINDOW *w_look, const int column, int &line,
                             bool mouse_hover)
{
    int veh_part = 0;
    vehicle *veh = m.veh_at(lx, ly, lz, veh_part);
    const Creature *critter = critter_at( lx, ly, lz );
    ...
    handle_multi_item_info(lx, ly, lz, w_look, column, line, mouse_hover);
}

void game::handle_multi_item_info(int lx, int ly, int lz, WINDOW *w_look, const int column, int &line,
                                  bool mouse_hover)
{
    if (m.sees_some_items(lx, ly, lz, u)) {
        if (mouse_hover) {
            // items are displayed from the live view, don't do this here
            return;
        }
        std::vector<item> &items = m.i_at(lx, ly, lz);
    ...
    } else if (m.has_flag("CONTAINER", lx, ly, lz) && !m.could_see_items(lx, ly, lz, u)) {
...
bool game::checkZone(const std::string p_sType, const int p_iX, const int p_iY, const int p_iZ)
{
    return u.Zones.hasZone(p_sType, m.getabs(p_iX, p_iY, p_iZ));
}
...
void game::zones_manager()
{
    ...
    do {
        if (action == "ADD_ZONE") {
            ...
            tripoint pFirst = look_around(w_zones_info, tripoint(-999, -999, -999));
            tripoint pSecond = tripoint(-1, -1, -1);
            ...
            if (pSecond.x != -1 && pSecond.y != -1) {
                ...
                u.Zones.add("", "", false, true,
                            m.getabs(std::min(pFirst.x, pSecond.x), std::min(pFirst.y, pSecond.y), pSecond.z),
                            m.getabs(std::max(pFirst.x, pSecond.x), std::max(pFirst.y, pSecond.y), pSecond.z)
                           );
            ...
            if (action == "UP") {
                ...
                tripoint pOMPlayer = overmapbuffer::ms_to_omt_copy(m.getabs(u.posx, u.posy, u.posz));
                tripoint pOMZone = overmapbuffer::ms_to_omt_copy(u.Zones.vZones[iActive].getCenterPoint());
                overmap::draw_overmap(tripoint(pOMPlayer.x, pOMPlayer.y, pOMPlayer.z),
                                      false,
                                      tripoint(pOMZone.x, pOMZone.y, pOMPlayer.z),
                                      iActive
                                     );
        ...
        if (iZonesNum == 0) {
            ...
            tripoint pointPlayer = m.getabs(u.posx, u.posy, u.posz);
            ...
            for (auto &i : u.Zones.vZones) {
                if (iNum >= iStartPos && iNum < iStartPos + ((iMaxRows > iZonesNum) ? iZonesNum : iMaxRows)) {
                    ...
                    tripoint pCenter = i.getCenterPoint();
        ...
        if (iZonesNum > 0) {
            ...
            tripoint pStart = m.getlocal(u.Zones.vZones[iActive].getStartPoint());
            tripoint pEnd = m.getlocal(u.Zones.vZones[iActive].getEndPoint());

            if (bBlink) {
                //draw marked area
                tripoint pOffset = tripoint(offset_x, offset_y, u.posz); //ASCII
#ifdef TILES
                if (use_tiles) {
...
tripoint game::look_around(WINDOW *w_info, const tripoint pairCoordsFirst)
{
    temp_exit_fullscreen();

    bool bSelectZone = (pairCoordsFirst.x != -1 && pairCoordsFirst.y != -1 && pairCoordsFirst.z != -1);
    bool bHasFirstPoint = (pairCoordsFirst.x != -999 && pairCoordsFirst.y != -999 && pairCoordsFirst.z != -999);

    const int offset_x = (u.posx + u.view_offset_x) - getmaxx(w_terrain) / 2;
    const int offset_y = (u.posy + u.view_offset_y) - getmaxy(w_terrain) / 2;

    int lx = u.posx + u.view_offset_x, ly = u.posy + u.view_offset_y, lz = u.posz;
    ...
    do {
        ...
        if (bSelectZone) {
            ...
            if (bHasFirstPoint) {
                ...
                if (bBlink) {
                    const tripoint pStart = tripoint(std::min(dx, POSX), std::min(dy, POSY), u.posz);
                    const tripoint pEnd = tripoint(std::max(dx, POSX), std::max(dy, POSY), u.posz);

                    tripoint pOffset = tripoint(0, 0, 0); //ASCII/SDL
#ifdef TILES
                    if (use_tiles) {
            ...
            if( m.has_graffiti_at( lx, ly, lz ) ) {
                mvwprintw(w_info, ++off + 1, 1, _("Graffiti: %s"), m.graffiti_at( lx, ly, lz ).c_str() );
            }
...
std::vector<map_item_stack> game::find_nearby_items(int iRadius)
{
    ...
    std::vector<tripoint> points = closest_points_first(iRadius, u.posx, u.posy, u.posz);

    int iLastX = 0;
    int iLastY = 0;

    for (std::vector<tripoint>::iterator p_it = points.begin(); p_it != points.end(); ++p_it) {
        if (p_it->y >= u.posy - iRadius && p_it->y <= u.posy + iRadius &&
            u_see(p_it->x, p_it->y) && m.sees_some_items(p_it->x, p_it->y, u.posz, u)) {

            here.clear();
            here = m.i_at(p_it->x, p_it->y, u.posz);
            for (std::vector<item>::iterator it = here.begin(); it != here.end(); ++it) {
                ...
                if (temp_items.find(name) == temp_items.end() || (iLastX != p_it->x || iLastY != p_it->y)) {
                    ...
                    if (std::find(vOrder.begin(), vOrder.end(), name) == vOrder.end()) {
                        ...
                        temp_items[name] = map_item_stack(*it, p_it->x - u.posx, p_it->y - u.posy);
                    } else {
    ...
    for( std::vector<std::string>::iterator it = vOrder.begin(); it != vOrder.end(); ++it ) {
        ret.push_back(temp_items[*it]);
    }
...
void game::draw_trail_to_square(int x, int y, bool bDrawX)
{
    //Reset terrain
    draw_ter();
    //Shooting between z-levels is going to be saved for not now
    std::vector<tripoint> vPoint;
    tripoint center = tripoint(u.posx + u.view_offset_x, u.posy + u.view_offset_y, u.posz);
    if (x != 0 || y != 0) {
        //Draw trail
        vPoint = line_to(tripoint(u.posx, u.posy, u.posz), tripoint(u.posx + x, u.posy + y, u.posz), 0, 0);
    } else {
        //Draw point
        vPoint.push_back(tripoint(u.posx, u.posy, u.posz));
    }

    draw_line(u.posx + x, u.posy + y, u.posz, center, vPoint);
    if (bDrawX) {
        if (vPoint.empty()) {
            mvwputch(w_terrain, POSY, POSX, c_white, 'X');
        } else {
        ...
        if (!vMonsters.empty() || iLastState == 1) {
            if (action == "UP") {
                ...
                if (iMonDex >= 0 && size_t(iMonDex) < num_zombies() &&
                    rl_dist(tripoint(u.posx, u.posy, u.posz), zombie(iMonDex).pos()) <= iWeaponRange) {
                    last_target = iMonDex;
                    u.view_offset_x = iStoreViewOffsetX;
                    u.view_offset_y = iStoreViewOffsetY;
                    return 2;
            ...
            if (vMonsters.empty() && iLastState == 1) {
                ...
                if (rl_dist(u.pos(), zombie(iMonDex).pos()) <= iWeaponRange) {
...
void game::grab()
{
    ...
    if (0 != u.grab_point.x || 0 != u.grab_point.y) {
        vehicle *veh = m.veh_at(u.posx + u.grab_point.x, u.posy + u.grab_point.y, u.posz);
        if (veh) {
            add_msg(_("You release the %s."), veh->name.c_str());
        } else if (m.has_furn(u.posx + u.grab_point.x, u.posy + u.grab_point.y, u.posz)) {
            add_msg(_("You release the %s."), m.furnname(u.posx + u.grab_point.x,
                    u.posy + u.grab_point.y, u.posz).c_str());
        }
        u.grab_point.x = 0;
        u.grab_point.y = 0;
    ...
    if (choose_adjacent(_("Grab where?"), grabx, graby)) {
        vehicle *veh = m.veh_at(grabx, graby, u.posz);
        ...
        } else if (m.has_furn(grabx, graby, u.posz)) { // If not, grab furniture if present
            if (m.furn_at(grabx, graby, u.posz).move_str_req < 0) {
                add_msg(_("You can not grab the %s"), m.furnname(grabx, graby, u.posz).c_str());
                return;
            }
            u.grab_point.x = grabx - u.posx;
            u.grab_point.y = graby - u.posy;
            u.grab_type = OBJECT_FURNITURE;
            if (!m.can_move_furniture(grabx, graby, u.posz, &u)) {
                add_msg(_("You grab the %s. It feels really heavy."), m.furnname(grabx, graby, u.posz).c_str());
            } else {
                add_msg(_("You grab the %s."), m.furnname(grabx, graby, u.posz).c_str());
            }
                         ...
                         item *cont)
{
    ...
    if( (liquid.type->id == "gasoline" || liquid.type->id == "diesel") &&
        ...
        vehicle *veh = m.veh_at(vx, vy, u.posz);
        if (veh == NULL) {
        ...
        if (fuel_cap <= 0) {
            ...
            u.activity.placement = tripoint(vx, vy, u.posz);
    ...
    if (!from_ground && liquid.rotten() &&
        ...
        //Pouring stuff is probably on the same level as the one pouring it. (That's us!)
        if (!m.can_put_items(dirx, diry, u.posz)) {
            add_msg(m_info, _("You can't pour there!"));
            return false;
        }
        m.add_item_or_charges(dirx, diry, u.posz, liquid, 1);
        return true;
    }
...
void game::drop_in_direction()
{
    ...
    make_drop_activity( ACT_DROP, tripoint(dirx, diry, u.posz) );
}
                              ...
                              int freed_volume_capacity) const
{
    ...
    for(std::vector<item>::iterator it = dropped.begin(); it != dropped.end(); ++it) {
        total_volume_dropped += it->volume();
                ...
                int freed_volume_capacity, int dirx, int diry)
{
    ...
    bool can_move_there = m.move_cost(dirx, diry, u.posz) != 0;
    ...
    if (to_veh) {
        ...
        for (std::vector<item>::iterator it = dropped.begin();
             it != dropped.end(); ++it) {
            vh_overflow = vh_overflow || !veh->add_item (veh_part, *it);
            if (vh_overflow) {
                m.add_item_or_charges(dirx, diry, u.posz, *it, 1);
            }
        }
        if (vh_overflow) {
        ...
        for (std::vector<item>::iterator it = dropped.begin();
             it != dropped.end(); ++it) {
            m.add_item_or_charges(dirx, diry, u.posz, *it, 2);
        }
    }
...
void game::plthrow(int pos)
{
    ...
    std::vector <point> trajectory = pl_target_ui(x, y, u.posz, range, &thrown);
    if (trajectory.empty()) {
        return;
    }
...
std::vector<point> game::pl_target_ui(int &x, int &y, int z, int range, item *relevant,
...
void game::plfire(bool burst, int default_target_x, int default_target_y)
{
    ...
    if (u.weapon.has_flag("MOUNTED_GUN")) {
        int vpart = -1;
        vehicle *veh = m.veh_at(u.posx, u.posy, vpart);
        if (!m.has_flag_ter_or_furn("MOUNTABLE", u.posx, u.posy, u.posz) &&
    ...
    std::vector<point> trajectory = pl_target_ui(x, y, u.posz, range, &u.weapon, default_target_x,
                                    default_target_y);
...
void game::complete_butcher(int index)
{
    ...
    if ((int)m.i_at(u.posx, u.posy, u.posz).size() <= index || m.i_at(u.posx, u.posy, u.posz)[index].corpse == NULL ||
        m.i_at(u.posx, u.posy, u.posz)[index].typeId() != "corpse") {
        add_msg(m_info, _("There's no corpse to butcher!"));
        return;
    }
    mtype *corpse = m.i_at(u.posx, u.posy, u.posz)[index].corpse;
    std::vector<item> contents = m.i_at(u.posx, u.posy, u.posz)[index].contents;
    int age = m.i_at(u.posx, u.posy, u.posz)[index].bday;
    m.i_rem(u.posx, u.posy, u.posz, index);
    ...
    if (bones > 0) {
        if (corpse->has_flag(MF_BONES)) {
            m.spawn_item(u.posx, u.posy, u.posz, "bone", bones, 0, age);
            add_msg(m_good, _("You harvest some usable bones!"));
        } else if (corpse->mat == "veggy") {
            m.spawn_item(u.posx, u.posy, u.posz, "plant_sac", bones, 0, age);
    ...
    if (sinews > 0) {
        if (corpse->has_flag(MF_BONES)) {
            m.spawn_item(u.posx, u.posy, u.posz, "sinew", sinews, 0, age);
            add_msg(m_good, _("You harvest some usable sinews!"));
        } else if (corpse->mat == "veggy") {
            m.spawn_item(u.posx, u.posy, u.posz, "plant_fibre", sinews, 0, age);
    ...
    if ((corpse->has_flag(MF_FUR) || corpse->has_flag(MF_LEATHER) ||
        ...
        if (chitin) {
            m.spawn_item(u.posx, u.posy, u.posz, "chitin_piece", chitin, 0, age);
        }
        if (fur) {
            m.spawn_item(u.posx, u.posy, u.posz, "raw_fur", fur, 0, age);
        }
        if (leather) {
            m.spawn_item(u.posx, u.posy, u.posz, "raw_leather", leather, 0, age);
        }
    }

    if (feathers > 0) {
        if (corpse->has_flag(MF_FEATHER)) {
            m.spawn_item(u.posx, u.posy, u.posz, "feather", feathers, 0, age);
    ...
    if (fats > 0) {
        if (corpse->has_flag(MF_FAT)) {
            m.spawn_item(u.posx, u.posy, u.posz, "fat", fats, 0, age);
    ...
    if (corpse->has_flag(MF_CBM_CIV)) {
        ...
        if (skill_shift >= 0) {
            ...
            if (rng(0, 1) == 1) { //The battery works
                m.spawn_item(u.posx, u.posy, u.posz, "bio_power_storage", 1, 0, age);
            } else { //There is a burnt out CBM
                m.spawn_item(u.posx, u.posy, u.posz, "burnt_out_bionic", 1, 0, age);
            }
        }
        if (skill_shift >= 0) {
            ...
            if (rng(0, 1) == 1) { //The CBM works
                ...
                m.spawn_item(u.posx, u.posy, u.posz, bionic_item, 1, 0, age);
            } else { //There is a burnt out CBM
                m.spawn_item(u.posx, u.posy, u.posz, "burnt_out_bionic", 1, 0, age);
            }
        }
    ...
    if (corpse->has_flag(MF_CBM_OP)) {
        ...
        if (skill_shift >= 0) {
            ...
            //To see if it spawns a battery
            if (rng(0, 1) == 1) { //The battery works
                m.spawn_item(u.posx, u.posy, u.posz, "bio_power_storage_mkII", 1, 0, age);
            } else { //There is a burnt out CBM
                m.spawn_item(u.posx, u.posy, u.posz, "burnt_out_bionic", 1, 0, age);
            }
        }
    ...
    for (std::vector<item>::iterator it = contents.begin(); it != contents.end(); ++it) {
        if ((skill_shift + 10) * 5 > rng(0, 100)) {
            add_msg(m_good, _("You discover a %s in the %s!"),
                    it->tname().c_str(), corpse->nname().c_str());
            m.add_item_or_charges(u.posx, u.posy, u.posz, *it);
    ...
    if (pieces <= 0) {
        ...
        while ( pieces > 0 ) {
            pieces--;
            m.add_item_or_charges(u.posx, u.posy, u.posz, tmpitem);
        }
    }
}
...
void game::forage()
{
    ...
    if (veggy_chance < ((u.skillLevel("survival") * 1.5) + ((u.per_cur / 2 - 4) + 3))) {
        ...
        if (cnt > 0) {
            ...
            m.ter_set(u.activity.placement.x, u.activity.placement.y, u.activity.placement.z, t_dirt);
            found_something = true;
        }
    } else {
        if (one_in(2)) {
...
void game::eat(int pos)
{
    if ((u.has_active_mutation("RUMINANT") || u.has_active_mutation("GRAZER")) &&
        m.ter(u.posx, u.posy, u.posz) == t_underbrush && query_yn(_("Eat underbrush?"))) {
        u.moves -= 400;
        u.hunger -= 10;
        m.ter_set(u.posx, u.posy, u.posz, t_grass);
        add_msg(_("You eat the underbrush."));
        return;
    }
    if (u.has_active_mutation("GRAZER") && m.ter(u.posx, u.posy, u.posz) == t_grass &&
        query_yn(_("Graze?"))) {
        u.moves -= 400;
        if ((u.hunger < 10) || one_in(20 - u.int_cur)) {
            ...
            u.hunger -= 5;
            m.ter_set(u.posx, u.posy, u.posz, t_dirt);
        }
        return;
    }
    if (pos == INT_MIN) {
...
void game::unload(item &it)
{
        ...
        while (!it.contents.empty()) {
            ...
            if (content.made_of(LIQUID)) {
                ...
                if (u.can_pickVolume(content.volume()) &&
                    ...
                    m.add_item_or_charges(u.posx, u.posy, u.posz, content, 1);
                }
            }
            it.contents.erase(it.contents.begin());
        }
    ...
    if (newam.made_of(LIQUID)) {
        ...
        if (u.can_pickWeight(newam.weight(), !OPTIONS["DANGEROUS_PICKUPS"]) &&
            ...
            m.add_item_or_charges(u.posx, u.posy, u.posz, newam, 1);
        }
    }
...
void game::chat()
{
    ...
    for( std::vector<npc *>::iterator it = active_npc.begin(); it != active_npc.end(); ++it ) {
        if( u_see((*it)->posx, (*it)->posy) &&
            rl_dist(u.posx, u.posy, (*it)->posx, (*it)->posy) <= 24 ) {
    ...
    if (available.empty()) {
        ...
        for( std::vector<npc *>::iterator it = available.begin(); it != available.end(); ++it ) {
            npcs.push_back((*it)->name);
        }
...
bool game::disable_robot( const tripoint p )
{
    ...
    if( !mon_item_id.empty() ) {
        ...
        u.moves -= 100;
        m.add_item_or_charges( p.x, p.y, p.z, critter.to_item() );
        for( auto & ammodef : critter.ammo ) {
            if( ammodef.second > 0 ) {
                m.spawn_item( p.x, p.y, p.z, ammodef.first, 1, ammodef.second, calendar::turn );
            }
        }
        remove_zombie( mondex );
        return true;
...
bool game::plmove(int dx, int dy, int dz)
{
    ...
    if( disable_robot( tripoint( x, y, u.posz ) ) ) {
        return false;
    }
    ...
    if (u.has_disease("amigara")) {
        ...
        for (int cx = 0; cx < SEEX * MAPSIZE; cx++) {
            for (int cy = 0; cy < SEEY * MAPSIZE; cy++) {
                for (int cz = 0; cz < SEEZ * MAPSIZE; cz++){
                    if (m.ter(cx, cy, cz) == t_fault) {
    ...
    if (u.grab_point.x != 0 || u.grab_point.y) {
        if (u.grab_type == OBJECT_VEHICLE) { // default; assume OBJECT_VEHICLE
            vehicle *grabbed_vehicle = m.veh_at(u.posx + u.grab_point.x, u.posy + u.grab_point.y, u.posz);
            ...
            tripoint fpos(u.posx + u.grab_point.x, u.posy + u.grab_point.y, u.posz);
            if (m.has_furn(fpos.x, fpos.y, fpos.z)) {
    ...
    //TODO: Make water have various z-levels?
    bool toSwimmable = m.has_flag("SWIMMABLE", x, y, u.posz);
    bool toDeepWater = m.has_flag(TFLAG_DEEP_WATER, x, y, u.posz);
    bool fromSwimmable = m.has_flag("SWIMMABLE", u.posx, u.posy, u.posz);
    bool fromDeepWater = m.has_flag(TFLAG_DEEP_WATER, u.posx, u.posy, u.posz);
    ...
    if (toSwimmable && toDeepWater && !toBoat) { // Dive into water!
    ...
    } else if (m.move_cost(x, y, u.posz) > 0 || pushing_furniture || shifting_furniture || pushing_vehicle) {
        ...
        const field &tmpfld = m.field_at(x, y, u.posz);
        for( auto &fld : tmpfld ) {
            const field_entry &cur = fld.second;
        ...
        const trap_id tid = m.tr_at(x, y, u.posz);
        if (tid != tr_null) {
            const struct trap &t = *traplist[tid];
            if ((t.can_see(u, x, y, u.posz)) && !t.is_benign() &&
        ...
        if (u.grab_point.x != 0 || u.grab_point.y != 0) {
            ...
            if (u.grab_type == OBJECT_VEHICLE) {
                grabbed_vehicle = m.veh_at(u.posx + u.grab_point.x, u.posy + u.grab_point.y, u.posz);
                ...
                if ( ! m.has_furn( fpos.x, fpos.y, u.posz ) ) {
                    ...
                    bool canmove = (
                        ( m.move_cost(fdest.x, fdest.y, u.posz) > 0) &&
                        npc_at(fdest.x, fdest.y, u.posz) == -1 &&
                        mon_at(fdest.x, fdest.y, u.posz) == -1 &&
                        m.has_flag("FLAT", fdest.x, fdest.y, u.posz) &&
                        !m.has_furn(fdest.x, fdest.y, u.posz) &&
                        m.veh_at(fdest.x, fdest.y, u.posz) == NULL &&
                        m.tr_at(fdest.x, fdest.y, u.posz) == tr_null
                        );
                    //TODO: Dragging furniture between floors
                    const furn_t furntype = m.furn_at(fpos.x, fpos.y, u.posz);
                    int furncost = furntype.movecost;
                    const int src_items = m.i_at(fpos.x, fpos.y, u.posz).size();
                    const int dst_items = m.i_at(fdest.x, fdest.y, u.posz).size();
                    bool dst_item_ok = ( ! m.has_flag("NOITEM", fdest.x, fdest.y, u.posz) &&
                                         ! m.has_flag("SWIMMABLE", fdest.x, fdest.y, u.posz) &&
                                         ! m.has_flag("DESTROY_ITEM", fdest.x, fdest.y, u.posz) );
                    bool src_item_ok = ( m.furn_at(fpos.x, fpos.y, u.posz).has_flag("CONTAINER") ||
                                         m.furn_at(fpos.x, fpos.y, u.posz).has_flag("SEALED") );

                    int str_req = furntype.move_str_req;
                    // Factor in weight of items contained in the furniture.
                    int furniture_contents_weight = 0;
                    for( auto contained_item : m.i_at( fpos.x, fpos.y, u.posz ) ) {
                    ...
                    if ( pulling_furniture ) {
                        ...
                        if ( furncost < 0 ) {
                            ...
                            movecost_modifier += m.ter_at(fpos.x, fpos.y, u.posz).movecost;
                    ...
                    sound(x, y, u.posz, furntype.move_str_req * 2, _("a scraping noise."));

                    m.furn_set(fdest.x, fdest.y, u.posz, m.furn(fpos.x, fpos.y, u.posz));    // finally move it.
                    m.furn_set(fpos.x, fpos.y, u.posz, f_null);

                    if ( src_items > 0 ) {  // and the stuff inside.
                        if ( dst_item_ok && src_item_ok ) {
                            // Assume contents of both cells are legal, so we can just swap contents.
                            m.i_at( fpos.x, fpos.y, u.posz).swap( m.i_at(fdest.x, fdest.y, u.posz) );
                        } else {
                    ...
                    if ( shifting_furniture ) { // we didn't move
                        ...
                        return false; // We moved furniture but stayed still.
                                ...
                                m.move_cost(x, y, u.posz) <= 0 ) { // Not sure how that chair got into a wall, but don't let player follow.
                        add_msg( _("You let go of the %s as it slides past %s"),
                                 furntype.name.c_str(), m.ter_at(x, y, u.posz).name.c_str() );
                        u.grab_point = point (0, 0);
                        u.grab_type = OBJECT_NONE;
                    }
                }
        ...
        u.moves -= int(u.run_cost(m.combined_movecost(u.posx, u.posy, u.posz, x, y, u.posz, grabbed_vehicle,
        ...
        if ((!u.has_trait("PARKOUR") && m.move_cost(x, y, u.posz) > 2) ||
            ( u.has_trait("PARKOUR") && m.move_cost(x, y, u.posz) > 4    )) {
            if (veh1 && m.move_cost(x, y, u.posz) != 2) {
                add_msg(m_warning, _("Moving past this %s is slow!"), veh1->part_info(vpart1).name.c_str());
            } else {
                add_msg(m_warning, _("Moving past this %s is slow!"), m.name(x, y, u.posz).c_str());
            }
        }
        if (veh1) {
        ...
        std::string signage = m.get_signage(x, y, u.posz);
        if (signage.size()) {
            add_msg(m_info, _("The sign says: %s"), signage.c_str());
        }
        if( m.has_graffiti_at( x, y, u.posz ) ) {
            add_msg(_("Written here: %s"), utf8_truncate(m.graffiti_at( x, y, u.posz ), 40).c_str());
        }
        if (m.has_flag("ROUGH", x, y, u.posz) && (!u.in_vehicle)) {
            bool ter_or_furn = m.has_flag_ter( "ROUGH", x, y, u.posz );
            if (one_in(5) && u.get_armor_bash(bp_foot_l) < rng(2, 5)) {
                add_msg(m_bad, _("You hurt your left foot on the %s!"),
                        ter_or_furn ? m.tername(x, y, u.posz).c_str() : m.furnname(x, y, u.posz).c_str() );
        ...
        if( m.has_flag("SHARP", x, y, u.posz) && !one_in(3) && !one_in(40 - int(u.dex_cur / 2)) &&
            (!u.in_vehicle) && (!u.has_trait("PARKOUR") || one_in(4)) ) {
            bool ter_or_furn = m.has_flag_ter( "SHARP", x, y, u.posz );
        ...
        if (m.has_flag("UNSTABLE", x, y, u.posz)) {
        ...
        if (u.has_trait("LEG_TENT_BRACE") && (!u.footwear_factor() ||
            ...
            if ((!(m.has_flag("SWIMMABLE", x, y, u.posz)) && (one_in(80 + u.dex_cur + u.int_cur)))) {
        ...
        if (!u.has_artifact_with(AEP_STEALTH) && !u.has_trait("LEG_TENTACLES") &&
            ...
            if (u.has_trait("LIGHTSTEP") || u.is_wearing("rm13_armor_on")) {
                sound(x, y, u.posz, 2, "");    // Sound of footsteps may awaken nearby monsters
        ...
        if (one_in(20) && u.has_artifact_with(AEP_MOVEMENT_NOISE)) {
            sound(x, y, u.posz, 40, _("You emit a rattling sound."));
        }
        // If we moved out of the nonant, we need update our map data
        if (m.has_flag("SWIMMABLE", x, y, u.posz) && u.has_effect("onfire")) {
        ...
        if (OPTIONS["AUTO_PICKUP"] && (!OPTIONS["AUTO_PICKUP_SAFEMODE"] || mostseen == 0) &&
            ((m.i_at(u.posx, u.posy, u.posz)).size() || OPTIONS["AUTO_PICKUP_ADJACENT"])) {
            Pickup::pick_up(u.posx, u.posy, -1);
        }
        ...
        if (m.tr_at(x, y, u.posz) != tr_null) {
            trap *tr = traplist[m.tr_at(x, y, u.posz)];
            if (!u.avoid_trap(tr, x, y, u.posz)) {
                tr->trigger(&u, x, y, u.posz);
            }
        }
        ...
        if (u.weapon.type->id == "style_capoeira") {
            ...
            for (int wallx = x - 1; wallx <= x + 1 && !wall; wallx++) {
                for (int wally = y - 1; wally <= y + 1 && !wall; wally++) {
                    if (m.has_flag("SUPPORTS_ROOF", wallx, wally, u.posz)) {
                        wall = true;
                    }
                }
            }
        ...
        if (m.has_flag("SWIMMABLE", x, y, u.posz)) {
            u.drench(40, mfb(bp_foot_l) | mfb(bp_foot_r) | mfb(bp_leg_l) | mfb(bp_leg_r));
        }

        // List items here
        if (!m.has_flag("SEALED", x, y, u.posz)) {
            if (u.has_effect("blind") && !m.i_at(x, y, u.posz).empty()) {
                add_msg(_("There's something here, but you can't see what it is."));
            } else if (!m.i_at(x, y, u.posz).empty()) {
                std::vector<std::string> names;
                std::vector<size_t> counts;
                std::vector<item> items;
                for (std::vector<item>::iterator it = m.i_at(x, y, u.posz).begin();
                     it != m.i_at(x, y, u.posz).end(); ++it) {
                    item &tmpitem = *it;
                ...
                } else if (m.i_at(x, y, u.posz).size() < 7) {
                    add_msg(ngettext("There is %d item here.", "There are %d items here.", m.i_at(x, y, u.posz).size()),
                            m.i_at(x, y, u.posz).size());
                } else {
        ...
        int tunneldist = 0;
        // tile is impassable
        //Probably only tunneling on our own z-level
        while ((m.move_cost(x + tunneldist * (x - u.posx), y + tunneldist * (y - u.posy), u.posz) == 0) ||
               // a monster is there
               ((mon_at(x + tunneldist * (x - u.posx), y + tunneldist * (y - u.posy), u.posz) != -1 ||
                 // so keep tunneling
                 npc_at(x + tunneldist * (x - u.posx), y + tunneldist * (y - u.posy), u.posz) != -1) &&
        ...
        if (tunneldist) { //you tunneled
            ...
            u.power_level -= (tunneldist * 250); //tunneling costs 250 bionic power per impassable tile
        ...
        if (u.has_effect("blind") || u.has_effect("stunned")) {
            ...
            add_msg(_("You bump into a %s!"), m.name(x, y, u.posz).c_str());
            u.moves -= 100;
        } else if (m.furn(x, y, u.posz) != f_safe_c && m.open_door(x, y, u.posz, !m.is_outside(u.posx, u.posy, u.posz))) {
            u.moves -= 100;
        } else if (m.ter(x, y, u.posz) == t_door_locked || m.ter(x, y, u.posz) == t_door_locked_peep || m.ter(x, y, u.posz) == t_door_locked_alarm ||
                   m.ter(x, y, u.posz) == t_door_locked_interior) {
            u.moves -= 100;
            add_msg(_("That door is locked!"));
        } else if (m.ter(x, y, u.posz) == t_door_bar_locked) {
            u.moves -= 80;
...
void game::plswim(int x, int y)
{
    ...
    if (!m.has_flag("SWIMMABLE", x, y, u.posz)) {
        dbg(D_ERROR) << "game:plswim: Tried to swim in "
                     << m.tername(x, y, u.posz).c_str() << "!";
        debugmsg("Tried to swim in %s!", m.tername(x, y, u.posz).c_str());
        return;
    }
...
void game::fling_creature(Creature *c, const int &dir, float flvel, bool controlled)
{
    ...
    while (range > 0) {
        ...
        } else if (m.move_cost(x, y, u.posz) == 0) {
            slam = true;
            int vpart;
            vehicle *veh = m.veh_at(x, y, vpart);
            dname = veh ? veh->part_info(vpart).name : m.tername(x, y, u.posz).c_str();
            if (m.is_bashable(x, y, u.posz)) {
                // Only go through if we successfully destroy what we hit
                thru = m.bash(x, y, u.posz, flvel).second;
            } else {
                thru = false;
            }
        }
...
void game::vertical_move(int movez, bool force)
{
    ...
    if (!coming_to_stairs.empty()) {
        ...
        if (coming_to_stairs.size() > 4) {
            add_msg(_("The are a lot of them on the %s!"), m.tername(u.posx, u.posy, u.posz).c_str());
            dexroll /= 4;
            strroll /= 2;
    ...
    if (m.has_flag("SWIMMABLE", u.posx, u.posy, u.posz) && m.has_flag(TFLAG_DEEP_WATER, u.posx, u.posy, u.posz)) {
        if (movez == -1) {
            if (u.is_underwater()) {
    ...
    if (!force && ((movez == -1 && !m.has_flag("GOES_DOWN", u.posx, u.posy, u.posz)) ||
                   (movez == 1 && !m.has_flag("GOES_UP", u.posx, u.posy, u.posz)))) {
        if (movez == -1) {
    ...
    real_coords rc( m.getabs(u.posx, u.posy, u.posz) );

    tripoint omtile_align_start(
    ...
    if (force) {
        ...
        //TODO: Change this to use the normal map, now that we'll have multiple slices loaded.
        int best = 999;
        for (int i = omtile_align_start.x; i <= omtile_align_start.x + omtilesz; i++) {
            for (int j = omtile_align_start.y; j <= omtile_align_start.y + omtilesz; j++) {
                if (rl_dist(u.posx, u.posy, i, j) <= best &&
                    ((movez == -1 && tmpmap.has_flag("GOES_UP", i, j, 0)) ||
                     (movez == 1 && (tmpmap.has_flag("GOES_DOWN", i, j, 0) ||
                                     tmpmap.ter(i, j, 0) == t_manhole_cover)) ||
                     ((movez == 2 || movez == -2) && tmpmap.ter(i, j, 0) == t_elevator))) {
                    stairx = i;
                    stairy = j;
        ...
        if (stairx == -1 || stairy == -1) { // No stairs found!
            if (movez < 0) {
                if (tmpmap.move_cost(u.posx, u.posy, 0) == 0) {
    ...
    //TODO: Rework this for z-levels
    for( unsigned int i = 0; i < num_zombies(); ) {
        monster &critter = zombie(i);
        int turns = critter.turns_to_reach(u.posx, u.posy, u.posz);
        if (turns < 10 && coming_to_stairs.size() < 8 && critter.will_reach(u.posx, u.posy, u.posz)
            && !slippedpast) {
    ...
    u.posy = stairy;
    if (rope_ladder) {
        m.ter_set(u.posx, u.posy, u.posz, t_rope_up);
    }
    if (m.ter(stairx, stairy, u.posz) == t_manhole_cover) {
        m.spawn_item(stairx + rng(-1, 1), stairy + rng(-1, 1), u.posz, "manhole_cover");
        m.ter_set(stairx, stairy, u.posz, t_manhole);
    }
    ...
    if (m.tr_at(u.posx, u.posy, u.posz) != tr_null) { // We stepped on a trap!
        trap *tr = traplist[m.tr_at(u.posx, u.posy, u.posz)];
        if (force || !u.avoid_trap(tr, u.posx, u.posy, u.posz)) {
            tr->trigger(&u, u.posx, u.posy, u.posz);
        }
    }
...
void game::update_map(int &x, int &y, int &z)
{
    ...
    while (z < SEEZ * int(MAPSIZE / 2)) {
        z += SEEZ;
        shiftz--;
    }
    while (z >= SEEZ * (1 + int(MAPSIZE / 2))){
        z -= SEEZ;
        shiftz++;
    }

    m.shift(shiftx, shifty, shiftz);
    levx += shiftx;
    levy += shifty;
    levz += shiftz;

    real_coords rc( m.getabs( 0, 0, 0 ) );
    ...
    for (std::vector<npc *>::iterator it = active_npc.begin();
        ...
        if ((*it)->posx < 0 - SEEX * 2 ||
            (*it)->posy < 0 - SEEX * 2 ||
            (*it)->posx >     SEEX * (MAPSIZE + 2) ||
            (*it)->posy >     SEEY * (MAPSIZE + 2)) {
    ...
    if (!coming_to_stairs.empty()) {
        for (int x = 0; x < SEEX * MAPSIZE; x++) {
            for (int y = 0; y < SEEY * MAPSIZE; y++) {
                if( ( from_below && m.has_flag( "GOES_DOWN", x, y, u.posz ) ) ||
                    ( !from_below && m.has_flag( "GOES_UP", x, y, u.posz ) ) ) {
                    stairx.push_back(x);
                    stairy.push_back(y);
        ...
        for (size_t i = 0; i < coming_to_stairs.size(); i++) {
            ...
            if (!(critter.posx() < 0 - (SEEX * MAPSIZE) / 6 ||
                ...
                if (u_see(mposx, mposy)) {
                        ...
                        if (coming_to_stairs[i].staircount > 0) {
                            dump << (from_below ?
                                     string_format(_("The %s is almost at the top of the %s!"),
                                                   critter.name().c_str(),
                                                   m.tername(mposx, mposy, u.posz).c_str()) :
                    ...
                    sound(mposx, mposy, u.posz, 5, _("a sound nearby from the stairs!"));
                }

                if (is_empty(mposx, mposy, u.posz) && coming_to_stairs[i].staircount <= 0) {
                    critter.setpos(mposx, mposy, u.posz, true);
                    ...
                    // Monster attempts to push player off stairs
                    int pushx = -1;
                    int pushy = -1;
                    int tries = 0;
                    ...
                    while (tries < creature_push_attempts) {
                        ...
                        if ((pushx != 0 || pushy != 0) && (mon_at(iposx, iposy) == -1) &&
                            critter.can_move_to(iposx, iposy, u.posz)) {
                    ...
                    while (tries < creature_push_attempts) {
                        ...
                        if ((mon_at(iposx, iposy, u.posz) == -1) && other.can_move_to(iposx, iposy, other.posz())) {
                            other.setpos(iposx, iposy, other.posz(), false);
                            other.moves -= 100;
                            std::string msg = "";
...
void game::teleport(player *p, bool add_teleglow)
{
    ...
    do {
        ...
        tries++;
    } while (tries < 15 && m.move_cost(newx, newy, p->posz) == 0);
    ...
    p->posx = newx;
    p->posy = newy;
    if (m.move_cost(newx, newy, u.posz) == 0) { //Teleported into a wall
        if (can_see) {
            if (is_u) {
                add_msg(_("You teleport into the middle of a %s!"),
                        m.name(newx, newy, u.posz).c_str());
                ...
                add_msg(_("%s teleports into the middle of a %s!"),
                        p->name.c_str(), m.name(newx, newy, u.posz).c_str());
            }
        }
...
void game::nuke(int x, int y)
{
    ...
    for (int i = 0; i < SEEX * 2; i++) {
        for (int j = 0; j < SEEY * 2; j++) {
            ...
            if (one_in(3)) {
                tmpmap.add_field(i, j, 0, fd_nuke_gas, 3);
            }
            tmpmap.adjust_radiation(i, j, 0, rng(20, 80));
        }
    }
    tmpmap.save();
...
bool game::spread_fungus(int x, int y, int z)
{
    int growth = 1;
    for (int i = x - 1; i <= x + 1; i++) {
        for (int j = y - 1; j <= y + 1; j++) {
            ...
            //TODO: Make fungus grow up/down?
            if (m.has_flag("FUNGUS", i, j, z)) {
                growth += 1;
            }
        }
    }
    ...
    if (!m.has_flag_ter("FUNGUS", x, y, z)) {
        // Terrain conversion
        if (m.has_flag_ter("DIGGABLE", x, y, z)) {
            if (x_in_y(growth * 10, 100)) {
                m.ter_set(x, y, z, t_fungus);
                converted = true;
            }
        } else if (m.has_flag("FLAT", x, y, z)) {
            if (m.has_flag("INDOORS", x, y, z)) {
                if (x_in_y(growth * 10, 500)) {
                    m.ter_set(x, y, z, t_fungus_floor_in);
                    converted = true;
                }
            } else if (m.has_flag("SUPPORTS_ROOF", x, y, z)) {
                if (x_in_y(growth * 10, 1000)) {
                    m.ter_set(x, y, z, t_fungus_floor_sup);
                    converted = true;
                }
            } else {
                if (x_in_y(growth * 10, 2500)) {
                    m.ter_set(x, y, z, t_fungus_floor_out);
                    converted = true;
                }
            }
        } else if (m.has_flag("SHRUB", x, y, z)) {
            if (x_in_y(growth * 10, 200)) {
                m.ter_set(x, y, z, t_shrub_fungal);
                converted = true;
            } else if (x_in_y(growth, 1000)) {
                m.ter_set(x, y, z, t_marloss);
                converted = true;
            }
        } else if (m.has_flag("THIN_OBSTACLE", x, y, z)) {
            if (x_in_y(growth * 10, 150)) {
                m.ter_set(x, y, z, t_fungus_mound);
                converted = true;
            }
        } else if (m.has_flag("YOUNG", x, y, z)) {
            if (x_in_y(growth * 10, 500)) {
                m.ter_set(x, y, z, t_tree_fungal_young);
                converted = true;
            }
        } else if (m.has_flag("WALL", x, y, z)) {
            if (x_in_y(growth * 10, 5000)) {
                converted = true;
                if (m.ter_at(x, y, z).sym == LINE_OXOX) {
                    m.ter_set(x, y, z, t_fungus_wall_h);
                } else if (m.ter_at(x, y, z).sym == LINE_XOXO) {
                    m.ter_set(x, y, z, t_fungus_wall_v);
                } else {
                    m.ter_set(x, y, z, t_fungus_wall);
                }
            }
        }
        // Furniture conversion
        if (converted) {
            if (m.has_flag("FLOWER", x, y, z)) {
                m.furn_set(x, y, z, f_flower_fungal);
            } else if (m.has_flag("ORGANIC", x, y, z)) {
                if (m.furn_at(x, y, z).movecost == -10) {
                    m.furn_set(x, y, z, f_fungal_mass);
                } else {
                    m.furn_set(x, y, z, f_fungal_clump);
                }
            } else if (m.has_flag("PLANT", x, y, z)) {
                for (size_t k = 0; k < m.i_at(x, y, z).size(); k++) {
                    m.i_rem(x, y, z, k);
                }
                item seeds("fungal_seeds", int(calendar::turn));
                m.add_item_or_charges(x, y, z, seeds);
            }
        }
        return true;
    } else {
        ...
        for (int i = x - 1; i <= x + 1; i++) {
            for (int j = y - 1; j <= y + 1; j++) {
                ...
                if (!m.has_flag("FUNGUS", i, j, z) && one_in(9 - growth)) {
                    //growth chance is 100 in X simplified
                    if (m.has_flag("DIGGABLE", i, j, z)) {
                        m.ter_set(i, j, z, t_fungus);
                        converted = true;
                    } else if (m.has_flag("FLAT", i, j, z)) {
                        if (m.has_flag("INDOORS", i, j, z)) {
                            if (one_in(5)) {
                                m.ter_set(i, j, z, t_fungus_floor_in);
                                converted = true;
                            }
                        } else if (m.has_flag("SUPPORTS_ROOF", i, j, z)) {
                            if (one_in(10)) {
                                m.ter_set(i, j, z, t_fungus_floor_sup);
                                converted = true;
                            }
                        } else {
                            if (one_in(25)) {
                                m.ter_set(i, j, z, t_fungus_floor_out);
                                converted = true;
                            }
                        }
                    } else if (m.has_flag("SHRUB", i, j, z)) {
                        if (one_in(2)) {
                            m.ter_set(i, j, z, t_shrub_fungal);
                            converted = true;
                        } else if (one_in(25)) {
                            m.ter_set(i, j, z, t_marloss);
                            converted = true;
                        }
                    } else if (m.has_flag("THIN_OBSTACLE", i, j, z)) {
                        if (x_in_y(10, 15)) {
                            m.ter_set(i, j, z, t_fungus_mound);
                            converted = true;
                        }
                    } else if (m.has_flag("YOUNG", i, j, z)) {
                        if (one_in(5)) {
                            if (m.get_field_strength(tripoint (x, y, z), fd_fungal_haze) != 0) {
                                if (one_in(3)) { // young trees are Vulnerable
                                    m.ter_set(i, j, z, t_fungus);
                                    m.add_spawn("mon_fungal_blossom", 1, x, y, z);
                                    if (u_see(x, y)) {
                                    add_msg(m_warning, _("The young tree blooms forth into a fungal blossom!"));
                                    }
                                } else if (one_in(2)) {
                                    m.ter_set(i, j, z, t_marloss_tree);
                                }
                            } else {
                                m.ter_set(i, j, z, t_tree_fungal_young);
                            }
                            converted = true;
                        }
                    } else if (m.has_flag("TREE", i, j, z)) {
                        if (one_in(10)) {
                            if (m.get_field_strength(tripoint (x, y, z), fd_fungal_haze) != 0) {
                                if (one_in(4)) {
                                    m.ter_set(i, j, z, t_fungus);
                                ...
                                m.ter_set(i, j, z, t_tree_fungal);
                            }
                            converted = true;
                        }
                    } else if (m.has_flag("WALL", i, j, z)) {
                        if (one_in(50)) {
                            converted = true;
                            if (m.ter_at(i, j, z).sym == LINE_OXOX) {
                                m.ter_set(i, j, z, t_fungus_wall_h);
                            } else if (m.ter_at(i, j, z).sym == LINE_XOXO) {
                                m.ter_set(i, j, z, t_fungus_wall_v);
                            } else {
                                m.ter_set(i, j, z, t_fungus_wall);
                            }
                        }
                    }

                    if (converted) {
                        if (m.has_flag("FLOWER", i, j, z)) {
                            m.furn_set(i, j, z, f_flower_fungal);
                        } else if (m.has_flag("ORGANIC", i, j, z)) {
                            if (m.furn_at(i, j, z).movecost == -10) {
                                m.furn_set(i, j, z, f_fungal_mass);
                            } else {
                                m.furn_set(i, j, z, f_fungal_clump);
                            }
                        } else if (m.has_flag("PLANT", i, j, z)) {
                            for (size_t k = 0; k < m.i_at(i, j, z).size(); k++) {
                                m.i_rem(i, j, z, k);
                            }
                            item seeds("fungal_seeds", int(calendar::turn));
                            m.add_item_or_charges(x, y, z, seeds);
        ...
        if (it->charges < tool->max_charges) {
            ...
            case ARTC_SOLAR:
                if (calendar::turn.seconds() == 0 && calendar::turn.minutes() % 10 == 0 &&
                    is_in_sunlight(p->posx, p->posy, p->posz)) {
                    it->charges++;
                }
                break;
    ...
    for (auto &i : effects) {
        ...
        case AEP_SMOKE:
            if (one_in(10)) {
                ...
                if (m.add_field(x, y, p->posz, fd_smoke, rng(1, 3))) {
