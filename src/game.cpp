        ...
        for (int y = u.posy - SCENT_RADIUS; y <= u.posy + SCENT_RADIUS; y++) {
            ...
            for (int i = y - 1; i <= y + 1; ++i) {
                if (reduce_scent_here[x][i] == true) {
                    ...
                    sum_3_scent_y[x][y]  += 2 * grscent[x][i];
                    squares_used_y[x][y] += 2; // only 20% diffuses into REDUCE_SCENT squares 
    ...
    for (int x = u.posx - SCENT_RADIUS; x <= u.posx + SCENT_RADIUS; x++) {
        for (int y = u.posy - SCENT_RADIUS; y <= u.posy + SCENT_RADIUS; y++) {
            if (has_wall_here[x][y] == false) {
                ...
                int squares_used = squares_used_y[x-1][y] 
                                 + squares_used_y[x][y] 
                                 + squares_used_y[x+1][y];
                ...
                grscent[x][y] = static_cast<int>(temp_scent + this_diffusivity * 
                                (sum_3_scent_y[x-1][y] + sum_3_scent_y[x][y] + 
                                sum_3_scent_y[x+1][y] )) / (1000 * 10);
