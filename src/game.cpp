...
bool game::start_game()
{
    ...
    u.mutations.set_highest_cat_level();
    //Calculate mutation drench protection stats
    u.mutations.drench_mut_calc();
...
void game::create_starting_npcs()
{
    ...
    tmp->mutations.toggle_trait( *tmp, trait_id( "NPC_STARTING_NPC" ) );
...
bool game::cleanup_at_end()
{
    ...
    if( uquit == QUIT_DIED || uquit == QUIT_SUICIDE ) {
        ...
        if( u.has_amount( "holybook_bible1", 1 ) || u.has_amount( "holybook_bible2", 1 ) ||
            ...
            if( !( u.mutations.has_trait( trait_id( "CANNIBAL" ) ) ||
                   u.mutations.has_trait( trait_id( "PSYCHOPATH" ) ) ) ) {
...
bool game::do_turn()
{
    ...
    if( !u.has_active_bionic( bionic_id( "bio_scent_mask" ) ) &&
        !u.mutations.has_trait( trait_id( "DEBUG_NOSCENT" ) ) ) {
    ...
    if( newseen > mostseen ) {
        if( newseen - mostseen == 1 ) {
            if( !new_seen_mon.empty() ) {
                ...
                if( u.mutations.has_trait( trait_id( "M_DEFENDER" ) ) && critter.type->in_species( PLANT ) ) {
                ...
                if( critter_at<monster>( traj_front ) ) {
                    ...
                    if( guy->male ) {
                        ...
                        add_msg( _( "%s collided with someone else and sent her flying!" ),
                           ...
                           ( u.mutations.has_trait( trait_LEG_TENT_BRACE ) && ( !u.footwear_factor() ||
...
{
    if( u.mutations.has_trait( trait_id( "ILLITERATE" ) ) ) {
    ...
    if( u.mutations.has_trait( trait_id( "HYPEROPIC" ) ) && !u.worn_with_flag( "FIX_FARSIGHT" ) &&
                               ...
                               int &line )
{
    ...
    if( !signage.empty() ) {
        trim_and_print( w_look, point( column, ++lines ), max_width, c_dark_gray,
                        ...
                        u.mutations.has_trait( trait_ILLITERATE ) ? _( "Sign: ???" ) : _( "Sign: %s" ), signage );
    }
...
{
    if( ( u.mutations.has_active_mutation( trait_RUMINANT ) ||
          u.mutations.has_active_mutation( trait_GRAZER ) ) &&
    ...
    if( u.mutations.has_active_mutation( trait_GRAZER ) && ( m.ter( u.pos() ) == t_grass ||
    ...
    if( u.mutations.has_active_mutation( trait_GRAZER ) ) {
...
bool game::check_safe_mode_allowed( bool repeat_safe_mode_warnings )
{
    ...
    if( u.has_effect( effect_laserlocked ) ) {
        ...
        if( u.get_int_base() < 5 || u.mutations.has_trait( trait_id( "PROF_CHURL" ) ) ) {
...
bool game::walk_move( const tripoint &dest_loc )
{
    ...
    const bool slowed = ( ( !u.mutations.has_trait( trait_PARKOUR ) && ( mcost_to > 2 ||
                            mcost_from > 2 ) ) ||
                          mcost_to > 4 || mcost_from > 4 ) &&
                        !( u.mutations.has_trait( trait_M_IMMUNE ) && fungus );
    ...
    if( !u.is_mounted() && u.mutations.has_trait( trait_id( "LEG_TENT_BRACE" ) ) &&
        ( !u.footwear_factor() ||
          ( u.footwear_factor() == .5 && one_in( 2 ) ) ) ) {
    ...
    if( !u.has_artifact_with( AEP_STEALTH ) && !u.mutations.has_trait( trait_id( "DEBUG_SILENT" ) ) ) {
        int volume = u.is_stealthy() ? 3 : 6;
        volume *= u.mutations.mutation_value( "noise_modifier" );
        if( volume > 0 ) {
...
point game::place_player( const tripoint &dest_loc )
{
    ...
    if( !signage.empty() ) {
        if( !u.mutations.has_trait( trait_ILLITERATE ) ) {
    ...
    if( m.has_flag( "SHARP", dest_loc ) && !one_in( 3 ) && !x_in_y( 1 + u.dex_cur / 2.0, 40 ) &&
        ( !u.in_vehicle && !g->m.veh_at( dest_loc ) ) && ( !u.mutations.has_trait( trait_PARKOUR ) ||
                one_in( 4 ) ) ) {
        if( u.is_mounted() ) {
            ...
            if( u.deal_damage( nullptr, bp, damage_instance( DT_CUT, rng( 1, 10 ) ) ).total_damage() > 0 ) {
                ...
                add_msg( m_bad, _( "You cut your %1$s on the %2$s!" ),
                         ...
                         m.has_flag_ter( "SHARP", dest_loc ) ? m.tername( dest_loc ) : m.furnname(
                             dest_loc ) );
                if( ( u.mutations.has_trait( trait_INFRESIST ) ) && ( one_in( 1024 ) ) ) {
                    u.add_effect( effect_tetanus, 1_turns, num_bp, true );
                } else if( ( !u.mutations.has_trait( trait_INFIMMUNE ) ||
                             !u.mutations.has_trait( trait_INFRESIST ) ) &&
                           ( one_in( 256 ) ) ) {
                    u.add_effect( effect_tetanus, 1_turns, num_bp, true );
                }
            }
    ...
    if( vp1.part_with_feature( "CONTROLS", true ) && u.in_vehicle && !u.is_mounted() ) {
        ...
        if( !u.mutations.has_trait( trait_id( "WAYFARER" ) ) ) {
...
cata::optional<tripoint> game::find_or_make_stairs( map &mp, const int z_after, bool &rope_ladder )
{
    ...
    if( u.mutations.has_trait( trait_id( "WEB_RAPPEL" ) ) ) {
    ...
    } else if( u.mutations.has_trait( trait_VINES2 ) || u.mutations.has_trait( trait_VINES3 ) ) {
        if( query_yn( _( "There is a sheer drop halfway down.  Use your vines to descend?" ) ) ) {
            ...
            if( u.has_trait( trait_VINES2 ) ) {
                if( query_yn( _( "Detach a vine?  It'll hurt, but you'll be able to climb back up…" ) ) ) {
                    == == == =
                    if( u.mutations.has_trait( trait_VINES2 ) ) {
                        if( query_yn( _( "Detach a vine?  It'll hurt, but you'll be able to climb back up..." ) ) ) {
                            >>> >>> > fix function calls
                            rope_ladder = true;
                    ...
                    while( tries < creature_push_attempts ) {
                        ...
                        if( ( pushx != 0 || pushy != 0 ) && !critter_at( pos ) &&
                            ...
                            bool resiststhrow = ( u.is_throw_immune() ) ||
                                                ( u.mutations.has_trait( trait_LEG_TENT_BRACE ) );
