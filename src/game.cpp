...
void game::load( std::string worldname, std::string name )
{
    ...
    read_from_file_optional( worldpath + name + ".weather", std::bind( &game::load_weather, this,
                             _1 ) );
    nextweather = int( calendar::turn );

    read_from_file_optional( worldpath + name + ".log", std::bind( &player::load_memorial_file, &u,
                             _1 ) );
