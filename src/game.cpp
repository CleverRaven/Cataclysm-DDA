...
void game::examine( const tripoint &examp )
{
    ...
    if( m.has_flag( TFLAG_SEALED, examp ) ) {
        ...
        if( m.tr_at( examp ).is_null() && m.i_at( examp ).empty() &&
            ...
            if( !u.is_mounted() && !m.has_flag( flag_NO_PICKUP_ON_EXAMINE, examp ) ) {
...
std::vector<std::string> game::get_dangerous_tile( const tripoint &dest_loc ) const
{
    ...
    if( m.has_flag( flag_ROUGH, dest_loc ) && !m.has_flag( flag_ROUGH, u.pos() ) && !veh_dest &&
...
bool game::walk_move( const tripoint &dest_loc, const bool via_ramp, const bool furniture_move )
{
    ...
    // calculate this _after_ calling grabbed_move
    const int mcost_to = m.move_cost( dest_loc );
    // fungal furniture has no slowing effect on mycus characters
    const bool fungus = m.has_flag_ter_or_furn( flag_FUNGUS, u.pos() ) ||
                        m.has_flag_ter_or_furn( flag_FUNGUS, dest_loc );
...
bool game::grabbed_furn_move( const tripoint &dp )
{
    ...
    if( !m.has_floor( fdest ) && !m.has_flag( flag_FLAT, fdest ) ) {
...
void game::vertical_move( int movez, bool force, bool peeking )
{
    ...
    if( here.has_zlevels() && !force && movez == 1 && !here.has_flag( flag_GOES_UP, u.pos() ) ) {
        // Climbing
    ...
    if( !force && movez == -1 && !here.has_flag( flag_GOES_DOWN, u.pos() ) ) {
        add_msg( m_info, _( "You can't go down here!" ) );
        return;
    } else if( !climbing && !force && movez == 1 && !here.has_flag( flag_GOES_UP, u.pos() ) ) {
    ...
    if( here.has_flag( flag_UNSTABLE, u.pos() ) ) {
        u.moves -= 500;
    ...
    if( !here.has_zlevels() ) {
        ...
        for( monster &critter : all_monsters() ) {
            ...
            if( ( here.has_flag( flag_DIFFICULT_Z, u.pos() ) && !critter.climbs() ) ||
    ...
    if( here.has_zlevels() && std::abs( movez ) == 1 ) {
        bool ladder = here.has_flag( flag_DIFFICULT_Z, u.pos() );
        for( monster &critter : all_monsters() ) {
