...
// Explicitly instanciantes func<std::string> and func<cata_path>
#define TEMPLATE_PATHS(func) \
    template decltype( ( func )<std::string> )( func )<std::string>; \
    template decltype( ( func )<cata_path> )( func )<cata_path>;
...
bool name_contains( const std::filesystem::directory_entry &entry, const std::string &match,
                    const bool at_end, const bool match_path )
{
...
std::vector<std::string> find_file_if_bfs( const std::string &root_path,
        const bool recursive_search, Predicate predicate )
{
...
TEMPLATE_PATHS( get_files_from_path )

template<typename P>
std::vector<P> get_files_from_path_with_path_exclusion( const std::string &pattern,
        const std::string &pattern_clash, const P &root_path, const bool recursive_search,
        const bool match_extension )
{
...
TEMPLATE_PATHS( get_files_from_path_with_path_exclusion )
...
// TEMPLATE_PATHS can't be used for overloaded function
...
TEMPLATE_PATHS( get_directories )
