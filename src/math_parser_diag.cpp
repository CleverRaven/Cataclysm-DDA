        ...
        std::vector<diag_value> const &params, diag_kwargs const &kwargs )
{
    ...
    return[beta = is_beta( scope ),
         spellcasting_property, whitelist, blacklist]( dialogue const & d, double val ) {
...
std::function<void( dialogue &, double )> spellcasting_adjustment_by_mod_ass( char scope,
        std::vector<diag_value> const &params, diag_kwargs const &kwargs )
{
    diag_value spellcasting_property = params[0];
    diag_value mod_id_val = params[1];
    ...
    return[mod_id_val, beta = is_beta( scope ),
                spellcasting_property, whitelist, blacklist]( dialogue const & d, double val ) {
        const mod_id target_mod_id( mod_id_val.str( d ) );
        for( auto spellIt : d.actor( beta )->get_character()->magic->get_spells() ) {
...
std::function<void( dialogue &, double )> spellcasting_adjustment_by_school_ass( char scope,
        std::vector<diag_value> const &params, diag_kwargs const &kwargs )
{
    diag_value spellcasting_property = params[0];
    diag_value school_val = params[1];
    ...
    return[school_val, beta = is_beta( scope ),
                spellcasting_property, whitelist, blacklist]( dialogue const & d, double val ) {
        const trait_id school_id( school_val.str( d ) );
        for( auto spellIt : d.actor( beta )->get_character()->magic->get_spells() ) {
...
std::function<void( dialogue &, double )> spellcasting_adjustment_by_spell_ass( char scope,
        std::vector<diag_value> const &params, diag_kwargs const &kwargs )
{
    diag_value spellcasting_property = params[0];
    diag_value spell_id_val = params[1];
    ...
    return[spell_id_val, beta = is_beta( scope ),
                  spellcasting_property, whitelist, blacklist]( dialogue const & d, double val ) {
        d.actor( beta )->get_character()->magic->get_spell( spell_id( spell_id_val.str(
                    d ) ) ).set_temp_adjustment(
