        ...
        std::vector<diag_value> const &params, diag_kwargs const &kwargs )
{
    ...
    return [beta = is_beta( scope ), category = params[0], type]( dialogue const & d ) {
        ...
        mutation_category_id x = mutation_category_id( category.str() );
        std::string thing = type.str( d );
        mut_count_type AAA;

        if( thing == "POSITIVE" ) {
            AAA = mut_count_type::POSITIVE;
        } else if( thing == "NEGATIVE" ) {
            AAA = mut_count_type::NEGATIVE;
        } else if( thing == "ALL" ) {
            AAA = mut_count_type::ALL;
        } else {
            debugmsg( "Incorrect type '%s' in sum_traits_of_category", type.str() );
            return 0;
        }

        return d.actor( beta )->get_total_in_category( x, AAA );
    };
}
