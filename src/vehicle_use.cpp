...
bool vehicle::can_close( int part_index )
{
    for( auto const &vec : find_lines_of_parts( part_index, "OPENABLE" ) ) {
        for( auto const &partID : vec ) {
            ...
            if( mon ) {
                if( mon->is_player() ) {
                    get_player_character().add_msg_if_player( m_info, _( "There's some buffoon in the way!" ) );
                } else if( mon->is_monster() ) {
                    // TODO: Houseflies, mosquitoes, etc shouldn't count
                    get_player_character().add_msg_if_player( m_info, _( "The %s is in the way!" ), mon->get_name() );
                } else {
                    get_player_character().add_msg_if_player( m_info, _( "%s is in the way!" ), mon->disp_name() );
                }
                return false;
            }
        }
