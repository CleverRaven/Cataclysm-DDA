...
void vehicle::use_dishwasher( int p )
{
    ...
    bool soft_armor = std::any_of( items.begin(), items.end(), []( const item & i ) {
        return i.is_soft();
    } );
    ...
    if( parts[p].enabled ) {
        ...
        add_msg( m_bad,
                 _( "Soft items can't be cleaned in a dishwasher, you should use a washing machine for that.  You need to remove them." ) );
    } else {
        parts[p].enabled = true;
        for( auto &n : items ) {
            n.set_age( 0_turns );
...
void vehicle::interact_with( const tripoint &pos, int interact_part )
{
    ...
    enum {
        ...
        USE_HOTPLATE, FILL_CONTAINER, DRINK, USE_WELDER, USE_PURIFIER, PURIFY_TANK, USE_WASHMACHINE, USE_DISHWASHER, USE_MONSTER_CAPTURE,
