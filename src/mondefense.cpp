                        ...
                        dealt_projectile_attack const *proj )
{
    ...
    if( const Character *const foe = dynamic_cast<Character *>( source ) ) {
        ...
        if( !foe->get_wielded_weapon().conductive() ) {
                           ...
                           dealt_projectile_attack const *const proj )
{
    ...
    if( proj ) {
        ...
        if( const Character *const foe = dynamic_cast<Character *>( source ) ) {
            if( foe->get_wielded_weapon().is_melee( damage_type::CUT ) ||
                foe->get_wielded_weapon().is_melee( damage_type::STAB ) ) {
                num_drops += rng( 3, 4 );
            }
...
void mdefense::return_fire( monster &m, Creature *source, const dealt_projectile_attack *proj )
{
    ...
    if( foe == nullptr ||
        foe->get_wielded_weapon().gun_noise().volume < rl_dist( m.pos(), source->pos() ) ) {
        return;
    }
    ...
    for( const std::pair<const std::string, mtype_special_attack> &attack : m.type->special_attacks ) {
        if( attack.second->id == "gun" ) {
            ...
            tmp.set_wielded_weapon( item( gunactor->gun_type ).ammo_set( gunactor->ammo_type,
