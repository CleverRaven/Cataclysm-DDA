                                     ...
                                     bool fullsize, bool verbose, bool desc, bool isHorizontal )
{
    ...
    for( int i = start_index; i < max_size; i++ ) {
        ...
        print_fuel_indicator( win, y + yofs, x, f, fuel_used_last_turn, verbose, desc );
        yofs++;
    }
                                    ...
                                    bool verbose, bool desc )
{
    ...
    if( verbose ) {
        ...
        if( fuel_type == itype_id( "battery" ) ) {
            rate += power_to_energy_bat( total_engine_epower_w() + total_solar_epower_w() + total_accessory_epower_w(), 1_hours );
            units = _( "kJ" );
        }
        if( rate != 0 ) {
            int tank_use = 0;
