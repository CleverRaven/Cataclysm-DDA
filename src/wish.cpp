...
class wish_mutate_callback: public uilist_callback
{
    public:
        ...
        void ValueRow( const char *key, std::vector<mutation_category_id> list ) {
            ...
            for( const auto &value : list ) {
                ImGui::TextColored( cataimgui::imvec4_from_color( gray ), "%s", value.c_str() );
            }
            ImGui::NewLine();
        }
        ...
        void ValueRow( const char *key, const char *value ) {
            ...
            ImGui::TableNextColumn();
            auto gray = c_light_gray;
            ImGui::TextColored( cataimgui::imvec4_from_color( gray ), "%s", value );
        }
        ...
        void refresh( uilist *menu ) override {
            ...
            auto green = c_green;
            auto white = c_white;
            ImGui::TextColored( cataimgui::imvec4_from_color( green ), "%s", msg.c_str() );
            msg.clear();
            ...
            if( only_active ) {
                ImGui::TextColored( cataimgui::imvec4_from_color( green ), _( "[a] show active traits (active)" ) );
            } else {
                ImGui::TextColored( cataimgui::imvec4_from_color( white ), _( "[a] show active traits" ) );
            }
        }
            ...
            if( ImGui::BeginChild( "monster info", info_size ) ) {
                ...
                if( valid_entnum ) {
                    ...
                    ImGui::SetCursorPosX( ( ImGui::GetContentRegionAvail().x - ImGui::CalcTextSize(
                                                header.c_str() ).x ) * 0.5 );
                    ImGui::TextColored( cataimgui::imvec4_from_color( cyan ), header.c_str() );
