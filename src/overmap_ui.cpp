    ...
    input_context *inp_ctxt, const draw_data_t &data )
{
    ...
    zone_of_influence *zone = overmap_buffer.nearby_zone( center );
    if( zone && zone->origin != center ) {
        zone = nullptr;
    }
    ...
    if( center_seen && !viewing_weather ) {
        if( zone ) {
            ...
            mvwprintz( wbar, point( 3, line_number++ ),
                       c_blue, "  area: %d", zone->area );
            mvwprintz( wbar, point( 3, line_number++ ),
                       c_blue, "  covered omt area: %d", zone->omt_coverage.size() );
            mvwprintz( wbar, point( 3, line_number++ ),
                       c_blue, "  damaged omts: %d", zone->lost_coverage.size() );
            mvwprintz( wbar, point( 3, line_number++ ),
                       c_blue, "  regrowing omts: %d", zone->omt_regrowth.size() );
