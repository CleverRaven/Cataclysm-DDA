                                      ...
                                      bool apply_night_vision_goggles, int &height_3d, int intensity, int overlay_count )
{
    return cata_tiles::draw_from_id_string_internal( id, category, subcategory, pos, subtile, rota,
            ll, -1, apply_night_vision_goggles, height_3d, intensity, "", point(), overlay_count );
}
...
{
    return cata_tiles::draw_from_id_string_internal( id, category, subcategory, pos, subtile, rota,
            ll, -1, apply_night_vision_goggles, height_3d, 0, "", point(), overlay_count );
}
        ...
        const point &offset, int overlay_count )
{
        ...
        if( sym != 0 && sym < 256 ) {
            ...
            if( tileset_ptr->find_tile_type( generic_id ) ) {
                return draw_from_id_string_internal( generic_id, pos, subtile, rota,
                                                     ll, retract, nv_color_active, overlay_count );
            }
    ...
    draw_tile_at( display_tile, screen_pos, loc_rand, rota, ll,
                  nv_color_active, retract, height_3d, offset, overlay_count );
    ...
    const point &offset, int overlay_count )
{
    ...
    draw_sprite_at( tile, tile.bg, p, loc_rand, /*fg:*/ false, rota, ll,
                    apply_night_vision_goggles, retract, fake_int, offset, overlay_count );
    draw_sprite_at( tile, tile.fg, p, loc_rand, /*fg:*/ true, rota, ll,
                    apply_night_vision_goggles, retract, height_3d, offset, overlay_count );
    return true;
}
                    ...
                    for( const layer_variant &layer_var : itt->second ) {
                        if( fld.id().str() == layer_var.id ) {
                            ...
                            ret_draw_field = draw_from_id_string( sprite_to_draw, TILE_CATEGORY::FIELD, empty_string, p,
                                                                  subtile, rotation, lit, nv, nullint, intensity, "", layer_var.offset, z_drop );
                            has_drawn = true;
                            break;
                        }
                    }
...
void cata_tiles::get_terrain_orientation( const tripoint &p, int &rota, int &subtile,
        ...
        const std::bitset<NUM_TERCONN> rotate_group, int z_drop )
{
    map &here = get_map();
