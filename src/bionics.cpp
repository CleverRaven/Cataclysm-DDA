    ...
    if( bio.id == "bio_night" ) {
        ...
        if( has_effect( effect_asthma ) ) {
            ...
            remove_effect( effect_asthma );
               ...
               "afs_bio_dopamine_stimulators" ) { // Aftershock-specific bionic; gives a constant mood boost
        add_morale( MORALE_FEELING_GOOD, 20, 20, 30_minutes, 20_minutes, true );
    } else if( bio.id ==
               "afs_bio_lightning_rod" ) { // Aftershock-specific bionic; charges power through lightning
        if( g->weather.lightning_active && g->m.is_outside( pos() ) && one_in( 3 ) ) {
            ...
            charge_power(
                100000 ); // the power of the heavens is something to behold! this should functionally cap out any bionic power
            deactivate_bionic( b );
