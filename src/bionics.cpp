...
static const trait_id trait_AFS_BIONIC_REJECTION("AFS_BIONIC_REJECTION"); // Aftershock-unique trait; makes installing bionics adversely affect you
...
bool player::activate_bionic(int b, bool eff_only)
{
    ...
    if (bionics[bio.id].gun_bionic) {
        ...
        remove_effect(effect_supercharged);
    }
    ...
    else if (bio.id == "afs_bio_lightning_rod") { // Aftershock
        if (has_effect(effect_emp)) {
            add_msg_if_player(m_info, _("Your lightning rod is temporarily inert due to overcharge."));
            bio.powered = false;
        }
    }
...
static bool attempt_recharge(player &p, bionic &bio, int &amount, int factor = 1, int rate = 1)
{
    ...
    if (power_cost > 0) {
        ...
        recharged = true; // We don't need to recharge the bionic, it just processes every now and then.
    }
...
void player::process_bionic(int b)
{
    ...
    else if (bio.id == "afs_bio_dopamine_stimulators") { // Aftershock-specific bionic; gives a constant mood boost
        add_morale(MORALE_FEELING_GOOD, 20, 20, 30_minutes, 20_minutes, true);
    }
    else if (bio.id == "afs_bio_lightning_rod") { // Aftershock-specific bionic; charges power through lightning
        if (g->weather.lightning_active && g->m.is_outside(pos()) && one_in(3)) {
            g->weather.lightning_active = false;
            sounds::sound(pos(), 300, sounds::sound_t::combat, _("BOOOOOOOOOOOOOOOOOOM!!!!"));
            add_msg_if_player(" ");
            ...
            charge_power(100000); // the power of the heavens is something to behold! this should functionally cap out any bionic power
            deactivate_bionic(b);
            g->cancel_activity_or_ignore_query(distraction_type::attacked,
                _("You've been struck by lightning!")); // might be something you'd want to know about
        }
    }
}
...
bool player::install_bionics(const itype &type, player &installer, bool autodoc, int skill_level)
{
    ...
    if (has_trait(trait_id("AFS_BIONIC_REJECTION"))) { // Aftershock
        if (!g->u.query_yn(
            _("Your body and mind will reject the bionic! Continue anyway?"))) {
            return false;
        }
    }
    ...
    if (success > 0) {
        ...
        if (bioid->upgraded_bionic) {
            ...
            if (!has_trait(trait_AFS_BIONIC_REJECTION)) {
                add_msg(m_good, _("Successfully installed %s."), bioid->name);
            }
            else { // Aftershock - proc bionic rejection when you install a new bionic, inflicting pain and side effects
                add_msg(m_mixed, _("Successfully installed %s."), bioid->name);
                mod_pain(5 * difficult);
                if (has_effect(efftype_id("afs_bionic_rejection"))) {
                    add_msg(m_bad, _("You groan and writhe as the pain of your existing bionics is amplified..."));
                }
                add_effect(efftype_id("afs_bionic_rejection"), rng(12_hours, 24_hours) * difficult);
            }
        }
...
void load_bionic(JsonObject &jsobj)
{
    ...
    new_bionic.storage_only = get_bool_or_flag(jsobj, "storage_only", "BIONIC_STORAGE_ONLY",
        false);
