...
#define INBOUNDS(x, y, z) \
 (x >= 0 && x < SEEX * my_MAPSIZE && y >= 0 && y < SEEY * my_MAPSIZE && z >= 0 && z < SEEZ * my_MAPSIZE)
...
bool map::displace_vehicle (int &x, int &y, int &z, const int dx, const int dy, const int dz, bool test)
{
    ...
    submap * const src_submap = get_submap_at(srcx, srcy, srcz, src_offset_x, src_offset_y, z);
    submap * const dst_submap = get_submap_at(dstx, dsty, dstz, dst_offset_x, dst_offset_y, z);
    ...
    if (src_submap != dst_submap) {
        veh->set_submap_moved( int( x2 / SEEX ), int( y2 / SEEY ), int( z2 / SEEZ) );
...
bool map::vehproceed()
{
    ...
    if (float_indices.empty()) {
        ...
        for (int w = 0; w < num_wheels; w++) {
            ...
            const int pz = z + veh->parts[p].precalc_dz[0];
            // deep water
            if (ter_at(px, py, pz).has_flag(TFLAG_DEEP_WATER)) {
                submerged_wheels++;
            }
        }
        ...
        for (int w = 0; w < num; w++) {
            ...
            if (!has_flag("SWIMMABLE", px, py, pz)) {
                moored++;
            }
        }
    ...
    float ter_turn_cost = 500.0 * move_cost_ter_furn (x,y, z) / abs(veh->velocity);
    ...
    if (!veh->valid_wheel_config()) {
        ...
        for (auto &p : veh->parts) {
            ...
            const int pz = z + p.precalc_dz[0];
            const ter_id &pter = ter(px, py, pz);
            if (pter == t_dirt || pter == t_grass) {
                ter_set(px, py, pz, t_dirtmound);
            }
        }
    }
    ...
    if (can_move) {
        ...
        for (auto &w : wheel_indices) {
            ...
            const int wheel_z = z + veh->parts[w].precalc_dz[0];
            if (one_in(2)) {
                if( displace_water( wheel_x, wheel_y, wheel_z) && pl_ctrl ) {
                    add_msg(m_warning, _("You hear a splash!"));
                }
            }
            veh->handle_trap( wheel_x, wheel_y, w );
            auto &item_vec = i_at( wheel_x, wheel_y, wheel_z );
    ...
    if (can_move) {
        ...
        displace_vehicle (x, y, z, dx, dy, z);
    } else { // can_move
        veh->stop();
    }
    // If the PC is in the currently moved vehicle, adjust the
    // view offset.
    if (g->u.controlling_vehicle && veh_at(g->u.posx, g->u.posy, g->u.posz) == veh) {
...
bool map::displace_water (const int x, const int y, const int z)
{
    if (has_flag("SWIMMABLE", x, y, z) && !has_flag(TFLAG_DEEP_WATER, x, y, z)) // shallow water
    { // displace it
        ...
        // then choose one within count and fill it with water on second pass
            ...
            for (int tx = -1; tx <= 1; tx++)
                ...
                {
                    if ((!tx && !ty)
                            || move_cost_ter_furn(x + tx, y + ty, z) == 0
                            || has_flag(TFLAG_DEEP_WATER, x + tx, y + ty, z))
                        continue;
                    ter_id ter0 = ter (x + tx, y + ty, z);
                    if (ter0 == t_water_sh ||
                        ter0 == t_water_dp)
                        continue;
                    if (pass && dis_places == sel_place)
                    {
                       ter_set(x + tx, y + ty, z, t_water_sh);
...
void map::set(const int x, const int y, const int z, const ter_id new_terrain, const furn_id new_furniture)
{
    furn_set(x, y, z, new_furniture);
    ter_set(x, y, z, new_terrain);
}

void map::set(const int x, const int y, const int z, const std::string new_terrain, const std::string new_furniture) {
    furn_set(x, y, z, new_furniture);
    ter_set(x, y, z, new_terrain);
}

std::string map::name(const int x, const int y, const int z)
{
 return has_furn(x, y, z) ? furnlist[furn(x, y, z)].name : terlist[ter(x, y, z)].name;
}

bool map::has_furn(const int x, const int y, const int z)
{
  return furn(x, y, z) != f_null;
}


std::string map::get_furn(const int x, const int y, const int z) const {
    return furnlist[ furn(x,y,z) ].id;
}

furn_t & map::furn_at(const int x, const int y, const int z) const
{
    return furnlist[ furn(x,y, z) ];
}

furn_id map::furn(const int x, const int y, const int z) const
{
 if (!INBOUNDS(x, y, z)) {
  return f_null;
 }
 ...
 int lx, ly, lz;
 submap * const current_submap = get_submap_at(x, y, z, lx, ly, lz);

 return current_submap->get_furn(lx, ly, lz);
}


void map::furn_set(const int x, const int y, const int z, const furn_id new_furniture)
{
 if (!INBOUNDS(x, y, z)) {
  return;
 }
 ...
 set_transparency_cache_dirty();
 current_submap->set_furn(lx, ly, lz, new_furniture);
}

void map::furn_set(const int x, const int y, const int z, const std::string new_furniture) {
    if ( furnmap.find(new_furniture) == furnmap.end() ) {
        return;
    }
    furn_set(x, y, z, (furn_id)furnmap[ new_furniture ].loadid );
}

bool map::can_move_furniture( const int x, const int y, const int z, player * p ) {
    furn_t furniture_type = furn_at(x, y, z);
...
std::string map::furnname(const int x, const int y, const int z) {
 return furnlist[furn(x, y, z)].name;
}
/*
 ...
 */
ter_id map::ter(const int x, const int y, const int z) const {
 if (!INBOUNDS(x, y, z)) {
  return t_null;
 }
 ...
 return current_submap->ter[lx][ly][lz];
}
 ...
 */
std::string map::get_ter(const int x, const int y, const int z) const {
    return terlist[ ter(x,y,z) ].id;
}

/*
 * Get the terrain harvestable string (what will get harvested from the terrain)
 */
std::string map::get_ter_harvestable(const int x, const int y, const int z) const {
    return terlist[ ter(x,y,z) ].harvestable;
}

/*
 * Get the terrain transforms_into id (what will the terrain transforms into)
 */
ter_id map::get_ter_transforms_into(const int x, const int y, const int z) const {
    return (ter_id)termap[ terlist[ ter(x,y,z) ].transforms_into ].loadid;
}

/*
 * Get the harvest season from the terrain
 */
int map::get_ter_harvest_season(const int x, const int y, const int z) const {
    return terlist[ ter(x,y,z) ].harvest_season;
}

/*
 * Get a reference to the actual terrain struct.
 */
ter_t & map::ter_at(const int x, const int y, const int z) const
{
    return terlist[ ter(x,y,z) ];
}

/*
 * set terrain via string; this works for -any- terrain id
 */
void map::ter_set(const int x, const int y, const int z, const std::string new_terrain) {
    if ( termap.find(new_terrain) == termap.end() ) {
        return;
    }
    ter_set(x, y, z, (ter_id)termap[ new_terrain ].loadid );
}
 ...
 * for mods
 */
void map::ter_set(const int x, const int y, const int z, const ter_id new_terrain) {
    if (!INBOUNDS(x, y, z)) {
        return;
    }
    ...
    int lx, ly, lz;
    submap * const current_submap = get_submap_at(x, y, z, lx, ly, lz);
    current_submap->ter[lx][ly][lz] = new_terrain;
}

std::string map::tername(const int x, const int y, const int z) const
{
 return terlist[ter(x, y, z)].name;
}

std::string map::features(const int x, const int y, const int z)
{
    ...
    std::string ret;
    if (is_bashable(x, y, z)) {
        ret += _("Smashable. ");
    }
    if (has_flag("DIGGABLE", x, y, z)) {
        ret += _("Diggable. ");
    }
    if (has_flag("ROUGH", x, y, z)) {
        ret += _("Rough. ");
    }
    if (has_flag("UNSTABLE", x, y, z)) {
        ret += _("Unstable. ");
    }
    if (has_flag("SHARP", x, y, z)) {
        ret += _("Sharp. ");
    }
    if (has_flag("FLAT", x, y, z)) {
        ret += _("Flat. ");
    }
    return ret;
}

int map::move_cost(const int x, const int y, const int z, const vehicle *ignored_vehicle) const
{
    int cost = move_cost_ter_furn(x, y, z); // covers !inbounds check too
    if ( cost == 0 ) {
        return 0;
    }
...
int map::move_cost_ter_furn(const int x, const int y, const int z) const
{
    if (!INBOUNDS(x, y, z)) {
        return 0;
    }
    ...
    const int tercost = terlist[ current_submap->ter[lx][ly][lz] ].movecost;
    if ( tercost == 0 ) {
        return 0;
    }
    const int furncost = furnlist[ current_submap->get_furn(lx, ly, lz) ].movecost;
    if ( furncost < 0 ) {
        return 0;
    }
...
int map::combined_movecost(const int x1, const int y1, const int z1,
                           const int x2, const int y2, const int z2,
                           const vehicle *ignored_vehicle, const int modifier)
{
    int cost1 = move_cost(x1, y1, z1, ignored_vehicle);
    int cost2 = move_cost(x2, y2, z2, ignored_vehicle);
    // 50 moves taken per move_cost (70.71.. diagonally)
    int mult = (trigdist && x1 != x2 && y1 != y2 ? 71 : 50);
    return (cost1 + cost2 + modifier) * mult / 2;
}

bool map::trans(const int x, const int y, const int z)
{
    ...
    if (veh) {
        ...
        tertr = has_flag_ter_and_furn(TFLAG_TRANSPARENT, x, y, z);
    }
    if( tertr ) {
        // Fields may obscure the view, too
        const field &curfield = field_at( x,y,z );
...
bool map::has_flag(const std::string &flag, const int x, const int y, const int z) const
{
    ...
    return has_flag_ter_or_furn(flag, x, y, z);
}

bool map::can_put_items(const int x, const int y, const int z)
{
    return !has_flag("NOITEM", x, y, z) && !has_flag("SEALED", x, y, z);
}

bool map::has_flag_ter(const std::string & flag, const int x, const int y, const int z) const
{
 return terlist[ter(x, y, z)].has_flag(flag);
}

bool map::has_flag_furn(const std::string & flag, const int x, const int y, const int z) const
{
 return furnlist[furn(x, y, z)].has_flag(flag);
}

bool map::has_flag_ter_or_furn(const std::string & flag, const int x, const int y, const int z) const
{
    if (!INBOUNDS(x, y, z)) {
        return false;
    }
    ...
    return ( terlist[ current_submap->ter[lx][ly][lz] ].has_flag(flag) || furnlist[ current_submap->get_furn(lx, ly, lz) ].has_flag(flag) );
}

bool map::has_flag_ter_and_furn(const std::string & flag, const int x, const int y, const int z) const
{
 return terlist[ter(x, y, z)].has_flag(flag) && furnlist[furn(x, y, z)].has_flag(flag);
}
/////
bool map::has_flag(const ter_bitflags flag, const int x, const int y, const int z) const
{
    if (!INBOUNDS(x, y, z)) {
        return false;
    }
...
bool map::has_flag_ter(const ter_bitflags flag, const int x, const int y, const int z) const
{
 return terlist[ter(x, y, z)].has_flag(flag);
}

bool map::has_flag_furn(const ter_bitflags flag, const int x, const int y, const int z) const
{
 return furnlist[furn(x, y, z)].has_flag(flag);
}

bool map::has_flag_ter_or_furn(const ter_bitflags flag, const int x, const int y, const int z) const
{
    if (!INBOUNDS(x, y, z)) {
        return false;
    }
...
bool map::has_flag_ter_and_furn(const ter_bitflags flag, const int x, const int y, const int z) const
{
    if (!INBOUNDS(x, y, z)) {
        return false;
    }
...
/////
bool map::is_bashable(const int x, const int y, const int z)
{
    if (!inbounds(x, y, z)) {
    ...
    if (veh_in_active_range && veh_exists_at[x][y][z]) {
        std::map< std::pair<int, int>, std::pair<vehicle *, int> >::const_iterator it;
        if ((it = veh_cached_parts.find( std::make_pair(x, y) )) != veh_cached_parts.end()) {
            const int vpart = it->second.second;
            vehicle *veh = it->second.first;
    ...
    if ( has_furn(x, y, z) && furn_at(x, y, z).bash.str_max != -1 ) {
        return true;
    } else if ( ter_at(x, y, z).bash.str_max != -1 ) {
        return true;
    }
    return false;
}

bool map::is_bashable_ter(const int x, const int y, const int z)
{
    if ( ter_at(x, y, z).bash.str_max != -1 ) {
        return true;
    }
    return false;
}

bool map::is_bashable_furn(const int x, const int y, const int z)
{
    if ( has_furn(x, y, z) && furn_at(x, y, z).bash.str_max != -1 ) {
        return true;
    }
...
bool map::is_bashable_ter_furn(const int x, const int y, const int z)
{
    return is_bashable_furn(x, y, z) || is_bashable_ter(x, y, z);
}

int map::bash_strength(const int x, const int y, const int z)
{
    if ( has_furn(x, y, z) && furn_at(x, y, z).bash.str_max != -1 ) {
        return furn_at(x, y, z).bash.str_max;
    } else if ( ter_at(x, y, z).bash.str_max != -1 ) {
        return ter_at(x, y, z).bash.str_max;
    }
    return -1;
}

int map::bash_resistance(const int x, const int y, const int z)
{
    if ( has_furn(x, y, z) && furn_at(x, y, z).bash.str_min != -1 ) {
        return furn_at(x, y, z).bash.str_min;
    } else if ( ter_at(x, y, z).bash.str_min != -1 ) {
        return ter_at(x, y, z).bash.str_min;
    }
    return -1;
}

int map::bash_rating(const int str, const int x, const int y, const int z)
{
    if (!is_bashable(x,y,z)) {
        return -1;
    }
    ...
    int bash_max = 0;
    if (furn_smash) {
        bash_min = furn_at(x, y, z).bash.str_min;
        bash_max = furn_at(x, y, z).bash.str_max;
    } else {
        bash_min = ter_at(x, y, z).bash.str_min;
        bash_max = ter_at(x, y, z).bash.str_max;
    }
    if (str < bash_min) {
        return 0;
...
void map::make_rubble(const int x, const int y, const int z, furn_id rubble_type, bool items, ter_id floor_type, bool overwrite)
{
    if (overwrite) {
        ter_set(x, y, z, floor_type);
        furn_set(x, y, z, rubble_type);
    } else {
        // First see if there is existing furniture to destroy
        if (is_bashable_furn(x, y, z)) {
            destroy_furn(x, y, z, true);
        }
        // Leave the terrain alone unless it interferes with furniture placement
        if (move_cost(x, y, z) <= 0 && is_bashable_ter(x, y, z)) {
            destroy(x, y, z, true);
        }
    ...
    if (items) {
        ...
        if (rubble_type == f_wreckage) {
            ...
            add_item_or_charges(x, y, z, chunk);
            add_item_or_charges(x, y, z, scrap);
            if (one_in(5)) {
                add_item_or_charges(x, y, z, pipe);
                add_item_or_charges(x, y, z, wire);
            }
        } else if (rubble_type == f_rubble_rock) {
            item rock("rock", calendar::turn);
            int rock_count = rng(1, 3);
            for (int i = 0; i < rock_count; i++) {
                add_item_or_charges(x, y, z, rock);
            }
            ...
            for (int i = 0; i < splinter_count; i++) {
                add_item_or_charges(x, y, z, splinter);
            }
            for (int i = 0; i < nail_count; i++) {
                add_item_or_charges(x, y, z, nail);
            }
        }
    }
}
 ...
 */
bool map::is_divable(const int x, const int y, const int z)
{
  return has_flag("SWIMMABLE", x, y, z) && has_flag(TFLAG_DEEP_WATER, x, y, z);
}

bool map::is_outside(const int x, const int y, const int z)
{
 if(!INBOUNDS(x, y, z))
  return true;
...
bool map::flammable_items_at(const int x, const int y, const int z)
{
    for (auto &i : i_at(x, y, z)) {
        int vol = i.volume();
...
bool map::moppable_items_at(const int x, const int y, const int z)
{
    for (auto &i : i_at(x, y, z)) {
        if (i.made_of(LIQUID)) {
            return true;
    ...
    const field &fld = field_at(x, y, z);
    ...
    int vpart;
    vehicle *veh = veh_at(x, y, z, vpart);
    if(veh != 0) {
...
tripoint map::random_outdoor_tile()
{
 std::vector<tripoint> options;
 for (int x = 0; x < SEEX * my_MAPSIZE; x++) {
  for (int y = 0; y < SEEY * my_MAPSIZE; y++) {
   for (int z = 0; z < SEEZ * my_MAPSIZE; z++) {
       if (is_outside(x, y, z))
        options.push_back(tripoint(x, y, z));
   }
  }
 }
...
bool map::has_adjacent_furniture(const int x, const int y, const int z)
{
    ...
    for (int i = 0; i < 4; i++)
    {
        ...
        if ( has_furn(adj_x, adj_y, z) && furn_at(adj_x, adj_y, z).has_flag("BLOCKSDOOR") ) {
            return true;
        }
    }
...
bool map::has_nearby_fire(int x, int y, int z, int radius)
{
    for(int dx = -radius; dx <= radius; dx++) {
        for(int dy = -radius; dy <= radius; dy++) {
            const tripoint p(x + dx, y + dy, z);
            if( get_field( p, fd_fire ) != nullptr ) {
                return true;
            }
            if (ter(p.x, p.y, z) == t_lava) {
                return true;
            }
        }
    }
...
void map::mop_spills(const int x, const int y, const int z) {
    for (size_t i = 0; i < i_at(x, y, z).size(); i++) {
        item *it = &(i_at(x, y, z)[i]);
        if (it->made_of(LIQUID)) {
            i_rem(x, y, z, i);
            i--;
        }
    }
    remove_field( x, y, z, fd_blood );
    remove_field( x, y, z, fd_blood_veggy );
    remove_field( x, y, z, fd_blood_insect );
    remove_field( x, y, z, fd_blood_invertebrate );
    remove_field( x, y, z, fd_bile );
    remove_field( x, y, z, fd_slime );
    remove_field( x, y, z, fd_sludge );
    int vpart;
    vehicle *veh = veh_at(x, y, z, vpart);
    if(veh != 0) {
        std::vector<int> parts_here = veh->parts_at_relative(veh->parts[vpart].mount_dx, veh->parts[vpart].mount_dy);
...
void map::create_spores(const int x, const int y, const int z, Creature* source)
{
    // TODO: Infect NPCs?
...
int map::collapse_check(const int x, const int y, const int z)
{
    int num_supports = 0;
    for (int i = x - 1; i <= x + 1; i++) {
        for (int j = y - 1; j <= y + 1; j++) {
            ...
            if (has_flag("COLLAPSES", x, y, z)) {
                if (has_flag("COLLAPSES", i, j, z)) {
                    num_supports++;
                } else if (has_flag("SUPPORTS_ROOF", i, j, z)) {
                    num_supports += 2;
                }
            } else if (has_flag("SUPPORTS_ROOF", x, y, z)) {
                if (has_flag("SUPPORTS_ROOF", i, j, z) && !has_flag("COLLAPSES", i, j, z)) {
                    num_supports += 3;
                }
            }
        }
...
void map::collapse_at(const int x, const int y, const int z)
{
    destroy (x, y, z, false);
    //Collapsing through multiple z-levels is going to be a royal PITA. Filing away under 'Future Feature'
    crush(x, y, z);
    make_rubble(x, y, z);
    for (int i = x - 1; i <= x + 1; i++) {
        for (int j = y - 1; j <= y + 1; j++) {
            ...
            if (has_flag("COLLAPSES", i, j, z) && one_in(collapse_check(i, j, z))) {
                destroy (i, j, z, false);
            // We only check for rubble spread if it doesn't already collapse to prevent double crushing
            } else if (has_flag("FLAT", i, j, z) && one_in(8)) {
                crush(i, j, z);
                make_rubble(i, j, z);
            }
        }
...
std::pair<bool, bool> map::bash(const int x, const int y, const int z, const int str,
                                bool silent, bool destroy, vehicle *bashing_vehicle )
{
    ...
    if( get_field( tripoint( x, y, z ), fd_web ) != nullptr ) {
        smashed_something = true;
        remove_field(x, y, z, fd_web);
    }

    // Destroy glass items, spilling their contents.
    std::vector<item> smashed_contents;
    std::vector<item> &bashed_items = i_at(x, y, z);
    ...
    if (veh && veh != bashing_vehicle) {
        ...
        if ( has_furn(x, y, z) && furn_at(x, y, z).bash.str_max != -1 ) {
            bash = &(furn_at(x,y,z).bash);
            smash_furn = true;
        } else if ( ter_at(x, y, z).bash.str_max != -1 ) {
            bash = &(ter_at(x,y,z).bash);
            smash_ter = true;
        }
        // TODO: what if silent is true? What if this was done by a hulk, not the player?
        if (has_flag("ALARMED", x, y, z) && !g->event_queued(EVENT_WANTED)) {
            g->sound(x, y, z, 40, _("An alarm sounds!"));
        ...
        if ( bash != NULL && (!bash->destroy_only || destroy)) {
            ...
            if (destroy) {
                ...
                if ( bash->str_min_blocked != -1 || bash->str_max_blocked != -1 ) {
                    if( has_adjacent_furniture(x, y, z) ) {
            ...
            if (success || destroy) {
                ...
                if (has_flag_ter_or_furn("PLANT", x, y, z)) {
                    for (size_t i = 0; i < i_at(x, y, z).size(); i++) {
                        i_rem(x, y, z, i);
                    }
                }
                ...
                if (smash_furn) {
                    if (has_flag_furn("FUNGUS", x, y, z)) {
                        create_spores(x, y, z);
                    }
                } else if (smash_ter) {
                    if (has_flag_ter("FUNGUS", x, y, z)) {
                        create_spores(x, y, z);
                    }
                }
                ...
                bool collapses = has_flag("COLLAPSES", x, y, z) && smash_ter;
                bool supports = has_flag("SUPPORTS_ROOF", x, y, z) && smash_ter;
                if (smash_furn == true) {
                    furn_set(x, y, z, bash->furn_set);
                    // Hack alert.
                    // Signs have cosmetics associated with them on the submap since
                    // furniture can't store dynamic data to disk. To prevent writing
                    // mysteriously appearing for a sign later built here, remove the
                    // writing from the submap.
                    delete_signage(x, y, z);
                } else if (smash_ter == true) {
                    ter_set(x, y, z, bash->ter_set);
                } else {
                    debugmsg( "data/json/terrain.json does not have %s.bash.ter_set set!",
                              ter_at(x,y,z).id.c_str() );
                }

                spawn_item_list(bash->items, x, y, z);
                if (bash->explosive > 0) {
                    g->explosion(x, y, z, bash->explosive, 0, false);
                }
                ...
                if (supports && !has_flag("SUPPORTS_ROOF", x, y, z)) {
                    for (int i = x - 1; i <= x + 1; i++) {
                        for (int j = y - 1; j <= y + 1; j++) {
                            if ((i == x && j == y) || !has_flag("COLLAPSES", i, j, z)) {
                                continue;
                            }
                            if (one_in(collapse_check(i, j, z))) {
                                collapse_at(i, j, z);
                            }
                        }
                    }
            ...
            if ( furnid == f_skin_wall || furnid == f_skin_door || furnid == f_skin_door_o ||
                ...
                if (str >= rng(0, 6) || destroy) {
                    ...
                    for (int i = -1; i <= 1; i++) {
                        for (int j = -1; j <= 1; j++) {
                            if (furn(x + i, y + j, z) == f_groundsheet ||
                                furn(x + i, y + j, z) == f_fema_groundsheet ||
                                furn(x + i, y + j, z) == f_skin_groundsheet){
                                tentx = x + i;
                                tenty = y + j;
                                break;
                            }
                    ...
                    for (int i = -1; i <= 1; i++) {
                        for (int j = -1; j <= 1; j++) {
                            if (furn(tentx + i, tenty + j, z) == f_groundsheet) {
                                spawn_item(tentx + i, tenty + j, z, "broketent");
                            }
                            if (furn(tentx + i, tenty + j, z) == f_skin_groundsheet) {
                                spawn_item(tentx + i, tenty + j, z, "damaged_shelter_kit");
                            }
                            furn_id check_furn = furn(tentx + i, tenty + j, z);
                            if (check_furn == f_skin_wall || check_furn == f_skin_door ||
                                ...
                                furn_set(tentx + i, tenty + j, z, f_null);
                            }
                        }
                    }
            ...
            // Made furniture seperate from the other tent to facilitate destruction
                ...
                if (str >= rng(0, 6) || destroy) {
                    ...
                    for (int i = -2; i <= 2; i++) {
                        for (int j = -2; j <= 2; j++) {
                            if (furn(x + i, y + j, z) == f_center_groundsheet){
                                tentx = x + i;
                                tenty = y + j;
                                break;
                            }
                    ...
                    for (int i = -2; i <= 2; i++) {
                        for (int j = -2; j <= 2; j++) {
                             if (furn(tentx + i, tenty + j, z) == f_center_groundsheet) {
                             spawn_item(tentx + i, tenty + j, z, "largebroketent");
                            }
                            furn_set(tentx + i, tenty + j, z, f_null);
                        }
                    }
    ...
    if( move_cost(x, y, z) <= 0  && !smashed_something ) {
        sound = _("thump!");
        sound_volume = 18;
        smashed_something = true;
    }
    if( !sound.empty() && !silent) {
        g->sound( x, y, z, sound_volume, sound);
    }
    return std::pair<bool, bool> (smashed_something, success);
}

void map::spawn_item_list(const std::vector<map_bash_item_drop> &items, int x, int y, int z) {
        ...
        if ( chance == -1 || rng(0, 100) >= chance ) {
            ...
            if ( numitems > 0 ) {
                ...
                for(int a = 0; a < numitems; a++ ) {
                    ...
                    add_item_or_charges(x, y, z, new_item);
                }
            }
        }
    }
...
void map::destroy(const int x, const int y,  const int z, const bool silent)
{
    ...
    int count = 0;
    while (count <= 25 && bash(x, y, z, 999, silent, true).second) {
        count++;
    }
}

void map::destroy_furn(const int x, const int y, const int z, const bool silent)
{
    // Break if it takes more than 25 destructions to remove to prevent infinite loops
    // Example: A bashes to B, B bashes to A leads to A->B->A->...
    int count = 0;
    while (count <= 25 && furn(x, y, z) != f_null && bash(x, y, z, 999, silent, true).second) {
        count++;
    }
}

void map::crush(const int x, const int y, const int z)
{
    int veh_part;
    player *crushed_player = nullptr;
    //The index of the NPC at (x,y), or -1 if there isn't one
    int npc_index = g->npc_at(x, y, z);
...
void map::shoot(const int x, const int y, const int z, int &dam,
                const bool hit_items, const std::set<std::string>& ammo_effects)
{
    ...
    if (has_flag("ALARMED", x, y, z) && !g->event_queued(EVENT_WANTED))
    {
        g->sound(x, y, z, 30, _("An alarm sounds!"));
        g->add_event(EVENT_WANTED, int(calendar::turn) + 300, 0, g->get_abs_levx(), g->get_abs_levy());
    }
    ...
    if( 0 == terrain.id.compare("t_wall_wood_broken") ||
        ...
        dam -= rng(15, 30);
        if (dam > 0) {
            g->sound(x, y, z, 10, _("smash!"));
            ter_set(x, y, z, t_door_b);
        }
        ...
        if (ammo_effects.count("LASER")) {
            ...
            dam -= rng(1,3);
            if (dam > 0) {
                g->sound(x, y, z, 16, _("glass breaking!"));
                ter_set(x, y, z, t_window_frame);
                spawn_item(x, y, z, "sheet", 1);
                spawn_item(x, y, z, "stick");
                spawn_item(x, y, z, "string_36");
            }
        }
        ...
        if (ammo_effects.count("LASER")) {
            ...
            dam -= rng(1,8);
            if (dam > 0) {
                g->sound(x, y, z, 20, _("glass breaking!"));
                ter_set(x, y, z, t_floor);
            }
        }
                ...
                if (ammo_effects.count("INCENDIARY") || ammo_effects.count("FLAME")) {
                    ...
                    for (int i = x - 2; i <= x + 2; i++) {
                        for (int j = y - 2; j <= y + 2; j++) {
                            if (move_cost(i, j, z) > 0 && one_in(3)) {
                                    spawn_item(i, j, z, "gasoline");
                            }
                        }
                    }
                    g->sound(x, y, z, 10, _("smash!"));
                }
                ter_set(x, y, z, t_gas_pump_smashed);
            }
            dam -= 60;
        }
    } else if( 0 == terrain.id.compare("t_vat") ) {
        if (dam >= 10) {
            g->sound(x, y, z, 20, _("ke-rash!"));
            ter_set(x, y, z, t_floor);
        } else {
            dam = 0;
        }
    } else {
        if (move_cost(x, y, z) == 0 && !trans(x, y, z)) {
    ...
    if (ammo_effects.count("TRAIL") && !one_in(4)) {
        add_field(x, y, z, fd_smoke, rng(1, 2));
    }

    if (ammo_effects.count("STREAM") && !one_in(3)) {
        add_field(x, y, z, fd_fire, rng(1, 2));
    }

    if (ammo_effects.count("LIGHTNING")) {
        add_field(x, y, z, fd_electricity, rng(2, 3));
    }

    if (ammo_effects.count("PLASMA") && one_in(2)) {
        add_field(x, y, z, fd_plasma, rng(1, 2));
    }

    if (ammo_effects.count("LASER")) {
        add_field(x, y, z, fd_laser, 2);
    }
    ...
    // Check fields?
    const field_entry *fieldhit = get_field( tripoint( x, y, z ), fd_web );
    if( fieldhit != nullptr ) {
        if (ammo_effects.count("INCENDIARY") || ammo_effects.count("FLAME")) {
            add_field(x, y, z, fd_fire, fieldhit->getFieldDensity() - 1);
    ...
    if ((move_cost(x, y, z) == 2 && !hit_items) || !INBOUNDS(x, y, z)) {
        return; // Items on floor-type spaces won't be shot up.
    }

    for (size_t i = 0; i < i_at(x, y, z).size(); i++) {
        bool destroyed = false;
        int chance = (i_at(x, y, z)[i].volume() > 0 ? i_at(x, y, z)[i].volume() : 1);
        // volume dependent chance

        if (dam > i_at(x, y, z)[i].bash_resist() && one_in(chance)) {
            i_at(x, y, z)[i].damage++;
        }
        if (i_at(x, y, z)[i].damage >= 5) {
            destroyed = true;
        }

        if (destroyed) {
            for (auto &j : i_at(x, y, z)[i].contents) {
                i_at(x, y, z).push_back(j);
            }
            i_rem(x, y, z, i);
            i--;
        }
...
bool map::hit_with_acid( const int x, const int y, const int z )
{
    if( move_cost( x, y, z ) != 0 ) {
        return false;    // Didn't hit the tile!
    }
    const ter_id t = ter( x, y, z );
    if( t == t_wall_glass_v || t == t_wall_glass_h || t == t_wall_glass_v_alarm || t == t_wall_glass_h_alarm ||
        ...
        if( one_in( 4 ) ) {
            ter_set( x, y, z, t_door_frame );
        } else {
            return false;
        }
    } else if( t == t_window || t == t_window_alarm ) {
        ter_set( x, y, z, t_window_empty );
    } else if( t == t_wax ) {
        ter_set( x, y, z, t_floor_wax );
    } else if( t == t_gas_pump || t == t_gas_pump_smashed ) {
        return false;
    } else if( t == t_card_science || t == t_card_military ) {
        ter_set( x, y, z, t_card_reader_broken );
    }
    return true;
}

// returns true if terrain stops fire
bool map::hit_with_fire(const int x, const int y, const int z)
{
    if (move_cost(x, y, z) != 0)
        return false; // Didn't hit the tile!

    // non passable but flammable terrain, set it on fire
    if (has_flag("FLAMMABLE", x, y, z) || has_flag("FLAMMABLE_ASH", x, y, z))
    {
        add_field(x, y, z, fd_fire, 3);
    }
    return true;
}

bool map::marlossify(const int x, const int y, const int z)
{
    if (one_in(25) && (terlist[ter(x, y, z)].movecost != 0 && !has_furn(x, y, z))
            && !ter_at(x, y, z).has_flag(TFLAG_DEEP_WATER)) {
        ter_set(x, y, z, t_marloss);
        return true;
    }
    for (int i = 0; i < 25; i++) {
        if(!g->spread_fungus(x, y, z)) {
            return true;
        }
    }
    return false;
...
bool map::open_door(const int x, const int y, const int z, const bool inside, const bool check_only)
{
 const std::string terid = get_ter(x,y,z);
 const std::string furnid = furnlist[furn(x,y,z)].id;
 if ( !termap[terid].open.empty() && termap[ terid ].open != "t_null" ) {
     ...
     if ( has_flag("OPENCLOSE_INSIDE", x, y, z) && inside == false ) {
         return false;
     }
     if(!check_only) {
         ter_set(x, y, z, termap[ terid ].open );
     }
     return true;
     ...
     if(!check_only) {
         furn_set(x, y, z, furnmap[ furnid ].open );
     }
     return true;
 }
 return false;
...
void map::translate_radius(const ter_id from, const ter_id to, float radi, int uX, int uY, int uZ)
{
 if (from == to) {
  debugmsg("map::translate %s => %s", terlist[from].name.c_str(),
                                      terlist[from].name.c_str());
...
bool map::close_door(const int x, const int y, const int z, const bool inside, const bool check_only)
{
 const std::string terid = get_ter(x,y,z);
 const std::string furnid = furnlist[furn(x,y,z)].id;
 if ( !termap[terid].close.empty() && termap[ terid ].close != "t_null" ) {
     ...
     if (!check_only) {
        ter_set(x, y, z, termap[ terid ].close );
     }
     return true;
     ...
     if (!check_only) {
         furn_set(x, y, z, furnmap[ furnid ].close );
     }
     return true;
 }
 return false;
...
const std::string map::get_signage(const int x, const int y, const int z) const
{
    if (!INBOUNDS(x, y, z)) {
        return "";
    }
    ...
    return current_submap->get_signage(lx, ly, lz);
}
void map::set_signage(const int x, const int y, const int z, std::string message) const
{
    if (!INBOUNDS(x, y, z)) {
        return;
    }
    ...
    current_submap->set_signage(lx, ly, lz, message);
}
void map::delete_signage(const int x, const int y, const int z) const
{
    if (!INBOUNDS(x, y, z)) {
        return;
    }
    ...
    current_submap->delete_signage(lx, ly, lz);
}

int map::get_radiation(const int x, const int y, const int z) const
{
    if (!INBOUNDS(x, y, z)) {
        return 0;
    }
    ...
    return current_submap->get_radiation(lx, ly, lz);
}

void map::set_radiation(const int x, const int y, const int z, const int value)
{
    if (!INBOUNDS(x, y, z)) {
        return;
    }
    ...
    current_submap->set_radiation(lx, ly, lz, value);
}

void map::adjust_radiation(const int x, const int y, const int z, const int delta)
{
    if (!INBOUNDS(x, y, z)) {
        return;
    }
    ...
    int current_radiation = current_submap->get_radiation(lx, ly, lz);
    current_submap->set_radiation(lx, ly, lz, current_radiation + delta);
}

int& map::temperature(const int x, const int y, const int z)
{
 if (!INBOUNDS(x, y, z)) {
  null_temperature = 0;
 ...
 return get_submap_at(x, y, z)->temperature;
}

void map::set_temperature(const int x, const int y, const int z, int new_temperature)
{
    temperature(x, y, z) = new_temperature;
    temperature(x + SEEX, y, z) = new_temperature;
    temperature(x, y + SEEY, z) = new_temperature;
    temperature(x, y, z + SEEZ) = new_temperature;
    temperature(x + SEEX, y + SEEY, z) = new_temperature;
    temperature(x, y + SEEY, z + SEEZ) = new_temperature;
    temperature(x + SEEX, y + SEEY, z + SEEZ) = new_temperature;
}

std::vector<item> &map::i_at(const int x, const int y, const int z)
{
 if (!INBOUNDS(x, y, z)) {
  nulitems.clear();
  return nulitems;
...
*/
 ...
 return current_submap->itm[lx][ly][lz];
}
...
bool map::sees_some_items(int x, int y, int z, const player &u)
{
    // can only see items if there are any items.
    return !i_at( x, y, z ).empty() && could_see_items( x, y, z, u );
}

bool map::could_see_items(int x, int y, int z, const player &u)
{
    const bool container = has_flag_ter_or_furn("CONTAINER", x, y, z);
    const bool sealed = has_flag_ter_or_furn("SEALED", x, y, z);
    if (sealed && container) {
...
item map::water_from(const int x, const int y, const int z)
{
    item ret("water", 0);
    if (ter(x, y, z) == t_water_sh && one_in(3))
        ret.poison = rng(1, 4);
    else if (ter(x, y, z) == t_water_dp && one_in(4))
        ret.poison = rng(1, 4);
    else if (ter(x, y, z) == t_sewage)
        ret.poison = rng(1, 7);
    return ret;
}
item map::swater_from(const int x, const int y, const int z)
{
...
item map::acid_from(const int x, const int y, const int z)
{
    (void)x; (void)y; (void)z; //all acid is acid, i guess?
    item ret("water_acid", 0);
    return ret;
}

void map::i_rem(const int x, const int y, const int z, const int index)
{
    if (index > (int)i_at(x, y, z).size() - 1) {
        return;
    }
    i_at(x, y, z).erase(i_at(x, y, z).begin() + index);
}

void map::i_rem(const int x, const int y, const int z, item* it)
{
    std::vector<item>& map_items = i_at(x, y, z);
...
void map::i_clear(const int x, const int y, const int z)
{
    i_at(x, y, z).clear();
}

void map::spawn_an_item(const int x, const int y, const int z, item new_item,
                        const long charges, const int damlevel)
{
    ...
    if( (new_item.made_of(LIQUID) && has_flag("SWIMMABLE", x, y, z)) ||
        has_flag("DESTROY_ITEM", x, y, z) ) {
        return;
    }
...
void map::spawn_items(const int x, const int y, const int z, const std::vector<item> &new_items)
{
    if (!inbounds(x, y, z) || has_flag("DESTROY_ITEM", x, y, z)) {
        return;
    }
    const bool swimmable = has_flag("SWIMMABLE", x, y, z);
    for (std::vector<item>::const_iterator a = new_items.begin(); a != new_items.end(); ++a) {
        ...
        if (new_item.is_armor() && new_item.has_flag("PAIRED") && x_in_y(4, 5)) {
            ...
            add_item_or_charges(x, y, z, new_item2);
        }
        add_item_or_charges(x, y, z, new_item);
    }
}

void map::spawn_artifact(const int x, const int y, const int z)
{
    add_item_or_charges( x, y, z, item( new_artifact(), 0 ) );
}

void map::spawn_natural_artifact(const int x, const int y, const int z, artifact_natural_property prop)
{
    add_item_or_charges( x, y, z, item( new_natural_artifact( prop ), 0 ) );
}

//New spawn_item method, using item factory
// added argument to spawn at various damage levels
void map::spawn_item(const int x, const int y, const int z, const std::string &type_id,
                     ...
                     const unsigned birthday, const int damlevel, const bool rand)
{
    ...
    {
        spawn_item(x, y, z, type_id, 1, charges, birthday, damlevel);
    }
    // spawn the item
    ...
    spawn_an_item(x, y, z, new_item, charges, damlevel);
}

int map::max_volume(const int x, const int y, const int z)
{
    const ter_t &ter = ter_at(x, y, z);
    if (has_furn(x, y, z)) {
        return furn_at(x, y, z).max_volume;
    }
    return ter.max_volume;
}

// total volume of all the things
int map::stored_volume(const int x, const int y, const int z) {
    if(!INBOUNDS(x, y, z)) {
        return 0;
    }
    int cur_volume=0;
    for (auto &n : i_at(x, y, z)) {
        item* curit = &n;
...
// free space
int map::free_volume(const int x, const int y, const int z) {
   const int maxvolume = this->max_volume(x, y, z);
   if(!INBOUNDS(x, y, z)) return 0;
   return ( maxvolume - stored_volume(x, y, z) );
}
...
bool map::is_full(const int x, const int y, const int z, const int addvolume, const int addnumber ) {
   const int maxitems = MAX_ITEM_IN_SQUARE; // (game.h) 1024
   const int maxvolume = this->max_volume(x, y, z);

   if( ! (INBOUNDS(x, y, z) && move_cost(x, y, z) > 0 && !has_flag("NOITEM", x, y, z) ) ) {
       return true;
   }

   if ( addvolume == -1 ) {
       if ( i_at(x, y, z).size() < maxitems ) return true;
       int cur_volume=stored_volume(x, y, z);
       return (cur_volume >= maxvolume ? true : false );
   } else {
       if ( i_at(x, y, z).size() + ( addnumber == -1 ? 1 : addnumber ) > maxitems ) return true;
       int cur_volume=stored_volume(x, y, z);
...
bool map::add_item_or_charges(const int x, const int y, const int z, item new_item, int overflow_radius) {

    if(!INBOUNDS(x,y, z) ) {
        // Complain about things that should never happen.
        dbg(D_INFO) << x << "," << y << ", liquid "
                    <<(new_item.made_of(LIQUID) && has_flag("SWIMMABLE", x, y, z)) <<
                    ", destroy_item "<<has_flag("DESTROY_ITEM", x, y, z);

        return false;
    }
    if( (new_item.made_of(LIQUID) && has_flag("SWIMMABLE", x, y, z)) ||
            has_flag("DESTROY_ITEM", x, y, z) || new_item.has_flag("NO_DROP") ) {
    ...
    std::vector<tripoint> ps = closest_points_first(overflow_radius, x, y, z);
    for(std::vector<tripoint>::iterator p_it = ps.begin(); p_it != ps.end(); p_it++)
    {
        if (!INBOUNDS(p_it->x, p_it->y, p_it->z) || new_item.volume() > this->free_volume(p_it->x, p_it->y, p_it->z) ||
                has_flag("DESTROY_ITEM", p_it->x, p_it->y, p_it->z) || has_flag("NOITEM", p_it->x, p_it->y, p_it->z)){
            continue;
        }

        if (tryaddcharges) {
            for (auto &i : i_at(p_it->x,p_it->y, p_it->z))
            {
        ...
        if (i_at(p_it->x, p_it->y, p_it->z).size() < MAX_ITEM_IN_SQUARE)
        {
            add_item(p_it->x, p_it->y, p_it->z, new_item, MAX_ITEM_IN_SQUARE);
            return true;
        }
    }
    return false;
...
void map::add_item(const int x, const int y, const int z, item new_item, const int maxitems)
{
    if (new_item.made_of(LIQUID) && has_flag("SWIMMABLE", x, y, z)) {
        return;
    }
    ...
    if (has_flag("DESTROY_ITEM", x, y, z) || ((int)i_at(x,y,z).size() >= maxitems)) {
        return;
    }
    if (new_item.has_flag("ACT_IN_FIRE") && get_field( tripoint( x, y, z ), fd_fire ) != nullptr ) {
        new_item.active = true;
    }

    int lx, ly, lz;
    submap * const current_submap = get_submap_at(x, y, z, lx, ly, lz);
    current_submap->itm[lx][ly][lz].push_back(new_item);
...
void map::process_items( bool active, T processor )
{
    for( int gx = 0; gx < my_MAPSIZE; gx++ ) {
        for( int gy = 0; gy < my_MAPSIZE; gy++ ) {
            for (int gz = 0; gz < my_MAPSIZE; gz++) {
                submap *const current_submap = get_submap_at_grid(gx, gy, gz);
...
std::list<item> map::use_amount_square(const int x, const int y, const int z, const itype_id type,
                                       int &quantity, const bool use_container)
{
  ...
  std::list<item> tmp = use_amount_map_or_vehicle(i_at(x,y,z), type, quantity, use_container);
  ret.splice(ret.end(), tmp);
  return ret;
}
      ...
      for (int y = origin.y - radius; y <= origin.y + radius; y++) {
        for (int z = origin.z - radius; z <= origin.z + radius; z++) {
            if (rl_dist(origin.x, origin.y, x, y) >= radius) {
              std::list<item> tmp;
              tmp = use_amount_square(x, y, z, type, quantity, use_container);
              ret.splice(ret.end(), tmp);
            }
        }
      }
                                 ...
                                 const itype_id type, const long amount)
{
    ...
    for (int radius = 0; radius <= range && quantity > 0; radius++) {
        for (int x = origin.x - radius; x <= origin.x + radius; x++) {
            for (int y = origin.y - radius; y <= origin.y + radius; y++) {
                for (int z = origin.z - radius; z <= origin.z + radius; z++) {
                    if (has_furn(x, y, z) && accessable_furniture(origin.x, origin.y, origin.z, x, y, z, range)) {
                        use_charges_from_furn(furn_at(x, y, z), type, quantity, i_at(x, y, z), ret);
                        if (quantity <= 0) {
                            return ret;
                        }
                    }
                    if(accessable_items( origin.x, origin.y, origin.z, x, y, z, range) ) {
                        continue;
                    }
                    if (rl_dist(origin.x, origin.y, x, y) >= radius) {
                        int vpart = -1;
                        ...
                        std::list<item> tmp = use_charges_from_map_or_vehicle(i_at(x,y,z), type, quantity);
                        ret.splice(ret.end(), tmp);
                        if (quantity <= 0) {
                            return ret;
                        }
...
std::list<std::pair<tripoint, item *> > map::get_rc_items( int x, int y, int z )
{
    ...
    for( pos.x = 0; pos.x < SEEX * MAPSIZE; pos.x++ ) {
        ...
        for( pos.y = 0; pos.y < SEEY * MAPSIZE; pos.y++ ) {
            ...
            std::vector<item> &item_stack = i_at( pos.x, pos.y, pos.z );
...
static bool trigger_radio_item( std::string signal, std::vector<item> &items, int n, tripoint pos )
{
    ...
    if( items[n].has_flag("RADIO_ACTIVATION") && items[n].has_flag(signal) ) {
        g->sound(pos.x, pos.y, pos.z, 6, "beep.");
...
*/
 ...
 if (terlist[ current_submap->ter[lx][ly][lz] ].trap != tr_null) {
  return terlist[ current_submap->ter[lx][ly][lz] ].trap;
 }

 return current_submap->get_trap(lx, ly, lz);
}
...
void map::add_trap(const int x, const int y, const int z, const trap_id t)
{
    ...
    if (current_submap->get_trap(lx, ly, lz) != tr_null) {
        remove_trap(x, y, z);
    }

    current_submap->set_trap(lx, ly, lz, t);
    if (t != tr_null) {
...
void map::disarm_trap(const int x, const int y, const int z)
{
    ...
    if (roll >= diff) {
        ...
        if (tr_at(x, y, z) == tr_shotgun_1 || tr_at(x,y, z) == tr_shotgun_2) {
            spawn_item(x,y, z, "shot_00",1,2);
        }
        remove_trap(x, y, z);
...
void map::remove_trap(const int x, const int y, const int z)
{
    ...
    trap_id t = current_submap->get_trap(lx, ly, lz);
    if (t != tr_null) {
        if (g != NULL && this == &g->m) {
            g->u.add_known_trap(x, y, z, "tr_null");
        }
        current_submap->set_trap(lx, ly, lz, tr_null);
 ...
 */
const field &map::field_at( const int x, const int y, const int z) const
{
    if( !inbounds( x, y, z ) ) {
        nulfield = field();
        return nulfield;
    ...
    return current_submap->fld[lx][ly][lz];
}

field &map::get_field( const int x, const int y, const int z )
{
    if( !inbounds( x, y, z ) ) {
        nulfield = field();
        return nulfield;
...
field_entry * map::get_field( const tripoint p, const field_id t ) {
    if (!INBOUNDS(p.x, p.y, p.z))
        return NULL;
    int lx, ly, lz;
    submap * const current_submap = get_submap_at(p.x, p.y, p.z, lx, ly, lz);
    return current_submap->fld[lx][ly][lz].findField(t);
}
...
bool map::add_field(const tripoint p, const field_id t, int density, const int age)
{
    ...
    if( current_submap->fld[lx][ly][lz].addField( t, density, age ) ) {
    ...
    if(g != NULL && this == &g->m && p.x == g->u.posx && p.y == g->u.posy) {
        step_in_field(p.x, p.y, p.z); //Hit the player with the field if it spawned on top of them.
    }
    return true;
}

bool map::add_field(const int x, const int y, const int z, const field_id t, const int new_density)
{
    return this->add_field(tripoint(x,y,z), t, new_density, 0);
}

void map::remove_field(const int x, const int y, const int z, const field_id field_to_remove)
{
 if (!INBOUNDS(x, y, z)) {
  return;
 }
 ...
 if (current_submap->fld[lx][ly][lz].findField(field_to_remove)) { //same as checking for fd_null in the old system
  current_submap->field_count--;
 }
 current_submap->fld[lx][ly][lz].removeField(field_to_remove);
}

computer* map::computer_at(const int x, const int y, const int z)
{
 if (!INBOUNDS(x, y, z))
  return NULL;

 submap * const current_submap = get_submap_at(x, y, z);
...
bool map::allow_camp(const int x, const int y, const int z, const int radius)
{
    return camp_at(x, y, z, radius) == NULL;
}

// locate the nearest camp in some radius (default CAMPSIZE)
basecamp* map::camp_at(const int x, const int y, const int z, const int radius)
{
    if (!INBOUNDS(x, y, z)) {
        return NULL;
    }
...
void map::add_camp(const std::string& name, const int x, const int y, const int z)
{
    if (!allow_camp(x, y, z)) {
    ...
    get_submap_at(x, y, z)->camp = basecamp(name, x, y);
}
...
void map::debug()
{
 ...
 for (int i = 0; i <= SEEX * 2; i++) {
  for (int j = 0; j <= SEEY * 2; j++) {
   for (int h = 0; h <= SEEZ * 2; h++) {
    if (i_at(i, j, h).size() > 0) {
     mvprintw(1, 0, "%d, %d: %d items", i, j, i_at(i, j, h).size());
     mvprintw(2, 0, "%c, %d", i_at(i, j, h)[0].symbol(), i_at(i, j, h)[0].color());
     getch();
    }
   }
  }
...
void map::draw(WINDOW* w, const tripoint center)
{
 ...
 for  (int realx = center.x - getmaxx(w)/2; realx <= center.x + getmaxx(w)/2; realx++) {
  for (int realy = center.y - getmaxy(w)/2; realy <= center.y + getmaxy(w)/2; realy++) {
   ...
   if (!is_outside(realx, realy, center.z)) {
...
void map::drawsq(WINDOW* w, player &u, const int x, const int y, const int z, const bool invert_arg,
                 ...
                 const bool low_light, const bool bright_light)
{
    ...
    nc_color tercol;
    const ter_id curr_ter = ter(x,y,z);
    const furn_id curr_furn = furn(x,y,z);
    const trap_id curr_trap = tr_at(x, y, z);
    const field &curr_field = field_at(x, y, z);
    const std::vector<item> &curr_items = i_at(x, y, z);
    long sym;
    bool hi = false;
    bool graf = false;
    ...
    if (has_flag(TFLAG_SWIMMABLE, x, y, z) && has_flag(TFLAG_DEEP_WATER, x, y, z) && !u.is_underwater()) {
        show_items = false; // Can only see underwater items if WE are underwater
    }
    // If there's a trap here, and we have sufficient perception, draw that instead
    if (curr_trap != tr_null && traplist[curr_trap]->can_see(g->u, x, y, z)) {
    ...
    if (show_items && sees_some_items(x, y, z, g->u)) {
    ...
    int veh_part = 0;
    vehicle *veh = veh_at(x, y, z, veh_part);
    if (veh) {
        sym = special_symbol (veh->face.dir_symbol(veh->part_sym(veh_part)));
        tercol = veh->part_color(veh_part);
    }
    // If there's graffiti here, change background color
    if( has_graffiti_at( x, y, z ) ) {
        graf = true;
    }
    ...
    if(sym == LINE_XOXO || sym == LINE_OXOX) { //vertical or horizontal
        sym = determine_wall_corner(x, y, z, sym);
    }
...
*/
bool map::sees(const int Fx, const int Fy, const int Fz, const int Tx, const int Ty, const int Tz,
...
// promote "mobbing" behavior (zombies surround you to beat on you)
  for (tc = abs(ay - (ax >> 1)) * 2 + 1; tc >= -1; tc--) {
   ...
   } while ((trans(x, y, z)) && (INBOUNDS(x,y,z)));
  }
  return false;
...
bool map::clear_path(const int Fx, const int Fy, const int Fz, const int Tx, const int Ty, const int Tz,
...
// promote "mobbing" behavior (zombies surround you to beat on you)
  for (tc = abs(ay - (ax >> 1)) * 2 + 1; tc >= -1; tc--) {
   ...
   } while (move_cost(x, y, z) >= cost_min && move_cost(x, y, z) <= cost_max &&
            INBOUNDS(x, y, z));
  }
  return false;
 } else { // Same as above, for mostly-vertical lines
...
bool map::accessable_items(const int Fx, const int Fy, const int Fz, const int Tx, const int Ty, const int Tz, const int range) const
{
    int junk = 0;
    return (has_flag("SEALED", Tx, Ty, Tz) && !has_flag("LIQUIDCONT", Tx, Ty, Tz)) ||
        ((Fx != Tx || Fy != Ty) &&
         !clear_path( Fx, Fy, Fz, Tx, Ty, Tz, range, 1, 100, junk ) );
}

bool map::accessable_furniture(const int Fx, const int Fy, const int Fz, const int Tx, const int Ty, const int Tz, const int range) const
{
    int junk = 0;
    return ((Fx == Tx && Fy == Ty) ||
           clear_path( Fx, Fy, Fz, Tx, Ty, Tz, range, 1, 100, junk ) );
...
std::vector<tripoint> map::getDirCircle(const int Fx, const int Fy, const int Fz, const int Tx, const int Ty, const int Tz)
{
    std::vector<tripoint> vCircle;
    vCircle.resize(8);

    const std::vector<point> vLine = line_to(Fx, Fy, Tx, Ty, 0);
    const std::vector<tripoint> vSpiral = closest_points_first(1, Fx, Fy, Fz);
    ...
    for (unsigned int i = 1; i < vSpiral.size(); i++) {
        ...
        vCircle[vPos[iPosOffset++]-1] = tripoint(vSpiral[i].x, vSpiral[i].y, vSpiral[i].z);
    }
...
// Bash defaults to true.
 std::vector<tripoint> map::route(const int Fx, const int Fy, const int Fz, const int Tx, const int Ty, const int Tz,
                          const bool can_bash)
{
    ...
    if (!INBOUNDS(Fx, Fy, Fz) || !INBOUNDS(Tx, Ty, Tz)) {
        int linet;
        if (sees(Fx, Fy, Fz, Tx, Ty, Tz, -1, linet)) {
            return line_to(tripoint(Fx, Fy, Fz), tripoint(Tx, Ty, Tz), linet, 0);
        } else {
    ...
    int linet = 0;
    if (clear_path(Fx, Fy, Fz, Tx, Ty, Tz, -1, 2, 2, linet)) {
        return line_to(tripoint(Fx, Fy, Fz), tripoint(Tx, Ty, Tz), linet, 0);
    }
    /*
    ...
    */
    ...
    int startx = Fx - 4, endx = Tx + 4, starty = Fy - 4, endy = Ty + 4, startz = Fz - 4, endz = Tz + 4;
    if (Tx < Fx) {
        startx = Tx - 4;
        endx = Fx + 4;
    }
    ...
    for (int x = startx; x <= endx; x++) {
        for (int y = starty; y <= endy; y++) {
            for (int z = startz; z <= endz; z++) {
                list  [x][y][z] = ASL_NONE; // Init to not being on any list
                score [x][y][z] = 0;        // No score!
                gscore[x][y][z] = 0;        // No score!
                parent[x][y][z] = tripoint(-1, -1, -1);
            }
        }
    }
    list[Fx][Fy][Fz] = ASL_OPEN;
    open.push_back(tripoint(Fx, Fy, Fz));
    ...
    do {
        ...
        for (size_t i = 0; i < open.size(); i++) {
            if (i == 0 || score[open[i].x][open[i].y][open[i].z] < best) {
                best = score[open[i].x][open[i].y][open[i].z];
                index = i;
            }
        }

        std::vector<tripoint> vDirCircle = getDirCircle(open[index].x, open[index].y, open[index].z, Tx, Ty, Tz);

        for (unsigned int i = 0; i < vDirCircle.size(); ++i) {
            const int x = vDirCircle[i].x;
            const int y = vDirCircle[i].y;
            const int z = vDirCircle[i].z;

            if (x == Tx && y == Ty) {
                done = true;
                parent[x][y][z] = open[index];
            } else if (x >= startx && x <= endx && y >= starty && y <= endy &&
                        (move_cost(x, y, z) > 0 || (can_bash && is_bashable(x, y, z)))) {
                if (list[x][y][z] == ASL_NONE) { // Not listed, so make it open
                    list[x][y][z] = ASL_OPEN;
                    open.push_back(tripoint(x, y, z));
                    parent[x][y][z] = open[index];
                    gscore[x][y][z] = gscore[open[index].x][open[index].y][open[index].z] + move_cost(x, y, z) + ((open[index].x - x != 0 && open[index].y - y != 0) ? 1 : 0);
                    if (ter(x, y, z) == t_door_c) {
                        gscore[x][y][z] += 4; // A turn to open it and a turn to move there
                    } else if (move_cost(x, y, z) == 0 && (can_bash && is_bashable(x, y, z))) {
                        gscore[x][y][z] += 18; // Worst case scenario with damage penalty
                    }
                    score[x][y][z] = gscore[x][y][z] + 2 * rl_dist(x, y, Tx, Ty);
                } else if (list[x][y][z] == ASL_OPEN) { // It's open, but make it our child
                    int newg = gscore[open[index].x][open[index].y][open[index].z] + move_cost(x, y, z) + ((open[index].x - x != 0 && open[index].y - y != 0) ? 1 : 0);
                    if (ter(x, y, z) == t_door_c) {
                        newg += 4; // A turn to open it and a turn to move there
                    } else if (move_cost(x, y, z) == 0 && (can_bash && is_bashable(x, y, z))) {
                        newg += 18; // Worst case scenario with damage penalty
                    }
                    if (newg < gscore[x][y][z]) {
                        gscore[x][y][z] = newg;
                        parent[x][y][z] = open[index];
                        score [x][y][z] = gscore[x][y][z] + 2 * rl_dist(x, y, Tx, Ty);
                    }
        ...
        list[open[index].x][open[index].y][open[index].z] = ASL_CLOSED;
        open.erase(open.begin() + index);
    } while (!done && !open.empty());

    std::vector<tripoint> tmp;
    ...
    if (done) {
        ...
        while (cur.x != Fx || cur.y != Fy) {
            ...
            tmp.push_back(cur);
            if (rl_dist(cur.x, cur.y, parent[cur.x][cur.y][cur.z].x, parent[cur.x][cur.y][cur.z].y)>1){
                debugmsg("Jump in our route! %d:%d->%d:%d", cur.x, cur.y,
                            parent[cur.x][cur.y][cur.z].x, parent[cur.x][cur.y][cur.z].y);
                return ret;
            }
...
int map::coord_to_angle ( const int x, const int y, const int z, const int tgtx, const int tgty, const int tgtz ) {
...
void map::save()
{
    for( int gridx = 0; gridx < my_MAPSIZE; gridx++ ) {
        for( int gridy = 0; gridy < my_MAPSIZE; gridy++ ) {
            for (int gridz = 0; gridz < my_MAPSIZE; gridz++) {
                saven( abs_sub.x, abs_sub.y, abs_sub.z, gridx, gridy, gridz );
            }
        }
    }
}
    ...
    for (int x = 0; x < SEEX; x++) {
        for (int y = 0; y < SEEY; y++) {
            for (int z = 0; z < SEEZ; z++) {
                trap_id t = grid[n]->get_trap(x, y, z);
                if (t != tr_null) {
                    const int fx = x + gridx * SEEX;
                    const int fy = y + gridy * SEEY;
                    const int fz = z + gridz * SEEZ;
                    traplocs[t].erase(tripoint(fx, fy, fz));
                }
            }
        }
    }
            ...
            if (sy >= 0) {
                    ...
                    if (sz >= 0) {
                        for (int gridz = 0; gridz < my_MAPSIZE; gridz++){
                            if (gridx + sx < my_MAPSIZE && gridy + sy < my_MAPSIZE) {
                                ...
                                loadn(absx + sx, absy + sy, wz, gridx, gridy, gridz);
                            }
                        }
                    }
                }
                ...
                for (int gridy = my_MAPSIZE - 1; gridy >= 0; gridy--) {
                    if (gridx + sx < my_MAPSIZE && gridy + sy >= 0) {
                        copy_grid(gridx + gridy * my_MAPSIZE,
                                  gridx + sx + (gridy + sy) * my_MAPSIZE);
                        update_vehicle_list(get_submap_at_grid(gridx, gridy, sz));
                    } else {
                        loadn(absx + sx, absy + sy, wz, gridx, gridy, sz);
                    }
                }
            }
        }
                 ...
                 const int gridx, const int gridy, const int gridz )
{
    dbg( D_INFO ) << "map::saven(worldx[" << worldx << "], worldy[" << worldy << "], gridx[" << gridx <<
                  "], gridy[" << gridy << "])";
    submap *submap_to_save = get_submap_at_grid( gridx, gridy, gridz );
    if( submap_to_save == NULL || submap_to_save->ter[0][0][0] == t_null ) {
...
void map::loadn(const int worldx, const int worldy, const int worldz,
                const int gridx, const int gridy, const int gridz, const bool update_vehicles) {
...
void map::grow_plant( const tripoint pnt )
{
    ...
    while( calendar::turn > seed.bday + plantEpoch && furn.loadid < f_plant_harvest ) {
        ...
        furn_set( pnt.x, pnt.y, pnt.z, static_cast<furn_id>( furn.loadid + 1 ) );
    }
}

void map::restock_fruits( const tripoint pnt, int time_since_last_actualize )
{
    const auto &ter = ter_at( pnt.x, pnt.y, pnt.z );
    ...
    if( ter.harvest_season != calendar::turn.get_season() ||
        ...
        ter_set( pnt.x, pnt.y, pnt.z, ter.transforms_into );
    }
}
...
void map::actualize( const int gridx, const int gridy, const int gridz )
{
    ...
    for( int x = 0; x < SEEX; x++ ) {
        for( int y = 0; y < SEEY; y++ ) {
            for ( int z = 0; z < SEEZ; z++) {
                const tripoint pnt( gridx * SEEX + x, gridy * SEEY + y, gridz * SEEZ + z );

                remove_rotten_items( tmpsub->itm[x][y][z], pnt );

                const auto trap_here = tmpsub->get_trap( x, y, z );
 ...
 for( std::vector<vehicle*>::iterator it = grid[to]->vehicles.begin(),
  ...
  (*it)->smy = to / my_MAPSIZE;
    ...
    for (int x = 0; x < SEEX; x++) {
        for (int y = 0; y < SEEY; y++) {
            for (int z = 0; z < SEEZ; z++) {
                trap_id t = grid[to]->get_trap(x, y, z);
                if (t != tr_null) {
                    traplocs[t].erase(tripoint(oldx + x, oldy + y, z));
                    traplocs[t].insert(tripoint(newx + x, newy + y, z));
                }
            }
        }
    }
...
void map::spawn_monsters( int gx, int gy, int gz, mongroup &group, bool ignore_sight )
{
    ...
    for( int x = 0; x < SEEX; ++x ) {
        for( int y = 0; y < SEEY; ++y ) {
            for ( int z = 0; z < SEEZ; ++z) {
                ...
                int fy = y + SEEY * gy;
                int fz = z + SEEZ * gz;
                if( g->critter_at( fx, fy, fz ) != nullptr ) {
                    continue; // there is already some creature
                }
                if( move_cost( fx, fy, fz ) == 0 ) {
                    continue; // solid area, impassable
                }
                int t;
                if( !ignore_sight && sees( g->u.posx, g->u.posy, g->u.posz, fx, fy, fz, s_range, t ) ) {
                    continue; // monster must spawn outside the viewing range of the player
                }
                if( has_flag_ter_or_furn( TFLAG_INDOORS, fx, fy, fz ) ) {
                    continue; // monster must spawn outside.
                }
                locations.push_back( tripoint( fx, fy, fz ) );
            }
        }
    }
    if( locations.empty() ) {
    ...
    for( int m = 0; m < pop; m++ ) {
        ...
        for( int i = 0; i < spawn_details.pack_size; i++) {
            for( int tries = 0; tries < 10 && !locations.empty(); tries++ ) {
                ...
                if( !tmp.can_move_to( p.x, p.y, p.z ) ) {
...
void map::spawn_monsters(bool ignore_sight)
{
    for (int gx = 0; gx < my_MAPSIZE; gx++) {
        for (int gy = 0; gy < my_MAPSIZE; gy++) {
            for (int gz = 0; gz < my_MAPSIZE; gz++) {
                auto groups = overmap_buffer.groups_at( abs_sub.x + gx, abs_sub.y + gy, abs_sub.z + gz );
                for( auto &mgp : groups ) {
                    spawn_monsters( gx, gy, gz, *mgp, ignore_sight );
                }
                ...
                for (auto &i : current_submap->spawns) {
                    for (int j = 0; j < i.count; j++) {
                        int tries = 0;
                        int mx = i.posx, my = i.posy, mz = i.posz;
                        ...
                        int fx = mx + gx * SEEX, fy = my + gy * SEEY, fz = mz + gz * SEEZ;

                        while ((!g->is_empty(fx, fy, fz) || !tmp.can_move_to(fx, fy, fz)) && tries < 10) {
                            mx = (i.posx + rng(-3, 3)) % SEEX;
                            my = (i.posy + rng(-3, 3)) % SEEY;
                            mz = (i.posz + rng(-3, 3)) % SEEZ;
                            if (mx < 0) {
                                mx += SEEX;
                            }
                            if (my < 0) {
...
void map::clear_traps()
{
    ...
    std::map<trap_id, std::set<tripoint> >::iterator i;
...
{
 return (x >= 0 && x < SEEX * my_MAPSIZE && y >= 0 && y < SEEY * my_MAPSIZE && z >= 0 && z < SEEZ * my_MAPSIZE);
}

void map::set_graffiti( int x, int y, int z, const std::string &contents )
{
    if( !inbounds( x, y, z ) ) {
        return;
    }
    int lx, ly, lz;
    submap *const current_submap = get_submap_at( x, y, z, lx, ly, lz );
    current_submap->set_graffiti( lx, ly, lz, contents );
}

void map::delete_graffiti( int x, int y, int z )
{
    if( !inbounds( x, y, z ) ) {
        return;
    }
    int lx, ly, lz;
    submap *const current_submap = get_submap_at( x, y, z, lx, ly, lz );
    current_submap->delete_graffiti( lx, ly, lz );
}

const std::string &map::graffiti_at( int x, int y, int z ) const
{
    if( !inbounds( x, y, z ) ) {
    ...
    int lx, ly, lz;
    submap *const current_submap = get_submap_at( x, y, z, lx, ly, lz );
    return current_submap->get_graffiti( lx, ly, lz );
}

bool map::has_graffiti_at( int x, int y, int z ) const
{
    if( !inbounds( x, y, z ) ) {
        return false;
    }
    int lx, ly, lz;
    submap *const current_submap = get_submap_at( x, y, z, lx, ly, lz );
    return current_submap->has_graffiti( lx, ly, lz );
}

long map::determine_wall_corner(const int x, const int y, const int z, const long orig_sym)
{
    long sym = orig_sym;
    //LINE_NESW
    const long above = terlist[ter(x, y-1, z)].sym;
    const long below = terlist[ter(x, y+1, z)].sym;
    const long left  = terlist[ter(x-1, y, z)].sym;
    const long right = terlist[ter(x+1, y, z)].sym;
...
void map::build_outside_cache()
{
    ...
    for(int x = 0; x < SEEX * my_MAPSIZE; x++)
    {
        ...
        {
            for (int z = 0; z < SEEZ * my_MAPSIZE; z++){
                if( has_flag_ter_or_furn(TFLAG_INDOORS, x, y, z))
                {
...
void map::build_transparency_cache()
{
    ...
    for( int x = 0; x < my_MAPSIZE * SEEX; x++ ) {
        for( int y = 0; y < my_MAPSIZE * SEEY; y++ ) {
            for ( int z = 0; z < my_MAPSIZE * SEEZ; z++) {
                // Default to fully transparent.
                transparency_cache[x][y] = LIGHT_TRANSPARENCY_CLEAR;

                if( !terlist[ter(x, y, z)].transparent || !furnlist[furn(x, y, z)].transparent ) {
...
void map::build_map_cache()
{
    ...
    for(auto &v : vehs) {
        for (size_t part = 0; part < v.v->parts.size(); part++) {
            ...
            int pz = v.z + v.v->parts[part].precalc_dz[0];
...
{
    return closest_points_first(radius, p.x, p.y, p.z);
}
...
std::vector<tripoint> closest_points_first(int radius, int center_x, int center_y, int center_z)
{
...
tripoint map::getabs(const int x, const int y, const int z) const
{
    return tripoint( x + abs_sub.x * SEEX, y + abs_sub.y * SEEY, z + abs_sub.z * SEEZ );
}

tripoint map::getlocal(const int x, const int y, const int z) const {
    return tripoint( x - abs_sub.x * SEEX, y - abs_sub.y * SEEY, z - abs_sub.z * SEEZ );
}
...
submap *map::get_submap_at(int x, int y, int z) const {
    ...
    const int nonant = int(x / SEEX) + int(y / SEEY) + int(z / SEEZ) * my_MAPSIZE;
    return grid[nonant];
}

submap *map::get_submap_at(int x, int y, int z, int& offset_x, int& offset_y, int& offset_z) const {
    offset_x = x % SEEX;
    offset_y = y % SEEY;
    offset_z = z % SEEZ;
...
submap *map::get_submap_at_grid(int gridx, int gridy, int gridz) const {
    ...
    const int nonant = gridx + gridy + gridz * my_MAPSIZE;
    return grid[nonant];
}
