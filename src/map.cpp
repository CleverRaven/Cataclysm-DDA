...
void map::vehicle_wheel_traction( const vehicle &veh, float* returns ) const
{
    ...
    for( auto &wheel_index : wheel_indices ) {
        ...
        //        g->u.add_msg_if_player(m_debug, "Continue with %f:%f:%f.", veh.parts[ wheel_index ].wheel_area(), sinkage, friction_mult * weather_mult * veh.parts[ wheel_index ].wheel_friction());
    }
    
    //    returns[K_FRICTION] /= num_wheels - submerged_wheels - air_wheels;
    //    returns[K_TRACTION] /= num_wheels - submerged_wheels - air_wheels;
    ...
    g->u.add_msg_if_player(m_debug, "End with %f:%f:%f and %f:%f.", returns[OBSTACLE], returns[K_FRICTION], returns[K_TRACTION], area, weight);
