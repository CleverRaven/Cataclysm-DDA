...
bool map::vehproceed( VehicleList &vehicle_list )
{
    ...
    if( cur_veh == nullptr ) {
        for( wrapped_vehicle &vehs_v : vehicle_list ) {
            if( vehs_v.v->is_falling || ( ( vehs_v.v->is_rotorcraft() || vehs_v.v->is_rotorlike_aircraft() ) &&
...
vehicle *map::move_vehicle( vehicle &veh, const tripoint_rel_ms &dp, const tileray &facing )
{
    ...
    if( velocity_before == 0 && !veh.is_rotorcraft() && !veh.is_rotorlike_aircraft() &&
    ...
    if( dp.z() != 0 && ( veh.is_rotorcraft() || veh.is_rotorlike_aircraft() ) ) {
        can_move = true;
    }
