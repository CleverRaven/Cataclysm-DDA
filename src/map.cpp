...
bool map::vehproceed()
{
    ...
    { // sink in water?
        ...
        if (num_wheels &&  (float)submerged_wheels / num_wheels > .666) {
            Messages::add_msg(_("Your %s sank."), veh->name.c_str());
            if (pl_ctrl) {
                veh->unboard_all ();
            }
    ...
    if (veh->skidding) {
        ...
        Messages::add_msg(_("You fumble with the %s's controls."), veh->name.c_str());
        veh->turn (one_in(2) ? -15 : 15);
    }
    ...
    if(veh_veh_colls.size()) { // we have dynamic crap!
        ...
        Messages::add_msg(_("The %1$s's %2$s collides with the %3$s's %4$s."),
    ...
    if (dmg_1 > 0) {
        ...
        for (int ps = 0; ps < ppl.size(); ps++) {
            ...
            if(d_vel > 60* rng(50,100)/100 && !throw_from_seat) {
                ...
                psg->hurtall(dmg);
                if (psg == &g->u) {
                    Messages::add_msg(_("You take %d damage by the power of the impact!"), dmg);
                } else if (psg->name.length()) {
                    Messages::add_msg(_("%s takes %d damage by the power of the impact!"),
                               psg->name.c_str(), dmg);
                }
            }
            ...
            if (throw_from_seat) {
                if (psg == &g->u) {
                    Messages::add_msg(_("You are hurled from the %s's seat by the power of the impact!"),
                               veh->name.c_str());
                } else if (psg->name.length()) {
                    Messages::add_msg(_("%s is hurled from the %s's seat by the power of the impact!"),
                ...
                if (lose_ctrl_roll > psg->dex_cur * 2 + psg->skillLevel("driving") * 3) {
                    if (psg == &g->u) {
                        Messages::add_msg(_("You lose control of the %s."), veh->name.c_str());
                    } else if (psg->name.length()) {
                        Messages::add_msg(_("%s loses control of the %s."), psg->name.c_str());
                    }
        ...
        for (int w = 0; w < wheel_indices.size(); w++) {
            ...
            if (one_in(2)) {
                if (displace_water (x + veh->parts[p].precalc_dx[0],
                    ...
                    Messages::add_msg(_("You hear a splash!"));
                }
            }
                ...
                const bool hit_items, const std::set<std::string>& ammo_effects)
{
    ...
    if( 0 == terrain.id.compare("t_wall_wood_broken") ||
        ...
        if (ammo_effects.count("LASER")) {
            ...
            dam -= 40;
            if (dam <= 0) {
                Messages::add_msg(_("The shot is stopped by the reinforced glass wall!"));
            } else if (dam >= 40) {
...
bool map::hit_with_acid(const int x, const int y)
{
 ...
 switch (oldter(x, y)) {
  ...
  case old_t_bars:
   ter_set(x, y, t_floor);
   Messages::add_msg(_("The metal bars melt!"));
   break;
                ...
                if (rng(0,it->volume()) > it->burnt && g->revive_corpse(mapx, mapy, it)) {
                    if (g->u_see(mapx, mapy)) {
                        if(it->corpse->in_species("ROBOT")) {
                            Messages::add_msg(_("A nearby robot has repaired itself and stands up!"));
                        } else {
                            Messages::add_msg(_("A nearby corpse rises and moves towards you!"));
                        }
                    }
                    return true;
                } else {
...
void map::disarm_trap(const int x, const int y)
{
 ...
 while ((rng(5, 20) < g->u.per_cur || rng(1, 20) < g->u.dex_cur) && roll < 50)
  roll++;
 if (roll >= diff) {
  Messages::add_msg(_("You disarm the trap!"));
  ...
  Messages::add_msg(_("You fail to disarm the trap."));
  if(diff > 1.25 * skillLevel)
    g->u.practice(calendar::turn, "traps", 1.5*(diff - skillLevel));
 }
 else {
  Messages::add_msg(_("You fail to disarm the trap, and you set it off!"));
