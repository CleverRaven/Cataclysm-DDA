...
void move_mode::load( const JsonObject &jo, const std::string & )
{
    ...
    optional( jo, was_loaded, "flavor_text_mount", flavor_text_mount,
...
void move_mode::check() const
{
    if( display_character.length() < 1 ) {
        debugmsg( "Attempt to load move mode %s failed because it did not have a valid display_character",
                  id.c_str() );
    }
}
