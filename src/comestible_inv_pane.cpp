...
struct col_data {
    ...
    col_data(comestible_inv_columns id, std::string col_name, int width, char hotkey, std::string sort_name) :
        id(id),
...
void comestible_inventory_pane::init(std::vector<comestible_inv_columns> c, int items_per_page, catacurses::window w, std::array<comestible_inv_area, comestible_inv_area_info::NUM_AIM_LOCATIONS>* s){
    columns = c;
    ...
    comestible_inv_area_info::aim_location location = static_cast<comestible_inv_area_info::aim_location>(uistate.comestible_save.area_idx);
    ...
    if (std::find(columns.begin(), columns.end(), uistate.comestible_save.sort_idx)!= columns.end()) {
...
void comestible_inventory_pane::add_sort_enries(uilist& sm) {
    ...
    for (size_t i = 0; i < columns.size(); i++) {
        ...
        if (c.id == COLUMN_SRC || c.id == COLUMN_AMOUNT) continue;
...
void comestible_inventory_pane::print_items()
{
    ...
    for (int i = page * itemsPerPage, cur_print_y = 6; i < static_cast<int>(items.size()) &&
        ...
        if (sitem.is_category_header()) {
            mvwprintz(window, point((max_width - utf8_width(sitem.name) - 6) / 2, cur_print_y), c_cyan, "[%s]",
                sitem.name);
            continue;
        }
...
struct comestible_inv_sorter {
    ...
    bool compare(comestible_inv_columns compare_by, const comestible_inv_listitem& d1, const comestible_inv_listitem& d2) {
        switch (compare_by) {
        case COLUMN_NAME:
            const std::string* n1;
            const std::string* n2;
...
void comestible_inventory_pane::add_items_from_area(comestible_inv_area* area, bool from_cargo, units::volume& ret_volume, units::mass& ret_weight)
{
...
void comestible_inventory_pane::redraw()
{
    ...
    bool same_as_dragged = (cur_area->info.id >= comestible_inv_area_info::AIM_SOUTHWEST && cur_area->info.id <= comestible_inv_area_info::AIM_NORTHEAST) &&
        // only cardinals
        cur_area->info.id != comestible_inv_area_info::AIM_CENTER && is_in_vehicle() && // not where you stand, and pane is in vehicle
        cur_area->info.default_offset == get_square(comestible_inv_area_info::AIM_DRAGGED)->info.default_offset; // make sure the offsets are the same as the grab point
    const comestible_inv_area* sq = same_as_dragged ? get_square(comestible_inv_area_info::AIM_DRAGGED) : cur_area;

    
    bool car = cur_area->has_vehicle() && is_in_vehicle() && sq->info.id != comestible_inv_area_info::AIM_DRAGGED;
...
int comestible_inventory_pane::print_header(comestible_inv_area* sel_area)
{
    ...
    for (size_t i = 0; i < squares->size(); ++i) {
        std::string key = comestible_inv_area::get_info(static_cast<comestible_inv_area_info::aim_location>(i)).minimapname;
        ...
        wprintz(window, kcolor, "%s", viewing_cargo && is_selected_by_item && is_single && cur_area->info.id != comestible_inv_area_info::AIM_DRAGGED ? "V" : key);
        wprintz(window, bcolor, "%c", bracket[1]);
    }
    return get_square(comestible_inv_area_info::AIM_INVENTORY)->info.hscreen.y + ofs;
}

void comestible_inv_listitem::print_columns(std::vector<comestible_inv_columns> columns, comestible_select_state selected_state, bool is_compact, catacurses::window window, int right_bound, int cur_print_y) {
    
    is_selected = selected_state;
    int w;
    for (std::vector<comestible_inv_columns>::reverse_iterator col_iter = columns.rbegin(); col_iter != columns.rend(); ++col_iter) {
...
bool comestible_inv_listitem::print_string_column(comestible_inv_columns col, catacurses::window window, int cur_print_x, int cur_print_y) {
    ...
    mvwprintz(window, point(cur_print_x, cur_print_y), print_color, "%*s", get_col_data(col).width, print_string);
    return true;
}

bool comestible_inv_listitem::print_int_column(comestible_inv_columns col, catacurses::window window, int cur_print_x, int cur_print_y) {
    int print_val;
    ...
    mvwprintz(window, point(cur_print_x, cur_print_y), print_color, "%*s", get_col_data(col).width, s);
    return true;
}
...
bool comestible_inv_listitem::print_default_columns(comestible_inv_columns col, catacurses::window window, int cur_print_x, int cur_print_y) {
    nc_color print_color;
    std::string s;
...
void comestible_inv_listitem::print_name(bool is_compact, catacurses::window window, int right_bound, int cur_print_y) {
    ...
    trim_and_print(window, point(name_startpos, cur_print_y), max_name_length, print_color, item_name);
