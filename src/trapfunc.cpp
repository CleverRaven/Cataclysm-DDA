...
{
 Messages::add_msg(_("You step on some bubble wrap!"));
...
{
 Messages::add_msg(_("The %s stumbles over the cot"), z->name().c_str());
 z->moves -= 100;
}

void trapfunc::beartrap(int x, int y)
{
    Messages::add_msg(_("A bear trap closes on your foot!"));
...
{
 Messages::add_msg(_("You step on a spiked board!"));
...
void trapfuncm::board(monster *z, int x, int y)
{
 ...
 if (g->u_see(z))
  Messages::add_msg(_("The %s steps on a spiked board!"), z->name().c_str());
 if (z->hurt(rng(6, 10)))
  g->kill_mon(g->mon_at(x, y));
 else
  z->moves -= 80;
...
{
 Messages::add_msg(_("You step on a sharp metal caltrop!"));
...
void trapfuncm::caltrops(monster *z, int x, int y)
{
 ...
 if (g->u_see(z))
  Messages::add_msg(_("The %s steps on a caltrop!"), z->name().c_str());
 if (z->hurt(rng(18, 30)))
  g->kill_mon(g->mon_at(x, y));
 else
  z->moves -= 80;
...
{
 Messages::add_msg(_("You trip over a tripwire!"));
...
void trapfuncm::tripwire(monster *z, int, int)
{
 ...
 if (g->u_see(z))
  Messages::add_msg(_("The %s trips over a tripwire!"), z->name().c_str());
 z->stumble(false);
...
{
 bool add_bolt = true;
 Messages::add_msg(_("You trigger a crossbow trap!"));
 ...
 if (!one_in(4) && rng(8, 20) > g->u.get_dodge()) {
  ...
  Messages::add_msg(_("Your %s is hit!"), body_part_name(hit, side).c_str());
  g->u.hit(NULL, hit, side, 0, rng(20, 30));
  add_bolt = !one_in(10);
 } else
  Messages::add_msg(_("You dodge the shot!"));
 g->m.remove_trap(x, y);
...
void trapfuncm::crossbow(monster *z, int x, int y)
{
    ...
    {
        if (seen)
            Messages::add_msg(_("A bolt shoots out and hits the %s!"), z->name().c_str());
        if (z->hurt(rng(20, 30)))
            g->kill_mon(g->mon_at(x, y));
        add_bolt = !one_in(10);
    }
    else if (seen)
        Messages::add_msg(_("A bolt shoots out, but misses the %s."), z->name().c_str());
    g->m.remove_trap(x, y);
    g->m.spawn_item(x, y, "crossbow");
    g->m.spawn_item(x, y, "string_6");
    if (add_bolt)
...
{
 Messages::add_msg(_("You trigger a shotgun trap!"));
...
void trapfuncm::shotgun(monster *z, int x, int y)
{
 ...
 if (g->m.tr_at(x, y) == tr_shotgun_1)
  shots = 1;
 if (seen)
  Messages::add_msg(_("A shotgun fires and hits the %s!"), z->name().c_str());
...
{
 Messages::add_msg(_("A blade swings out and hacks your torso!"));
...
{
 if (g->u_see(z))
  Messages::add_msg(_("A blade swings out and hacks the %s!"), z->name().c_str());
...
void trapfunc::snare_light(int x, int y)
{
 ...
 Messages::add_msg(_("A snare closes on your leg."));
...
void trapfuncm::snare_light(monster *z, int x, int y)
{
 ...
 switch (z->type->size) {
  case MS_TINY:
   if(seen){
    Messages::add_msg(_("The %s has been snared!"), z->name().c_str());
   }
   if(z->hurt(10)){
    g->kill_mon(g->mon_at(x, y));
   } else {
  ...
  case MS_LARGE:
   if(seen){
    Messages::add_msg(_("The snare has no effect on the %s!"), z->name().c_str());
   }
   break;
  case MS_HUGE:
   if(seen){
...
void trapfunc::snare_heavy(int x, int y)
{
 ...
 Messages::add_msg(_("A snare closes on your %s."), body_part_name(hit, side).c_str());
...
{
 Messages::add_msg(_("You trigger a land mine!"));
...
void trapfuncm::landmine(monster *z, int x, int y)
{
 ...
 if (g->u_see(x, y))
  Messages::add_msg(_("The %s steps on a land mine!"), z->name().c_str());
 g->explosion(x, y, 10, 8, false);
 g->m.remove_trap(x, y);
}
...
{
 Messages::add_msg(_("You trigger a booby trap!"));
...
{
 if (g->u_see(x, y))
  Messages::add_msg(_("The %s triggers a booby trap!"), z->name().c_str());
 g->explosion(x, y, 18, 12, false);
 g->m.remove_trap(x, y);
}
...
void trapfunc::telepad(int x, int y)
{
    ...
    Messages::add_msg(_("The air shimmers around you..."));
...
void trapfuncm::telepad(monster *z, int x, int y)
{
 g->sound(x, y, 6, _("vvrrrRRMM*POP!*"));
 if (g->u_see(z))
  Messages::add_msg(_("The air shimmers around the %s..."), z->name().c_str());
...
{
 Messages::add_msg(_("You step in a puddle of thick goo."));
 ...
 if (one_in(3)) {
  Messages::add_msg(_("The acidic goo eats away at your feet."));
...
{
 Messages::add_msg(_("Electrical beams emit from the floor and slice your flesh!"));
...
{
    Messages::add_msg(_("You fall in a pit!"));
...
void trapfuncm::pit(monster *z, int x, int y)
{
    ...
    if (g->u_see(x, y)) {
        Messages::add_msg(_("The %s falls in a pit!"), z->name().c_str());
    }
...
void trapfunc::pit_spikes(int x, int y)
{
    ...
    if ( (g->u.has_trait("WINGS_BIRD")) || ((one_in(2)) && (g->u.has_trait("WINGS_BUTTERFLY"))) ) {
        ...
        Messages::add_msg(_("You avoid the spikes within."));
    } else {
        body_part hit = num_bp;
        switch (rng(1, 10)) {
            case  1:
        ...
        int side = random_side(hit);
        Messages::add_msg(_("The spikes impale your %s!"), body_part_name(hit, side).c_str());
        g->u.hit(NULL, hit, side, 0, damage);
        if (one_in(4)) {
            Messages::add_msg(_("The spears break!"));
...
void trapfuncm::pit_spikes(monster *z, int x, int y)
{
    ...
    bool sees = g->u_see(z);
    if (sees) {
        Messages::add_msg(_("The %s falls in a spiked pit!"), z->name().c_str());
    }
...
{
 Messages::add_msg(_("The %s burns you horribly!"), g->m.tername(x, y).c_str());
...
void trapfuncm::lava(monster *z, int x, int y)
{
 bool sees = g->u_see(z);
 if (sees)
  Messages::add_msg(_("The %s burns the %s!"), g->m.tername(x, y).c_str(),
                                     z->name().c_str());
...
void trapfunc::sinkhole(int x, int y)
{
 ...
 Messages::add_msg(_("You step into a sinkhole, and start to sink down!"));
...
// Determine safe places for the character to get pulled to
    ...
    if (safe.empty()) {
     Messages::add_msg(_("There's nowhere to pull yourself to, and you sink!"));
     g->u.use_amount("grapnel", 1);
     g->m.spawn_item(g->u.posx + rng(-1, 1), g->u.posy + rng(-1, 1), "grapnel");
     g->m.ter_set(g->u.posx, g->u.posy, t_hole);
     g->vertical_move(-1, true);
    } else {
     Messages::add_msg(_("You pull yourself to safety!  The sinkhole collapses."));
   ...
   Messages::add_msg(_("Your throw misses completely, and you sink!"));
  ...
  int throwroll = rng(g->u.skillLevel("throw"),
                      g->u.skillLevel("throw") + g->u.str_cur + g->u.dex_cur);
  if (throwroll >= 12) {
   Messages::add_msg(_("The rope catches something!"));
   if (rng(g->u.skillLevel("unarmed"),
...
// Determine safe places for the character to get pulled to
  ...
  Messages::add_msg(_("You sink into the sinkhole!"));
  g->m.ter_set(g->u.posx, g->u.posy, t_hole);
  g->vertical_move(-1, true);
 }
}
...
{
 Messages::add_msg(_("You fall down a level!"));
...
{
 Messages::add_msg(_("The %s falls down a level!"), z->name().c_str());
 g->kill_mon(g->mon_at(x, y));
}
...
{
    (void)x; (void)y; // unused
 Messages::add_msg(_("You step on a loose tile, and water starts to flood the room!"));
...
{
 Messages::add_msg(_("You hear the grinding of shifting rock."));
...
{
 if (one_in(3)) {
  Messages::add_msg(_("You're bathed in radiation!"));
  g->u.radiation += rng(10, 30);
 } else if (one_in(4)) {
  Messages::add_msg(_("A blinding flash strikes you!"));
  g->flashbang(g->u.posx, g->u.posy);
 } else
  Messages::add_msg(_("Small flashes surround you."));
}
...
void trapfunc::shadow(int x, int y)
{
 ...
 if (tries < 5) {
  Messages::add_msg(_("A shadow forms nearby."));
...
void trapfunc::drain(int, int)
{
 ...
 Messages::add_msg(_("You feel your life force sapping away."));
 g->u.hurtall(1);
}
...
void trapfunc::snake(int x, int y)
{
 ...
 if (one_in(3)) {
  ...
  if (tries < 5) {
   Messages::add_msg(_("A shadowy snake forms nearby."));
   spawned.spawn(monx, mony);
   g->add_zombie(spawned);
   g->m.remove_trap(x, y);
   return;
