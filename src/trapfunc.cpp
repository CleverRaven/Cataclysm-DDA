...
{
    add_msg(m_warning, _("You step on some bubble wrap!"));
...
void trapfuncm::board(monster *z, int x, int y)
{
    ...
    if (g->u_see(z)) {
        add_msg(m_good, _("The %s steps on a spiked board!"), z->name().c_str());
    }
    if (z->hurt(rng(6, 10))) {
        g->kill_mon(g->mon_at(x, y));
    } else {
...
{
    add_msg(m_bad, _("You step on a sharp metal caltrop!"));
...
void trapfuncm::caltrops(monster *z, int x, int y)
{
    ...
    if (g->u_see(z)) {
        add_msg(m_good, _("The %s steps on a caltrop!"), z->name().c_str());
    }
    if (z->hurt(rng(18, 30))) {
        g->kill_mon(g->mon_at(x, y));
    } else {
...
{
    add_msg(m_bad, _("You trip over a tripwire!"));
...
void trapfuncm::tripwire(monster *z, int, int)
{
    ...
    if (g->u_see(z)) {
        add_msg(m_good, _("The %s trips over a tripwire!"), z->name().c_str());
    }
    z->stumble(false);
...
{
    bool add_bolt = true;
    add_msg(m_neutral, _("You trigger a crossbow trap!"));
    ...
    if (!one_in(4) && rng(8, 20) > g->u.get_dodge()) {
        ...
        add_msg(m_bad, _("Your %s is hit!"), body_part_name(hit, side).c_str());
        g->u.hit(NULL, hit, side, 0, rng(20, 30));
        add_bolt = !one_in(10);
    } else {
        add_msg(m_neutral, _("You dodge the shot!"));
    }
    g->m.remove_trap(x, y);
...
{
    add_msg(m_neutral, _("You trigger a shotgun trap!"));
...
{
    add_msg(m_bad, _("A blade swings out and hacks your torso!"));
...
{
    if (g->u_see(z)) {
        add_msg(m_bad, _("A blade swings out and hacks the %s!"), z->name().c_str());
    }
...
void trapfunc::snare_light(int x, int y)
{
    ...
    add_msg(m_bad, _("A snare closes on your leg."));
...
void trapfuncm::snare_light(monster *z, int x, int y)
{
    ...
    switch (z->type->size) {
        case MS_TINY:
            if(seen) {
                add_msg(m_good, _("The %s has been snared!"), z->name().c_str());
            }
            if(z->hurt(10)) {
                g->kill_mon(g->mon_at(x, y));
            } else {
        ...
        case MS_HUGE:
            if(seen) {
                add_msg(m_neutral, _("The snare has no effect on the %s!"), z->name().c_str());
            }
            break;
    }
    g->m.remove_trap(x, y);
...
void trapfunc::snare_heavy(int x, int y)
{
    ...
    add_msg(m_bad, _("A snare closes on your %s."), body_part_name(hit, side).c_str());
...
{
    add_msg(m_bad, _("You trigger a land mine!"));
...
void trapfuncm::landmine(monster *z, int x, int y)
{
    ...
    if (g->u_see(x, y)) {
        add_msg(m_good, _("The %s steps on a land mine!"), z->name().c_str());
    }
    g->explosion(x, y, 10, 8, false);
    g->m.remove_trap(x, y);
}
...
{
    add_msg(m_bad, _("You trigger a booby trap!"));
...
{
    if (g->u_see(x, y)) {
        add_msg(m_good, _("The %s triggers a booby trap!"), z->name().c_str());
    }
    g->explosion(x, y, 18, 12, false);
    g->m.remove_trap(x, y);
}
...
void trapfunc::telepad(int x, int y)
{
    ...
    add_msg(m_warning, _("The air shimmers around you..."));
...
{
    add_msg(m_bad, _("You step in a puddle of thick goo."));
    ...
    if (one_in(3)) {
        add_msg(m_bad, _("The acidic goo eats away at your feet."));
...
{
    add_msg(m_bad, _("Electrical beams emit from the floor and slice your flesh!"));
...
{
    add_msg(m_bad, _("You fall in a pit!"));
...
void trapfuncm::pit(monster *z, int x, int y)
{
    ...
    if (g->u_see(x, y)) {
        add_msg(m_good, _("The %s falls in a pit!"), z->name().c_str());
    }
...
void trapfunc::pit_spikes(int x, int y)
{
    ...
    if ( (g->u.has_trait("WINGS_BIRD")) || ((one_in(2)) && (g->u.has_trait("WINGS_BUTTERFLY"))) ) {
        ...
        int side = random_side(hit);
        add_msg(m_bad, _("The spikes impale your %s!"), body_part_name(hit, side).c_str());
...
void trapfuncm::pit_spikes(monster *z, int x, int y)
{
    ...
    bool sees = g->u_see(z);
    if (sees) {
        add_msg(m_good, _("The %s falls in a spiked pit!"), z->name().c_str());
    }
...
{
    add_msg(m_bad, _("The %s burns you horribly!"), g->m.tername(x, y).c_str());
...
void trapfuncm::lava(monster *z, int x, int y)
{
    bool sees = g->u_see(z);
    if (sees)
        add_msg(m_good, _("The %s burns the %s!"), g->m.tername(x, y).c_str(),
                z->name().c_str());
...
void trapfunc::sinkhole(int x, int y)
{
    (void)x;
    (void)y; // unused
    add_msg(m_bad, _("You step into a sinkhole, and start to sink down!"));
...
void trapfunc::drain(int, int)
{
    ...
    add_msg(m_bad, _("You feel your life force sapping away."));
    g->u.hurtall(1);
}
