...
bool trapfunc::glass( const tripoint &p, Creature *c, item * )
{
    if( c != nullptr ) {
        ...
        if( dmg > 0 ) {
            c->deal_damage( nullptr, foot_l_bp, damage_instance( DT_CUT, dmg ) );
            c->deal_damage( nullptr, foot_r_bp, damage_instance( DT_CUT, dmg ) );
            c->check_dead_state();
        }
    }
...
bool trapfunc::beartrap( const tripoint &p, Creature *c, item * )
{
    ...
    if( c != nullptr ) {
        // What got hit?
        const bodypart_id hit = one_in( 2 ) ? leg_l_bp : leg_r_bp;
        ...
        if( n != nullptr ) {
            if( ( n->has_trait( trait_INFRESIST ) ) && ( one_in( 512 ) ) ) {
                n->add_effect( effect_tetanus, 1_turns, default_bp, true );
...
bool trapfunc::board( const tripoint &, Creature *c, item * )
{
    ...
    if( z != nullptr ) {
        ...
        z->deal_damage( nullptr, foot_l_bp, damage_instance( DT_CUT, rng( 3, 5 ) ) );
        z->deal_damage( nullptr, foot_r_bp, damage_instance( DT_CUT, rng( 3, 5 ) ) );
    } else {
        c->deal_damage( nullptr, foot_l_bp, damage_instance( DT_CUT, rng( 6, 10 ) ) );
        c->deal_damage( nullptr, foot_r_bp, damage_instance( DT_CUT, rng( 6, 10 ) ) );
...
bool trapfunc::caltrops( const tripoint &, Creature *c, item * )
{
    ...
    if( z != nullptr ) {
        ...
        c->deal_damage( nullptr, foot_l_bp, damage_instance( DT_CUT, rng( 9, 15 ) ) );
        c->deal_damage( nullptr, foot_r_bp, damage_instance( DT_CUT, rng( 9, 15 ) ) );
    } else {
        c->deal_damage( nullptr, foot_l_bp, damage_instance( DT_CUT, rng( 9, 30 ) ) );
        c->deal_damage( nullptr, foot_r_bp, damage_instance( DT_CUT, rng( 9, 30 ) ) );
    }
    c->check_dead_state();
    return true;
}
...
bool trapfunc::tripwire( const tripoint &p, Creature *c, item * )
{
    ...
    if( z != nullptr ) {
        ...
        if( rng( 0, 10 ) > z->get_dodge() ) {
            z->deal_damage( nullptr, torso_bp, damage_instance( DT_TRUE, rng( 1, 4 ) ) );
        }
    } else if( n != nullptr ) {
        std::vector<tripoint> valid;
        for( const tripoint &jk : g->m.points_in_radius( p, 1 ) ) {
    ...
    if( c != nullptr ) {
        ...
        if( n != nullptr ) {
            ...
            if( one_in( chance ) ) {
                ...
                z->deal_damage( nullptr, torso_bp, damage_instance( DT_CUT, rng( 20, 30 ) ) );
                add_bolt = !one_in( 10 );
            } else if( seen ) {
...
bool trapfunc::shotgun( const tripoint &p, Creature *c, item * )
{
    ...
    if( c != nullptr ) {
        ...
        if( n != nullptr ) {
            ...
            z->deal_damage( nullptr, torso_bp, damage_instance( DT_CUT, rng( 40 * shots, 60 * shots ) ) );
        }
        c->check_dead_state();
    }
...
bool trapfunc::blade( const tripoint &, Creature *c, item * )
{
    ...
    c->deal_damage( nullptr, torso_bp, d );
    c->check_dead_state();
    return true;
}
...
bool trapfunc::goo( const tripoint &p, Creature *c, item * )
{
    ...
    player *n = dynamic_cast<player *>( c );
    if( n != nullptr ) {
        n->add_env_effect( effect_slimed, foot_l_bp, 6, 2_minutes );
        n->add_env_effect( effect_slimed, foot_r_bp, 6, 2_minutes );
        if( one_in( 3 ) ) {
            n->add_msg_if_player( m_bad, _( "The acidic goo eats away at your feet." ) );
            n->deal_damage( nullptr, foot_l_bp, damage_instance( DT_CUT, 5 ) );
            n->deal_damage( nullptr, foot_r_bp, damage_instance( DT_CUT, 5 ) );
            n->check_dead_state();
        }
        return true;
    } else if( z != nullptr ) {
...
bool trapfunc::dissector( const tripoint &p, Creature *c, item * )
{
    ...
    if( z != nullptr ) {
        ...
        if( z->has_effect( effect_ridden ) && z->mounted_player ) {
            ...
            ch->deal_damage( nullptr, head_bp, damage_instance( DT_CUT, 15 ) );
            ch->deal_damage( nullptr, torso_bp, damage_instance( DT_CUT, 20 ) );
            ch->deal_damage( nullptr, arm_r_bp, damage_instance( DT_CUT, 12 ) );
            ch->deal_damage( nullptr, arm_l_bp, damage_instance( DT_CUT, 12 ) );
            ch->deal_damage( nullptr, hand_r_bp, damage_instance( DT_CUT, 10 ) );
            ch->deal_damage( nullptr, hand_l_bp, damage_instance( DT_CUT, 10 ) );
            ch->deal_damage( nullptr, leg_r_bp, damage_instance( DT_CUT, 12 ) );
            ch->deal_damage( nullptr, leg_r_bp, damage_instance( DT_CUT, 12 ) );
            ch->deal_damage( nullptr, foot_l_bp, damage_instance( DT_CUT, 10 ) );
            ch->deal_damage( nullptr, foot_r_bp, damage_instance( DT_CUT, 10 ) );
            if( g->u.sees( p ) ) {
    ...
    c->deal_damage( nullptr, head_bp, damage_instance( DT_CUT, 15 ) );
    c->deal_damage( nullptr, torso_bp, damage_instance( DT_CUT, 20 ) );
    c->deal_damage( nullptr, arm_r_bp, damage_instance( DT_CUT, 12 ) );
    c->deal_damage( nullptr, arm_l_bp, damage_instance( DT_CUT, 12 ) );
    c->deal_damage( nullptr, hand_r_bp, damage_instance( DT_CUT, 10 ) );
    c->deal_damage( nullptr, hand_l_bp, damage_instance( DT_CUT, 10 ) );
    c->deal_damage( nullptr, leg_r_bp, damage_instance( DT_CUT, 12 ) );
    c->deal_damage( nullptr, leg_r_bp, damage_instance( DT_CUT, 12 ) );
    c->deal_damage( nullptr, foot_l_bp, damage_instance( DT_CUT, 10 ) );
    c->deal_damage( nullptr, foot_r_bp, damage_instance( DT_CUT, 10 ) );
...
bool trapfunc::pit( const tripoint &p, Creature *c, item * )
{
    ...
    c->add_effect( effect_in_pit, 1_turns, default_bp, true );
    monster *z = dynamic_cast<monster *>( c );
    player *n = dynamic_cast<player *>( c );
    if( n != nullptr ) {
        if( ( n->has_trait( trait_WINGS_BIRD ) ) || ( ( one_in( 2 ) ) &&
            ...
            if( damage > 0 ) {
                ...
                n->deal_damage( nullptr, leg_l_bp, damage_instance( DT_BASH, damage ) );
                n->deal_damage( nullptr, leg_r_bp, damage_instance( DT_BASH, damage ) );
            } else {
        ...
        z->deal_damage( nullptr, leg_l_bp, damage_instance( DT_BASH, eff * rng( 10, 20 ) ) );
        z->deal_damage( nullptr, leg_r_bp, damage_instance( DT_BASH, eff * rng( 10, 20 ) ) );
    }
    c->check_dead_state();
    return true;
}
...
bool trapfunc::pit_spikes( const tripoint &p, Creature *c, item * )
{
    ...
    if( n != nullptr ) {
        ...
        z->deal_damage( nullptr, torso_bp, damage_instance( DT_CUT, rng( 20, 50 ) ) );
    }
    c->check_dead_state();
    if( one_in( 4 ) ) {
        if( g->u.sees( p ) ) {
...
bool trapfunc::lava( const tripoint &p, Creature *c, item * )
{
    ...
    player *n = dynamic_cast<player *>( c );
    if( n != nullptr ) {
        n->deal_damage( nullptr, foot_l_bp, damage_instance( DT_HEAT, 20 ) );
        n->deal_damage( nullptr, foot_r_bp, damage_instance( DT_HEAT, 20 ) );
        n->deal_damage( nullptr, leg_l_bp, damage_instance( DT_HEAT, 20 ) );
        n->deal_damage( nullptr, leg_r_bp, damage_instance( DT_HEAT, 20 ) );
    } else if( z != nullptr ) {
        if( z->has_effect( effect_ridden ) ) {
        ...
        z->deal_damage( nullptr, torso_bp, damage_instance( DT_HEAT, dam ) );
    }
    c->check_dead_state();
    return true;
}
...
bool trapfunc::glow( const tripoint &p, Creature *c, item * )
{
    ...
    if( z != nullptr ) {
        if( one_in( 3 ) ) {
            z->deal_damage( nullptr, torso_bp, damage_instance( DT_ACID, rng( 5, 10 ) ) );
...
bool trapfunc::drain( const tripoint &, Creature *c, item * )
{
    if( c != nullptr ) {
        ...
        if( n != nullptr ) {
            ...
            z->deal_damage( nullptr, torso_bp, damage_instance( DT_TRUE, 1 ) );
        }
        c->check_dead_state();
        return true;
    }
