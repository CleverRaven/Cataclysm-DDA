...
static item *set_item_inventory( player &p, item &newit )
{
    ...
    if( newit.made_of( LIQUID ) ) {
        g->liquid_handling.handle_all_liquid( newit, PICKUP_RANGE );
    } else {
    ...
    if( making.has_byproducts() ) {
        ...
        for( auto &bp : bps ) {
            ...
            if( bp.made_of( LIQUID ) ) {
                g->liquid_handling.handle_all_liquid( bp, PICKUP_RANGE );
            } else if( loc == tripoint_zero ) {
                                   ...
                                   bool from_ground, const recipe &dis )
{
    ...
    for( const item &newit : components ) {
        ...
        if( act_item.made_of( LIQUID ) ) {
            g->liquid_handling.handle_all_liquid( act_item, PICKUP_RANGE );
        } else {
