...
void player::armor_absorb(damage_unit& du, item& armor) {
    if (rng(0,100) <= armor.get_coverage()) {
        ...
        const auto armor_res = resistances( armor );
        const float effective_resist = armor_res.get_effective_resist(du);
        ...
        // Armor piercing is more like "armor bypass" than "armor breaking"
        const int armor_dmg_resist = armor_res.type_resist( du.type );
        // if the post-mitigation amount is greater than the amount
        if( (du.amount > armor_dmg_resist && !one_in(du.amount) && one_in(2)) ||
            // or if it isn't, but 1/50 chance
            (du.amount <= armor_dmg_resist && !armor.has_flag("STURDY") &&
