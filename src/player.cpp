...
bool player::eat( item &food )
{
    int to_eat = 1;
    const auto comest = dynamic_cast<const it_comest *>( eaten.type );
    if (comest == NULL) {
        debugmsg("player::eat(%s); comest is NULL!", eaten->tname().c_str());
        return false;
    }

    const edible_rating edible = can_eat( food );
    if (comest->tool != "null") {
        bool has = has_amount(comest->tool, 1);
        if( item::count_by_charges( comest->tool ) ) {
            has = has_charges(comest->tool, 1);
...
morale_type player::allergy_type( const item &food ) const
{
    ...
    static const std::array<allergy_tuple, 8> allergy_tuples = {{
        {"VEGETARIAN", "flesh", MORALE_VEGETARIAN},
        {"VEGETARIAN", "hflesh", MORALE_VEGETARIAN},
        {"VEGETARIAN", "iflesh", MORALE_VEGETARIAN},
        {"MEATARIAN", "veggy", MORALE_MEATARIAN},
        {"LACTOSE", "milk", MORALE_LACTOSE},
        {"ANTIFRUIT", "fruit", MORALE_ANTIFRUIT},
        {"ANTIJUNK", "junk", MORALE_ANTIJUNK},
        {"ANTIWHEAT", "wheat", MORALE_ANTIWHEAT}
    }};
...
edible_rating player::can_eat( const item &food, bool interactive, bool force ) const
{
    ...
    const auto maybe_print = interactive ? [itname]( game_message_type type,
        const char *str ) {
        add_msg( type, str, itname );
    } : [itname]( game_message_type, const char * ) { };
    // As above, but for queries
    // Asks if interactive and not force
    // Always true if force
    // Never true otherwise
    const auto maybe_query = (!force && interactive) ?
        [itname]( const char *str ) {
            return query_yn( type, str, itname );
    } : force ? [itname]( const char * ) { return true; }
    : [itname]( const char * ) { return false; };
    ...
    if( is_underwater() ) {
        maybe_print( interactive, m_info, _("You can't do that while underwater.") );
        return INEDIBLE;
    }
    // For all those folks who loved eating marloss berries.  D:< mwuhahaha
    ...
    bool overeating = (!gourmand && get_hunger() < 0 &&
                       nutr >= 5);
    // Hibernate on, but we aren't full enough
    bool hiberfood = (hibernate && (get_hunger() > -60 && thirst > -60 ));
    bool spoiled = food.rotten();

    if( overeating && !hibernate && !eathealth && !slimespawner &&
        !maybe_query(_("You're full.  Force yourself to eat?") ) {
        return TOO_FULL;
    } else if( interactive && gourmand &&
        get_hunger() < 0 && nutrition_for(comest) >= 5 &&
        !maybe_query(_("You're fed.  Try to pack more in anyway?") ) {
        return TOO_FULL;
    }

    int temp_hunger = get_hunger() - nutr;
    int temp_thirst = thirst - quench;
    int capacity = gourmand ? -60 : -20;
    ...
    if( carnivore && food.made_of_any( carnivore_blacklist ) && nutrition_for(comest) > 0 ) {
    ...
    if( is_allergic( food ) && !query_yn(_("Really eat that %s?  Your stomach won't be happy.") ) {
        return ALLERGY;
    }
    ...
    const bool digestion = has_bionic( "bio_digestion" );
    if( spoiled ) {
        if( !saprophage && !has_trait("SAPROVORE") &&
            !maybe_query( _("This %s smells awful!  Eat it?") ) ) {
            return ROTTEN;
        }
    } else if ( saprophage && !spoiled && !drinkable && !digestion ) {
        return ROTTEN;
    }
    ...
    if( ( nutr > 0 && temp_hunger < capacity ) ||
        ( comest->quench > 0 && temp_thirst < capacity ) ) {
        if( spoiled && !saprophage &&
            !maybe_query( _("You can hardly finish it all.  Consume it?") ) ) {
            return TOO_FULL;
...
bool player::eat( const item &food, bool force )
{
    ...
    if( hiberfood && (get_hunger() - nutr < -60 || thirst - quench < -60) ) {
        add_memorial_log( pgettext("memorial_male", "Began preparing for hibernation."),
                          pgettext("memorial_female", "Began preparing for hibernation.") );
        add_msg_if_player(_("You've begun stockpiling calories and liquid for hibernation.  You get the feeling that you should prepare for bed, just in case, but...you're hungry again, and you could eat a whole week's worth of food RIGHT NOW."));
    }
    ...
    const bool saprophage = has_trait( "SAPROPHAGE" );
    if( spoiled && !saprophage ) {
        add_msg_if_player(m_bad, _("Ick, this %s doesn't taste so good..."), eaten->tname().c_str());
        if( !has_trait("SAPROVORE") && !has_trait("EATDEAD") &&
            (!has_bionic("bio_digestion") || one_in(3))) {
            add_effect( effect_foodpoison, rng(60, (nutr + 1) * 60) );
        }
        consume_effects( eaten, spoiled );
    } else if( spoiled && saprophage ) {
        add_msg_if_player(m_good, _("Mmm, this %s tastes delicious..."), eaten->tname().c_str());
        consume_effects( eaten, spoiled );
    } else {
        consume_effects( eaten, spoiled );
    }

    if( !gourmand && !hibernate && !eathealth && !slimespawner &&
    ...
    if( !is_npc() && slimespawner && get_hunger() < capacity + 40 && thirst < capacity + 40 ) {
        add_msg_if_player(m_mixed, _("You feel as though you're going to split open!  In a good way??"));
        mod_pain(5);
        std::vector<tripoint> valid;
    ...
    if( eaten->poison > 0 && !has_trait("EATPOISON") && !has_trait("EATDEAD") ) {
        if( eaten->poison >= rng(2, 4) ) {
            add_effect( effect_poison, eaten->poison * 100 );
        }
...
void player::consume_effects( item &food, bool rotten )
{
    ...
    if( eaten->made_of_any( herbivore_blacklist ) &&
    ...
    if( has_trait("CARNIVORE") && eaten->made_of_any( carnivore_whitelist ) ) {
        // At least partially edible
        if( eaten->made_of_any( carnivore_blacklist ) ) {
            // Other things are in it, we only get partial benefits
            factor *= .5;
        } else {
            // Carnivores don't get unhealthy off pure meat diets
    ...
    const bool gourmand = has_trait("GOURMAND");
    const bool hibernate = has_active_mutation("HIBERNATE");
    int min_hunger = gourmand ? -60 : -20;
    int min_thirst = gourmand ? -60 : -20;
    if( gourmand ) {
        if (fun < -2) {
            add_morale(MORALE_FOOD_BAD, fun * 0.5, fun, 60, 30, false, comest);
        } else if (fun > 0) {
    ...
    if( ( nutr > 0 && get_hunger() < min_hunger ) ||
        ( comest->quench > 0 && thirst < min_thirst ) ) {
        add_msg_if_player(_("You can't finish it all!"));
    }

    if( get_hunger() < min_hunger ) {
        set_hunger( min_hunger );
    }
    if( get_thirst() < min_thirst ) {
        thirst = min_thirst;
    }
}
