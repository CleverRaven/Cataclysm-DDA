...
#include "mapdata.h"
#include "martialarts.h"#include "player.h"
...
int player::run_cost( int base_cost, bool diag ) const
{
    ...
    if( move_mode == "sprint" && stamina > 0 ) {
        stamina_modifier *= 8.0;
    }
...
void player::set_movement_mode( const std::string &new_mode )
{
    if( new_mode == "sprint" ) {
        if( stamina > 0 && !has_effect( effect_winded ) ) {
            ...
            if( has_effect( effect_riding ) ) {
                add_msg( _( "You push your steed to its limit." ) );
            } else {
                add_msg( _( "You start sprinting." ) );
            }
        } else {
            if( has_effect( effect_riding ) ) {
...
void player::toggle_sprint_mode()
{
    if( move_mode == "sprint" ) {
...
void player::burn_move_stamina( int moves )
{
    ...
    if( current_weight > max_weight ) {
        overburden_percentage = ( current_weight - max_weight ) * 50 / max_weight;
    }
