...
bool player::read( int inventory_position, const bool continuous )
{
    ...
    for( player *elem : apply_morale ) {
        ...
        elem->add_morale( MORALE_BOOK, book_fun_for( it, *elem ), book_fun_for( it, *elem ) * 15, decay_start + 3_minutes,
...
void player::do_read( item &book )
{
    ...
    for( auto &elem : learners ) {
        ...
        if( book_fun_for( book, *learner ) != 0 ) {
            ...
            learner->add_morale( MORALE_BOOK, book_fun_for( book, *learner ), book_fun_for( book, *learner ) * 5, 6_minutes, 3_minutes, true,
                book.type );
        }
