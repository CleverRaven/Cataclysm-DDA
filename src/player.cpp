...
void player::suffer()
{
    ...
    if( has_trait( trait_ASTHMA ) &&
        ...
        bool auto_use = has_charges( "inhaler", 1 ) || has_charges( "oxygen_tank", 1 ) || has_charges( "smoxygen_tank", 1 );
        ...
        if( in_sleep_state() && !has_effect( effect_narcosis ) ) {
            ...
            if( oxygenator ) {
                ...
                } else if( use_charges_if_avail( "oxygen_tank", 1 ) || use_charges_if_avail( "smoxygen_tank", 1 ) ) {
                    add_msg_if_player( m_info, _( "You take a deep breath from your oxygen tank and go back to sleep." ) );
                }
                // check if an inhaler is somewhere near
            } else if( map_inv.has_charges( "inhaler", 1 ) || map_inv.has_charges( "oxygen_tank", 1 ) || map_inv.has_charges( "smoxygen_tank", 1 )  ) {
                add_msg_if_player( m_bad, _( "You have an asthma attack!" ) );
                // create new variable to resolve a reference issue
                long amount = 1;
                if ( !g->m.use_charges( g->u.pos(), 2, "inhaler", amount ).empty() ) {
                    add_msg_if_player( m_info, _( "You use your inhaler and go back to sleep." ) );
                } else if ( !g->m.use_charges( g->u.pos(), 2, "oxygen_tank", amount ).empty() || !g->m.use_charges( g->u.pos(), 2, "smoxygen_tank", amount ).empty() ) {
                    add_msg_if_player( m_info, _( "You take a deep breath from your oxygen tank and go back to sleep." ) );
                }
            } else {
            ...
            if ( use_charges_if_avail( "inhaler", 1 ) ) {
                ...
                if( charges == 0 ) {
                    ...
                    add_msg_if_player( m_info, ngettext( "You take a deep breath from your oxygen tank, only %d charge left.",
