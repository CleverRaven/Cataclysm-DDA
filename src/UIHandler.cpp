...
int UIWindow::UpdateWindow()
{
    ...
    if (m_thisSize.x < m_minSize.x) m_thisSize.x = m_minSize.x;
    if (m_thisSize.y < m_minSize.y) m_thisSize.y = m_minSize.y;

    if (m_thisSize.x > TERMX) m_thisSize.x = TERMX;
    if (m_thisSize.y > TERMY) m_thisSize.y = TERMY;

    if (m_thisSize.x > TERMX) DebugLog(D_ERROR, DC_ALL) << "Not enought screen space...";
    if (m_thisSize.y > TERMY) DebugLog(D_ERROR, DC_ALL) << "Not enought screen space...";
