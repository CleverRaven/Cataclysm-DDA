...
bool main_menu::new_character_tab()
{
    ...
    hints.push_back( "Select from one of previously created character templates." );
    hints.push_back( "Creates random character, but lets you preview the generated character and the scenario and change character and/or scenario if needed." );
    hints.push_back( "Puts you right in the game, randomly choosing character's traits, profession, skills and other parameters.  Scenario is fixed to Evacuee." );
    hints.push_back( "Puts you right in the game, randomly choosing scenario and character's traits, profession, skills and other parameters." );
    ...
    while( !start && sel1 == 1 && ( layer == 2 || layer == 3 ) ) {
        ...
        if( layer == 2 && sel1 == 1 ) {
            center_print( w_open, getmaxy( w_open ) - 7, c_yellow, _( hints[sel2] ) );
