...
void vehicle::smart_controller_handle_turn( const std::optional<float> &k_traction_cache )
{
    ...
    bool aircraft = is_flying && is_aircraft();

    // bail and shut down
    if( rotorcraft || aircraft || c_engines.empty() || ( has_electric_engine && c_engines.size() == 1 ) ||
        c_engines.size() > 5 ) {
...
void vehicle::thrust( int thd, int z )
{
    ...
    if( ( is_watercraft() && can_float() ) || ( is_rotorcraft() && ( z != 0 || is_flying ) ) || (is_aircraft() && (z != 0 || is_flying))) {
        // we're good
    ...
    if( (is_rotorcraft() || is_aircraft()) && ( z > 0 || is_flying_in_air() ) ) {
    ...
    if( load >= 1 && thrusting ) {
        ...
        if( ( is_rotorcraft() || is_aircraft()) && is_flying_in_air() ) {
        ...
        if( z != 0 && ( is_rotorcraft() || is_aircraft()) ) {
                         ...
                         bool just_detect, bool bash_floor )
{
    ...
    // Skip collisions when there is no apparent movement, except verticially moving flying vehicles.
    if( coll_velocity == 0 && !is_rotorcraft() && !is_aircraft() ) {
        just_detect = true;
    }
...
vehicle *vehicle::act_on_map()
{
    ...
    if( turn_cost >= of_turn && ( ( !is_flying && requested_z_change == 0 ) || ( !is_rotorcraft() && !is_aircraft() ) ) ) {
        if( !should_fall ) {
    ...
    if( should_fall ) {
        ...
        if( dp.z() > 0 && (is_rotorcraft() || is_aircraft())) {
            is_flying = true;
        }
    }
