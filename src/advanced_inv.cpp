...
int getsquare( const std::string action, int &off_x, int &off_y, std::string &areastring,
               advanced_inv_area *squares )
{
    int ret = - 1;
        ...
        case SORTBY_NONE:
            if( i != isinventory ) {
                std::stable_sort( panes[i].items.begin(), panes[i].items.end(),
                                  advanced_inv_sorter( SORTBY_NONE ) );
            }
            break;
        default:
            std::stable_sort( panes[i].items.begin(), panes[i].items.end(),
...
void advanced_inventory::display()
{
    ...
    while( !exit ) {
        ...
        if( action == "CATEGORY_SELECTION" ) {
                ...
                if( stack.size() > 1 ) { // if the item is stacked
                    ...
                    if( amount != 0 && amount <= it->charges ) {
                        ...
                        if( squares[destarea].vstor >= 0 ) {
                            ...
                            if( g->m.add_item_or_charges( squares[destarea].x, squares[destarea].y, moving_item,
                                                          0 ) == false ) {
                                // fixme add item back
                                g->u.i_add( moving_item );
                                popup( _( "Destination full.  Please report a bug if items have vanished." ) );
            ...
            sm.text = _( "Sort by... " );
            sm.entries.push_back( uimenu_entry( SORTBY_NONE, true, 'u',
                                                _( "Unsorted (recently added first)" ) ) );
