...
bool advanced_inventory::move_all_items()
{
        ...
        if(query_yn(_("Really move everything from your inventory?"))) {
            ...
            for (int ip = u.inv.size() - 1; ip >= 0; /* noop */ ) {
                ...
                if (volume > 0 && volume * amount > free_volume) {
                    ...
                    if (volmax == 0) {
                        Messages::add_msg(_("Unable to move item, the destination is too full."));
                        --ip;
                        continue;
                    }
                            ...
                            if (chargeback == true) {
                                ...
                                if (panes[dest].vstor >= 0) {
                                    if (veh->add_item(part, *iter) == false) {
                                        u.i_add(*iter);
                                        Messages::add_msg(_("Destination full. %d / %d moved. Please report a bug if items have vanished."), moved, amount);
                                        chargeback = true;
                                    }
                                } else {
                    ...
                    if (amount != 0 && amount <= it->charges) {
                        ...
                        if (panes[dest].vstor >= 0) {
                            if (veh->add_item(part, moving_item) == false) {
                                u.i_add(moving_item);
                                Messages::add_msg(_("Destination full. Please report a bug if items have vanished."));
                            }
                        } else {
            ...
            if (it->made_of(LIQUID)) {
                ...
                if (destarea == isinventory) {
                    ...
                    if(squares[destarea].size >= MAX_ITEM_IN_SQUARE) {
                        Messages::add_msg(_("You are carrying too many items."));
                        return true;
                    }
                    ...
                    if (it->count_by_charges() && it->charges > 1) {
                        ...
                        if (max != 0) {
                            ...
                            if ( trycharges == 0 ) {
                                Messages::add_msg(_("Unable to pick up %s."), it->name.c_str());
                                ++it;
                                continue;
                            }
                        } else {
                    ...
                    if(!u.can_pickVolume(tryvolume)) {
                        Messages::add_msg(_("There's no room in your inventory for %s."), it->name.c_str());
                        ++it;
                        continue;
                    } else if (!u.can_pickWeight(tryweight, false)) {
                        Messages::add_msg(_("%s is too heavy."), it->name.c_str());
                        ++it;
                        continue;
                    }
                }
...

    {
        ...
        Messages::add_msg("Item %s", ait->it->name.c_str());
