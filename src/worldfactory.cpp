...
WORLDPTR worldfactory::make_new_world(bool make_default)
{
    // Window variables
    ...
    tab_strings.push_back(_("Mod Selection"));
    ...
    if (!make_default){
        ...
        // no mods, will use the mainline core files
        if (retworld->world_options.size() == 0) {
            for (std::map<std::string, cOpt>::iterator it = OPTIONS.begin(); it != OPTIONS.end(); ++it) {
                if (it->second.getPage() == "world_default") {
                    retworld->world_options[it->first] = it->second;
                }
            }
        }
        retworld->world_name = get_next_valid_worldname("World", this);
    }
