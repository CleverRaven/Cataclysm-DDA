...
int worldfactory::show_worldgen_tab_options( WINDOW *win, WORLDPTR world )
{
    ...
    do {
        ...
        int iBlankOffset = 0;
        for( int i = iStartPos;
             i < iStartPos + ( ( iContentHeight > ( int )mPageItems[iWorldOptPage].size() ) ?
...
void worldfactory::draw_mod_list( WINDOW *w, int &start, int &cursor,
    ...
    while( tab_output == 0 ) {
        ...
        if( recalc_tabs ) {
            ...
            for( const auto &item : mman_ui->usable_mods ) {
                const auto &iter = get_mod_list_cat_tab().find(
                                       get_mod_list_categories()[mman->mod_map[item]->category.first].first );
        ...
        if( redraw_description ) {
            ...
            for( size_t i = 0; i < get_mod_list_tabs().size(); i++ ) {
                wprintz( win, c_white, "[" );
                wprintz( win, ( iCurrentTab == ( int )i ) ? hilite( c_ltgreen ) : c_ltgreen,
                         ( get_mod_list_tabs()[i].second ).c_str() );
                wprintz( win, c_white, "]" );
...
#endif
            if( worldname.empty() ) {
                mvwprintz( w_confirmation, namebar_y, namebar_x, h_ltgray,
                           _( "_______NO NAME ENTERED!!!!______" ) );
                noname = true;
                wrefresh( w_confirmation );
...
bool worldfactory::valid_worldname( std::string name, bool automated )
{
    ...
    } else if( std::find( all_worldnames.begin(), all_worldnames.end(),
                          name ) == all_worldnames.end() ) {
        return true;
    } else {
...
bool worldfactory::load_world_options( WORLDPTR &world )
{
    ...
    if( read_from_file_optional( path, [world]( JsonIn & js ) {
    ...
    if( read_from_file_optional( legacy_path, [world]( std::istream & is ) {
