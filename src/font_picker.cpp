...
static bool ShowFontFileOpenDialog()
{
    bool ret = false;
    if( ImGuiFileDialog::Instance()->Display( "ChooseFont", ImGuiWindowFlags_None, { 960.0f, 540.0f } ) ) {
        if( ImGuiFileDialog::Instance()->IsOk() ) {
            std::string filePathName = ImGuiFileDialog::Instance()->GetFilePathName();
            font_config *face = static_cast<font_config *>( ImGuiFileDialog::Instance()->GetUserDatas() );
            if( face ) {
                face->path = filePathName;
                ret = true;
            }
        }
...
static void InitFontFileOpenDialog( font_config &face )
{
    IGFD::FileDialogConfig config;
    config.countSelectionMax = 1;
    config.flags = ImGuiFileDialogFlags_Modal;
    config.userDatas = static_cast<void *>( &face );
    ImGuiFileDialog::Instance()->AddPlacesGroup( _( "Fonts" ), 0, false, true );
    auto *places_ptr = ImGuiFileDialog::Instance()->GetPlacesGroupPtr( _( "Fonts" ) );
    if( places_ptr != nullptr ) {
        // NOLINTBEGIN(cata-u8-path)
#if defined(MACOSX)
        places_ptr->AddPlace( "System Fonts", fs::u8path( "/" ) / "Library" / "Fonts", false );
        char *user_dir;
        if( ( user_dir = getenv( "HOME" ) ) ) {
            places_ptr->AddPlace( "User Fonts", fs::u8path( user_dir ) / "Library" / "Fonts", false );
        }
#elif defined(_WIN32)
        char *user_dir;
        if( ( user_dir = getenv( "windir" ) ) ) {
            places_ptr->AddPlace( "System Fonts", ( fs::u8path( user_dir ) / "fonts" ).generic_string(),
                                  false );
        }
        if( ( user_dir = getenv( "LOCALAPPDATA" ) ) ) {
            places_ptr->AddPlace( "User Fonts",
                                  ( fs::u8path( user_dir ) / "Microsoft" / "Windows" / "Fonts" ).generic_string(),
                                  false );
        }
#elif defined(__ANDROID__)
        places_ptr->AddPlace( "System Fonts", fs::u8path( "/" ) / "system" / "fonts", false );
#else
        places_ptr->AddPlace( "System Fonts", fs::u8path( "/" ) / "usr" / "share" / "fonts",
                              false );
        char *user_dir;
        fs::path xdg_path;
        if( ( user_dir = getenv( "XDG_DATA_HOME" ) ) ) {
            xdg_path = fs::u8path( user_dir ) / "fonts";
        } else if( ( user_dir = getenv( "HOME" ) ) ) {
            xdg_path = fs::u8path( user_dir ) / ".local" / "share" / "fonts";
        }
        places_ptr->AddPlace( "User XDG Fonts", xdg_path, false );
        if( ( user_dir = getenv( "HOME" ) ) ) {
            places_ptr->AddPlace( "User Fonts", fs::u8path( user_dir ) / ".fonts", false );
        }
#endif
        places_ptr->AddPlace( "Game Fonts", PATH_INFO::fontdir(), false );
        // NOLINTEND(cata-u8-path)
    }
    ImGuiFileDialog::Instance()->OpenDialog(
        "ChooseFont", "Choose Font",
        "Fonts (*.ttf, *.otf){.ttf,.otf}All Files{}", config );
}
...
void FontPickerWindow::draw_controls()
{
    ...
    picker->WantRebuild |= ShowFontFileOpenDialog();
}
    ...
    if( ImGui::Begin( window_name, p_open ) ) {
        ...
        if( cataimgui::BeginRightAlign( "+" ) ) {
            if( ImGui::Button( ICON_IGFD_ADD ) ) {
                typefaces.insert( typefaces.begin(), font_config() );
                InitFontFileOpenDialog( typefaces[0] );
