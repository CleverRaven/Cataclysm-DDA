...
ret_val<void> Character::can_wear( const item &it, bool with_equip_change ) const
{
    ...
    if( !it.has_flag( flag_OVERSIZE ) && !it.has_flag( flag_SEMITANGIBLE ) && !it.has_flag( flag_UNRESTRICTED ) ) {
...
{
    for( const item &i : worn ) {
        if( i.covers( bp ) && !i.has_flag( flag_INTEGRATED ) && !i.has_flag( flag_OVERSIZE ) && !i.has_flag( flag_UNRESTRICTED ) ) {
            return true;
        }
    }
    return false;
