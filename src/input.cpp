...
void input_manager::init()
{
    ...
    try {
        load( path.get_path_for_value_key( "KEYBINDINGS" ), false );
    } catch( const JsonError &err ) {
        throw std::runtime_error( path.get_path_for_value_key( "KEYBINDINGS" ) + ": " + err.what() );
    }
    try {
        load( path.get_path_for_value_key( "KEYBIND_VEHIC" ), false );
    } catch( const JsonError &err ) {
        throw std::runtime_error( path.get_path_for_value_key( "KEYBIND_VEHIC" ) + ": " + err.what() );
    }
    try {
        load( path.get_path_for_value_key( "KEYBIND_USER" ), true );
    } catch( const JsonError &err ) {
        throw std::runtime_error( path.get_path_for_value_key( "KEYBIND_USER" ) + ": " + err.what() );
    }
...
void input_manager::save()
{
    ...
    write_to_file( path.get_path_for_value_key( "KEYBIND_USER" ), [&]( std::ostream & data_file ) {
