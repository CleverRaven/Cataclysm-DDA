...
void GeometryRenderer::horizontal_line( const SDL_Renderer_Ptr &renderer, const point &pos, int x2, int thickness, const SDL_Color &color ) 
{
    SDL_Rect rect { pos.x, pos.y, x2 - pos.x, thickness };
    this->rect( renderer, rect, color );
}

void GeometryRenderer::vertical_line( const SDL_Renderer_Ptr &renderer, const point &pos, int y2, int thickness, const SDL_Color &color )
{
    SDL_Rect rect { pos.x, pos.y, thickness, y2 - pos.y };
    this->rect( renderer, rect, color );
}

void GeometryRenderer::rect( const SDL_Renderer_Ptr &renderer, const point &pos, int width, int height, const SDL_Color &color )
{
...
void DefaultGeometryRenderer::rect( const SDL_Renderer_Ptr &renderer, const SDL_Rect &rect, const SDL_Color &color ) 
{
    ...
    : DefaultGeometryRenderer()
{
    ...
    if( alt_surf ) {
        ...
        DebugLog( D_INFO, DC_ALL ) << "ColorModulatedGeometryRenderer constructor() = " << ( tex_enable ? "FAIL" : "SUCCESS" ) << ". tex_enable = " << tex_enable;
    } else {
        DebugLog( D_ERROR, DC_ALL ) << "CreateRGBSurface failed: " << SDL_GetError();
    }
}

void ColorModulatedGeometryRenderer::rect( const SDL_Renderer_Ptr &renderer, const SDL_Rect &rect, const SDL_Color &color )
{
    if ( tex ) {
