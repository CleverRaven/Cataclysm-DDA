...
int item_contents::ammo_consume( int qty, const tripoint &pos, float fuel_efficiency )
{
    ...
    for( item_pocket &pocket : contents ) {
        if( pocket.is_type( item_pocket::pocket_type::MAGAZINE_WELL ) ) {
            ...
            if( pocket.front().is_fuel() && fuel_efficiency >= 0 ) {
                ...
                int charges_used = roll_remainder( static_cast<float>( units::from_kilojoule( qty ).value() ) / (
                                                       static_cast<float>( pocket.front().fuel_energy().value() ) * fuel_efficiency ) );
                const int res = pocket.ammo_consume( charges_used );
                if( res == charges_used ) {
                    // avoid rounding weirdness, if we used all the charges we wanted to, count that as good enough
                    consumed += qty;
                    qty = 0;
                } else {
                    // otherwise calculate the ammount of energy generated
