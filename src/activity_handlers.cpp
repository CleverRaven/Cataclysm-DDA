    ...
    if( ( corpse->has_flag( MF_FUR ) || corpse->has_flag( MF_LEATHER ) ||
        ...
        while( skins > 0 ) {
            ...
            if( corpse->has_flag( MF_LEATHER ) ) {
                if( corpse->has_flag( MF_POISON ) ) {
                    tainted_leather = rng_int( 0, skins );
                    skins -= tainted_leather;
                } else if( corpse->has_flag( MF_HUMAN ) ) {
                    human_leather = rng_int( 0, skins );
                    skins -= human_leather;
                } else {
                    leather = rng_int( 0, skins );
                    skins -= leather;
                }
                skins = std::max( skins, 0 );
            }
...
void activity_handlers::fish_finish( player_activity *act, player *p )
{
    ...
    p->practice( skill_survival, rng_int(5, 15) );
    act->set_to_null();
}

void activity_handlers::forage_finish( player_activity *act, player *p )
{
    int veggy_chance = rng_int(1, 100);
    ...
    p->practice( skill_survival, rng_int(1, max_exp), max_forage_skill );
...
void activity_handlers::hotwire_finish( player_activity *act, player *pl )
{
    ...
    if( veh ) {
        ...
        if( mech_skill > rng_int(1, 6) ) {
            //success
            veh->is_locked = false;
            add_msg(_("This wire will start the engine."));
        } else if( mech_skill > rng_int(0, 4) ) {
            //soft fail
            veh->is_locked = false;
...
void activity_handlers::make_zlave_finish( player_activity *act, player *p )
{
    ...
    if( success > 0 ) {
        ...
        p->practice( skill_firstaid, rng_int(2, 5) );
        p->practice( skill_survival, rng_int(2, 5) );
        ...
        if( success > -20 ) {
            ...
            p->practice( skill_firstaid, rng_int(3, 6) );
            p->practice( skill_survival, rng_int(3, 6) );
            ...
            p->practice( skill_firstaid, rng_int(1, 8) );
            p->practice( skill_survival, rng_int(1, 8) );
    ...
    for( auto &corpse : corpse_pile ) {
        ...
        while( corpse.damage() < corpse.max_damage() ) {
            ...
            if( x_in_y( pulp_power, corpse.volume() / units::legacy_volume_factor ) ) { // Splatter some blood around
                ...
                const tripoint dest( pos.x + rng_int( -radius, radius ), pos.y + rng_int( -radius, radius ), pos.z );
...
void activity_handlers::chop_tree_finish( player_activity *act, player *p ) {
    ...
    tripoint to = pos + point( 3 * direction.x + rng_int( -1, 1 ), 3 * direction.y + rng_int( -1, 1 ) );
    std::vector<tripoint> tree = line_to( pos, to, rng_int( 1, 8 ) );
    for( auto &elem : tree ) {
        g->m.destroy( elem );
