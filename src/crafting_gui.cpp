...
const recipe *select_crafting_recipe( int &batch_size )
{
    ...
    do {
        ...
        if( !current.empty() ) {
            ...
            component_print_buffer = current[line]->requirements.obj().get_folded_components_list(
            ...
            if( display_mode == 0 || display_mode == 1 ) {
                ypos += current[line]->requirements.obj().print_tools(
        ...
        if( subtab == "CSC_ALL" || rec->subcat == subtab ||
            ...
            if( filter != "" ) {
                if( ( search_name && !lcmatch( item::nname( rec->result ), filter ) )
                    || ( search_tool && !lcmatch_any( rec->requirements.obj().get_tools(), filter ) )
                    || ( search_component && !lcmatch_any( rec->requirements.obj().get_components(), filter ) ) ) {
                    continue;
                }
                ...
                if( search_qualities ) {
                    for( auto quality_reqs : rec->requirements.obj().get_qualities() ) {
