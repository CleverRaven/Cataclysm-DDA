...
static bool are_requirements_nearby( const std::vector<tripoint> &loot_spots,
                                     ...
                                     const activity_id &act_id, bool in_loot_zones )
{
    ...
    const bool check_weight = act_id == activity_id( "ACT_MULTIPLE_FARM" );
        ...
        const player_activity &act )
{
    ...
    zone_manager &mgr = zone_manager::get_manager();
    const bool pickup_task = act.id() == activity_id( "ACT_MULTIPLE_FARM" );
    ...
    for( const tripoint &elem : g->m.points_in_radius( g->m.getlocal( act.placement ),
            PICKUP_RANGE - 1 ) ) {
...
static bool tidy_activity( player &p, const tripoint src_loc, const player_activity &act )
{
...
static void fetch_activity( player &p, const tripoint src_loc, const player_activity &act )
{
    if( act.coords.empty() ) {
        return;
    }
    const tripoint &coord = *act.coords.begin();
    if( !g->m.can_put_items_ter_furn( g->m.getlocal( coord ) ) ) {
        return;
    }
    const std::vector<std::tuple<tripoint, itype_id, int>> mental_map_2 = requirements_map( p, act );
    int pickup_count = 1;
    auto items_there = g->m.i_at( src_loc );
        ...
        for( auto &veh_elem : src_veh->get_items( src_part ) ) {
            for( auto elem : mental_map_2 ) {
                if( std::get<0>( elem ) == src_loc && veh_elem.typeId() == std::get<1>( elem ) ) {
                    if( act.id() == activity_id( "ACT_MULTIPLE_CONSTRUCTION" ) ) {
                        move_item( p, veh_elem, veh_elem.count_by_charges() ? std::get<2>( elem ) : 1, src_loc,
                                   g->m.getlocal( coord ), src_veh, src_part );
                        return;
                    }
                }
            }
    ...
    for( auto it = items_there.begin(); it != items_there.end(); it++ ) {
        for( auto elem : mental_map_2 ) {
            if( std::get<0>( elem ) == src_loc && it->typeId() == std::get<1>( elem ) ) {
                ...
                } else if( act.id() == activity_id( "ACT_MULTIPLE_FARM" ) ) {
                                ...
                                player_activity &act )
{
    ...
    int &num_processed = act.values[ NUM_PROCESSED ];
...
static void generic_multi_activity_finish( player &p )
{
    ...
    if( p.backlog.empty() || p.activity.coord_set.empty() ) {
        if( p.is_npc() ) {
...
void generic_multi_activity_handler( player_activity &act, player &p )
{
    ...
    if( stage == INIT ) {
        ...
        if( act.id() == activity_id( "ACT_MOVE_LOOT" ) ) {
            dark_capable = true;
            ...
            if( act.values.size() < 1 ) {
                act.values.resize( 1 );
                act.values[NUM_PROCESSED] = 0;
            }
        }
        if( act.id() == activity_id( "ACT_TIDY_UP" ) ) {
            dark_capable = true;
        ...
        if( act.id() == activity_id( "ACT_FETCH_REQUIRED" ) ) {
            ...
            std::vector<std::tuple<tripoint, itype_id, int>> mental_map = requirements_map( p, act );
    ...
    if( stage == THINK ) {
        ...
        for( const tripoint &src : coords ) {
            ...
            if( !g->m.inbounds( src_loc ) ) {
                ...
                stage = THINK;
                //mark all coords before this one as "already processed" to avoid looping between two distant locations
                for( const tripoint &src_to_remove : coords ) {
                    act.coord_set.erase( src_to_remove );
                    if( src == src_to_remove ) {
                        break;
                    }
                }
                p.set_moves( 0 );
    ...
    if( stage == DO ) {
        ...
        if( reason == NEEDS_HARVESTING && g->m.has_flag_furn( "GROWTH_HARVEST", src_loc ) ) {
            ...
            return;
        } else if( reason == CAN_DO_FETCH && act.id() == activity_id( "ACT_TIDY_UP" ) ) {
            if( !tidy_activity( p, src_loc, act ) ) {
                return;
            }
        } else if( reason == CAN_DO_FETCH && act.id() == activity_id( "ACT_FETCH_REQUIRED" ) ) {
            fetch_activity( p, src_loc, act );
            return;
        } else if( reason == CAN_DO_FETCH && act.id() == activity_id( "ACT_MOVE_LOOT" ) ) {
            if( move_loot_activity( p, src_loc, mgr, act ) ) {
                return;
            }
        } else if( reason != ALREADY_DONE ) {
            debugmsg( "Unhandled activity [%s] reason %d", act.name, reason );
        }

        //done, think about what to do next
        stage = THINK;
        act.coord_set.erase( src );
    }
