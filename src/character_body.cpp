...
static const trait_id trait_BARK2_a( "BARK2_a" );
static const trait_id trait_BARK2_b( "BARK2_b" );
static const trait_id trait_BARK2_c( "BARK2_c" );
...
void Character::update_bodytemp()
{
    ...
    for( const bodypart_id &bp : get_all_body_parts() ) {
        ...
        const bool cold_immune = has_flag( json_flag_ECTOTHERM );
        // PENALTIES
        if( temp_after < BODYTEMP_FREEZING && !cold_immune ) {
            add_effect( effect_cold, 1_turns, bp, true, 3 );
        } else if( temp_after < BODYTEMP_VERY_COLD && !cold_immune ) {
            add_effect( effect_cold, 1_turns, bp, true, 2 );
        } else if( temp_after < BODYTEMP_COLD && !cold_immune ) {
