        ...
        for( const point &gate_offset : four_adjacent_offsets ) {
            ...
            if( !open ) { // Closing the gate...
                ...
                while( here.ter( cur_pos ) == gate.floor.id() ) {
                    fail = !doors::forced_door_closing( cur_pos, gate.door.id(), gate.bash_dmg ) || fail;
                    close = !fail;
                    cur_pos += gate_offset;
                }
            }
...
bool doors::forced_door_closing(const tripoint& p, const ter_id& door_type, int bash_dmg)
{
    map& m = get_map();
    avatar& u = get_avatar();
    // TODO: Z
...
bool doors::forced_door_closing(const tripoint_bub_ms& p, const ter_id& door_type, int bash_dmg)
{
