...
void live_view::show(const int x, const int y)
{
    ...
    g->print_all_tile_info(x, y, g->u.posz, w_live_view, START_COLUMN, line, true);

    if (m.can_put_items(x, y, g->u.posz) && m.sees_some_items(x, y, g->u.posz, g->u)) {
        if(g->u.has_effect("blind")) {
            ...
            print_items(m.i_at(x, y, g->u.posz), line);
        }
    }
