...
std::string base64_encode_bitset( const std::bitset<576> &bitset_to_encode )
{
    ...
    for( int i = 0; i < 9; ++i ) {
        ...
        memcpy( &raw_bitset_data[( 8 - i ) * 8 ], &bits, sizeof( bits ) );
    }
...
void base64_decode_bitset( const std::string &packed_bitset, std::bitset<576> &destination_bitset )
{
    ...
    for( int i = 0; i < 9; i++ ) {
        uint64_t bits;
        memcpy( &bits, &decoded_string.data()[i * 8], sizeof( bits ) );
        ...
        for( int z = 0; z < OVERMAP_LAYERS; ++z ) {
            ...
            for( int j = 0; j < OMAPY; j++ ) {
                for( int i = 0; i < OMAPX; i++ ) {
                    ...
                    count--;
                    set_passable( project_combine( loc, tripoint_om_omt{ i, j, z - OVERMAP_DEPTH } ),
