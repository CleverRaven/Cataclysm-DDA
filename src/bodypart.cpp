...
/** @relates int_id */
template<>
const string_id<body_part_type> &int_id<body_part_type>::id() const
{
...
/** @relates string_id */
template<>
int_id<body_part_type> string_id<body_part_type>::id() const
{
...
{
    const body_part_type &bdy = get_bp( bp );
    // See comments in `body_part_type::load` about why these two strings are
    // not a single translation object with plural enabled.
