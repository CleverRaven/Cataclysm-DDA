                              ...
                              const cata::optional<tripoint> &mouse_target )
{
    ...
    switch( act ) {
        ...
        case ACTION_CONTROL_VEHICLE:
            vehicle *veh = veh_pointer_or_null( get_map().veh_at( player.character.pos() ) );
            if( player_character.has_active_mutation( trait_SHELL2 ) ) {
                ...
                add_msg( m_info, _( "You lack the substance to affect anything." ) );
            } else if( ( player_character.creature_size::huge ) && ( veh && veh->has_part( player.character.pos(), "MUTANT_OK" ) ) ) {
