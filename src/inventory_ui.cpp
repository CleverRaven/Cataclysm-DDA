    ...
    public:
        ...
        virtual std::string get_caption( const inventory_entry &entry ) const override {
            ...
            if (entry.location->ammo_type() == "money") {
                if (entry.chosen_count > 0 && entry.chosen_count < available_count) {
					//~ In the following string, the %s is the ammount of money on the selected cards as passed by the display money function, out of the total ammount of money on the cards, specified as ($%.2f).(e.g. "cash cards ($15.35) of ($128.55)")
