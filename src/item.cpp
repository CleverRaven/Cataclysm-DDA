...
std::string item::info(bool showtext, std::vector<iteminfo> &info) const
{
    ...
    if( !is_null() ) {
        ...
        if ( damage_bash() > 0 || damage_cut() > 0 ) {
            ...
            info.push_back(iteminfo("BASE", space + _("To-hit bonus: "), ((type->m_to_hit > 0) ? "+" : ""),
                                    type->m_to_hit, true, ""));
    ...
    if( is_gun() ) {
        ...
        if( mod == nullptr ) {
            ...
            info.push_back( iteminfo( "DESCRIPTION", string_format( _( "Stats of the active <info>gunmod (%s)</info> are shown." ),
                                                                     mod->tname().c_str() ) ) );
        }
        ...
        info.push_back(iteminfo("GUN", _("Range: "), "", mod->gun_range( false ), true, "", false, false));
        if (has_ammo) {
            temp1.str("");
            temp1 << (ammo_range >= 0 ? "+" : "" );
        ...
        info.push_back(iteminfo("GUN", _("Recoil: "), "", mod->gun_recoil( false ), true, "", false, true));
        if (has_ammo) {
            temp1.str("");
            temp1 << (ammo_recoil >= 0 ? "+" : "" );
    ...
    if( is_gunmod() ) {
        ...
        if( is_auxiliary_gunmod() ) {
            info.push_back( iteminfo( "DESCRIPTION", _( "This mod <info>must be attached to a gun</info>, it can not be fired separately." ) ) );
        }
        if( has_flag( "REACH_ATTACK" ) ) {
            info.push_back( iteminfo( "DESCRIPTION", _( "When attached to a gun, <good>allows</good> making <info>reach melee attacks</info> with it." ) ) );
        }
        if (mod->dispersion != 0) {
    ...
    if( is_armor() ) {
        ...
        info.push_back(iteminfo("ARMOR", _("Protection: Bash: "), "", bash_resist(), true, "", false));
    ...
    if( is_book() ) {
        ...
        if( g->u.has_identified( type->id ) ) {
            ...
            if( recipe_list.size() != book->recipes.size() ) {
                info.push_back( iteminfo( "DESCRIPTION", _( "It might help you figuring out some <good>more recipes</good>." ) ) );
            }
        } else {
            info.push_back(iteminfo("BOOK", _("You need to <info>read this book to see its contents</info>.")));
        }
    ...
    if (!components.empty()) {
        info.push_back( iteminfo( "DESCRIPTION", string_format( _("Made from: %s"), components_to_string().c_str() ) ) );
    } else {
    ...
    if ( showtext && !is_null() ) {
        ...
        if( !is_gunmod() && has_flag( "REACH_ATTACK" ) ) {
            ...
            if( has_flag( "REACH3" ) ) {
                ...
                info.push_back(iteminfo("DESCRIPTION", _("This item can be used to make <info>reach attacks</info>.")));
            }
        }
        ...
        for( auto &u : type->use_methods ) {
            ...
            if( time_to_do <= 0 ) {
                ...
                info.push_back( iteminfo( "DESCRIPTION", string_format( _( "It will be done in %s." ), time.c_str() ) ) );
            }
        }
