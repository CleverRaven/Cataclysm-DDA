                     ...
                     int /* batch */, bool /* debug */ ) const
{
    ...
    const std::string ranged_damage_text = colorize( _( "Ranged damage:" ), c_bold ) + " ";
    ...
    if( mod->ammo_required() ) {
        ...
        if( parts->test( iteminfo_parts::GUN_DAMAGE_PELLETS ) && pellets ) {
            ...
            info.emplace_back( "GUN", _( "<bold>Point blank damage:</bold> " ), "<num>", iteminfo::no_flags,
                               gun_damage );
            info.emplace_back( "GUN", ranged_damage_text, iteminfo::no_newline );
    ...
    int gun_dispersion = mod->gun_dispersion( false, false ) / 100.0;
    if( mod->ammo_required() ) {
    ...
    const int final_value = adj_disp ?
                            adj_disp :
                            point_shooting_limit <= eff_disp ? point_shooting_limit : act_disp;
