...
bool item::magazine_integral() const
{
    ...
    // We have an integral magazine if we're a gun with an ammo capacity (clip) or we have no magazines or batteries.
    return ( is_gun() && type->gun->clip > 0 ) || ( type->magazines.empty() && !battery_powered() );
}
