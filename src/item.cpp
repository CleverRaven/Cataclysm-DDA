...
std::string item::info( bool showtext, std::vector<iteminfo> &info ) const
{
    ...
    if( !is_null() ) {
        ...
        if( count_by_charges() && type->volume > 0 && type->stack_size > 1 ) {
            if( type->volume == 1 ) {
                ...
                info.emplace_back( "BASE", string_format( _( "Stacks in groups of <stat>%1$d</stat>" ),
                                   type->stack_size ) );
            } else {
                //~ %1$d is stack size and %2$d is base volume with both guaranteed to be > 1
                info.emplace_back( "BASE", string_format(
                                       _( "Stack of <stat>%1$d</stat> consumes <stat>%2$d</stat> volume" ),
        ...
        if( damage_bash() > 0 || damage_cut() > 0 ) {
            ...
            if( !conductive() ) {
                info.push_back( iteminfo( "BASE",
                                          string_format( _( "* This weapon <good>does not conduct</good> electricity." ) ) ) );
            } else if( has_flag( "CONDUCTIVE" ) ) {
                info.push_back( iteminfo( "BASE",
                                          string_format(
                                              _( "* This weapon effectively <bad>conducts</bad> electricity, as it has no guard." ) ) ) );
            } else {
                info.push_back( iteminfo( "BASE",
                                          string_format( _( "* This weapon <bad>conducts</bad> electricity." ) ) ) );
            }
        }
    ...
    if( food_item != nullptr ) {
        if( g->u.nutrition_for( food_item->type ) != 0 || food_item->type->comestible->quench != 0 ) {
            info.push_back( iteminfo( "FOOD", _( "<bold>Nutrition</bold>: " ), "",
                                      g->u.nutrition_for( food_item->type ),
                                      true, "", false, true ) );
        ...
        if( food_item->has_flag( "HIDDEN_POISON" ) &&
            g->u.get_skill_level( skill_survival ).level() >= 3 ) {
    ...
    if( !is_gun() ) {
        ...
        if( eff_range > 0 ) {
            info.emplace_back( "GUN", _( "<bold>Effective range: </bold>" ), "<num>", eff_range, true, "",
                               false );
        ...
        if( aim_mv > 0 ) {
            info.emplace_back( "GUN", _( "Maximum aiming time: " ), "<num> seconds", int( aim_mv / 16.67 ),
                               true, "", true, true );
        }
    ...
    if( is_gunmod() ) {
        ...
        mod->usable.end(), []( const std::string & used_on ) {
    ...
    auto name_quality = [&info]( const std::pair<quality_id, int> &q ) {
        ...
        if( q.first == quality_jack || q.first == quality_lift ) {
            str = string_format(
                      _( "Has level <info>%1$d %2$s</info> quality and is rated at <info>%3$dkg</info>" ),
    ...
    if( std::any_of( contents.begin(), contents.end(), []( const item & e ) {
    ...
    if( showtext && !is_null() ) {
        ...
        if( has_flag( "RADIO_ACTIVATION" ) ) {
            ...
            info.emplace_back( "DESCRIPTION",
                               string_format( _( "* It will be activated by the %s radio signal." ), signame.c_str() ) );
        ...
        if( !contents.empty() ) {
            if( is_gun() ) { //Mods description
                for( const auto mod : gunmods() ) {
                    ...
                    temp1 << _( "Mod: " ) << "<bold>" << mod->tname() << "</bold> (" << _
                          ( mod->type->gunmod->location.c_str() ) <<
                          ")";
                    insert_separation_line();
                    info.push_back( iteminfo( "DESCRIPTION", temp1.str() ) );
        ...
        if( u->has_identified( typeId() ) ) {
            ...
            if( tmp.skill && // Book can improve skill: blue
                ...
                ret = ( is_marked_important() ) ? c_ltblue_white : c_ltblue;
                           ...
                           *type ) ) { // Book can't improve skill right now, but has more recipes: yellow
                ret = ( is_marked_important() ) ? c_yellow_white : c_yellow;
...
std::string item::tname( unsigned int quantity, bool with_prefix ) const
{
    ...
    if( corpse != NULL && typeId() == "corpse" ) {
        ...
        if( contents.front().made_of( LIQUID ) ) {
            maintext = rmp_format( _( "<item_name>%s of %s" ), label( quantity ).c_str(),
                                   contents.front().tname( quantity, with_prefix ).c_str() );
        } else if( contents.front().is_food() ) {
            maintext = contents.front().charges > 1 ? rmp_format( _( "<item_name>%s of %s" ),
            ...
            maintext = rmp_format( _( "<item_name>%s with %s" ), label( quantity ).c_str(),
                                   contents.front().tname( quantity, with_prefix ).c_str() );
        }
...
bool item::is_food( player const *u ) const
{
    ...
    if( ( u->has_active_bionic( "bio_reactor" ) || u->has_active_bionic( "bio_advreactor" ) ) &&
        is_ammo() && ( ammo_type() == ammotype( "reactor_slurry" ) ||
                       ammo_type() == ammotype( "plutonium" ) ) ) {
        return true;
    }
...
skill_id item::weap_skill() const
{
    ...
    if( type->melee_dam >= type->melee_cut ) {
        return skill_bashing;
    }
    if( has_flag( "STAB" ) || has_flag( "SPEAR" ) ) {
...
bool item::magazine_integral() const
{
    ...
    return std::none_of( mags.begin(),
    mags.end(), []( const std::pair<ammotype, const std::set<itype_id>> &e ) {
...
bool item::gunmod_compatible( const item &mod, bool alert, bool effects ) const
{
    ...
    if( !is_gun() ) {
        ...
        msg = string_format( _( "Your %1$s already has a %2$s." ), tname().c_str(),
                             mod.tname( 1 ).c_str() );
        ...
        msg = string_format( _( "Your %1$s doesn't have enough room for another %2$s mod." ),
                             tname().c_str(), _( mod.type->gunmod->location.c_str() ) );

    } else if( effects && ( mod.type->gunmod->ammo_modifier ||
                            !mod.type->gunmod->magazine_adaptor.empty() )
        ...
        msg = string_format( _( "That %s cannot be attached to a %s" ), mod.tname().c_str(),
                             _( gun_type().c_str() ) );
    ...
    } else if( !mod.type->gunmod->acceptable_ammo.empty() &&
               !mod.type->gunmod->acceptable_ammo.count( ammo_type( false ) ) ) {
        msg = string_format( _( "That %1$s cannot be used on a %2$s." ), mod.tname( 1 ).c_str(),
        ...
        msg = string_format( _( "This %s is eminently reliable. You can't improve upon it this way." ),
                             tname().c_str() );
...
bool item::detonate( const tripoint &p, std::vector<item> &drops )
{
    if( type->explosion.power >= 0 ) {
        ...
        return true;
    } else if( !contents.empty() && ( type->magazine == nullptr ||
                                      !type->magazine->protects_contents ) ) {
