                      ...
                      bool /* debug */ ) const
{
    ...
    if( !book.skill && !type->can_use( "MA_MANUAL" ) && parts->test( iteminfo_parts::BOOK_SUMMARY ) && player_character.studied_all_recipes(*type)) {
...
nc_color item::color_in_inventory( const Character *const ch ) const
{
    ...
    if( type->can_use( "learn_spell" ) ) {
        ...
        } else if( (tmp.skill || type->can_use( "MA_MANUAL" )) && !player_character.studied_all_recipes(*type)) {
