...
item::armor_status item::damage_armor_durability( damage_unit &du, const bodypart_id &bp, double enchant_multiplier )
{
    ...
    if( has_flag( flag_FRAGILE ) && du.amount / armors_own_resist > (0.15f / enchant_multiplier ) ) {
        return mod_damage( itype::damage_scale * enchant_multiplier ) ? armor_status::DESTROYED : armor_status::DAMAGED;
    }
    ...
    const float damaged_chance = ( 0.11 * ( post_mitigated_dmg / ( armors_own_resist + 2 ) ) + 0.1 ) * enchant_multiplier;
