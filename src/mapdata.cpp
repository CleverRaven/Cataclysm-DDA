...
void ter_t::load( JsonObject &json, const std::string &src )
{
    map_data_common_t::load( json, src );
    mandatory( json, was_loaded, "name", name_ );
    mandatory( json, was_loaded, "move_cost", movecost );
    optional( json, was_loaded, "max_volume", max_volume, legacy_volume_reader, DEFAULT_MAX_VOLUME_IN_SQUARE );
    optional( json, was_loaded, "trap", trap_id_str );
    optional( json, was_loaded, "sinkage", sinkage );
    ...
    for( auto &flag : json.get_string_array( "flags" ) ) {
        set_flag( flag );
    }
    ...
    if( json.has_member( "connects_to" ) ) {
        set_connects( json.get_string( "connects_to" ) );
    }
    
    optional( json, was_loaded, "open", open, ter_str_id::NULL_ID() );
    optional( json, was_loaded, "close", close, ter_str_id::NULL_ID() );
    optional( json, was_loaded, "transforms_into", transforms_into, ter_str_id::NULL_ID() );
    optional( json, was_loaded, "roof", roof, ter_str_id::NULL_ID() );
    
    bash.load( json, "bash", false );
    deconstruct.load( json, "deconstruct", false );
}
