...
//Isn't storing these as four ints in the case of no seasonal variation (overwhelmingly the norm) really inefficient compared to one int for not seasonal and an optional array?
...
void map_data_common_t::load_symbol( const JsonObject &jo, const std::string &context )
{
    ...
    if( jo.has_member( "symbol" ) || !was_loaded ) {
        load_season_array( jo, "symbol", context, symbol_, [&]( const std::string & /*str*/ ) {
            uint32_t symbol;
...
void map_data_common_t::set_flag( const ter_furn_flag flag )
{
    ...
    bitflags.set( flag );
    transparent |= flag == ter_furn_flag::TFLAG_TRANSPARENT;
}
...
void furn_t::load( const JsonObject &jo, const std::string &src )
{
    ...
    if( jo.has_object( "plant_data" ) ) { //TODO: Does this support copy-from?
