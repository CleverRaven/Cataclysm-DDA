...
void start_location::place_player( player &u ) const
{
    ...
    u.setx( SEEX * HALF_MAPSIZE );
    u.sety( SEEY * HALF_MAPSIZE );
    u.setz( g->get_levz() );
    ...
    while( !found_good_spot && tries < 100 ) {
        tripoint rand_point( ( SEEX * HALF_MAPSIZE ) + rng( 0, SEEX * 2 - 1 ),
                             ( SEEY * HALF_MAPSIZE ) + rng( 0, SEEY * 2 - 1 ),
                             u.posz() );
                           ...
                           const size_t count, const int rad ) const
{
    ...
    const int ux = g->u.posx() % ( SEEX * HALF_MAPSIZE );
    const int uy = g->u.posy() % ( SEEY * HALF_MAPSIZE );
