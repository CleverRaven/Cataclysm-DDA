...
int Character::run_cost( int base_cost, bool diag ) const
{
    ...
    const bool on_road = flatground && here.has_flag( flag_ROAD, pos() );
    const bool on_fungus = here.has_flag_ter_or_furn( flag_FUNGUS, pos() );
            ...
            for( const tripoint &point : path ) {
                if( here.impassable( point ) &&
                    !( weapon.has_flag( flag_SPEAR ) && // Fences etc. Spears can stab through those
                       here.has_flag( flag_THIN_OBSTACLE, point ) ) ) { //this mirrors melee.cpp function reach_attack
                    can_see = false;
                    break;
                }
            }
...
void Character::knock_back_to( const tripoint &to )
{
    ...
    if( here.has_flag( flag_LIQUID, to ) && here.has_flag( TFLAG_DEEP_WATER, to ) ) {
        if( !is_npc() ) {
