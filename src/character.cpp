...
units::mass Character::weight_carried() const
{
    ...
    cached_weight_carried = bodyweight_fat() + weight_carried_with_tweaks( item_tweaks() );
    return *cached_weight_carried;
}
...
void Character::calc_bmi_encumb( std::map<bodypart_id, encumbrance_data> &vals ) const
{
    ...
    for( std::pair<const bodypart_id, encumbrance_data> &val : vals ) {
        if( val.first.get_bmi_encumbrance_scalar() > 0 ) {
            int penalty = std::floor( val.first.get_bmi_encumbrance_scalar() * ( std::max( 0,
                                      ( get_bmi() - val.first.get_bmi_encumbrance_threshold() ) ) ) );
...
{
    return std::max( 0, ( ( get_bmi() * std::pow( height() / 100.0f, 2 ) ) - ( 25 * std::pow( height() / 100.0f, 2 ) ) ) );
}
