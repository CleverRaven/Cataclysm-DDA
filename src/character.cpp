        ...
        if( calendar::once_every( 10_minutes ) && ( has_trait( trait_CHLOROMORPH ) ||
            ...
            if( has_trait( trait_M_SKIN3 ) ) {
                ...
                if( here.has_flag_ter_or_furn( ter_furn_flag::TFLAG_FUNGUS, pos() ) ) {
                    if( get_sleepiness() >= 0 ) {
                        mod_sleepiness( -5 ); // Local guides need less sleep on fungal soil
                    }
...
needs_rates Character::calc_needs_rates() const
{
    ...
    rates.sleepiness *= 1.0f + mutation_value( sleepiness_modifier );
...
void Character::calc_sleep_recovery_rate( needs_rates &rates ) const
{
    ...
    rates.recovery = 1.0f + mutation_value( sleepiness_regen_modifier );
        ...
        if( calendar::once_every( 60_minutes ) ) {
            ...
            if( can_pass_out && calendar::once_every( 10_minutes ) ) {
                ...
                if( one_in( static_cast<int>( ( 1 - sleep_deprivation_pct ) * 100 ) + per_cur ) ||
                    ...
                    add_msg_player_or_npc( m_bad,
                                           _( "Your body collapses due to sleep deprivation, your neglected sleepiness rushing back all at once, and you pass out on the spot." )
...
mutation_value_map = {
    ...
    { "sleepiness_regen_modifier", calc_mutation_value<&mutation_branch::sleepiness_regen_modifier> },
    { "sleepiness_modifier", calc_mutation_value<&mutation_branch::sleepiness_modifier> },
