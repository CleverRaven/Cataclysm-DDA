...
long field_dress_actor::use( player &p, item &it, bool t, const tripoint &pos ) const
{
    ...
    if( corpses.empty() ) {
        p.add_msg_if_player( _( "No corpses here." ) );
        return 0;
    }

    if( pos != p.pos() ) {
        p.add_msg_if_player( _( "You have to stand above the corpse." ) );
        return 0;
    }
    ...
    amenu.selected = 0;
    amenu.text = _( "Perform a field dressing on a corpse?" );
    amenu.addentry( cancel, true, 'q', _( "Cancel" ) );
    for( size_t i = 0; i < corpses.size(); i++ ) {
        amenu.addentry( i + 1, true, -1, corpses[i]->display_name().c_str() );
    }
    ...
    if( !mt->in_category( "WILDLIFE" ) ) {
        if( mt->in_species( ZOMBIE ) ) {
            add_msg( m_info, _("How would you benefit from field dressing a zombie?") );
            return 0;
        } else if( mt->in_species( HUMAN ) && ( !g->u.has_trait_flag( "CANNIBAL" ) || !g->u.has_trait_flag( "PSYCHOPATH" ) ) ) {
    ...
    int tool_q = 0;
    if( it.get_quality( quality_id( "BUTCHER" ) ) < p.max_quality( quality_id( "BUTCHER" ) ) ) {
        tool_q = p.max_quality( quality_id( "BUTCHER" ) );
        add_msg( m_info, _("You have something better then a %s so you choose a different tool."), it.type->nname( 1 ).c_str() );
        if( tool_q < 0 ) {
           add_msg( m_info, _("You don't trust the quality of your tool, but carry on anyway.") );
        }
    } else {
        tool_q = it.get_quality( quality_id( "BUTCHER" ) );
        if( tool_q < 0 ) {
           add_msg( m_info, _("You don't trust the quality of your %s, but carry on anyway."), it.type->nname( 1 ).c_str() );
        }
    }
    
    int difficulty = 50; //base difficulty

    //damaged bodies can have damaged organs, so overall body state is a very important factor
    difficulty += ( body->damage() * 100 / body->max_damage() ); // 0-100% damage

    // reversed butchering quality of tool; minus qualities bump up the difficulty, while plus quality items reduce the difficulty 
    // example: sharp rock quality = -50, difficulty + 50;
    // example: hunting knife quality = 30, difficulty = - 30;
    difficulty -= tool_q; 

    //bonus for fine cutting tools vs. simple cutting tools
    if( it.has_quality( quality_id( "CUT_FINE" ) ) ) {
        difficulty -= rng( 0, 10 ); 
    }
    
    //malus for power tools and other butchering items that don't have cutting quality
    if( !it.has_quality( quality_id( "CUT" ) ) ) {
        difficulty += rng( 10, 20 ) ; 
    }

    // Larger creatures are easier to cut without mistakes.
    // reversed get_meat_chunks_count() is a good representation of this bonus.
    difficulty -= mt->get_meat_chunks_count();

    // at SURVIVAL = 3 (entry lvl),  Dex = 10,  Skill = 40
    // at SURVIVAL = 6,  Dex = 10,  Skill = 55
    // at SURVIVAL = 10,  Dex = 10,  Skill = 75
    int skills = p.get_skill_level( skill_survival ) + p.get_skill_level( skill_firstaid ) +
                 ( p.dex_cur / 2 );
    skills *= 5; 

    // 50% of skills and difficulty is ruled by RNG
    int success = ( skills / 2 )  + rng( 0, skills / 2 ) - (difficulty / 2) - rng( 0, difficulty / 2 );

    // speed scales down with survival 
    int moves = (difficulty + 1) * 100 / p.get_skill_level( skill_survival );
    if ( moves < 500 ) {
        moves = 500;
    }

    p.assign_activity( activity_id( "ACT_FIELD_DRESS" ), moves );
    p.activity.values.push_back( success );
    p.activity.str_values.push_back( corpses[selected_corpse]->display_name() );
...
ret_val<bool> field_dress_actor::can_use( const player &p, const item &, bool, const tripoint & ) const
{

    if( p.get_skill_level( skill_survival ) < 3 ) {
        return ret_val<bool>::make_failure( _( "You don't know enough about %s to try it yet." ),
                                            skill_survival->name().c_str() );
    }
    return ret_val<bool>::make_success();
}
