...
void repair_item_actor::load( JsonObject &obj )
{
    ...
    charges_to_use = obj.get_int( "charges_to_use", 0 );
}
...
bool repair_item_actor::can_use( const player*, const item* it, bool, const tripoint& ) const
{
    return it->charges >= charges_to_use;
}
...
bool sew_advanced_actor::can_use( const player*, const item* it, bool, const tripoint& ) const
{
    return it->charges >= charges_to_use;
}


long sew_advanced_actor::use(player *p, item *it, bool, const tripoint& ) const
{
    if( p->is_npc() ) {
        return 0;
    }
