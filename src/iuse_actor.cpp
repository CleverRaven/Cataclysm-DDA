...
iuse_actor *clear_rubble_actor::clone() const
{
    return new clear_rubble_actor( *this );
}

bool clear_rubble_actor::can_use( const player *p, const item *, bool, const tripoint &pos ) const
{
    if( pos != p->pos() ) {
        return g->m.furn( pos ).obj().examine == iexamine::rubble;
    }
...
long clear_rubble_actor::use( player *p, item *it, bool t, const tripoint &pos ) const
{
    if( !can_use( p, it, t, pos ) ) {
