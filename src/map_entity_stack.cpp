...
void map_entity_stack<item>::make_name_cache()
{
    ...
    if( count > 1 ) {
        text += string_format( "%s ",
                               get_selected_entity()->type->item_measure_prefix( count ) );
    }
...
template<typename T>
std::string map_entity_stack<T>::get_category() const
{
    return std::string();
}

template<>
std::string map_entity_stack<item>::get_category() const
{
...
template<>
std::string map_entity_stack<Creature>::get_category() const
{
...
template<>
std::string map_entity_stack<map_data_common_t>::get_category() const
{
