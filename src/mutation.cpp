...
void player::mutate_towards(std::string mut)
{
    ...
    if (threshold) {
        Messages::player_messages.add_msg(_("You feel something straining deep inside you, yearning to be free..."));
        mutate();
        return;
    }
    ...
    if (replacing2 != "") {
        Messages::player_messages.add_msg(_("Your %1$s mutation turns into %2$s!"), traits[replacing2].name.c_str(), traits[mut].name.c_str());
        add_memorial_log(pgettext("memorial_male","'%s' mutation turned into '%s'"),
            pgettext("memorial_female", "'%s' mutation turned into '%s'"),
...
void player::remove_mutation(std::string mut)
{
    ...
    if (replacing != "") {
        Messages::player_messages.add_msg(_("Your %1$s mutation turns into %2$s."), traits[mut].name.c_str(),
...
void mutation_effect(player &p, std::string mut)
{
    ...
    if (mut == "TOUGH" || mut == "TOUGH2" || mut == "TOUGH3" || mut == "GLASSJAW" ||
      ...
      mut == "ARM_TENTACLES_4" || mut == "ARM_TENTACLES_8") {
        ...
        Messages::player_messages.add_msg(_("You rip out of your clothing!"));
        destroy = true;
        bps.push_back(bp_torso);
