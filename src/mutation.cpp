...
int Character::get_instability_per_category( const mutation_category_id &categ ) const
{
    ...
    for( const trait_id &mut : get_mutations() ) {
        ...
        if( ( mut.obj().points > -1 && !mut.obj().threshold && mut.obj().valid && !has_base_trait( mut ) ) || ( mut.obj().transform && mut.obj().transform.obj().valid && ( mut.obj().points > -1 || mut.obj().transform.obj().points > -1 ) ) ) {
            bool in_categ = false;
