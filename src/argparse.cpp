...
ArgHandler::ArgHandler(const std::string &name, const std::string &description)
    : _name(name), _description(description), _handling_succeeded(true) {}

ArgHandler::ArgHandler(const std::string &name,
    const std::string &param_description,
    const std::string &description)
    : ArgHandler(name, description)
{
  _param_description = param_description;
}

bool ArgHandler::ParseArglist(const std::string &arg_prefix,
...
FlagHandler::FlagHandler(const std::string &name,
    ...
    : ArgHandler(name, description), _flag(flag), _value_to_set(value_if_present) {}

size_t FlagHandler::HandleArg(const arglist_citerator_t &,
...
LambdaFlagHandler::LambdaFlagHandler(const std::string &name,
    const std::string &description,
    const handler_t handler)
    : ArgHandler(name, description), _handler(handler) {}

size_t LambdaFlagHandler::HandleArg(const arglist_citerator_t &, const arglist_citerator_t &, arglist_t &) {
...
LambdaArgHandler::LambdaArgHandler(const std::string &name, const std::string &param_description,
    ...
    : ArgHandler(name, param_description, description), _handler(handler) {}
