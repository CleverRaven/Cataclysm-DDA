...
int get_rot_since( const int since, const int endturn )
{
    ...
    for( ;
        ...
        if ( wit->first <= endturn ) {
            ...
            if ( tbegin < since ) {
                tbegin = since;
                ret += ( (tend - tbegin) * get_hourly_rotpoints_at_temp(wit->second.temperature) );
                tend = wit->first;
                break;
            }
            ret += ( (tend - tbegin) * get_hourly_rotpoints_at_temp(wit->second.temperature) );
...
void funnelcontainer_accumulates(int rain_depth_mm_per_hour, bool acid, int funnel_quality,
                                 item *fcontainer)
{
...
std::string weather_forecast(radio_tower tower)
{
    ...
    for(std::map<int, weather_segment>::iterator it = g->weather_log.lower_bound( int(calendar::turn) );
        it != g->weather_log.end(); ++it ) {
