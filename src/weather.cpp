...
void fill_funnels(int rain_depth_mm_per_hour, bool acid, trap_id t)
{
    ...
    for (i = funnel_locs.begin(); i != funnel_locs.end(); ++i) {
        ...
        if (one_in(turns_per_charge)) { // todo; fixme. todo; fixme
            //Messages::add_msg("%d mm/h %d tps %.4f: fill",int(calendar::turn),rain_depth_mm_per_hour,turns_per_charge);
...
void weather_effect::thunder()
{
    ...
    if (one_in(THUNDER_CHANCE)) {
        if (g->levz >= 0) {
            Messages::add_msg(_("You hear a distant rumble of thunder."));
        } else if (g->u.has_trait("GOODHEARING") && one_in(1 - 2 * g->levz)) {
            Messages::add_msg(_("You hear a rumble of thunder from above."));
...
void weather_effect::lightning()
{
    ...
    if(one_in(LIGHTNING_CHANCE)) {
        if(g->levz >= 0) {
            Messages::add_msg(_("A flash of lightning illuminates your surroundings!"));
...
void weather_effect::light_acid()
{
    ...
    if (int(calendar::turn) % 10 == 0 && PLAYER_OUTSIDE) {
        if (g->u.weapon.has_flag("RAIN_PROTECT") && !one_in(3)) {
            Messages::add_msg(_("Your %s protects you from the acidic drizzle."), g->u.weapon.name.c_str());
        } else {
            if (g->u.worn_with_flag("RAINPROOF") && !one_in(4)) {
                Messages::add_msg(_("Your clothing protects you from the acidic drizzle."));
            } else {
                bool has_helmet = false;
                if (g->u.is_wearing_power_armor(&has_helmet) && (has_helmet || !one_in(4))) {
                    Messages::add_msg(_("Your power armor protects you from the acidic drizzle."));
                } else {
                    Messages::add_msg(_("The acid rain stings, but is mostly harmless for now..."));
...
void weather_effect::acid()
{
    if (int(calendar::turn) % 2 == 0 && PLAYER_OUTSIDE) {
        if (g->u.weapon.has_flag("RAIN_PROTECT") && one_in(4)) {
            Messages::add_msg(_("Your umbrella protects you from the acid rain."));
        } else {
            if (g->u.worn_with_flag("RAINPROOF") && one_in(2)) {
                Messages::add_msg(_("Your clothing protects you from the acid rain."));
            } else {
                bool has_helmet = false;
                if (g->u.is_wearing_power_armor(&has_helmet) && (has_helmet || !one_in(2))) {
                    Messages::add_msg(_("Your power armor protects you from the acid rain."));
                } else {
                    Messages::add_msg(_("The acid rain burns!"));
