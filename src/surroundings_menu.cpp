...
static void move_selection( T &data, const int amount )
{
    ...
    if( amount < 0 && it == data.filtered_list.begin() ) {
    ...
    } else if( amount > 0 && amount > std::distance( it, data.filtered_list.end() ) ) {
        data.selected_entry = *data.filtered_list.rbegin();
    } else {
    ...
    tab_data( title )
{
    ...
    hotkey_groups = { { "EXAMINE", "COMPARE", "TRAVEL_TO" }, { "FILTER", "RESET_FILTER", "PRIORITY_INCREASE", "PRIORITY_DECREASE" }
    };
}
    ...
    tab_data( title )
{
    ...
    hotkey_groups = { { "fire", "SAFEMODE_BLACKLIST_ADD", "SAFEMODE_BLACKLIST_REMOVE", "FILTER", "RESET_FILTER", "TRAVEL_TO" } };
}
    ...
    tab_data( title )
{
    ...
    hotkey_groups = { { "FILTER", "RESET_FILTER", "TRAVEL_TO" } };
}
...
void surroundings_menu::draw_item_tab()
{
    ...
    if( ImGui::BeginTabItem( title.c_str(), nullptr, flags ) ) {
        ...
        float list_height = str_height_to_pixels( TERMY - info_height ) - get_hotkey_buttons_height() -
                            magic_number_other_elements_height;
...
void surroundings_menu::draw_hotkey_buttons()
{
    tab_data *cur_tab = get_selected_data();
    ...
    for( const std::unordered_set<std::string> &group : cur_tab->hotkey_groups ) {
        for( const std::string &button : group ) {
            // todo: don't just hardcode this
            if( button == "RESET_FILTER" && !cur_tab->has_filter() ) {
                continue;
            }

            std::string buttontxt = cur_tab->ctxt.get_button_text( button );
...
float surroundings_menu::get_hotkey_buttons_height()
{
    tab_data *cur_tab = get_selected_data();

    if( cur_tab->hotkey_groups.empty() ) {
        return 0;
    }
