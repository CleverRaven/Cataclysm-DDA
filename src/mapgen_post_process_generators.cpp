...
void pp_generator::apply_pp_generators( map &md, std::list<tripoint_bub_ms> &all_points_in_map )
{
    for( const auto &gen : map_generator_functions ) {

        overmap_buffer.ter_existing( coords::project_to<coords::omt>( md.get_abs(
                                         all_points_in_map.begin()->abs() ) ) )->get_type_id()->get_generator_var(
                                             map_generator::bash_damage );
...
{
    // pp_generator bash_vars = overmap_buffer.ter_existing( coords::project_to<coords::omt>( md.get_abs(
    //                              all_points_in_map.begin()->abs() ) ) )->get_type_id()->get_generator_var(
    //                              map_generator::bash_damage );
    // bash_vars.num_attempts = 250; // Roughly half as many attempts as old version, may need tweaking
    //   bash_vars.percent_chance = 10; REMOVE
...
void generator_instance::riot_damage( map &md, const tripoint_abs_omt &p, bool is_a_road )
{
    ...
    //  bash_damage( md, all_points_in_map );
    // move_items( md, all_points_in_map );
    // add_fire( md, all_points_in_map );
    // HACK: Don't burn roads to the ground! This should be resolved when the system is moved to json
    if( !is_a_road ) {
        //     pre_burn( md, all_points_in_map );
    }

}

void generator_instance::aftershock_ruin( map &md, const tripoint_abs_omt &p )
{
    std::list<tripoint_bub_ms> all_points_in_map;
