...
struct malloc_impl : mmap_file::impl {
    ...
    virtual bool resize_file( size_t desired_size ) override {
    ...
    virtual bool flush( size_t offset, size_t length ) override {
        return true;
    }
...
struct file_impl : mmap_file::impl {
    ...
    virtual bool map_view() override {
        struct stat buf {};
        if( fstat( file, &buf ) ) {
            return false;
        }
    ...
    virtual bool unmap_view() override {
        if( base != nullptr ) {
            munmap( base_, len_ );
        }
        base_ = nullptr;
    ...
    virtual bool resize_file( size_t desired_size ) {
        if( desired_size == len() ) {
            return true;
        }
        if( !unmap_view() ) {
