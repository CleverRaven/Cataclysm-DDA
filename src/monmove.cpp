...
void monster::move()
{
    ...
    if (has_flag(MF_REGENERATES_50)) {
        if (hp < type->hp) {
            if (one_in(2)) {
                Messages::player_messages.add_msg(_("The %s is visibly regenerating!"), name().c_str());
            }
            hp += 50;
            if(hp > type->hp) {
                hp = type->hp;
    ...
    if (has_flag(MF_REGENERATES_10)) {
        if (hp < type->hp) {
            if (one_in(2)) {
                Messages::player_messages.add_msg(_("The %s seems a little healthier."), name().c_str());
            }
            hp += 10;
            if(hp > type->hp) {
                hp = type->hp;
    ...
    if (has_flag(MF_ABSORBS)) {
        if(!g->m.i_at(posx(), posy()).empty()) {
            Messages::player_messages.add_msg(_("The %s flows around the objects on the floor and they are quickly dissolved!"), name().c_str());
    ...
    if (g->is_in_sunlight(posx(), posy()) && has_flag(MF_SUNDEATH)) {
        Messages::player_messages.add_msg(_("The %s burns horribly in the sunlight!"), name().c_str());
        hp -= 100;
        if(hp < 0) {
            hp = 0  ;
        }
