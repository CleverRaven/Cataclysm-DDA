...
bool monster::will_move_to( const tripoint &p ) const
{
    ...
    if( here.impassable( p ) ) {
        if( digging() ) {
            if( !here.has_flag( flag_BURROWABLE, p ) ) {
                return false;
            }
        } else if( !( can_climb() && here.has_flag( flag_CLIMBABLE, p ) ) ) {
            return false;
        }
    }
    ...
    if( digs() && !here.has_flag( flag_DIGGABLE, p ) && !here.has_flag( flag_BURROWABLE, p ) ) {
        return false;
    }
    ...
    if( avoid_fire || avoid_fall || avoid_simple || avoid_complex ) {
        ...
        if( attitude( &get_player_character() ) != MATT_ATTACK ) {
            ...
            if( avoid_simple && here.has_flag( flag_SHARP, p ) &&
...
int monster::calc_movecost( const tripoint &f, const tripoint &t ) const
{
    ...
    if( flies() || ( digging() && here.has_flag( flag_DIGGABLE, t ) ) ) {
        movecost = 100;
        ...
        if( here.has_flag( flag_CLIMBABLE, f ) ) {
            movecost += 150;
        } else {
            movecost += 50 * here.move_cost( f );
        }
        if( here.has_flag( flag_CLIMBABLE, t ) ) {
            movecost += 150;
        } else {
            movecost += 50 * here.move_cost( t );
        }
...
bool monster::bash_at( const tripoint &p )
{
    ...
    if( !( here.is_bashable_furn( p ) || here.veh_at( p ).obstacle_at_part() ) ) {
        ...
        bool flat_ground = here.has_flag( flag_ROAD, p ) || here.has_flag( flag_FLAT, p );
