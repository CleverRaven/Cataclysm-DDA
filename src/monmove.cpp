...
bool monster::can_reach_to( const tripoint &p ) const
{
    ...
    if( p.z > pos().z && z_is_valid( pos().z ) ) {
        ...
        if( !here.has_flag( TFLAG_GOES_UP, pos() ) && !g->m.has_flag( TFLAG_NO_FLOOR, p ) ) {
...
void monster::move()
{
    ...
    if( moved ) {
        ...
        for( tripoint &candidate : squares_closer_to( pos(), destination ) ) {
            bool via_ramp = false;
            if( g->m.has_flag( TFLAG_RAMP_UP, candidate ) ) {
                via_ramp = true;
                candidate.z += 1;
            } else if( g->m.has_flag( TFLAG_RAMP_DOWN, candidate ) ) {
                via_ramp = true;
                candidate.z -= 1;
            }
            ...
            if( candidate.z != posz() ) {
                ...
                if( !g->m.valid_move( pos(), candidate, false, true, via_ramp ) ) {
