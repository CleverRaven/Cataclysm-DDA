                       ...
                       const float stagger_adjustment )
{
    ...
    if( type->size != MS_TINY && on_ground ) {
        ...
        if( g->m.has_flag( "SHARP", pos() ) && !one_in( 4 ) && get_armor_cut( bp_chest ) < sharp_damage ) {
            apply_damage( nullptr, bp_chest, sharp_damage );
        }
        if( g->m.has_flag( "ROUGH", pos() ) && one_in( 6 ) && get_armor_cut( bp_chest ) < rough_damage ) {
            apply_damage( nullptr, bp_chest, rough_damage );
        }
    }
...
void monster::knock_back_to( const tripoint &to )
{
    ...
    if( monster *const z = g->critter_at<monster>( to ) ) {
        apply_damage( z, bp_chest, z->type->size );
        add_effect( effect_stunned, 1_turns );
        if( type->size > 1 + z->type->size ) {
            z->knock_back_from( pos() ); // Chain reaction!
            z->apply_damage( this, bp_chest, type->size );
    ...
    if( npc *const p = g->critter_at<npc>( to ) ) {
        apply_damage( p, bp_chest, 3 );
        add_effect( effect_stunned, 1_turns );
        p->deal_damage( this, bp_chest, damage_instance( DT_BASH, type->size ) );
        if( u_see ) {
    ...
    if( g->m.impassable( to ) ) {
        ...
        apply_damage( nullptr, bp_chest, type->size );
