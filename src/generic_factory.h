...
class generic_factory
{
    ...
    public:
        ...
        /**
        * Perform JSON inheritence handling for `T def` and returns true if JsonObject is real.
        *
        * If the object contains a "copy-from" member the corrisponding abstract gets copied if found.
        ...
        * 
        * @return true if `jo` is real and false if it's abstract. (has "abstract" member)
        * @throws JsonError If `jo` is both abstract and real. (contains "abstract" and "id" members)
        */
        bool handle_inharitance( T &def, JsonObject &jo, const std::string &src ) {
        ...
        void load( JsonObject &jo, const std::string &src ) {
            ...
            if( handle_inharitance( def, jo, src ) && jo.has_string( id_member_name ) ) {
