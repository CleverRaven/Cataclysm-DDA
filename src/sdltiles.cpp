...
bool input_context::get_coordinates(WINDOW* capture_win, int& x, int& y) {
    ...
    if (use_tiles)
    {
        ...
        int win_left = capture_win->x * tile_width;
        int win_right = (capture_win->x + capture_win->width) * tile_width;
        int win_top = capture_win->y * tile_height;
        int win_bottom = (capture_win->y + capture_win->height) * tile_height;
