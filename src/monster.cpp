...
void monster::print_info_imgui() const
{
    ...
    std::string bar_str;
    get_HP_Bar( bar_color, bar_str );
    ImGui::TextColored( cataimgui::imvec4_from_color( bar_color ), "%s", bar_str.c_str() );
    auto unbar_str = std::string( 5 - utf8_width( bar_str ), '.' );
    auto white = c_white;
    ImGui::SameLine( 0, 0 );
    ImGui::TextColored( cataimgui::imvec4_from_color( white ), "%s", unbar_str.c_str() );
    auto symbol_color = basic_symbol_color();
    ImGui::SameLine();
    ImGui::TextColored( cataimgui::imvec4_from_color( symbol_color ), "%s %s", name().c_str(),
    ...
    std::pair<std::string, nc_color> att = get_attitude();
    if( player_knows ) {
        ImGui::TextColored( cataimgui::imvec4_from_color( att.second ), "%s", att.first.c_str() );
    }
