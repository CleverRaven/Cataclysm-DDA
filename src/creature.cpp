                                     ...
                                     const projectile &proj, dealt_damage_instance &dealt_dam)
{
    ...
    if (dodge_roll() >= dice(10, proj.speed)) {
        if (is_player())
            Messages::player_messages.add_msg(_("You dodge %s projectile!"),
                       source->disp_name(true).c_str());
        else if (u_see_this)
            Messages::player_messages.add_msg(_("%s dodges %s projectile."),
    ...
    if(u_see_this) {
        if (damage_mult == 0) {
            if(source != NULL) {
                Messages::player_messages.add_msg(source->is_player() ? _("You miss!") : _("The shot misses!"));
            }
        } else if (dealt_dam.total_damage() == 0) {
            Messages::player_messages.add_msg(_("The shot reflects off %s %s!"), disp_name(true).c_str(),
                       skin_name().c_str());
        } else if (source != NULL) {
            if (source->is_player()) {
                Messages::player_messages.add_msg(_("You hit the %s for %d damage."),
                ...
                Messages::player_messages.add_msg(_("%s shoots %s."),
...
{
    if (e.get_duration() <= 0) {
        Messages::player_messages.add_msg(e.get_effect_type()->get_remove_message().c_str());
...
void Creature::add_effect(efftype_id eff_id, int dur, int intensity, bool permanent)
{
    ...
    if (first_eff != effects.end()) {
        ...
        if (is_player()) { // only print the message if we didn't already have it
            Messages::player_messages.add_msg(effect_types[eff_id].get_apply_message().c_str());
...
body_part Creature::select_body_part(Creature *source, int hit_roll)
{
    ...
    if(g->debugmon) {
        Messages::player_messages.add_msg("source size = %d", source->get_size());
        Messages::player_messages.add_msg("target size = %d", get_size());
        Messages::player_messages.add_msg("difference = %d", szdif);
    }
    ...
    if(g->debugmon) {
        Messages::player_messages.add_msg("eyes = %f", hit_weights.at(bp_eyes));
        Messages::player_messages.add_msg("head = %f", hit_weights.at(bp_head));
        Messages::player_messages.add_msg("torso = %f", hit_weights.at(bp_torso));
        Messages::player_messages.add_msg("arms = %f", hit_weights.at(bp_arms));
        Messages::player_messages.add_msg("legs = %f", hit_weights.at(bp_legs));
    }
