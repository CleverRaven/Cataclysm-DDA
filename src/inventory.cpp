...
bool inventory::can_make_recipe(recipe *r) const
{
    ...
    */
    ...
    for (invstack::const_iterator iter = items.begin(); iter != items.end(); ++iter) {
        for(std::list<item>::const_iterator stack_iter = iter->begin(); stack_iter != iter->end();
            ++stack_iter) {
            int item_assigned = 0; //flag to see if item is assigned, plus to see how much of it has been assigned
            //if it's ammunition or liquid
                ...
                while (tool_it != tool_choices.end()) {
                    ...
                    if(tool.type == item_name || stack_iter->has_quality(tool.type, tool.level)){
                        ...
                        if(tool.charges != 0 && tool.chargeperc != 0){
                            //charges required
                            if(charges >= tool.charges ||
                               charges >= (int)(item_controller->find_template(tool.type)->charges_to_use() / 100 * tool.chargeperc){
...
//                    }
                    ...
                    component &comp = *comp_it;
                     ...
                     if(comp.type == item_name || stack_iter->has_quality(comp.type, comp.level)){
                        ...
                        if(comp.charges != 0 && comp.chargeperc != 0){
                            //charges required
                            if(charges >= comp.charges ||
                               charges >= (int)(item_controller->find_template(comp.type)->charges_to_use() / 100 * comp.chargeperc){
    ...
    std::vector< item > items_used; //this will keep track of items selected for the requirements
        ...
        while (tool_it != tool_choices.end()) {
            ...
            if(tool.items_assigned.empty()){
                ...
                while (item_it != tool.items_assigned.end()) {
                    ...
                    if(!items_used.empty() && items_used.find(item_it) != items_used.end()){
