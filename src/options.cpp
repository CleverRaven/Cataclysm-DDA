...
options_manager::options_manager() : general_page_("general", to_translation("General")),
...
std::string options_manager::cOpt::getDefaultText(const bool bTranslated) const
{
    if (sType == "string_select")
    {
        ...
        const std::string defaultName = iter == vItems.end() ? std::string() : bTranslated ? iter->second.translated()
                                                                                           : iter->first;
        const std::string &sItems = enumerate_as_string(
...
void options_manager::cOpt::setNext()
{
    ...
    else if (sType == "string_input")
    {
        ...
        string_input_popup()
            .width(iMaxLength > 80 ? 80 : iMaxLength < iMenuTextLength ? iMenuTextLength
                                                                       : iMaxLength + 1)
...
void options_manager::add_options_general()
{
    ...
    add("SOUNDPACKS", "general", to_translation("Choose soundpack"),
        ...
        build_soundpacks_list(), "basic", COPT_NO_SOUND_HIDE); // populate the options dynamically
...
void options_manager::add_options_graphics()
{
    ...
    add("TILES", "graphics", to_translation("Choose tileset"),
        ...
        build_tilesets_list(), "UltimateCataclysm", COPT_CURSES_HIDE); // populate the options dynamically
    ...
    add("OVERMAP_TILES", "graphics", to_translation("Choose overmap tileset"),
        ...
        build_tilesets_list(), "retrodays", COPT_CURSES_HIDE); // populate the options dynamically
    ...
    add("PIXEL_MINIMAP_MODE", "graphics", to_translation("Pixel minimap drawing mode"),
        to_translation("Specified the mode in which the minimap drawn."), {{"solid", to_translation("Solid")}, {"squares", to_translation("Squares")}, {"dots", to_translation("Dots")}}, "dots", COPT_CURSES_HIDE);
    ...
    add("SCALING_FACTOR", "graphics", to_translation("Scaling factor"),
        to_translation("Factor by which to scale the display.  Requires restart."), {{"1", to_translation("1x")}, {"2", to_translation("2x")}, {"4", to_translation("4x")}},
        "1", COPT_CURSES_HIDE);
#endif
}
...
void options_manager::add_options_world_default()
{
    ...
    add("WORLD_END", "world_default", to_translation("World end handling"),
        to_translation("Handling of game world when last character dies."), {{"reset", to_translation("Reset")}, {"delete", to_translation("Delete")}, {"query", to_translation("Query")}, {"keep", to_translation("Keep")}}, "reset");
...
std::string options_manager::show(bool ingame, const bool world_options_only, bool with_tabs)
{
    ...
    options_container &ACTIVE_WORLD_OPTIONS = world_options.has_value() ? *world_options.value() : OPTIONS;
    ...
    for (auto &iter : OPTIONS_OLD)
    {
        if (iter.second != OPTIONS[iter.first])
        {
            ...
            if (iter.first == "PIXEL_MINIMAP_HEIGHT" || iter.first == "PIXEL_MINIMAP_RATIO" || iter.first == "PIXEL_MINIMAP_MODE" || iter.first == "PIXEL_MINIMAP_SCALE_TO_FIT")
            {
