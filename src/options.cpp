...
bool android_get_default_setting(const char* settings_name, bool default_value) {
    ...
    jboolean ans = env->CallBooleanMethod(activity, method_id, env->NewStringUTF(settings_name), default_value);
...
#ifdef __ANDROID__
         android_get_default_setting("Use tiles", false), COPT_CURSES_HIDE // take default setting from pre-game settings screen - important as many devices lack memory to run with tiles
#else
        true, COPT_CURSES_HIDE
#endif
        );
...
#ifdef __ANDROID__
         android_get_default_setting("Software rendering", false), COPT_CURSES_HIDE // take default setting from pre-game settings screen - important as both software + hardware rendering have issues with specific devices
#else
