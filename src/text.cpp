...
static std::string trim_substring( const std::string &text, float width )
{
    std::string ellipsis = "\u2026";
    ...
    do {
        std::string trimmed_str = text.substr( 0, visible_chars ) + ellipsis;
...
static std::string trim_by_width( const std::string &text, float width )
{
    float text_width = ImGui::CalcTextSize( text.c_str(), nullptr, true ).x;
    ...
    for( size_t i = 0; i < color_segments.size(); ++i ) {
        ...
        if( i + 1 < color_segments.size() ) {
            char buf[5];
            ImTextCharToUtf8( buf, UTF8_getch( remove_color_tags( color_segments[i + 1] ) ) );
            next_char_width = ImGui::CalcTextSize( buf, nullptr, true ).x;
        }
...
void TextColoredTrimmed( const std::string &text, nc_color default_color, float width )
{
    draw_colored_text( trim_by_width( text, width ), default_color );
}
