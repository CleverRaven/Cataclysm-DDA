...
void vehicle::control_doors()
{
    ...
    doors_with_motors.reserve( door_motors.size() *
                               2 ); // it is possible to have one door to open and one to close for single motor
...
void vehicle::use_controls( const tripoint &pos )
{
    ...
    if (!veh->is_furniture) {
        ...
        actions.push_back([&] {
            if (tracking_on)
            {
                overmap_buffer.remove_vehicle(this);
                tracking_on = false;
                add_msg(_("You stop keeping track of the furniture position."));
            }
            else
            {
                overmap_buffer.add_vehicle(this);
                tracking_on = true;
                add_msg(_("You start keeping track of this furniture's position."));
            }
        });
    }
    ...
    if( has_electronic_controls && ( camera_on || ( has_part( "CAMERA" ) &&
                                     has_part( "CAMERA_CONTROL" ) ) ) ) {
    ...
    } else { menu.text = _("Furniture controls"); }
 ...
 */
int vehicle::print_part_desc( WINDOW *win, int y1, const int max_y, int width, int p,
                              int hl /*= -1*/ ) const
{
    if( p < 0 || p >= ( int )parts.size() ) {
        return y1;
 ...
 */
void vehicle::print_fuel_indicators( WINDOW *win, int y, int x, int start_index, bool fullsize,
                                     bool verbose, bool desc, bool isHorizontal ) const
{
 ...
 * @param desc true if the name of the fuel should be at the end
 */
void vehicle::print_fuel_indicator( void *w, int y, int x, itype_id fuel_type, bool verbose,
                                    bool desc ) const
{
