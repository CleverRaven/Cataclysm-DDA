   ...
   if (has_overhead_lights) {
    ...
    case toggle_fridge:
        if( !fridge_on || fuel_left(fuel_type_battery) ) {
            fridge_on = !fridge_on;
            Messages::player_messages.add_msg((fridge_on) ? _("Fridge turned on") :
                       _("Fridge turned off"));
        } else {
            Messages::player_messages.add_msg(_("The fridge won't turn on!"));
        }
        break;
    case toggle_recharger:
        if( !recharger_on || fuel_left(fuel_type_battery) ) {
            recharger_on = !recharger_on;
            Messages::player_messages.add_msg((recharger_on) ? _("Recharger turned on") :
                       _("Recharger turned off"));
        } else {
            Messages::player_messages.add_msg(_("The recharger won't turn on!"));
        }
        break;
    case toggle_reactor:
        if(!reactor_on || fuel_left(fuel_type_plutonium)) {
            reactor_on = !reactor_on;
            Messages::player_messages.add_msg((reactor_on) ? _("Reactor turned on") :
                       _("Reactor turned off"));
        }
        else {
            Messages::player_messages.add_msg(_("The reactor won't turn on!"));
        }
        break;
    case toggle_engine:
        if (g->u.controlling_vehicle) {
            if (engine_on) {
                engine_on = false;
                Messages::player_messages.add_msg(_("You turn the engine off and let go of the controls."));
            } else {
            ...
            engine_on = false;
            Messages::player_messages.add_msg(_("You turn the engine off."));
        } else {
          if (total_power () < 1) {
              if (total_power (false) < 1) {
                  Messages::player_messages.add_msg (_("The %s doesn't have an engine!"), name.c_str());
              } else if( has_pedals ) {
                  Messages::player_messages.add_msg (_("The %s's pedals are out of reach!"), name.c_str());
              } else if( has_hand_rims ) {
                  Messages::player_messages.add_msg (_("The %s's hand rims are out of reach!"), name.c_str());
              } else {
    ...
    {
        if(g->u.controlling_vehicle) {
            Messages::player_messages.add_msg(_("As the pitiless metal bars close on your nether regions, you reconsider trying to fold the %s while riding it."), name.c_str());
            break;
        }
        Messages::player_messages.add_msg(_("You painstakingly pack the %s into a portable configuration."), name.c_str());
...
void vehicle::start_engine()
{
    ...
    if(engine_on == true) {
        Messages::player_messages.add_msg(_("The %s's engine starts up."), name.c_str());
    } else if (!muscle_powered) {
        Messages::player_messages.add_msg (_("The %s's engine fails to start."), name.c_str());
    }
}
...
void vehicle::power_parts ()//TODO: more categories of powered part!
{
    ...
    if(reactor_on && battery_discharge - epower > 0) {
        ...
        if(reactors_epower > 0) {
            ...
            if(player_in_control(&g->u) || g->u_see(global_x(), global_y())) {
                Messages::player_messages.add_msg(_("The %s's reactor dies!"), name.c_str());
            }
        }
    }
    ...
    if(battery_deficit) {
        ...
        if(gas_epower < 0) {
            ...
            engine_on = false;
            if(player_in_control(&g->u) || g->u_see(global_x(), global_y())) {
                Messages::player_messages.add_msg("The %s's engine dies!",name.c_str());
            }
        }
    }
}
...
void vehicle::idle() {
    ...
    if( engine_on && total_power() > 0 && !has_pedals && !has_hand_rims ) {
        ...
        if (g->u_see(global_x(), global_y()) && engine_on) {
            Messages::player_messages.add_msg(_("The %s's engine dies!"), name.c_str());
        }
        engine_on = false;
    }
...
void vehicle::thrust (int thd) {
    ...
    if (thrusting && rng(1, accel) <= vel_inc )
    {
        if (total_power () < 1)
        {
          ...
          Messages::player_messages.add_msg (_("The %s's engine isn't on!"), name.c_str());
          cruise_velocity = 0;
          return;
...
void vehicle::handle_trap (int x, int y, int part)
{
    ...
    if (!msg.empty() && g->u_see(x, y)) {
        Messages::player_messages.add_msg (msg.c_str(), name.c_str(), part_info(part).name.c_str(), traplist[t]->name.c_str());
    }
    if (noise > 0) {
...
void vehicle::gain_moves()
{
    ...
    if( turret_mode ) { // handle turrets
        ...
        if( !can_fire ) {
            if( player_in_control(&g->u) || g->u_see(global_x(), global_y()) ) {
                Messages::player_messages.add_msg( _("The %s's turrets run out of ammo and switch off."), name.c_str() );
            }
           turret_mode = 0;
        }
    }
                    ...
                    if(parts_in_square[index] != p) {
                        if(parts[parts_in_square[index]].hp == 0) {
                            ...
                            if(g->u_see(x_pos, y_pos)) {
                                Messages::player_messages.add_msg(_("The %s's %s breaks into pieces!"), name.c_str(),
...
bool vehicle::fire_turret_internal (int p, it_gun &gun, it_ammo &ammo, long charges, const std::string &extra_sound)
{
    ...
    if (target == NULL) {
        if (u_see) {
            Messages::player_messages.add_msg(ngettext("%s points in your direction and emits an IFF warning beep.",
