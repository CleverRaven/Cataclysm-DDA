...
void vehicle::control_turrets() {
    ...
    while( true ) {
        ...
        for( int i = 0; i < (int)turrets.size(); i++ ) {
            ...
            if( parts[p].mode == 0 ) {
                sym = ' ';
            } else if( parts[p].mode > 9 && gun.gun_has_mode( "AUTO" ) ) {
                sym = 'B'; // Burst
...
void vehicle::cycle_turret_mode( int p, bool only_manual_modes )
{
    ...
    if( tr.mode < -1 && !auto_only ) {
        ...
        tr.mode = gun.gun_has_mode( "AUTO" ).qty > 1 ? 1000 : -1;
    } else {
        tr.mode = gun.gun_has_mode( "AUTO" ).qty > 1 ? -1000 : -1;
    }
...
bool vehicle::fire_turret( int p, bool manual )
{
    ...
    if( abs(parts[p].mode) > 1 ){
        charges *= std::max(turret_data.gun.gun_has_mode( "AUTO" ).qty, 1 );
...
int vehicle::automatic_fire_turret( int p, item& gun  )
{
    ...
    const int to_fire = std::min( abs(parts[p].mode), std::max( gun.gun_has_mode( "AUTO" ).qty, 1 ) );
...
int vehicle::manual_fire_turret( int p, player &shooter, item &gun )
{
    ...
    if( !trajectory.empty() ) {
        ...
        int to_fire = abs(parts[p].mode) > 1 ? std::max( gun.gun_has_mode( "AUTO" ).qty, 1 ) : 1;
