...
void mission_end::evac_construct_5(mission *miss)
{
    ...
    char direction;
    if (overmap_buffer.find_closest(site, "evac_center_19_north",1,true) != overmap::invalid_tripoint) {
        //dbg(D_ERROR) << "NORTH SITE FOUND"; //TODO: delete debug
        direction = 'N';
    }
    else if (overmap_buffer.find_closest(site, "evac_center_19_east", 1, true) != overmap::invalid_tripoint) {
        //dbg(D_ERROR) << "EAST SITE FOUND"; //TODO: delete debug
        direction = 'E';  
    }
    else if (overmap_buffer.find_closest(site, "evac_center_19_south", 1, true) != overmap::invalid_tripoint) {
        //dbg(D_ERROR) << "SOUTH SITE FOUND"; //TODO: delete debug
        direction = 'S';  
    }
    else if (overmap_buffer.find_closest(site, "evac_center_19_west", 1, true) != overmap::invalid_tripoint) {
    ...
    for (int i = 0; i < 18; i++) {
        ...
        case 'E':
            ...
            break;
        case 'S':
            storageRoom.spawn_item(max_tile_index - x_coords[i], max_tile_index - y_coords[i], item[i], quantity[i]);
            break;
        case 'W':
