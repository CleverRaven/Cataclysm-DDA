...
w_point weather_generator::get_weather(double x, double y, calendar t) {
    ...
    double H2;
    double P;
    T = Temperature.noise(x, y, z) * 8.0;
    H = Humidity.noise(x, y, z / 5);
    H2 = Humidity.noise(x, y, z) / 4;
    P = Pressure.noise(x, y, z / 3) * 70;
    // temperature variation
    double now = (day_of_year + (minute_of_day / 1440)) / this->year_length; // Add the minutes to the day and return the current time as a decimal [0-1].
    ...
    double daily_variation = cos( 2 * pi * minute_of_day / 1440 ) * season_atenuation + season_dispersion; // Day-night temperature variation.
