...
struct generator_instance {
    ...
    // should be obsoleted in favor of json definition
    static void riot_damage( map &md, const tripoint_abs_omt &p, bool is_a_road );

    static void aftershock_ruin( map &md, const tripoint_abs_omt &p );
...
class pp_generator
{
    public:
        ...
        void load_pp_generator( const JsonObject &jo, const std::string &src );

        static void apply_pp_generators( map &md, std::list<tripoint_bub_ms> &all_points_in_map );
