...
void npc::move_to( const tripoint &pt, bool no_bashing, std::set<tripoint> *nomove )
{
    ...
    if( p.z != posz() ) {
        ...
        moves -= 100;
        moved = true;
    } else if( here.passable( p ) && !here.has_flag( flag_DOOR, p ) ) {
    ...
    } else if( get_dex() > 1 && here.has_flag_ter_or_furn( flag_CLIMBABLE, p ) ) {
...
void npc::worker_downtime()
{
    ...
    if( here.has_flag_furn( flag_CAN_SIT, pos() ) ) {
        // just chill here
        move_pause();
        return;
    }
    ...
    if( chair_pos != tripoint_min ) {
        if( here.has_flag_furn( flag_CAN_SIT, here.getlocal( chair_pos ) ) ) {
        ...
        if( !is_mounted() ) {
            for( const tripoint &elem : here.points_in_radius( pos(), 30 ) ) {
                if( here.has_flag_furn( flag_CAN_SIT, elem ) && !creatures.creature_at( elem ) &&
...
void npc::go_to_omt_destination()
{
    ...
    add_msg_debug( debugmode::DF_NPC, "%s going %s->%s", name, omt_pos.to_string_writable(),
