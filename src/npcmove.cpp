    ...
    */
    ...
    case npc_reload: {
        ...
        if (g->u_see(posx, posy)) {
            Messages::add_msg(_("%s reloads their %s."), name.c_str(),
                                              weapon.tname().c_str());
        }
    }
    break;
...
void npc::pick_up_item()
{
    ...
    if (u_see_me) {
        if (pickup.size() == 1) {
            if (u_see_items)
                Messages::add_msg(_("%s picks up a %s."), name.c_str(),
                           (*items)[pickup[0]].tname().c_str());
            else {
                Messages::add_msg(_("%s picks something up."), name.c_str());
            }
        } else if (pickup.size() == 2) {
            if (u_see_items)
                Messages::add_msg(_("%s picks up a %s and a %s."), name.c_str(),
                           (*items)[pickup[0]].tname().c_str(),
                           (*items)[pickup[1]].tname().c_str());
            else {
                Messages::add_msg(_("%s picks up a couple of items."), name.c_str());
            }
        } else {
            Messages::add_msg(_("%s picks up several items."), name.c_str());
        }
    } else if (u_see_items) {
        if (pickup.size() == 1) {
            Messages::add_msg(_("Someone picks up a %s."),
                                              (*items)[pickup[0]].tname().c_str());
        } else if (pickup.size() == 2)
            Messages::add_msg(_("Someone picks up a %s and a %s"),
                       (*items)[pickup[0]].tname().c_str(),
                       (*items)[pickup[1]].tname().c_str());
        else {
            Messages::add_msg(_("Someone picks up several items."));
        }
    }
...
void npc::drop_items(int weight, int volume)
{
    ...
    if (g->u_see(posx, posy)) {
        if (num_items_dropped >= 3) {
            Messages::add_msg(ngettext("%s drops %d item.", "%s drops %d items.",
                                                       num_items_dropped), name.c_str(),
                                              num_items_dropped);
        } else {
            Messages::add_msg(_("%s drops a %s."), name.c_str(),
                                              item_name_str.c_str());
        }
    }
...
void npc::alt_attack(int target)
{
    ...
    if (!thrown_item(used)) {
        ...
        if (dist <= confident_range(invlet) && wont_hit_friend(tarx, tary, invlet)) {
            ...
            moves -= 125;
            if (g->u_see(posx, posy)) {
                Messages::add_msg(_("%s throws a %s."),
            ...
            if (!used->active || used->charges > 2) { // Safe to hold on to, for now
                ...
                moves -= 125;
                if (g->u_see(posx, posy)) {
                    Messages::add_msg(_("%s throws a %s."), name.c_str(),
                                                      used->tname().c_str());
                }
...
void npc::heal_player(player &patient)
{
    ...
    if (dist > 1) { // We need to move to the player
        ...
        if (patient.is_npc()) {
            if (u_see_me) {
                if (u_see_patient) {
                    Messages::add_msg(_("%s heals %s."),
                                                      name.c_str(), patient.name.c_str());
                } else {
                    Messages::add_msg(_("%s heals someone."), name.c_str());
                }
            } else if (u_see_patient) {
                Messages::add_msg(_("Someone heals %s."), patient.name.c_str());
            }
        } else if (u_see_me) {
            Messages::add_msg(_("%s heals you."), name.c_str());
        } else {
            Messages::add_msg(_("Someone heals you."));
        }
...
void npc::heal_self()
{
    ...
    if (g->u_see(posx, posy)) {
        Messages::add_msg(_("%s heals %s."), name.c_str(),
...
void npc::use_painkiller()
{
    ...
    if (it->is_null()) {
        ...
        if (g->u_see(posx, posy)) {
            Messages::add_msg(_("%s takes some %s."), name.c_str(), it->name.c_str());
        }
    ...
    if (rl_dist(posx, posy, mark.posx, mark.posy) > 1) { // We have to travel
        ...
        if (mark.cash > 0) {
            ...
            if (mark.is_npc()) {
                if (u_see_me) {
                    if (u_see_mark) {
                        Messages::add_msg(_("%s takes %s's money!"),
                                                          name.c_str(), mark.name.c_str());
                    } else {
                        Messages::add_msg(_("%s takes someone's money!"),
                                                          name.c_str());
                    }
                } else if (u_see_mark) {
                    Messages::add_msg(_("Someone takes %s's money!"),
                                                      mark.name.c_str());
                }
            } else {
                if (u_see_me) {
                    Messages::add_msg(_("%s takes your money!"), name.c_str());
                } else {
                    Messages::add_msg(_("Someone takes your money!"));
                }
            }
            ...
            if (invlet == 0) { // Didn't find anything worthwhile!
                ...
                if (mark.is_npc()) {
                    if (u_see_me) {
                        if (u_see_mark)
                            Messages::add_msg(_("%s takes %s's %s."), name.c_str(),
                                                              mark.name.c_str(),
                                                              stolen.tname().c_str());
                        else {
                            Messages::add_msg(_("%s takes something from somebody."),
                                                              name.c_str());
                        }
                    } else if (u_see_mark)
                        Messages::add_msg(_("Someone takes %s's %s."),
                                                          mark.name.c_str(), stolen.tname().c_str());
                } else {
                    if (u_see_me) {
                        Messages::add_msg(_("%s takes your %s."),
                                                          name.c_str(), stolen.tname().c_str());
                    } else {
                        Messages::add_msg(_("Someone takes your %s."),
                                                          stolen.tname().c_str());
                    }
                }
                i_add(stolen);
