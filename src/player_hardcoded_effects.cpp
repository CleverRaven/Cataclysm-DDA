...
void player::hardcoded_effects( effect &it )
{
    ...
    if( id == effect_dermatik ) {
        ...
        if (calendar::once_every(10 _minutes) && has_trait(trait_id("CHLOROMORPH")) && g - > m.is_outside(pos()) && g - > natural_light_level(posz()) >= 12 && (g - > weather == WEATHER_CLEAR || g - > weather == WEATHER_SUNNY || g - > weather == WEATHER_DRIZZLE || g - > weather == WEATHER_RAINY || g - > weather == WEATHER_FLURRIES || g - > weather == WEATHER_CLOUDY || g - > weather == WEATHER_SNOW)) {
