...
void player::hardcoded_effects( effect &it )
{
    ...
    if( id == effect_dermatik ) {
        ...
        if( calendar::once_every( 10_minutes ) && has_trait( trait_id( "CHLOROMORPH" ) ) &&
            g->is_outside( pos() ) && weather_type( WEATHER_SUNNY ) || weather_type( WEATHER_CLEAR ) || weather_type( WEATHER_CLOUDY ) ||  weather_type( WEATHER_DRIZZLE ) || weather_type( WEATHER_RAINY ) || weather_type( WEATHER_CLOUDY ) || weather_type( WEATHER_FLURRIES ) || weather_type( WEATHER_SNOW ) ) {
