...
int iuse::flu_vaccine( player *p, item *it, bool, const tripoint & )
{
    ...
    p->add_effect( effect_flushot, 30_days, default_bp, true );
    p->mod_pain( 3 );
    item syringe( "syringe", it->birthday() );
    p->i_add( syringe );
    return it->type->charges_to_use();
...
int iuse::blech( player *p, item *it, bool, const tripoint & )
{
    ...
    if( it->has_flag( flag_ACID ) && ( p->has_trait( trait_ACIDPROOF ) ||
        ...
        p->add_effect( effect_poison, 1_hours );
        p->apply_damage( nullptr, torso_bp, rng( 4, 12 ) );
        p->vomit();
    }
    return it->type->charges_to_use();
}
        ...
        switch( animal_food_type ) {
            case DOGFOOD:
                if( mon.type->id == mon_dog_thing ) {
                    p.deal_damage( &mon, hand_r_bp, damage_instance( DT_CUT, rng( 1, 10 ) ) );
...
int iuse::extinguisher( player *p, item *it, bool, const tripoint & )
{
    ...
    if( monster *const mon_ptr = g->critter_at<monster>( dest, true ) ) {
        ...
        if( critter.made_of( LIQUID ) ) {
            ...
            critter.apply_damage( p, torso_bp, rng( 20, 60 ) );
...
int iuse::tazer( player *p, item *it, bool, const tripoint &pos )
{
    ...
    if( tazer_was_dodged ) {
        ...
        int dam = target->deal_damage( p, torso_bp, damage_instance( DT_ELECTRIC, rng( 5,
                                       25 ) ) ).total_damage();
        if( dam > 0 ) {
...
int iuse::artifact( player *p, item *it, bool, const tripoint & )
{
    ...
    for( size_t i = 0; i < num_used && !effects.empty(); i++ ) {
        ...
        switch( used ) {
                ...
                if( bug ) {
                    for( int j = 0; j < num; j++ ) {
                        if( monster *const b = g->place_critter_around( bug, p->pos(), 1 ) ) {
                            b->friendly = -1;
                            b->add_effect( effect_pet, 1_turns, default_bp, true );
                        }
                    }
                }
            }
            ...
            case AEA_HURTALL:
                for( monster &critter : g->all_monsters() ) {
                    critter.apply_damage( nullptr, torso_bp, rng( 0, 5 ) );
                }
                break;
...
int iuse::jet_injector( player *p, item *it, bool, const tripoint & )
{
    if( !it->ammo_sufficient() ) {
        ...
        p->add_effect( effect_jetinjector, 20_minutes, default_bp, false, 2 );
        p->mod_painkiller( 20 );
        p->mod_stim( 10 );
        p->healall( 20 );
    }
...
int iuse::stimpack( player *p, item *it, bool, const tripoint & )
{
    ...
    if( !it->ammo_sufficient() ) {
        ...
        // Intensity is 2 here because intensity = 1 is the comedown
        p->add_effect( effect_stimpack, 25_minutes, default_bp, false, 2 );
        p->mod_painkiller( 2 );
        p->mod_stim( 20 );
        p->mod_fatigue( -100 );
        ...
        if( ( it->ammo_remaining() > it->type->maximum_charges() - 1000 ) && ( x != pos.x ||
            ...
            if( p->has_item( *it ) && p->weapon.typeId() == "e_handcuffs" ) {
                ...
                if( p->is_elec_immune() ) {
                    ...
                    p->apply_damage( nullptr, arm_l_bp, rng( 0, 2 ) );
                    p->apply_damage( nullptr, arm_r_bp, rng( 0, 2 ) );
                    p->mod_pain( rng( 2, 5 ) );
        ...
        case 6:
            if( !one_in( 5 ) ) {
                ...
                if( monster *const m = g->place_critter_around( mon_hallu_multicooker, p.pos(), 1 ) ) {
                    ...
                    m->add_effect( effect_run, 1_turns, default_bp, true );
                }
            } else {
