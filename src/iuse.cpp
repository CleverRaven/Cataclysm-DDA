...
int iuse::flu_vaccine( player *p, item *it, bool, const tripoint & )
{
    ...
    if( remaining_time > 0_turns ) {
        ...
        p->add_effect( effect_flushot, remaining_time, bodypart_id( "bp_null" ), false );
    } else {
        p->add_msg_if_player( m_bad,
...
int iuse::antiasthmatic( player *p, item *it, bool, const tripoint & )
{
    ...
    p->add_effect( effect_took_antiasthmatic, 1_days, bodypart_id( "bp_null" ), true );
    return it->type->charges_to_use();
}
...
static int feedpet( player &p, monster &mon, item &it, m_flag food_flag, const char *message )
{
    if( mon.has_flag( food_flag ) ) {
        ...
        mon.friendly = -1;
        mon.add_effect( effect_pet, 1_turns, bodypart_id( "bp_null" ), true );
    ...
    for( monster &critter : g->all_monsters() ) {
        if( critter.friendly != 0 && critter.has_flag( MF_DOGFOOD ) ) {
            ...
            if( critter.has_effect( effect_docile ) ) {
                ...
                critter.add_effect( effect_docile, 1_turns, bodypart_id( "bp_null" ), true );
            }
        }
    }
    return it->type->charges_to_use();
    ...
    for( size_t i = 0; i < num_used && !effects.empty(); i++ ) {
        ...
        switch( used ) {
            case AEA_STORM: {
                ...
                for( int j = 0; j < num_bolts; j++ ) {
                    ...
                    for( int n = 0; n < dist; n++ ) {
                        bolt.x += dir.x;
                        bolt.y += dir.y;
                        here.add_field( {bolt, p->posz()}, fd_electricity, rng( 2, 3 ) );
                        if( one_in( 4 ) ) {
                            if( dir.x == 0 ) {
                                dir.x = rng( 0, 1 ) * 2 - 1;
                            } else {
            ...
            case AEA_FATIGUE: {
                ...
                here.add_field( {p2, p->posz()}, fd_fatigue, rng( 1, 2 ) );
            }
            break;

            case AEA_ACIDBALL: {
                if( const cata::optional<tripoint> acidball = g->look_around() ) {
                    for( const tripoint &tmp : here.points_in_radius( *acidball, 1 ) ) {
                        here.add_field( tmp, fd_acid, rng( 2, 3 ) );
                    }
                }
            }
            break;
            ...
            case AEA_PULSE:
                sounds::sound( p->pos(), 30, sounds::sound_t::combat, _( "The earth shakes!" ), true, "misc",
                               "earthquake" );
                for( const tripoint &pt : here.points_in_radius( p->pos(), 2 ) ) {
                    here.bash( pt, 40 );
                    here.bash( pt, 40 );  // Multibash effect, so that doors &c will fall
                    here.bash( pt, 40 );
                    if( here.is_bashable( pt ) && rng( 1, 10 ) >= 3 ) {
                        here.bash( pt, 999, false, true );
                    }
                }
                break;
            ...
            case AEA_CONFUSED:
                for( const tripoint &dest : here.points_in_radius( p->pos(), 8 ) ) {
                ...
                if( bug ) {
                    for( int j = 0; j < num; j++ ) {
                        if( monster *const b = g->place_critter_around( bug, p->pos(), 1 ) ) {
                            b->friendly = -1;
                            b->add_effect( effect_pet, 1_turns, bodypart_id( "bp_null" ), true );
                        }
                    }
                }
            }
            ...
            case AEA_RADIATION:
                ...
                for( const tripoint &dest : here.points_in_radius( p->pos(), 1 ) ) {
                    here.add_field( dest, fd_nuke_gas, rng( 2, 3 ) );
                }
                break;
...
int iuse::jet_injector( player *p, item *it, bool, const tripoint & )
{
    if( !it->ammo_sufficient() ) {
        ...
        p->add_effect( effect_jetinjector, 20_minutes, bodypart_id( "bp_null" ), false, 2 );
        p->mod_painkiller( 20 );
        p->mod_stim( 10 );
        p->healall( 20 );
    }
...
int iuse::stimpack( player *p, item *it, bool, const tripoint & )
{
    ...
    if( !it->ammo_sufficient() ) {
        ...
        // Intensity is 2 here because intensity = 1 is the comedown
        p->add_effect( effect_stimpack, 25_minutes, bodypart_id( "bp_null" ), false, 2 );
        p->mod_painkiller( 2 );
        p->mod_stim( 20 );
        p->mod_fatigue( -100 );
    ...
    if( choice < 0 ) {
        ...
        if( it->active ) {
            ...
            for( auto &rc_pairs_rc_pair : rc_pairs ) {
                if( rc_pairs_rc_pair.second->has_flag( "RADIOCAR" ) &&
        ...
        case 6:
            if( !one_in( 5 ) ) {
                ...
                if( monster *const m = g->place_critter_around( mon_hallu_multicooker, p.pos(), 1 ) ) {
                    ...
                    m->add_effect( effect_run, 1_turns, bodypart_id( "bp_null" ), true );
                }
            } else {
