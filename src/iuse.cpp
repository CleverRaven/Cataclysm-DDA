                              ...
                              bool carveable)
{
    ...
    if (!cut->made_of(SOLID)) {
        ...
        Messages::add_msg(_("You can't %s an item that's not solid!"), lower_verb.c_str());
        return false;
    }
    if(carveable && !(cut->made_of("wood") || cut->made_of("plastic") || cut->made_of("glass") ||
        ...
        Messages::add_msg(_("You can't %1$s an item made of %2$s!"),
...
int iuse::sew(player *p, item *, bool)
{
    ...
    if (p->fine_detail_vision_mod() > 4) {
        Messages::add_msg(_("You can't see to sew!"));
        return 0;
    }
    int thread_used = 1;
...
static bool valid_fabric(player *p, item *it, bool)
{
    ...
    if (it->type->id == "string_6" || it->type->id == "string_36" || it->type->id == "rope_30" ||
        it->type->id == "rope_6") {
        Messages::add_msg(_("You cannot cut that, you must disassemble it using the disassemble key"));
        return false;
    }
    ...
    if (!it->made_of("cotton") && !it->made_of("leather") && !it->made_of("nomex")) {
        Messages::add_msg(_("You can only slice items made of fabric or leather."));
        return false;
    }
    if (it->is_container() && !it->contents.empty()) {
        Messages::add_msg(_("That %s is not empty!"), it->tname().c_str());
        return false;
    }
...
int iuse::directional_antenna(player *p, item *it, bool)
{
    ...
    {
        Messages::add_msg( _("Must have an active radio to check for signal direction.") );
        return 0;
    }
    ...
    if( tower == NULL )
    {
        Messages::add_msg( _("You can't find the direction if your radio isn't tuned.") );
        return 0;
    }
    // Report direction.
    direction angle = direction_from( g->levx, g->levy, tower->x, tower->y );
    Messages::add_msg( _("The signal seems strongest to the %s."), direction_name(angle).c_str());
    return it->type->charges_to_use();
}
...
int iuse::picklock(player *p, item *it, bool)
{
 ...
 if (type == t_chaingate_l) {
   ...
   Messages::add_msg(_("That door isn't locked."));
   return 0;
 } else {
  Messages::add_msg(_("That cannot be picked."));
  return 0;
 }
...
int iuse::siphon(player *p, item *it, bool)
{
    ...
    if ( fillv != NULL ) {
        ...
        Messages::add_msg(ngettext("Siphoned %d unit of %s from the %s into the %s%s", "Siphoned %d units of %s from the %s into the %s%s", got), got,
...
int iuse::geiger(player *p, item *it, bool t)
{
    if (t) { // Every-turn use when it's on
        ...
        if (rads > 50) {
            Messages::add_msg(_("The geiger counter buzzes intensely."));
        } else if (rads > 35) {
            Messages::add_msg(_("The geiger counter clicks wildly."));
        } else if (rads > 25) {
            Messages::add_msg(_("The geiger counter clicks rapidly."));
        } else if (rads > 15) {
            Messages::add_msg(_("The geiger counter clicks steadily."));
        } else if (rads > 8) {
            Messages::add_msg(_("The geiger counter clicks slowly."));
        } else if (rads > 4) {
            Messages::add_msg(_("The geiger counter clicks intermittently."));
        } else {
            Messages::add_msg(_("The geiger counter clicks once."));
        }
        return it->type->charges_to_use();;
    }
    ...
    if (is_on) {
        Messages::add_msg(_("The geiger counter's SCANNING LED flicks off."));
...
int iuse::can_goo(player *p, item *it, bool)
{
 ...
 if (mondex != -1) {
  if (g->u_see(goox, gooy)) {
      Messages::add_msg(_("Black goo emerges from the canister and envelopes a %s!"),
  ...
  if (g->u_see(goox, gooy)) {
   Messages::add_msg(_("Living black goo emerges from the canister!"));
  }
 ...
 while (!one_in(4) && tries < 10) {
  ...
  if (tries < 10) {
   if (g->u_see(goox, gooy)) {
    Messages::add_msg(_("A nearby splatter of goo forms into a goo pit."));
   }
...
int iuse::pipebomb_act(player *, item *it, bool t)
{
    ...
    if (t) { // Simple timer effects
        ...
        Messages::add_msg(_("You've already lit the %s, try throwing it instead."), it->name.c_str());
        return 0;
    } else { // The timer has run down
        if (one_in(10) && g->u_see(pos.x, pos.y)) {
            Messages::add_msg(_("The pipe bomb fizzles out."));
        } else {
...
int iuse::granade_act(player *, item *it, bool t)
{
    ...
    if (t) { // Simple timer effects
        ...
        Messages::add_msg(_("You've already pulled the %s's pin, try throwing it instead."), it->name.c_str());
        return 0;
...
int iuse::pheromone(player *p, item *it, bool)
{
 ...
 if (g->u_see(p)) {
  if (converts == 0) {
   Messages::add_msg(_("...but nothing happens."));
  } else if (converts == 1) {
   Messages::add_msg(_("...and a nearby zombie turns friendly!"));
  } else{
   Messages::add_msg(_("...and several nearby zombies turn friendly!"));
  }
 }
 return it->type->charges_to_use();
}
...
int iuse::knife(player *p, item *it, bool t)
{
    ...
    if (cut == it)
    {
        Messages::add_msg(_("You can not cut the %s with itself!"), it->tname().c_str());
        return 0;
    }
    if (cut == &p->weapon)
    {
    ...
    if(amount == 0) {
        Messages::add_msg(_("This object is too small to salvage a meaningful quantity of anything from!"));
        return 0;
    }
    ...
    if (cut->is_container() && !cut->contents.empty()) {
        Messages::add_msg(_("That container is not empty!"));
        return 0;
    }
    ...
    if ((cut->made_of("cotton") || cut->made_of("leather") || cut->made_of("nomex")) ) {
        ...
        Messages::add_msg(_("Material of this item is not applicable for cutting up."));
        return 0;
    }
    // check again
    if ( result == NULL ) {
    ...
    if ( cut->typeId() == result->typeId() ) {
        Messages::add_msg(_("There's no point in cutting a %s."), cut->tname().c_str());
        return 0;
    }
    ...
    if (action == "carve") {
        if(count > 0) {
            Messages::add_msg(ngettext("You carve the %1$s into %2$i %3$s.",
                            "You carve the %1$s into %2$i %3$ss.", count),
                   cut->tname().c_str(), count, result->tname().c_str());
        } else {
            Messages::add_msg("You clumsily carve the %s into useless pieces.",
                       cut->tname().c_str());
        }
    } else {
        if(count > 0){
            Messages::add_msg(ngettext("You cut the %1$s into %2$i %3$s.",
                            "You cut the %1$s into %2$i %3$ss.", count),
                   cut->tname().c_str(), count, result->tname().c_str());
        } else {
            Messages::add_msg("You clumsily cut the %s into useless pieces.",
                       cut->tname().c_str());
        }
    }
...
{
    p->moves -= 300;
    Messages::add_msg(_("You cut the log into planks."));
    ...
    if (scraps >= planks) {
        Messages::add_msg(_("You waste a lot of the wood."));
    }
...
int iuse::lumber(player *p, item *it, bool)
{
 ...
 if (cut->type->id == "log") {
     ...
     Messages::add_msg(_("You can't cut that up!"));
     return it->type->charges_to_use();
 }
}
...
int iuse::hacksaw(player *p, item *it, bool)
{
    ...
    if (dirx == p->posx && diry == p->posy) {
        Messages::add_msg(_("Why would you do that?"));
        Messages::add_msg(_("You're not even chained to a boiler."));
        return 0;
    }
    ...
    default:
        Messages::add_msg(_("You can't cut that."));
        return 0;
    }
    return it->type->charges_to_use();
}
 ...
 for (int i = -1; i <= 1; i++) {
     for (int j = -1; j <= 1; j++) {
         if (!g->m.has_flag("FLAT", posx + i, posy + j) ||
             ...
             Messages::add_msg(_("You need a 3x3 flat space to place a tent"));
             return 0;
         }
     }
 }
...
int iuse::shelter(player *p, item *, bool)
{
 ...
 for (int i = -1; i <= 1; i++) {
     for (int j = -1; j <= 1; j++) {
         if (!g->m.has_flag("FLAT", posx + i, posy + j) ||
             g->m.has_furn(posx + i, posy + j)) {
             Messages::add_msg(_("You need a 3x3 flat space to place a shelter"));
             return 0;
         }
     }
 }
...
int iuse::bullet_puller(player *p, item *it, bool)
{
    ...
    if (a == bullet_pulling_recipes.end()) {
        Messages::add_msg(_("You cannot disassemble that."));
        return 0;
    }
    if (p->skillLevel("gun") < 2) {
        Messages::add_msg(_("You need to be at least level 2 in the firearms skill before you\
  can disassemble ammunition."));
    ...
    Messages::add_msg(_("You take apart the ammunition."));
    p->moves -= 500;
...
int iuse::mop(player *p, item *it, bool)
{
 ...
 if (g->m.moppable_items_at(dirx, diry)) {
   ...
   Messages::add_msg(_("You mop up the spill."));
   p->moves -= 15;
 } else {
...
int iuse::artifact(player *p, item *it, bool)
{
 ...
 for (size_t i = 0; i < num_used; i++) {
  ...
  case AEA_RADIATION:
   Messages::add_msg(_("Horrible gasses are emitted!"));
   for (int x = p->posx - 1; x <= p->posx + 1; x++) {
    for (int y = p->posy - 1; y <= p->posy + 1; y++)
...
int iuse::radglove(player *p, item *it, bool)
{
    if ( p->get_item_position( it ) >= -1 ) {
        ...
        if (p->radiation >= 1) {
            ...
            Messages::add_msg(_("Your radiation level: %d"), p->radiation);
        } else {
