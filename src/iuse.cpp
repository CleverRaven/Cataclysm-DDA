...
{
  Messages::player_messages.add_msg(_("You can't do anything interesting with your %s."),
             it->tname(g).c_str());
...
int iuse::cig(player *p, item *it, bool t) {
    ...
    if (it->type->id == "cig") {
        p->add_msg_if_player(_("You light a cigarette and smoke it."));
    } else {  // cigar
        p->add_msg_if_player(_("You take a few puffs from your cigar."));
    }
    p->thirst += 2;
    p->hunger -= 3;
...
int iuse::fungicide(player *p, item *it, bool t) {
    p->add_msg_if_player(_("You take some fungicide."));
    if (p->has_disease("fungus")) {
        p->rem_disease("fungus");
...
int iuse::weed(player *p, item *it, bool t) {
    ...
    if (!canSmoke) {
        p->add_msg_if_player(_("You haven't got anything to smoke out of."));
        return 0;
    ...
    if (p->use_charges_if_avail("fire", 1)) {
        ...
        p->add_msg_if_player(_("You need something to light it."));
        return 0;
    }
    return it->type->charges_to_use();
}
...
int iuse::sew(player *p, item *it, bool t)
{
    ...
    if (!fix->is_armor()) {
        p->add_msg_if_player(_("That isn't clothing!"));
        return 0;
    }
    ...
    if(repair_items.empty()) {
        p->add_msg_if_player(_("Your %s is not made of cotton, wool, leather or fur."),
                             fix->tname().c_str());
        return 0;
    }
...
int iuse::extra_battery(player *p, item *it, bool t)
{
    ...
    if (!modded->is_tool())
    {
        p->add_msg_if_player(_("You can only mod tools with this battery mod."));
        return 0;
    }
...
static bool valid_fabric(player *p, item *it, bool t)
{
    ...
    if (!it->made_of("cotton") && !it->made_of("leather")) {
        Messages::player_messages.add_msg(_("You can only slice items made of cotton or leather."));
        return false;
    }
...
int iuse::gasoline_lantern_off(player *p, item *it, bool t)
{
    if (it->charges == 0)
    {
        p->add_msg_if_player(_("The lantern is empty."));
        return 0;
    }
    ...
    {
        p->add_msg_if_player(_("You turn the lantern on."));
...
// Do nothing... player::active_light and the lightmap::generate deal with this
    ...
    {
        p->add_msg_if_player(_("The lantern is extinguished."));
...
{
    if (it->charges == 0) {
        p->add_msg_if_player(_("The flashlight's batteries are dead."));
        return 0;
    } else {
        p->add_msg_if_player(_("You turn the flashlight on."));
...
// Do nothing... player::active_light and the lightmap::generate deal with this
  ...
  p->add_msg_if_player(_("The flashlight flicks off."));
...
{
    p->add_msg_if_player(_("You irreversibly activate the lightstrip."));
...
int iuse::lightstrip_active(player *p, item *it, bool t)
{
    if (t) { // Normal use
        ...
        p->add_msg_if_player(_("The lightstrip dies."));
...
{
    p->add_msg_if_player(_("You activate the glowstick."));
...
int iuse::glowstick_active(player *p, item *it, bool t)
{
    if (t) { // Normal use
        ...
        if (it->charges > 0) {
            p->add_msg_if_player(_("You can't turn off a glowstick."));
            return 0;
        } else {
            p->add_msg_if_player(_("The glowstick fades out."));
            it->active = false;
        }
    }
    return it->type->charges_to_use();
...
{
    p->add_msg_if_player(_("You strike your flare and light it."));
...
int iuse::handflare_lit(player *p, item *it, bool t)
{
    if (t) { // Normal use
        ...
        if (it->charges > 0) {
            p->add_msg_if_player(_("You can't turn off a flare."));
            return 0;
        } else {
            p->add_msg_if_player(_("The flare sputters out."));
            it->active = false;
        }
    }
    return it->type->charges_to_use();
...
int iuse::solder_weld(player *p, item *it, bool t)
{
    ...
    switch (choice)
    {
        ...
        {
            if(it->charges <= 0) {
                p->add_msg_if_player(_("Your repair tool does not have enough charges to do that."));
                return 0;
            }
            ...
            if(repair_items.empty()) {
                p->add_msg_if_player(_("Your %s is not made of kevlar, plastic or metal."),
                                     fix->tname().c_str());
                return 0;
            }
...
"0: Cancel"));
 ...
 if (ch == '1') {
  ...
  if (bonus >= 25) {
   ...
   g->add_event(EVENT_HELP, int(calendar::turn) + fac->response_time(g), fac->id, -1, -1);
...
int iuse::siphon(player *p, item *it, bool t)
{
    ...
    if ( fillv != NULL ) {
        ...
        Messages::player_messages.add_msg(_("Siphoned %d units of %s from the %s into the %s%s"), got,
...
int iuse::shishkebab_off(player *p, item *it, bool t)
{
    ...
    case 1:
    {
        ...
        if (rng(0, 10) - it->damage > 5 && it->charges > 0) {
            ...
            p->add_msg_if_player(_("Aw, dangit."));
        }
        return it->type->charges_to_use();
    }
    break;
...
int iuse::shishkebab_on(player *p, item *it, bool t)
{
    if (t)    // Effects while simply on
    {
        ...
        if (one_in(75))
        {
            p->add_msg_if_player(_("Bummer man, wipeout!")),
    ...
    {
        p->add_msg_if_player(_("Uncool, outta gas."));
        it->make(itypes["shishkebab_off"]);
        it->active = false;
    }
    else
    {
        ...
        case 1:
        {
            p->add_msg_if_player(_("Peace out."));
            it->make(itypes["shishkebab_off"]);
            it->active = false;
        }
        break;
...
{
  p->add_msg_if_player(_("Whoa buddy! You can't go cheating in items and"));
  p->add_msg_if_player(_("just expect them to work! Now put the pickaxe"));
  p->add_msg_if_player(_("down and go play the game."));
  return 0;
}
...
int iuse::geiger(player *p, item *it, bool t)
{
    ...
    switch (ch) {
    case 1: p->add_msg_if_player(_("Your radiation level: %d"), p->radiation); break;
...
int iuse::pipebomb(player *p, item *it, bool t)
{
    ...
    p->add_msg_if_player(_("You light the fuse on the pipe bomb."));
...
{
    p->add_msg_if_player(_("You pull the pin on the grenade."));
...
{
    p->add_msg_if_player(_("You pull the pin on the flashbang."));
...
{
 p->add_msg_if_player(_("You pull the pin on the EMP grenade."));
...
{
    p->add_msg_if_player(_("You pull the pin on the scrambler grenade."));
...
{
    p->add_msg_if_player(_("You pull the pin on the teargas canister."));
...
{
    p->add_msg_if_player(_("You pull the pin on the smoke bomb."));
...
{
 p->add_msg_if_player(_("You remove the divider, and the chemicals mix."));
 p->moves -= 150;
...
int iuse::dynamite(player *p, item *it, bool t)
{
 ...
 p->add_msg_if_player(_("You light the dynamite."));
...
int iuse::matchbomb(player *p, item *it, bool t) {
    ...
    p->add_msg_if_player(_("You light the match head bomb."));
  ...
  if(ch == 'I') {
   ...
   if(charges == it->charges) {
    ...
    if(charges == 1) {
     ...
     p->add_msg_if_player(_("You light a string of %d firecrackers."), charges);
...
int iuse::turret(player *p, item *it, bool t)
{
 ...
 if (rng(0, p->int_cur / 2) + p->skillLevel("electronics") / 2 +
  ...
  p->add_msg_if_player(_("You misprogram the turret; it's hostile!"));
 } else {
  mturret.friendly = -1;
 }
...
int iuse::UPS_off(player *p, item *it, bool t)
{
 if (it->charges == 0) {
  ...
  if (p->is_wearing("goggles_nv"))
   p->add_msg_if_player(_("Your light amp goggles power on."));
    ...
    if ((cut->made_of("cotton") || cut->made_of("leather")) ) {
        ...
        if(cut->is_container()) {
            if(cut->is_food_container()) {
                Messages::player_messages.add_msg(_("That container has liquid in it!"));
                return 0;
            }
        }
...
{
    if (!p->use_charges_if_avail("fire", 1)) {
        p->add_msg_if_player(_("You need a lighter or fire to light this."));
        return 0;
    } else {
        p->add_msg_if_player(_("You light the torch."));
...
int iuse::battletorch(player *p, item *it, bool t)
{
    if (!p->use_charges_if_avail("fire", 1)) {
        ...
        p->add_msg_if_player(_("You light the Louisville Slaughterer."));
...
{
    if (!p->use_charges_if_avail("fire", 1)) {
        p->add_msg_if_player( _("You need a lighter to light this."));
        return 0;
    } else {
        p->add_msg_if_player( _("You light the candle."));
...
// Do nothing... player::active_light and the lightmap::generate deal with this
  ...
  p->add_msg_if_player(_("The candle winks out"));
...
{
    if (it->charges == 0) {
        p->add_msg_if_player(_("The PDA's batteries are dead."));
        return 0;
    } else {
        p->add_msg_if_player(_("You activate the flashlight app."));
...
// Do nothing... player::active_light and the lightmap::generate deal with this
  ...
  p->add_msg_if_player(_("The PDA screen goes blank."));
  it->make(itypes["pda"]);
  it->active = false;
 }
 return it->type->charges_to_use();
...
int iuse::dejar(player *p, item *it, bool t)
{
    if( (it->type->id).substr(0,4) == "jar_" ) {
        p->add_msg_if_player(_("You open the jar, exposing it to the atmosphere."));
    } else if( (it->type->id).substr(0,4) == "bag_" ) {
        p->add_msg_if_player(_("You open the vacuum pack, exposing it to the atmosphere."));
    } else {
    ...
    it->contents[0].bday = calendar::turn + 3600 - (calendar::turn % 3600);
    return it->type->charges_to_use();
}

int iuse::rad_badge(player *p, item *it, bool t)
{
    p->add_msg_if_player(_("You remove the badge from its wrapper, exposing it to ambient radiation."));
...
int iuse::towel(player *p, item *it, bool t)
{
    ...
    if( abs(p->has_morale(MORALE_WET)) )
    {
        ...
        p->add_msg_if_player(_("You use the %s to dry off!"), it->name.c_str());
    }
    else
    {
        p->add_msg_if_player(_("You are already dry, %s has no effect"), it->name.c_str());
    }
    return it->type->charges_to_use();
}
...
int iuse::unfold_bicycle(player *p, item *it, bool t)
{
    ...
    if( bicycle ) {
        ...
        p->add_msg_if_player( _("You painstakingly unfold the bicycle and make it ready to ride."));
        p->moves -= 500;
    } else {
        p->add_msg_if_player( _("There's no room to unfold the bicycle."));
        return 0;
    }
    return 1;
}
