                              ...
                              bool carveable)
{
    ...
    if (!cut->made_of(SOLID)) {
        ...
        add_msg(_("You can't %s an item that's not solid!"), lower_verb.c_str());
        return false;
    }
    if(carveable && !(cut->made_of("wood") || cut->made_of("plastic") || cut->made_of("glass") ||
        ...
        add_msg(_("You can't %1$s an item made of %2$s!"),
...
// TODO: Add other diseases here; royal jelly is a cure-all!
 ...
 if (p->has_effect("poison") || p->has_effect("foodpoison") ||
     p->has_effect("bad_poison") || p->has_effect("paralyzepoison")) {
                           ...
                           int bleed, int bite, int infect, bool force)
{
    ...
    if(p->hp_cur[hp_head] < p->hp_max[hp_head] ||
      (p->has_effect("infection", bp_head)) ||
    ...
    if(p->hp_cur[hp_torso] < p->hp_max[hp_torso] ||
      (p->has_effect("infection", bp_torso)) ||
    ...
    if(p->hp_cur[hp_arm_l] < p->hp_max[hp_arm_l] ||
      (p->has_effect("infection", bp_arms, side)) ||
      (p->has_effect("bite", bp_arms, side)) ||
      (p->has_disease("bleed", bp_arms, side)) || force) {
        color = g->limb_color(p, bp_arms, side, bleed, bite, infect);
    ...
    if(p->hp_cur[hp_leg_l] < p->hp_max[hp_leg_l] ||
      (p->has_effect("infection", bp_legs, side)) ||
      (p->has_effect("bite", bp_legs, side)) ||
      (p->has_disease("bleed", bp_legs, side)) || force) {
        color = g->limb_color(p, bp_legs, side, bleed, bite, infect);
        if (color != c_ltgray || normal_bonus != 0) {
    ...
    do {
        ...
        if (ch == '1'){
            ...
            (!((p->has_effect("infection", bp_arms, 0)) ||
              (p->has_effect("bite", bp_arms, 0)) ||
            ...
            if ( (p->hp_cur[hp_arm_r] == 0) &&
              (!((p->has_effect("infection", bp_arms, 1)) ||
              (p->has_effect("bite", bp_arms, 1)) ||
            ...
            if ( (p->hp_cur[hp_leg_l] == 0) &&
              (!((p->has_effect("infection", bp_legs, 0)) ||
              (p->has_effect("bite", bp_legs, 0)) ||
            ...
            if ( (p->hp_cur[hp_leg_r] == 0) &&
              (!((p->has_effect("infection", bp_legs, 1)) ||
              (p->has_effect("bite", bp_legs, 1)) ||
                                ...
                                int bite, int infect, bool force)
{
    ...
    if (p->has_effect("bite", bp_healed, side)) {
        if (x_in_y(bite, 100)) {
            int bite_dur = p->effect_duration("bite", false, bp_healed, side);
            p->remove_effect("bite", bp_healed, side);
    ...
    if (p->has_effect("infection", bp_healed, side)) {
        if (x_in_y(infect, 100)) {
            int infected_dur = p->effect_duration("infection", false, bp_healed, side);
            p->remove_effect("infection", bp_healed, side);
...
int iuse::antibiotic(player *p, item *it, bool)
{
    ...
    if (p->has_effect("infection")) {
        // cheap model of antibiotic resistance, but it's something.
        if (x_in_y(95, 100)) {
            int infected_dur = p->effect_duration("infection", true);
...
int iuse::sew(player *p, item *, bool)
{
    ...
    if (p->fine_detail_vision_mod() > 4) {
        add_msg(_("You can't see to sew!"));
        return 0;
    }
    int thread_used = 1;
...
static bool valid_fabric(player *p, item *it, bool)
{
    ...
    if (it->type->id == "string_6" || it->type->id == "string_36" || it->type->id == "rope_30" ||
        it->type->id == "rope_6") {
        add_msg(_("You cannot cut that, you must disassemble it using the disassemble key"));
        return false;
    }
    ...
    if (!it->made_of("cotton") && !it->made_of("leather") && !it->made_of("nomex")) {
        add_msg(_("You can only slice items made of fabric or leather."));
        return false;
    }
    if (it->is_container() && !it->contents.empty()) {
        add_msg(_("That %s is not empty!"), it->tname().c_str());
        return false;
    }
...
static bool cauterize_effect(player *p, item *it, bool force = true)
{
    ...
    if (hpart != num_hp_parts) {
        ...
        if (p->has_effect("bite", bp, side)) {
            g->u.add_effect("bite", 2600, false, 1, bp, side);
        }
        return true;
    }
    return 0;
...
static int cauterize_elec(player *p, item *it)
{
    if (it->charges == 0) {
        ...
        return 0;
    } else if (!p->has_effect("bite") && !p->has_disease("bleed") && !p->is_underwater()) {
...
int iuse::solder_weld(player *p, item *it, bool)
{
    ...
    if ((p->has_effect("bite") || p->has_disease("bleed")) && !p->is_underwater()) {
...
int iuse::directional_antenna(player *p, item *it, bool)
{
    ...
    {
        add_msg( _("Must have an active radio to check for signal direction.") );
        return 0;
    }
    ...
    if( tower == NULL )
    {
        add_msg( _("You can't find the direction if your radio isn't tuned.") );
        return 0;
    }
    // Report direction.
...
int iuse::picklock(player *p, item *it, bool)
{
 ...
 if (type == t_chaingate_l) {
  ...
  add_msg(_("That cannot be picked."));
  return 0;
 }
...
int iuse::geiger(player *p, item *it, bool t)
{
    if (t) { // Every-turn use when it's on
        ...
        if (rads > 50) {
            add_msg(_("The geiger counter buzzes intensely."));
        } else if (rads > 35) {
            add_msg(_("The geiger counter clicks wildly."));
        } else if (rads > 25) {
            add_msg(_("The geiger counter clicks rapidly."));
        } else if (rads > 15) {
            add_msg(_("The geiger counter clicks steadily."));
        } else if (rads > 8) {
            add_msg(_("The geiger counter clicks slowly."));
        } else if (rads > 4) {
...
int iuse::can_goo(player *p, item *it, bool)
{
 ...
 while (!one_in(4) && tries < 10) {
  ...
  if (tries < 10) {
   if (g->u_see(goox, gooy)) {
    add_msg(_("A nearby splatter of goo forms into a goo pit."));
   }
...
int iuse::pipebomb_act(player *, item *it, bool t)
{
    ...
    if (t) { // Simple timer effects
        ...
        add_msg(_("You've already lit the %s, try throwing it instead."), it->name.c_str());
        return 0;
...
int iuse::granade_act(player *, item *it, bool t)
{
    ...
    if (t) { // Simple timer effects
        ...
        add_msg(_("You've already pulled the %s's pin, try throwing it instead."), it->name.c_str());
        return 0;
...
int iuse::pheromone(player *p, item *it, bool)
{
 ...
 if (g->u_see(p)) {
  if (converts == 0) {
   ...
   add_msg(_("...and a nearby zombie turns friendly!"));
  } else{
   add_msg(_("...and several nearby zombies turn friendly!"));
  }
 }
 return it->type->charges_to_use();
}
...
{
 if (t) { // Normal use
  if (!p->has_item(it) || p->has_effect("deaf") ) {
...
int iuse::knife(player *p, item *it, bool t)
{
    ...
    if( (p->has_effect("bite") || p->has_disease("bleed") || p->has_trait("MASOCHIST") ||
        ...
        if ( !p->has_charges("fire", 4) ) {
            ...
            kmenu.addentry( cauterize, true, -1,
                            ((p->has_effect("bite") || p->has_disease("bleed")) &&
    ...
    if ( choice == cauterize) {
        bool has_disease = p->has_effect("bite") || p->has_disease("bleed");
    ...
    if (cut == it)
    {
        add_msg(_("You can not cut the %s with itself!"), it->tname().c_str());
        return 0;
    }
    if (cut == &p->weapon)
    {
    ...
    if (cut->is_container() && !cut->contents.empty()) {
        add_msg(_("That container is not empty!"));
        return 0;
    }
    ...
    if ((cut->made_of("cotton") || cut->made_of("leather") || cut->made_of("nomex")) ) {
        ...
        add_msg(_("Material of this item is not applicable for cutting up."));
        return 0;
    }
    // check again
    if ( result == NULL ) {
    ...
    if ( cut->typeId() == result->typeId() ) {
        add_msg(_("There's no point in cutting a %s."), cut->tname().c_str());
        return 0;
    }
    ...
    if (action == "carve") {
        if(count > 0) {
            ...
            add_msg("You clumsily carve the %s into useless pieces.",
                       cut->tname().c_str());
        }
    } else {
        if(count > 0){
            add_msg(ngettext("You cut the %1$s into %2$i %3$s.",
                            "You cut the %1$s into %2$i %3$ss.", count),
                   cut->tname().c_str(), count, result->tname().c_str());
        } else {
            add_msg("You clumsily cut the %s into useless pieces.",
                       cut->tname().c_str());
        }
    }
...
int iuse::cut_log_into_planks(player *p, item *it)
{
    ...
    if (scraps >= planks) {
        add_msg(_("You waste a lot of the wood."));
    }
...
int iuse::lumber(player *p, item *it, bool)
{
 ...
 if (cut->type->id == "log") {
     ...
     add_msg(_("You can't cut that up!"));
     return it->type->charges_to_use();
 }
}
...
int iuse::hacksaw(player *p, item *it, bool)
{
    ...
    if (dirx == p->posx && diry == p->posy) {
        add_msg(_("Why would you do that?"));
        add_msg(_("You're not even chained to a boiler."));
        return 0;
    }
 ...
 for (int i = -1; i <= 1; i++) {
     for (int j = -1; j <= 1; j++) {
         if (!g->m.has_flag("FLAT", posx + i, posy + j) ||
             ...
             add_msg(_("You need a 3x3 flat space to place a tent"));
             return 0;
         }
     }
 }
...
int iuse::shelter(player *p, item *, bool)
{
 ...
 for (int i = -1; i <= 1; i++) {
     for (int j = -1; j <= 1; j++) {
         if (!g->m.has_flag("FLAT", posx + i, posy + j) ||
             g->m.has_furn(posx + i, posy + j)) {
             add_msg(_("You need a 3x3 flat space to place a shelter"));
             return 0;
         }
     }
 }
...
int iuse::bullet_puller(player *p, item *it, bool)
{
    ...
    if (a == bullet_pulling_recipes.end()) {
        add_msg(_("You cannot disassemble that."));
        return 0;
    }
    if (p->skillLevel("gun") < 2) {
        add_msg(_("You need to be at least level 2 in the firearms skill before you\
...
int iuse::artifact(player *p, item *it, bool)
{
 ...
 for (size_t i = 0; i < num_used; i++) {
  ...
  case AEA_RADIATION:
   add_msg(_("Horrible gasses are emitted!"));
   for (int x = p->posx - 1; x <= p->posx + 1; x++) {
    for (int y = p->posy - 1; y <= p->posy + 1; y++)
...
static bool heat_item(player *p)
{
    ...
    add_msg(_("You can't heat that up!"));
    return false;
}
...
int iuse::radglove(player *p, item *it, bool)
{
    if ( p->get_item_position( it ) >= -1 ) {
        ...
        if (p->radiation >= 1) {
            ...
            add_msg(_("Your radiation level: %d"), p->radiation);
        } else {
