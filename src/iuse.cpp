...
cata::optional<int> iuse::lux_meter( player *p, item *, bool, const tripoint &pos )
{
    if( debug_mode ) {
        ...
        p->add_msg_if_player( m_neutral, "The illumination is %.1f, Sun illumination %.1f, Sun altitude %.1f.",
                              g->natural_light_level( pos.z ), sun_light_at( calendar::turn ), to_degrees( altitude ) );
    } else {
        p->add_msg_if_player( m_neutral, _( "The illumination is %.1f." ), g->natural_light_level( pos.z ) );
    }
