...
int iuse::install_software( player *p, item *it, bool, const tripoint & )
{
    ...
    auto filter = [it]( const item & e ) {
        ...
        return it != &e && !contained.is_null() && contained.has_flag( "SOFTWARE" ) &&
               !it->toolmod_find( contained.typeId() ) && !contained.has_flag( "SOFTWARE_INSTALLED" );
    };
    ...
    p->assign_activity( player_activity( install_software_activity_actor( item_location( *p, it ),
                                         loc,
                                         5_minutes ) ),
                        true );
    return 0;
